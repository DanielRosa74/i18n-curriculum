...
`.cat-whisker-left-top` セレクターには `absolute` に設定された `position` プロパティがあるはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.position, 'absolute');
```

`.cat-whisker-left-top` セレクターには `120px` に設定された `top` プロパティがあるはずです。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-top')?.top, '120px');
```

`.cat-whisker-left-top` セレクターには `52px` に設定された `left` プロパティがあるはずです。
