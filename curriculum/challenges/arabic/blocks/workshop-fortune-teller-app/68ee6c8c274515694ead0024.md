...
يجب أن تكون النبوءة عشوائية، لذا تحتاج إلى دالة مساعدة لاختيار نبوءة عشوائية. أعلن عن متغير باسم `chosenCard` داخل الطريقة `singleCardSelected`. استخدم الدالة `getRandomItem` للحصول على بطاقة عشوائية ومرر `this.cards` كمعلمة.
