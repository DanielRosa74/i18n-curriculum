...
title: Renderizar condicionalmente desde props
challengeType: 6
forumTopicId: 301405
...
Hasta ahora, has visto cómo usar `if/else`, `&&` y el operador ternario (`condition ? expressionIfTrue : expressionIfFalse`) para tomar decisiones condicionales sobre qué renderizar y cuándo. Sin embargo, queda un tema importante por tratar que te permite combinar cualquiera o todos estos conceptos con otra poderosa funcionalidad de React: las props. Usar props para renderizar código condicionalmente es muy común entre los desarrolladores de React — es decir, usan el valor de una prop dada para tomar decisiones automáticas sobre qué renderizar.
...
Primero, necesitarás una expresión simple que devuelva un valor diferente de forma aleatoria cada vez que se ejecute. Puedes usar `Math.random()`. Este método devuelve un valor entre `0` (inclusive) y `1` (exclusivo) cada vez que se llama. Así que para probabilidades 50/50, usa `Math.random() >= .5` en tu expresión. Estadísticamente hablando, esta expresión devolverá `true` el 50% del tiempo y `false` el otro 50%. En el método render, reemplaza `null` con la expresión anterior para completar la declaración de la variable.
