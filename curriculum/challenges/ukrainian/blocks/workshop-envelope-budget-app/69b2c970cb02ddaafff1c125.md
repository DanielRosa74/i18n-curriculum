...
Тепер, коли у вас є списки елементів, ви можете передати їх у вашу функцію `getTotalFromInputs`, щоб отримати загальну суму.

Оголосіть змінну `rent` і призначте їй результат виклику `getTotalFromInputs` з `[rentInput]` як аргументом.

# --hints--

У вашій функції `calculateBudget` має бути змінна `rent`.
...
Ви маєте викликати `getTotalFromInputs` з `[rentInput]` як аргументом.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

Ви маєте призначити результат `getTotalFromInputs` змінній `rent`.
