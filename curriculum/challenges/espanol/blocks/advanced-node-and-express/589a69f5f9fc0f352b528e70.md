...
Sigue <a href="https://www.freecodecamp.org/news/how-to-set-up-a-github-oauth-application/" target="_blank" rel="noopener noreferrer nofollow">estas instrucciones</a> para obtener tu *Client ID y Secret* de GitHub. Configura la URL de la página principal a tu página principal (**no la URL del código del proyecto**), y establece la URL de callback a la misma URL de la página principal con `/auth/github/callback` agregado al final. Guarda el client ID y tu client secret en el archivo `.env` de tu proyecto como `GITHUB_CLIENT_ID` y `GITHUB_CLIENT_SECRET`.
