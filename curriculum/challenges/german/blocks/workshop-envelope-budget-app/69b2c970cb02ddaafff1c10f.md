...
Erstellen Sie schließlich in einer neuen Zeile nach Ihrer zweiten `label` ein weiteres `input`-Element. Geben Sie diesem ein `type`-Attribut mit dem Wert `number`, ein `min`-Attribut mit dem Wert `0` (um sicherzustellen, dass keine negativen Beträge eingegeben werden können), ein `placeholder`-Attribut mit dem Wert `Amount` und ein `id`-Attribut, das mit dem `for`-Attribut Ihres zweiten `label`-Elements übereinstimmt.
...
Ihr `input`-Element sollte ein `placeholder`-Attribut mit dem Wert `Amount` haben.
...
Ihr `input`-Element sollte ein `id`-Attribut mit dem Wert `${category}-${entryNumber}-amount` haben.
...
Ihr neues `input`-Element sollte ein `min`-Attribut mit dem Wert `0` haben.
