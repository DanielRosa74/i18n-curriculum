...
記住 `return` 會結束函式（程式）的執行。在你的 `if` 區段之後，你需要處理輸入有效時的邏輯。因為你的 if 述語會傳回一個值，所以不需要 `else` 述語。

使用加法指定運算子將 `currVal` 加到你的 `total`。你需要使用 `Number` 建構式將 `currVal` 轉換成數字。
...
在你的 `if` 述語之後，應該對 `total` 使用加法指定運算子。
...
你應該將 `Number(currVal)` 的值加到 `total`。
