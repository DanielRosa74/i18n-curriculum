...
Neste laboratório, você vai gerar um OTP (senha única) de 6 dígitos e exibi-lo para o usuário. O OTP vai expirar após 5 segundos, e um novo OTP será gerado quando o usuário clicar no botão `"Generate OTP"`.
...
3. Seu `div` com a classe `container` deve incluir os seguintes elementos:

    - Um elemento `h1` com o ID `otp-title` e o texto `"OTP Generator"`.
    - Um elemento `h2` com o ID `otp-display` que exiba a mensagem `"Click 'Generate OTP' to get a code"` ou mostre o OTP gerado, se disponível.
    - Um elemento `p` com o ID `otp-timer` e um atributo `aria-live` definido para um valor válido que:
        - Comece vazio.
        - Exiba `"Expires in: X seconds"` após o clique no botão, onde `X` representa o tempo restante antes do OTP expirar.
        - Mostre a mensagem `"OTP expired. Click the button to generate a new OTP."` quando a contagem regressiva chegar a `0`.
    - Um elemento `button` com o ID `generate-otp-button` rotulado como `"Generate OTP"`. Ao ser clicado, deve gerar um novo OTP e iniciar uma contagem regressiva de 5 segundos.
    - O botão `"Generate OTP"` deve estar desabilitado enquanto a contagem regressiva estiver ativa.

4. Você deve garantir que o temporizador regressivo pare automaticamente quando chegar a `0` segundos para evitar atualizações desnecessárias.
5. Seu OTP gerado deve ter 6 dígitos.
...
Seu `.container` deve conter um elemento `h1` com o ID `otp-title` e o texto `"OTP Generator"`.
...
Seu `.container` deve conter um elemento `h2` com o ID `otp-display` que exiba o OTP quando gerado.
...
Seu `h2` dentro do `.container` deve inicialmente exibir a mensagem `"Click 'Generate OTP' to get a code"`.

```js
assert.strictEqual(document.querySelector('.container h2#otp-display').textContent.trim(), "Click 'Generate OTP' to get a code");
```

Seu elemento `.container` deve conter um elemento `p` com o ID `otp-timer`.
...
Seu elemento `p` deve ter um atributo `aria-live` definido como `assertive` ou `polite`.
...
Seu elemento `p` deve estar inicialmente vazio.
...
Seu `.container` deve conter um elemento `button` com o ID `generate-otp-button` e o texto `"Generate OTP"`.
...
Seu elemento `h2` com ID `otp-display` deve exibir um novo OTP quando o botão for clicado.
...
Seu OTP gerado deve ter 6 dígitos.
...
Seu elemento `p` com ID `otp-timer` deve mostrar uma contagem regressiva de 5 segundos quando o botão for clicado.
...
Seu elemento `p` com ID `otp-timer` deve atualizar sua mensagem a cada segundo para mostrar o tempo restante.
...
Seu botão `"Generate OTP"` deve estar desabilitado enquanto o temporizador regressivo estiver funcionando.
...
Seu botão `"Generate OTP"` deve ser habilitado assim que o temporizador regressivo chegar a 0.
...
Você deve exibir a mensagem `OTP expired. Click the button to generate a new OTP.` quando o temporizador regressivo chegar a `0`.
