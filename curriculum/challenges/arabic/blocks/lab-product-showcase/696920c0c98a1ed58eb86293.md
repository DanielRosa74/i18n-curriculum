...
عند استدعاء `renderProduct` مع منتج غير صالح، يجب أن ترجع خطأً بالصيغة `Unknown product type: [p]`. استبدل `[p]` بالمنتج غير الصالح بعد تحويله إلى سلسلة باستخدام `JSON.stringify`.
