...
Debido a estas posibles inconsistencias, ES6 (la sexta edición de JavaScript, lanzada en 2015) introdujo `Number.isNaN()`. Este método no intenta convertir el parámetro a un número antes de evaluarlo. Solo devuelve `true` si el valor es exactamente `NaN`:
