...
프로그래밍에서 `assignment operator`는 변수에 값을 할당하는 데 사용됩니다. Python을 포함한 많은 프로그래밍 언어에서 `assignment operator`는 `=` 기호로 나타냅니다. 이 기호는 `=`의 오른쪽에 있는 값을 왼쪽에 있는 변수 이름에 저장하라고 컴퓨터에 지시합니다. 예를 들어, `number = 10`는 변수 `number`에 값 `10`을 할당합니다.
