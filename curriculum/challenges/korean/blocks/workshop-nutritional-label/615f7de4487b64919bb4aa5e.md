...
마지막 `.divider` 뒤에 텍스트 `Cholesterol 0mg 0%`를 가진 새로운 `p` 요소를 만드세요. 그런 다음 텍스트 `Cholesterol`를 `span` 요소에, `0%`를 또 다른 요소에 감싸고 각각에 `bold` 클래스를 지정하세요.
...
텍스트 `Cholesterol`를 포함하는 `span` 요소는 `class` 속성이 `bold`로 설정되어야 합니다.
...
텍스트 `0%`를 포함하는 `span` 요소는 `class` 속성이 `bold`로 설정되어야 합니다.
