...
Sie sollten eine `const`-Variable namens `header` mit dem Wert `"Title,Author,Year,Location"` deklarieren.
...
Sie sollten eine `const`-Variable namens `rows` deklarieren, die auf ein leeres Array gesetzt ist.
