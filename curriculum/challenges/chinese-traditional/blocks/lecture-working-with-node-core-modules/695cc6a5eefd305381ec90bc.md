...
本課程將聚焦於 Node 的 `fs` 模組、它的運作方式，以及你可以使用它所提供方法的不同方式。
...
`fs` 模組提供的彈性在於它允許你同步和非同步地使用其方法。許多 `fs` 模組的方法同時提供非同步和同步兩種形式，而有些方法則僅提供非同步形式。
...
基於 promises 的 API 也提供了一種非同步操作檔案的方式。你可以搭配 `async/await` 使用 promises，這能讓非同步程式碼更容易閱讀和維護。

回呼和基於 promises 的 API 都可使用，最佳選擇取決於你的需求。

`writeFile()` 方法讓你寫入檔案。如果檔案不存在，它會在指定路徑創建該檔案。如果你提供相對路徑，檔案會相對於目前工作目錄被創建。
...
`appendFile()` 方法讓你向檔案添加內容。如果檔案不存在，它會創建該檔案。以下是使用方式：
...
以下哪一項不是使用 Node `fs` 模組寫入資料到檔案的有效方法？
