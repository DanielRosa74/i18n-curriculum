...
سلسلة `output.innerHTML` تحتاج إلى عنصر `span`. أنشئ هذا العنصر، وأعطه خاصية صنف (class) بقيمة المتغير `statusClass`.

داخل عنصر `span`، أضف المتغير `statusText` كمحتوى نصي.
...
يجب أن يحتوي `span` على الخاصية `class` مضبوطة على `${statusClass}`.
...
يجب أن يحتوي عنصر `span` على `${statusText}` كمحتوى نصي.
