...
Katika kitendakazi chako kipya, tangaza `total` kama kigezo na weka thamani yake kuwa `0`. Tumia `let` kuitangaza, kwa sababu utakuwa unarekebisha thamani yake baadaye.

# --hints--

Unapaswa kutangaza kigezo cha `total`.
...
Kigezo chako cha `total` kinapaswa kupewa thamani `0`.
...
Unapaswa kutangaza kigezo chako cha `total` kwa kutumia `let`.}
