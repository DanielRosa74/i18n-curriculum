...
要素のリストができたので、それらを `getTotalFromInputs` 関数に渡して合計を取得できます。

`rent` 変数を宣言し、`[rentInput]` を引数として `getTotalFromInputs` を呼び出した結果を代入してください。

# --hints--

`calculateBudget` 関数には `rent` 変数が必要です。
...
`getTotalFromInputs` を `[rentInput]` を引数にして呼び出してください。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

`getTotalFromInputs` の結果を `rent` に代入してください。
