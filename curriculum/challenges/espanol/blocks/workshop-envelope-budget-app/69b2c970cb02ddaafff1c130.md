...
Ahora necesitas manejar el caso cuando el usuario no está por encima del presupuesto (cuando `netRemaining` es mayor o igual a `0`).

Agrega un bloque `else` a tu sentencia `if` que asigne a `statusText` el valor `"$X Remaining"`, donde `X` es el valor de `netRemaining`. Además, asigna `"surplus"` a `statusClass`.
...
Debes usar un literal de plantilla para `statusText`.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

Tu `statusText` debe asignarse como `$${netRemaining} Remaining`.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

Debes asignar `"surplus"` a `statusClass`.
