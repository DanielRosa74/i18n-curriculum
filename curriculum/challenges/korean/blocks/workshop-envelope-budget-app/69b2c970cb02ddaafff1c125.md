...
이제 요소 목록을 얻었으니, `getTotalFromInputs` 함수에 전달해 총합을 구하세요.

`rent` 변수를 선언하고, `[rentInput]`을 인수로 하여 `getTotalFromInputs`를 호출한 결과를 할당하세요.

# --hints--

`calculateBudget` 함수에는 `rent` 변수가 있어야 합니다.
...
`getTotalFromInputs`를 `[rentInput]` 인수와 함께 호출해야 합니다.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

`getTotalFromInputs`의 결과를 `rent`에 할당해야 합니다.
