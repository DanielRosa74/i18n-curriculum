...
La palabra clave `function` le indica a JavaScript que la variable `name` será una función. `parameter` es una variable que representa un valor que se pasa a la función cuando se usa. Una función puede tener tantos o tan pocos <dfn>parámetros</dfn> como quieras. Al igual que un ciclo `for`, el espacio entre las llaves es el <dfn>cuerpo de la función</dfn>.
