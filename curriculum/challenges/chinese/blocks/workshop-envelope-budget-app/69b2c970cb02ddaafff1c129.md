...
如果检测到无效的输入，你的 `getTotalFromInputs` 函数会将全局的误差标记 `isError` 设置为 `true`。

为你的 `calculateBudget` 函数添加一个 `if` 语句，用来查看你的全局误差标记的真实性，如果为真，则使用 `return` 结束函数执行。

# --hints--

你的 `calculateBudget` 函数应该有一个 `if` 语句，用来查看 `isError` 变量的真实性。
...
你的 `if` 语句应该放在最后一次调用 `getTotalFromInputs` 函数之后。
