...
然後，當你點擊 `span` 元素時，你會在主控台看到 `<span>Click me~!</span>` 被記錄。
...
現在，每次我們直接點擊 `span`，該元素的文字就會變成紅色。

因為監聽器是附加在父層的 `p` 元素上，點擊 `p` 元素內的任何地方，包括 `span` 元素周圍或之間的空白區域，也會觸發事件。當你點擊那裡時，`event.target` 是 `p` 元素本身，所以紅色會套用到 `p` 元素。而且因為 `color` 是繼承的 CSS 屬性，所有四個 `span` 元素會同時變成紅色。
