...
La etiqueta `audio` soporta el atributo `controls`. Esto muestra los controles predeterminados del navegador para reproducir, pausar y otros, y soporta la funcionalidad del teclado. Es un atributo booleano, lo que significa que no necesita un valor; su presencia en la etiqueta activa la configuración.
