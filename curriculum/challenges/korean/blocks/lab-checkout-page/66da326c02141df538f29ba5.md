...
텍스트가 `Checkout`인 `h1` 요소가 있어야 합니다.
...
첫 번째 섹션 내에 텍스트가 `Your Cart`인 `h2` 요소가 있어야 합니다.
...
두 번째 섹션 내에 텍스트가 `Payment Information`인 `h2` 요소가 있어야 합니다.
...
`required` 속성이 있는 `input` 요소가 최소 두 개 있어야 합니다.
