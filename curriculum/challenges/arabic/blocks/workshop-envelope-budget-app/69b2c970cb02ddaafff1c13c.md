...
بعد انتهاء الحلقة، يجب مسح حقلي `incomeInput` و `rentInput`. عيّن خاصية `value` لكل من `incomeInput` و `rentInput` إلى سلسلة نصية فارغة.

# --hints--

يجب أن تصل دالة `clearForm` إلى خاصية `value` لعنصر `incomeInput`.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

يجب أن تعيّن دالة `clearForm` خاصية `value` لعنصر `incomeInput` إلى سلسلة نصية فارغة.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

يجب تعديل `incomeInput` بعد حلقة `for` الخاصة بك.
...
يجب أن تصل دالة `clearForm` إلى خاصية `value` لعنصر `rentInput`.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

يجب أن تعيّن دالة `clearForm` خاصية `value` لعنصر `rentInput` إلى سلسلة نصية فارغة.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

يجب تعديل `rentInput` بعد حلقة `for` الخاصة بك.
