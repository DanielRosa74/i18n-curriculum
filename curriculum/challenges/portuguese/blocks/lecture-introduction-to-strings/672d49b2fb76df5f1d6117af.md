...
Sempre que você usar o operador `+` para concatenar strings, é importante verificar cuidadosamente qualquer possível problema de espaçamento.
...
É importante lembrar que strings são imutáveis, o que significa que, uma vez criada uma string, você não pode alterá-la.

Neste caso, a string original `Hello` não é modificada. Em vez disso, `greeting` agora referencia a nova string `Hello, John!`.
