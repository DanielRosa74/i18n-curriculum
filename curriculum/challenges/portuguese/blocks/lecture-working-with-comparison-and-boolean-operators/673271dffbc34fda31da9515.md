...
Operadores de comparação permitem comparar dois valores e retornar um resultado `true` ou `false`. Você pode então usar esse resultado para tomar uma decisão ou controlar o fluxo do seu programa. Você usa comparações em declarações `if`, loops e muitas outras situações onde precisa tomar decisões baseadas em certas condições. Vamos explorar os operadores de comparação mais comuns e ver como eles funcionam.
