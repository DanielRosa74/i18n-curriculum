...
在你的 `if` 语句中，你需要设置 `statusText` 来显示用户超出预算的金额。

将 `statusText` 赋值为一个模板字面量，显示 `"Over Budget by $X"`，其中 `X` 是 `netRemaining` 的绝对值。使用 `Math.abs()` 来获取绝对值。

# --hints--

你应该在 `if` 语句内为 `statusText` 赋值。

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*statusText\s*=/);
```

你的 `statusText` 应该使用模板字面量。

```js
assert.match(code, /statusText\s*=\s*`/);
```

你的模板字面量应包含文本 "Over Budget by $"。

```js
assert.match(code, /statusText\s*=\s*`Over\s+Budget\s+by\s+\$\$\{/);
```

你应该使用 `Math.abs(netRemaining)` 来获取绝对值。
