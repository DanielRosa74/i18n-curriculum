...
오버플로우가 있을 때(`result.overflow > 0`) `handleGateAtTick` 함수가 먼저 메시지 `"Overflow of " + result.overflow + " attendees. Rerouting..."`를 기록해야 합니다.
...
오버플로우가 있을 때(`result.overflow > 0`) `handleGateAtTick` 함수가 `gates`, `gate`, `tickIndex`, 그리고 `result.overflow`를 인수로 하여 `rerouteOverflow()`를 호출해 오버플로우를 라우트해야 합니다.
