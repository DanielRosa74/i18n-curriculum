...
Tu `catch` ya no debe ser un método encadenado.

```js
assert.notMatch(code, /\.catch/);
```

Tu bloque `try` debe contener las tres sentencias que obtienen los datos, crean el `json` y luego lo muestran.
...
Tu página de autores de fCC debería renderizarse correctamente.
