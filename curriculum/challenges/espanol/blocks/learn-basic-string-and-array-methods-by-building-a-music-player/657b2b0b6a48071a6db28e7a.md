...
Observa que el método `map()` toma una función como argumento. Esto se llama <dfn>función callback</dfn>, que es una función que se pasa a otra función como argumento. En el ejemplo anterior, la función callback es `(number) => number * 2`, y se ejecuta en cada elemento del arreglo `numbers`. Luego, el método `map()` devuelve un nuevo arreglo con los resultados.
