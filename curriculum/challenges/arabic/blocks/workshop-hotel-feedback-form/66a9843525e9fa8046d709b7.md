...
تذكّر أنك تعلمت كيفية العمل مع أزرار الإرسال في الدروس السابقة.
...
يجب أن يحتوي نموذجك على عنصر `button` داخل `form`.

```js
assert.exists(document.querySelector('form button'));
```

يجب أن يحتوي عنصر `button` على خاصية `type` مضبوطة على القيمة `"submit"`.

```js
assert.exists(document.querySelector('button[type="submit"]'));
```

يجب أن يحتوي عنصر `button` على نص `Submit`.
