...
Sasa unahitaji kubainisha hali inayofaa kulingana na kama mtumizi yuko juu ya bajeti au chini ya bajeti.

Anza kwa kuunda usemi wa `if` unaokagua kama `netRemaining` ni chini ya `0`.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na usemi wa `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

Usemi wako wa `if` unapaswa kukagua kama `netRemaining` ni chini ya `0`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

Mwili wa usemi wako wa `if` unapaswa kuwa tupu kwa sasa.
