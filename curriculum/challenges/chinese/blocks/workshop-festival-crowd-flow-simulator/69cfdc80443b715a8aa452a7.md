...
- 使用 `for` 循环迭代输入 `gates` 数组中的每个 gate。对于每个 gate，向 `summary` 添加一个属性：
  - 该属性的键是 gate 的 `id`。
  - 该属性的值是 `0`。
- 循环结束后，返回 `summary` 对象。
...
当调用 `initializeThroughput` 函数并传入 `[]` 时，应返回 `{}`，且不修改源输入。
...
当调用 `initializeThroughput` 函数并传入 `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]` 时，应返回 `{ North: 0 }`，且不修改源输入。
...
当调用 `initializeThroughput` 函数并传入 `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]` 时，应返回 `{ North: 0, East: 0, South: 0 }`，且不修改源输入。
...
你的 `initializeThroughput` 函数应在循环结束后，作为函数的最后一行，返回 `summary`。
