...
Ahora necesitas determinar el estado apropiado según si el usuario está por encima o por debajo del presupuesto.

Comienza creando una sentencia `if` que verifique si `netRemaining` es menor que `0`.

# --hints--

Tu función `calculateBudget` debe tener una sentencia `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

Tu sentencia `if` debe verificar si `netRemaining` es menor que `0`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

El cuerpo de tu sentencia `if` debe estar vacío por ahora.
