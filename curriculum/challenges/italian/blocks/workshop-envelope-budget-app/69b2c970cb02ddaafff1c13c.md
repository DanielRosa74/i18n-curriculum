...
Dopo che il tuo loop è terminato, devi cancellare i campi `incomeInput` e `rentInput`. Imposta la proprietà `value` di `incomeInput` e `rentInput` su una stringa vuota.

# --hints--

La tua funzione `clearForm` dovrebbe consultare la proprietà `value` dell'elemento `incomeInput`.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

La tua funzione `clearForm` dovrebbe impostare la proprietà `value` dell'elemento `incomeInput` su una stringa vuota.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

Dovresti modificare `incomeInput` dopo il tuo `for` loop.
...
La tua funzione `clearForm` dovrebbe consultare la proprietà `value` dell'elemento `rentInput`.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

La tua funzione `clearForm` dovrebbe impostare la proprietà `value` dell'elemento `rentInput` su una stringa vuota.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

Dovresti modificare `rentInput` dopo il tuo `for` loop.
