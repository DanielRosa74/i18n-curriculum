...
Em Python, variáveis são como caixas rotuladas para armazenar e referenciar dados de diferentes tipos. Para declarar variáveis em Python, você atribui um valor a um identificador com o operador de atribuição (`=`). Você não precisa usar palavras-chave especiais como `let` ou `const` em Javascript, ou `char` em C#.

Em Python, você só precisa escrever o nome da variável à esquerda, seguido pelo operador de atribuição, e o valor que quer atribuir à variável à direita. Aqui está um exemplo de como declarar as variáveis `name` e `age`:
