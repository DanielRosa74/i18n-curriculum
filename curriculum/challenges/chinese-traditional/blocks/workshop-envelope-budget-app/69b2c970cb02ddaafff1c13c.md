...
當你的迴圈完成後，你需要清除 `incomeInput` 和 `rentInput` 欄位。將 `incomeInput` 和 `rentInput` 的 `value` 屬性設為空字串。

# --hints--

你的 `clearForm` 函式（程式）應該存取 `incomeInput` 元素的 `value` 屬性。

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

你的 `clearForm` 函式（程式）應該將 `incomeInput` 元素的 `value` 屬性設為空字串。

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

你應該在 `for` 迴圈之後修改 `incomeInput`。
...
你的 `clearForm` 函式（程式）應該存取 `rentInput` 元素的 `value` 屬性。

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

你的 `clearForm` 函式（程式）應該將 `rentInput` 元素的 `value` 屬性設為空字串。

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

你應該在 `for` 迴圈之後修改 `rentInput`。
