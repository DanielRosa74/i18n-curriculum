...
Kufuata muundo uleule, fanya hoja ya utafutaji kwa ingizo zako za `number` katika kipengele cha `#entertainment` na weka thamani zao katika `entertainmentInputs`.

# --hints--

Unapaswa kutangaza kigezo cha `entertainmentInputs`.
...
Unapaswa kufanya wito wa `document.querySelectorAll()` kwa kutumia `#entertainment input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito lako la `document.querySelectorAll()` katika `entertainmentInputs`.
