...
いよいよすべてを組み合わせていく時です。`e`という名前のパラメータを受け取る空の`calculateBudget`関数を宣言してください。この関数は別のイベントリスナーになるので、最初に渡される引数はブラウザのイベントです。`e`はこのパラメータの一般的な名前です。

# --hints--

`calculateBudget`変数を宣言してください。

```js
assert.isDefined(calculateBudget);
```

`calculateBudget`変数は関数である必要があります。

```js
assert.isFunction(calculateBudget);
```

`calculateBudget`関数は`e`という引数を受け取る必要があります。
