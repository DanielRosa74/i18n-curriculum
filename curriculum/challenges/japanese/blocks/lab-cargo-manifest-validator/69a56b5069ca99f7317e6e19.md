...
**注意:** テストで再代入が必要なため、`normalizeUnits`、`validateManifest`、`processManifest` は `const` を使って宣言しないでください。
...
`normalizeUnits`、`validateManifest`、`processManifest` はテストで再代入が必要なため、`const` を使って宣言しないでください。
