...
`#income` の入力値も取得する必要があります。コードの先頭で既にこれを取得し、`incomeInput` 変数に設定しています。ただし、`getElementById` を使っているため、返されるのは `NodeList` ではなく `Element` です。

`NodeList` は配列のようなオブジェクトで、繰り返し処理ができ、配列と共通のメソッドもいくつか持っています。`getTotalFromInputs` 関数には、引数として配列でも `NodeList` と同じように動作します。

`income` 変数を宣言し、`getTotalFromInputs` を呼び出した結果に設定してください。引数には `incomeInput` を含む配列を渡します。

# --hints--

`calculateBudget` 関数には `income` 変数が必要です。
...
`getTotalFromInputs` を `[incomeInput]` を引数にして呼び出してください。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

`getTotalFromInputs` の結果を `income` に代入してください。
