...
오른쪽 안쪽 귀를 `absolute`로 설정된 `position` 속성, `top`인 `22px`, 그리고 `left`인 `-20px`로 위치시키세요.

# --hints--

`.cat-right-inner-ear` 선택자에 `absolute`로 설정된 `position` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.position, 'absolute')
```

`.cat-right-inner-ear` 선택자에 `22px`로 설정된 `top` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-ear')?.top, '22px')
```

`.cat-right-inner-ear` 선택자에 `-20px`로 설정된 `left` 속성이 있어야 합니다.
