...
どのカテゴリに費用が入るかを知る必要があります。幸いにも、ユーザーがカテゴリを選択できるドロップダウンを追加しました。

以前のJavaScriptでそのドロップダウンを問い合わせて、`entryDropdown` 変数に代入したことを覚えていますか。選択されたオプションの値を取得するには、<dfn>value</dfn> プロパティを使えます。

`entryDropdown` の `value` プロパティを `category` という変数に代入してください。


# --hints--

`addEntry` 関数には `category` 変数が必要です。
...
`category` 変数には `entryDropdown.value` の値を代入してください。
