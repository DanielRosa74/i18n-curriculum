...
`innerHTML` 是 `Element` 对象的一个属性，你可以用它来设置它们的 HTML 结构。通过 `innerHTML`，你可以用字串设置现有元素的 HTML 结构，从而创建所有必要的节点。
...
然后，我们将容器的 `innerHTML` 属性设置为一个字串。这个字串必须包含表现你想要创建的节点和结构所需的所有标记。你可以把它看作是在一个字串中编写 HTML。
...
如果这样能让标记更易理解，你也可以将字串写成多行。重要的是标记能准确表现你想要创建的 HTML 结构。
