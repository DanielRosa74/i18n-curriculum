...
Successivamente, crea un secondo elemento `fieldset` con l'`id` impostato su `food`.

All'interno di quell'elemento, crea un elemento `legend` con il testo `Food` e un `div` vuoto con la `class` impostata su `input-container`.
...
Il tuo secondo elemento `fieldset` dovrebbe avere un `id` impostato su `food`.
...
Il tuo nuovo elemento `legend` dovrebbe avere il testo `Food`.
