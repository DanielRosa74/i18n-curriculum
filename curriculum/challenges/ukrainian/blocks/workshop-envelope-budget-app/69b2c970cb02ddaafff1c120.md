...
Тепер настав час усе зібрати докупи. Оголосіть порожню `calculateBudget` функцію, яка приймає параметр із назвою `e`. Ця функція буде ще одним слухачем подій, тому першим аргументом, що передається, буде подія браузера – `e` є загальною назвою для цього параметра.

# --hints--

Ви повинні оголосити змінну `calculateBudget`.

```js
assert.isDefined(calculateBudget);
```

Ваша змінна `calculateBudget` має бути функцією.

```js
assert.isFunction(calculateBudget);
```

Ваша функція `calculateBudget` має приймати аргумент `e`.
