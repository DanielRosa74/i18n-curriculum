...
`label` 要素に `for` 属性を付け、その値を `X-#-name` にしてください。ここで `X` は `category` 変数の `value` で、`#` は `entryNumber` の値です。HTMLの属性は二重引用符で囲む必要があることを忘れないでください。

# --hints--

`label` 要素の `for` 属性は `${category}-${entryNumber}-name` に設定してください。

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

`label` 要素のテキストは `"Expense ${entryNumber} Name"` にしてください。
