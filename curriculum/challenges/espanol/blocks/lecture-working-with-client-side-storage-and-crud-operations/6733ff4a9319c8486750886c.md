...
`sessionStorage` es otra parte de la API de almacenamiento web que guarda datos durante la sesión de la página, lo que significa que los datos están disponibles mientras la pestaña o ventana del navegador esté abierta. Sin embargo, a diferencia de `localStorage`, los datos en `sessionStorage` se borran cuando se cierra la pestaña o ventana. Aprenderás más sobre `sessionStorage` en la próxima lección.
