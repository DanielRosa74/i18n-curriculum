...
`foodInputs` 변수를 선언하고, 선택자 `#food input[type='number']`를 사용하여 `document.querySelectorAll()`을 호출한 값을 할당하세요. 이렇게 하면 `#food` 요소 안에 있는 모든 `number` 입력 요소를 가져옵니다.

# --hints--

`foodInputs` 변수를 선언하세요.
...
`#food input[type='number']`를 사용하여 `document.querySelectorAll()`을 호출하세요.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` 호출 결과를 `foodInputs`에 할당하세요.
