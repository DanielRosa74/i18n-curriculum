...
- 메시지 `"\nTick " + (tickIndex + 1)`를 기록하세요.
- `gates` 배열의 각 게이트를 반복하며 다음 인수로 `handleGateAtTick()`을 호출하세요:
  - `gates`: 게이트 객체 전체 배열입니다.
  - `gate`: 현재 처리 중인 게이트입니다.
  - `tickIndex`: `queue` 배열에서 현재 틱 위치입니다.
  - `throughputSummary`: 게이트별 처리된 총 참석자 수를 추적하는 로컬 객체입니다.
- 다음 틱으로 이동하기 위해 `tickIndex`를 1만큼 증가시키세요.
...
콘솔에 기록한 후 `gates` 배열의 각 게이트를 반복하며 `gates`, `gate`, `tickIndex`, `throughputSummary`를 인수로 `handleGateAtTick()`을 호출하세요.
...
`gates` 배열을 반복한 후 `tickIndex`를 1만큼 증가시키세요.
