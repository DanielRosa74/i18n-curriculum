...
Ora devi determinare lo stato appropriato in base al fatto che l'utente abbia superato il budget o sia sotto il budget.

Inizia creando un'istruzione `if` che verifichi se `netRemaining` è minore di `0`.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere un'istruzione `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

La tua istruzione `if` dovrebbe verificare se `netRemaining` è minore di `0`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

Il corpo della tua istruzione `if` dovrebbe essere vuoto per ora.
