...
これを解決するには、2番目の`legend`内の`label`要素の前に、テキスト`Account type (required)`を持つ`fieldset`要素を追加して、必要な内容の文脈を提供します。そして、フォームが必須データを含んで提出されるように、`checked`入力に`Personal`属性を追加してください。
...
`legend`要素に`Account type (required)`テキストを追加してください。
