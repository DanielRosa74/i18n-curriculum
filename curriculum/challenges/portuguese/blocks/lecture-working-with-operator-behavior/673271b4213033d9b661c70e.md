...
Em JavaScript, todos os operadores aritméticos têm uma forma de atribuição composta. Os operadores de atribuição composta oferecem uma forma abreviada e concisa para realizar uma operação em uma variável seguida de armazenar o resultado nessa mesma variável. Eles combinam a operação e a atribuição em uma forma mais curta como `x += y`, que é equivalente a escrever `x = x + y` mas sem repetir o nome da variável. Em vez de escrever algo assim:
