...
Ваш рядок `output.innerHTML` потребуватиме елемента `span`. Створіть його та додайте атрибут класу зі значенням змінної `statusClass`.

Всередині вашого елемента `span` додайте змінну `statusText` як текстовий вміст.
...
Ваш елемент `span` повинен мати атрибут `class`, встановлений у `${statusClass}`.
...
Ваш елемент `span` повинен мати `${statusText}` як текстовий вміст.
