...
在你的 `HTMLString` 結尾處（新的一行）創建另一個 `label` 元素。這個 `label` 應該包含文字 `Expense # Amount`，使用你的模板字串語法將 `#` 替換為 `entryNumber` 的值，並且 `for` 屬性設為 `X-#-amount`，其中 `X` 是 `category` 的值，`#` 是 `entryNumber` 的值。
...
你的新 `label` 元素的 `for` 屬性應設為 `${category}-${entryNumber}-amount`。
...
你的新 `label` 元素應該包含文字 `Expense ${entryNumber} Amount`。
