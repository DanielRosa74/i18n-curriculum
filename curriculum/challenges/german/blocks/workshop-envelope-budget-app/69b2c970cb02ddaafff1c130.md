...
Jetzt müssen Sie den Fall behandeln, wenn der Benutzer nicht über dem Budget liegt (wenn `netRemaining` größer oder gleich `0` ist).

Fügen Sie Ihrem `if`-Statement einen `else`-Block hinzu, der `statusText` auf `"$X Remaining"` setzt, wobei `X` der Wert von `netRemaining` ist. Setzen Sie außerdem `statusClass` auf `"surplus"`.
...
Sie sollten für `statusText` ein Template Literal verwenden.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

Ihr `statusText` sollte auf `$${netRemaining} Remaining` gesetzt werden.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

Sie sollten `"surplus"` `statusClass` zuweisen.
