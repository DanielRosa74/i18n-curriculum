...
`@media` の `only screen` を持つ `max-width` に対して、3つ目の `550px` クエリを作成してください。その中に、`6rem` に設定された `font-size` を持つ `.hero-title` セレクター、`1.8rem` に設定された `font-size` を持つ `.hero-subtitle, .author, .quote, .list-title` セレクター、`2rem` に設定された `font-size` を持つ `.social-icons` セレクター、そして `1.6rem` に設定された `font-size` を持つ `.text` セレクターを作成してください。
