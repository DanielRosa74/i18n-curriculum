...
Пам’ятайте, що `return` завершує виконання функції. Після вашого блоку `if` потрібно опрацювати логіку для випадку, коли вхідні дані є дійсними. Оскільки ваш оператор if повертає значення, вам не потрібен оператор `else`.

Використайте оператор додавання з присвоєнням, щоб додати `currVal` до вашого `total`. Вам потрібно буде використати конструктор `Number`, щоб перетворити `currVal` на число.
...
Після вашого оператора `if` слід використати оператор додавання з присвоєнням для `total`.
...
Ви повинні додати значення `Number(currVal)` до `total`.
