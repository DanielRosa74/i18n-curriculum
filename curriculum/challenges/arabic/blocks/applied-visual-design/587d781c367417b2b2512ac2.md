...
تُستخدم خاصية `font-size` لتحديد حجم النص داخل عنصر معين. يمكن تطبيق هذه القاعدة على عدة عناصر لإنشاء تناسق بصري للنص على الصفحة. في هذا التحدي، ستضبط القيم لجميع الوسوم من `h1` إلى `h6` لموازنة أحجام العناوين.
...
يجب أن يضبط كودك خاصية `font-size` للوسم `h1` إلى 68 بكسل.
...
يجب أن يضبط كودك خاصية `font-size` للوسم `h2` إلى 52 بكسل.
...
يجب أن يضبط كودك خاصية `font-size` للوسم `h3` إلى 40 بكسل.
...
يجب أن يضبط كودك خاصية `font-size` للوسم `h4` إلى 32 بكسل.
...
يجب أن يضبط كودك خاصية `font-size` للوسم `h5` إلى 21 بكسل.
...
يجب أن يضبط كودك خاصية `font-size` للوسم `h6` إلى 14 بكسل.
