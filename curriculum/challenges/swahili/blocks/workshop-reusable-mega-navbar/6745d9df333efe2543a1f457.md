...
Bado ndani ya kipengele chako cha tatu cha `li`, tengeneza kipengele cha `ul` kwa ajili ya popup. Mpe `className` ya `sub-menu` na `aria-label` ya `Apps`.
...
Kipengele chako cha `ul` kinapaswa kuwa na `className` ya `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Kipengele chako cha `ul` kinapaswa kuwa na `aria-label` ya `Apps`.
