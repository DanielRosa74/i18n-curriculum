...
`foundCard` yako inapaswa kupewa thamani kwa kuita `find` kwenye `this.cards`, ukipitisha wito wa kurudisha unaochukua kigezo cha `card` na kurudisha matokeo ya kulinganisha `card.name_short` na `cardId` kwa kutumia usawa mkali.
