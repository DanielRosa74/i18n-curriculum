...
Verwenden Sie dieselbe Interpolationssyntax, um unter dem vorhandenen `p`-Element ein zweites `p`-Element mit dem Text `$${expenses} Total Expenses` hinzuzufügen.

# --hints--

Sie sollten unter dem vorhandenen ein zweites `p`-Element hinzufügen.
...
Ihr zweites `p`-Element sollte den Text `$${expenses} Total Expenses` enthalten.
