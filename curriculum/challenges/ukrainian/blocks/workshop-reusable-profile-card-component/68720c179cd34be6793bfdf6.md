...
Видаліть три компоненти `<Card />` всередині елемента `<div className="flex-container">`. У наступних кількох кроках ви створите масив даних і використаєте метод `map()` для відтворення компонентів `Card`, що є більш загальним підходом у React.
