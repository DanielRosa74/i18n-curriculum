...
现在是时候把所有内容组合起来了。声明一个空的 `calculateBudget` 函数，它接受一个名为 `e` 的参数。这个函数将是另一个事件监听器，因此传入的第一个参数将是浏览器事件——`e` 是这个参数的常用名称。

# --hints--

你应该声明一个 `calculateBudget` 变量。

```js
assert.isDefined(calculateBudget);
```

你的 `calculateBudget` 变量应该是一个函数。

```js
assert.isFunction(calculateBudget);
```

你的 `calculateBudget` 函数应该接受一个 `e` 参数。
