...
حان الوقت الآن لبدء تجميع كل شيء معًا. أعلن دالة `calculateBudget` فارغة، تأخذ معلمة باسم `e`. ستكون هذه الدالة مستمع حدث آخر، لذا المعلمة الأولى التي تُمرر ستكون حدث المتصفح – `e` هو اسم شائع لهذه المعلمة.

# --hints--

يجب أن تعلن عن متغير `calculateBudget`.

```js
assert.isDefined(calculateBudget);
```

يجب أن يكون متغير `calculateBudget` دالة.

```js
assert.isFunction(calculateBudget);
```

يجب أن تأخذ دالة `calculateBudget` معلمة `e`.
