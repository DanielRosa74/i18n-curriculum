...
Для вашого `textarea` елемента додайте атрибути `id` та `name` зі значенням `"comments"`.

Для атрибута `cols` встановіть значення `"30"`, а для атрибута `rows` — значення `"10"`.
