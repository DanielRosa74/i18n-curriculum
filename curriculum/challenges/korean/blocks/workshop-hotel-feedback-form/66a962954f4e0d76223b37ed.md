...
그 `input` 아래에 텍스트가 `Location`이고 `for` 속성이 `"location"`으로 설정된 `label` 요소를 추가하세요.

그 아래 `label` 요소 아래에 `input` 체크박스를 하나 더 추가하고 `id`와 `value` 속성을 `"reputation"`로 설정하며, `name` 속성은 `"choice"`로 설정하세요.

그 `input` 아래에 텍스트가 `Reputation`이고 `for` 속성이 `"reputation"`으로 설정된 `label` 요소를 추가하세요.
...
네 번째 체크박스 `input`에는 `id` 속성이 `"reputation"`으로 설정되어야 합니다.
...
평판 체크박스 아래에 텍스트 `Reputation`가 있는 `label` 요소가 있어야 합니다.
