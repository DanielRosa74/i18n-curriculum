...
Usando essa mesma sintaxe, faça uma consulta dos seus `number` inputs no elemento `#utilities` e atribua-os a `utilitiesInputs`.

# --hints--

Você deve declarar uma variável `utilitiesInputs`.
...
Você deve fazer uma chamada a `document.querySelectorAll()` com `#utilities input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Você deve atribuir o resultado da sua chamada `document.querySelectorAll()` a `utilitiesInputs`.
