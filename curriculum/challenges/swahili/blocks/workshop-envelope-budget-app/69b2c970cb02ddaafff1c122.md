...
Tangaza kigezo cha `foodInputs`, na mweka thamani ya wito wa `document.querySelectorAll()` kwa kutumia kichaguzi `#food input[type='number']`. Hii itarudisha ingizo lolote la `number` lililoko ndani ya kipengele cha `#food`.

# --hints--

Unapaswa kutangaza kigezo cha `foodInputs`.
...
Unapaswa kufanya wito wa `document.querySelectorAll()` kwa kutumia `#food input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito wako wa `document.querySelectorAll()` katika `foodInputs`.
