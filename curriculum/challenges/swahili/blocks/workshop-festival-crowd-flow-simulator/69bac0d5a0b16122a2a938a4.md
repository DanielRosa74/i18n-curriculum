...
Ifuatayo, sahihisha kitu cha ingizo cha `throughputSummary` kuonyesha ni wangapi washiriki waliendeshwa kwa mafanikio na lango hili katika tick ya sasa. Hasa, chukua thamani ya `processed` katika `result` na uiongeze kwenye sifa ya `throughputSummary` kwa kutumia ufunguo `gate.id`.
