...
No seu elemento `span`, crie um elemento `label` com um atributo `for` definido como `entry-dropdown` e dê a ele o texto `Add expense to:`. Depois, crie um elemento `select` com o `id` definido como `entry-dropdown` e um `name` definido como `options`. Abaixo disso, adicione um elemento `button` com o `id` definido como `add-entry` e o texto `Add Entry`.
...
Seu novo elemento `label` deve ter o texto `Add expense to:`.
...
Seu novo elemento `button` deve ter um atributo `type` definido como `button`.
