...
Щоб завершити, встановіть текст першого `a` елемента на `Calendar`, другого — на `Chat`, а третього — на `Email`.

От і все, ваш компонент `Navbar` готовий!

# --hints--

Ви повинні встановити текст першого `a` елемента на `Calendar`.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Ви повинні встановити текст другого `a` елемента на `Chat`.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Ви повинні встановити текст третього `a` елемента на `Email`.
