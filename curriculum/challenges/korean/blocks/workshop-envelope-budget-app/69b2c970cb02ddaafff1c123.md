...
같은 구문을 사용해 `#utilities` 요소 안의 `number` 입력을 쿼리하고 `utilitiesInputs`에 할당하세요.

# --hints--

`utilitiesInputs` 변수를 선언하세요.
...
`#utilities input[type='number']`를 인수로 하여 `document.querySelectorAll()`을 호출하세요.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` 호출 결과를 `utilitiesInputs`에 할당하세요.
