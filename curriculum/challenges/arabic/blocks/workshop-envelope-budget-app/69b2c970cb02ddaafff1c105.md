...
ستحتاج إلى معرفة الفئة التي ينتمي إليها المصروف. لحسن الحظ، أضفت قائمة منسدلة ليختار المستخدم منها الفئة.
...
عيّن خاصية `value` من `entryDropdown` إلى متغير اسمه `category`.


# --hints--

يجب أن تحتوي دالة `addEntry` على متغير `category`.
...
يجب تعيين متغير `category` إلى قيمة `entryDropdown.value`.
