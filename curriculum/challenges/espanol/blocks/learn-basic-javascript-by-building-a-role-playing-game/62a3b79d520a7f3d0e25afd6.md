...
Un método para encontrar elementos específicos en tu HTML es usar el método `querySelector()`. El método `querySelector()` toma un selector CSS como argumento y devuelve el primer elemento que coincide con ese selector. Por ejemplo, para encontrar el elemento `<h1>` en tu HTML, escribirías:
