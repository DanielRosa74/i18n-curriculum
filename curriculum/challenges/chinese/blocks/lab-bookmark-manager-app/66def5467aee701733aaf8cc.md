...
对于本实验，所有的 HTML 和 CSS 都已提供给你。你将使用 JavaScript 完成书签管理器应用，以便它可以将书签集合保存到局部存储中并从中读取数据。
...
当你点击 `#view-category-button` 时，你应该调用 `displayOrHideCategory` 来显示书签列表视图并隐藏主视图。
