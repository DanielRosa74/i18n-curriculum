...
Було б добре перевірити вашу `getBooksByAuthor` функцію з іншим автором.

Почніть із виведення повідомлення `"\nList of books by James Clear:\n"` у консоль.

Під тим `console.log()` додайте ще один `console.log()`. Всередині цього `console.log()` викличте функцію `getBooksByAuthor()` з аргументами `library` та `"James Clear"`.
