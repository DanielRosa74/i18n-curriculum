...
Preencha `.bb3` com um `repeating-linear-gradient`. Use `90deg` para a direção, seu `building-color3` para as duas primeiras cores e `window-color3` em `15%` para a terceira. Quando você não especifica uma distância para uma cor, ela usará os valores que fazem sentido. Neste caso, as duas primeiras cores terão como padrão `0%` e `7.5%` porque começa em `0%` e `7.5%` é metade de `15%`.
