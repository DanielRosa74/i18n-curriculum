...
이제 `renderCard` 함수의 템플릿 리터럴 안에 `div` 요소를 만드세요. `div` 안에는 `h2`를 출력하는 `drawingType` 요소와 `card_container ${isReversed ? "reversed-card" : ""}` 클래스를 가진 `data-id="${shortName}"` 속성이 있는 `figure` 요소를 추가하세요.
...
`figure` 요소는 `${shortName}`으로 설정된 `data-id` 속성을 가져야 합니다.
