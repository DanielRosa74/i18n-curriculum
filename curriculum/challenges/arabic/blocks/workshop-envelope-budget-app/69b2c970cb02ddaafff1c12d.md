...
الآن عليك تحديد الحالة المناسبة بناءً على ما إذا كان المستخدم تجاوز الميزانية أو لم يتجاوزها.

ابدأ بإنشاء جملة `if` تتحقق مما إذا كانت قيمة `netRemaining` أقل من `0`.

# --hints--

يجب أن تحتوي دالة `calculateBudget` على جملة `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

يجب أن تتحقق جملة `if` مما إذا كانت قيمة `netRemaining` أقل من `0`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

يجب أن يكون جسم جملة `if` فارغًا في الوقت الحالي.
