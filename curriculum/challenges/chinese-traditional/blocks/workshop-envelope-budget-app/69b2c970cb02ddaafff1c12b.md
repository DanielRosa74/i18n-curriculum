...
現在宣告一個 `netRemaining` 變數，並將 `income` 減去 `expenses` 的結果指定給它。

# --hints--

你的 `calculateBudget` 函式（程式）應該要有一個 `netRemaining` 變數。
...
你應該要計算 `income - expenses` 的值。

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

你應該將 `income - expenses` 的值指定給 `netRemaining`。
