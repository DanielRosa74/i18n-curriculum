...
帽子中总是至少会创建一个球。创建帽子对象时传入的参数应转换为一个名为 `contents` 的实例变量。`contents` 应该是一个字串列表，每个项代表帽子中的一个球。列表中的每个项都应该是一个颜色名称，表示该颜色的一个球。例如，如果你的帽子是 `{'red': 2, 'blue': 1}`，那么 `contents` 应该是 `['red', 'red', 'blue']`。
