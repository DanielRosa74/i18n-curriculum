...
Você precisa construir a string HTML que será exibida no elemento `output`.

Comece atribuindo um template literal vazio à propriedade `innerHTML` do elemento `output` em uma nova linha no final da declaração `if`.
