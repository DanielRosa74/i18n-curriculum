...
الآن بعد أن أصبحت لديك قوائم العناصر، يمكنك تمريرها إلى دالتك `getTotalFromInputs` لاستخراج المجموع.

أعلن عن متغير `rent`، وقم بتعيين نتيجة استدعاء `getTotalFromInputs` مع `[rentInput]` كمعلمة.

# --hints--

يجب أن تحتوي دالة `calculateBudget` على متغير `rent`.
...
يجب أن تستدعي `getTotalFromInputs` مع `[rentInput]` كمعلمة.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

يجب أن تعيّن نتيجة `getTotalFromInputs` إلى `rent`.
