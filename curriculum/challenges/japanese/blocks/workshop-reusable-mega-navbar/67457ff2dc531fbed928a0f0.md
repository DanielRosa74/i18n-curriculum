...
`className` はここで `class` の代わりに使われています。なぜなら、`class` は JavaScript の予約語だからです。そのため、React では要素にクラスを適用する方法として `className` を使います。
...
あなたの `nav` 要素には `className` として `navbar` を設定してください。
