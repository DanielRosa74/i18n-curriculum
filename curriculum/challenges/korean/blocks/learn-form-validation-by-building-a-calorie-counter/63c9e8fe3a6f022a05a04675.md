...
같은 보간 구문을 사용하여 텍스트가 `consumedCalories Calories Consumed`인 두 번째 `p` 요소와 텍스트가 `exerciseCalories Calories Burned`인 세 번째 요소를 추가하세요. `consumedCalories`와 `exerciseCalories` 변수를 적절한 값으로 바꾸는 것도 잊지 마세요.
