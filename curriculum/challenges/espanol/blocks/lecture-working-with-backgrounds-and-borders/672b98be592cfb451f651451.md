...
Este CSS crea un degradado lineal que pasa de `red` en el `left` a `yellow` en el `right`. El degradado se aplica a un elemento con una altura del `40%` de la altura del viewport. Aprenderás más sobre las unidades `vh` en una lección futura.
