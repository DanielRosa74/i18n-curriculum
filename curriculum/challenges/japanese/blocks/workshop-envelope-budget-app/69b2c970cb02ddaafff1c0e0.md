...
収入の`input`要素には、ユーザーが有効な値を入力できるように追加の属性が必要です。

作成した`#income`要素を次の属性で更新してください。

- `type`を`number`に設定します（数値のみの入力を許可するため）
- `min`を`0`に設定します（正の数のみを許可するため）
- `placeholder`を`e.g. 2000`に設定します
- 最後に、`input`要素を`required`としてマークします。
...
あなたの`input`要素には、`placeholder`属性が`e.g. 2000`に設定されている必要があります。
