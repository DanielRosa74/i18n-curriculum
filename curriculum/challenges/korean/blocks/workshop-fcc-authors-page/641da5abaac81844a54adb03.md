...
`authors` 매개변수를 받는 `displayAuthors`라는 함수를 먼저 만드세요. 그런 다음 `authors` 배열을 순회하며 각 저자마다 카드 하나를 `authorContainer` 요소에 추가해야 합니다.
...
`displayAuthors` 함수는 각 저자에 대한 카드를 `authorContainer` 요소에 추가해야 합니다.
