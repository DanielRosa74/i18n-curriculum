...
- **Constructor `RegExp`**: Cuando tu patrón o banderas provienen de variables, puedes construir el mismo tipo de regex con el constructor `RegExp`. Acepta una cadena de patrón y una cadena de banderas opcional.
...
- **Definición**: Cada evento que se activa en el DOM tiene algún tipo de comportamiento predeterminado. El evento de clic en una casilla de verificación cambia el estado de esa casilla, por defecto. Presionar la <kbd>barra espaciadora</kbd> en un botón enfocado activa el botón. El método `preventDefault()` en estos objetos `Event` detiene que ese comportamiento ocurra.
...
- **Definición**: Hay tres formas en que un formulario puede ser enviado. La primera es cuando el usuario hace clic en un botón del formulario que tiene el atributo `type` establecido en `submit`. La segunda es cuando el usuario presiona la tecla <kbd>Enter</kbd> en cualquier campo `input` editable del formulario. La tercera es mediante una llamada en JavaScript a los métodos `requestSubmit()` o `submit()` del elemento `form`.
- **Atributo `action`**: El atributo `action` debe contener una URL o una ruta relativa para el dominio actual. Este valor determina a dónde intenta enviar datos el formulario; si no estableces un atributo `action`, el formulario enviará los datos a la URL de la página actual.
...
- **Definición**: El almacenamiento en caché es el proceso de guardar copias de archivos en una ubicación temporal para que puedan accederse más rápidamente. La API de caché se usa para almacenar solicitudes y respuestas de red, haciendo que las aplicaciones web funcionen de manera más eficiente e incluso sin conexión. Forma parte de la API más amplia de Service Worker y es crucial para crear aplicaciones web progresivas (PWA) que puedan funcionar bajo condiciones de red poco fiables o lentas.
La API de caché es un mecanismo de almacenamiento que guarda objetos Request y Response. Cuando se realiza una solicitud a un servidor, la aplicación puede almacenar la respuesta y luego recuperarla de la caché en lugar de hacer una nueva solicitud de red. Esto reduce los tiempos de carga, ahorra ancho de banda y mejora la experiencia general del usuario.
- **Almacenamiento en caché**: Se usa para guardar pares clave-valor de solicitudes HTTP y sus respuestas correspondientes. Esto permite una recuperación eficiente de recursos solicitados previamente, reduciendo la necesidad de obtenerlos de la red en visitas posteriores y mejorando el rendimiento.
...
- **Soporte sin conexión**: Usando la API de caché, puedes crear aplicaciones web que priorizan el funcionamiento sin conexión. Por ejemplo, una PWA puede servir recursos almacenados en caché cuando el usuario está desconectado de la red.
...
- **Guardar contraseñas en localStorage**: Esto puede parecer un patrón negativo más obvio, pero almacenar cualquier dato sensible como contraseñas en local storage representa un riesgo de seguridad. El local storage no está encriptado y puede ser accedido fácilmente. Por eso, nunca deberías almacenar ningún tipo de dato sensible allí.
