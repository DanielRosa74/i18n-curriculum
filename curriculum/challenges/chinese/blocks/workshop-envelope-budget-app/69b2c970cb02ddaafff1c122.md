...
声明一个 `foodInputs` 变量，并将调用 `document.querySelectorAll()` 并传入选择器 `#food input[type='number']` 的结果赋值给它。这将返回 `#food` 元素中所有的 `number` 输入。

# --hints--

你应该声明一个 `foodInputs` 变量。
...
你应该调用 `document.querySelectorAll()` 并传入 `#food input[type='number']`。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你应该将 `document.querySelectorAll()` 调用的结果赋值给 `foodInputs`。
