...
正如我们在前面的例子中看到的，相邻兄弟元素的外边距会发生折叠。这是外边距折叠最直接的情况。让我们来探讨更多可能发生外边距折叠的情况。
...
理解外边距折叠对于精确控制 CSS 中的版型和间距非常重要。虽然它有时会导致意想不到的结果，但这是一个旨在创建更美观且一致的文档间距的特色。通过了解外边距折叠何时发生以及如何在必要时防止它，你可以在网页设计中创建更可预测且易于维护的版型。
