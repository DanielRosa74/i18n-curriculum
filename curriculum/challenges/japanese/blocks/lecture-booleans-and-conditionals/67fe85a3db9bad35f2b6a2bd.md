...
`if`文の本体は<dfn>コードブロック</dfn>を構成しており、これは一緒に属する文のグループです。行の先頭の空白はインデントと呼ばれます。Pythonでは、インデントによってどの文がコードブロックに属するかが決まります。
...
`print('You are an adult')`の前の4つのスペースがその行をインデントし、`if`ブロックの中に配置します。
...
ターミナルに`You are an adult`が出力されます。
...
ターミナルに`You are a teenager`が出力されます。
...
ターミナルに`You are a child`が出力されます。
