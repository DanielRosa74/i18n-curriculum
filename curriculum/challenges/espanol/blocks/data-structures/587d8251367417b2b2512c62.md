...
¿Pero qué pasa si nuestra lista ya tiene uno o más miembros? ¿Cómo agregamos un elemento a la lista? Recuerda que cada nodo en una lista enlazada tiene una propiedad `next`. Para agregar un nodo a la lista, encuentra el último nodo y apunta la propiedad `next` de ese nodo al nuevo nodo. (Consejo: sabes que has llegado al final de una lista enlazada cuando la propiedad `next` de un nodo es `null`.)
