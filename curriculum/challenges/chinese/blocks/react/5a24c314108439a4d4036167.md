...
React 组件传入 `ReactDOM.render()` 的方式与 JSX 元素略有不同。对于 JSX 元素，你传入你想渲染的元素名称。然而，对于 React 组件，你需要使用与渲染嵌套组件相同的语法，例如 `ReactDOM.render(<ComponentToRender />, targetNode)`。这种语法适用于 ES6 类组件和函数组件。
