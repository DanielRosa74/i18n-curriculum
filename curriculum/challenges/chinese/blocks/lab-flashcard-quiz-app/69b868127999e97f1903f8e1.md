...
1) 你应该有一个 id 为 `flashcard` 的 HTML 元素。
...
你应该有一个 id 为 `flashcard` 的 HTML 元素。
...
`FlashCard` 接口应有一个类型为 `string` 的 `questionText` 属性。
...
`FlashCard` 接口应有一个类型为 `string` 的 `questionAnswer` 属性。
