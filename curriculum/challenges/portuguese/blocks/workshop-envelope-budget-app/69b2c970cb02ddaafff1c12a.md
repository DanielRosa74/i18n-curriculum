...
É hora de começar a preparar seus cálculos. Comece declarando uma variável `expenses` e atribua a soma de `rent`, `food`, `utilities` e `entertainment` (observe que a ordem importa para os testes). Certifique-se de fazer isso depois da sua instrução if.

# --hints--

A função deve ter uma variável `expenses`.
...
A variável `expenses` deve vir depois da sua instrução `if`.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

Você deve calcular a soma de `rent`, `food`, `utilities` e `entertainment`, nessa ordem.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

Você deve atribuir a soma de `rent`, `food`, `utilities` e `entertainment` para `expenses`.
