...
ユーザーが予算オーバーか予算内かに基づいて、適切なステータスを決定する必要があります。

`netRemaining` が `0` 未満かどうかをチェックする `if` 文を作成することから始めてください。

# --hints--

`calculateBudget` 関数には `if` 文が必要です。

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

`if` 文では `netRemaining` が `0` 未満かどうかをチェックしてください。

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

`if` 文の本体は今のところ空のままにしてください。
