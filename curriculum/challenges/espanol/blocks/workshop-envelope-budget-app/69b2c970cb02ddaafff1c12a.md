...
Es hora de empezar a preparar tus cálculos. Comienza declarando una variable `expenses` y asígnale la suma de `rent`, `food`, `utilities` y `entertainment` (ten en cuenta que el orden importa para las pruebas). Asegúrate de hacer esto después de tu sentencia if.

# --hints--

La función debe tener una variable `expenses`.
...
La variable `expenses` debe ir después de tu sentencia `if`.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

Debes calcular la suma de `rent`, `food`, `utilities` y `entertainment`, en ese orden.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

Debes asignar la suma de `rent`, `food`, `utilities` y `entertainment` a `expenses`.
