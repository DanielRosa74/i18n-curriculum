...
同じ構文を使って、`#utilities` 要素内の `number` 入力をクエリし、それを `utilitiesInputs` に代入してください。

# --hints--

`utilitiesInputs` 変数を宣言してください。
...
`document.querySelectorAll()` を `#utilities input[type='number']` で呼び出してください。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` の呼び出し結果を `utilitiesInputs` に代入してください。
