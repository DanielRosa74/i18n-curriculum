...
React Redux 提供了一个包含两个关键特色的简洁 API：`Provider` 和 `connect`。另一个挑战会讲解 `connect`。`Provider` 是 React Redux 提供的一个包装器组件，用来包裹你的 React 应用。这个包装器允许你在整个组件树中访问 Redux 的 `store` 和 `dispatch` 函数。`Provider` 接受两个属性：Redux 的保存和你的应用的子组件。为一个 App 组件定义 `Provider` 可能看起来像这样：
