...
Nella tua nuova funzione, dichiara una variabile `total` e assegnale il valore `0`. Usa `let` per dichiararla, dato che la riassegnerai più avanti.

# --hints--

Dovresti dichiarare una variabile `total`.
...
La tua variabile `total` dovrebbe avere assegnato il valore `0`.
...
Dovresti dichiarare la tua variabile `total` con `let`.
