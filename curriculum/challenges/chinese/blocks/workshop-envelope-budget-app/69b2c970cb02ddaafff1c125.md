...
既然你已经有了元素列表，就可以将它们传递给你的 `getTotalFromInputs` 函数来提取总数。

声明一个 `rent` 变量，并将调用 `getTotalFromInputs` 并以 `[rentInput]` 作为参数的结果赋值给它。

# --hints--

你的 `calculateBudget` 函数应该有一个 `rent` 变量。
...
你应该调用 `getTotalFromInputs` 并以 `[rentInput]` 作为参数。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

你应该将 `getTotalFromInputs` 的结果赋值给 `rent`。
