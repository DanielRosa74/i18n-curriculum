...
十进制数字 11 可以写成 `0*13 + 1*8 + 0*5 + 1*3 + 0*2 + 0*1`，或者在位置记数法中写成 `010100`，其中各列代表乘以序列中的特定成员。前导零会被省略，因此十进制的 11 变成了 `10100`。不过，10100 并不是用斐波那契数表示 11 的唯一方式，`0*13 + 1*8 + 0*5 + 0*3 + 1*2 + 1*1` 或 010011 也表示十进制的 11。对于真正的 Zeckendorf 数，还有一个额外限制，即*不能使用两个连续的斐波那契数*，这就导致了前者的唯一解。
