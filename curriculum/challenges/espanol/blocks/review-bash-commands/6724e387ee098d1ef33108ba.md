...
- <kbd>Flecha arriba</kbd>/<kbd>Flecha abajo</kbd>: Recorrer los comandos anteriores/siguientes en el historial.
- <kbd>Tab</kbd>: Autocompletar comandos.
- <kbd>Control</kbd> + <kbd>L</kbd> (Linux/macOS) o escribir `cls` (Windows): Limpiar la pantalla del terminal.
- <kbd>Control</kbd> + <kbd>C</kbd>: Interrumpir un comando en ejecución (también se usa para copiar en PowerShell si hay texto seleccionado).
- <kbd>Control</kbd> + <kbd>Z</kbd> (solo Linux/macOS): Suspender una tarea en segundo plano; usa `fg` para reanudarla.
- **`!!`**: Ejecutar inmediatamente el último comando ejecutado.
