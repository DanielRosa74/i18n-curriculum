...
Es ist Zeit, Ihre Berechnungen vorzubereiten. Beginnen Sie damit, eine Variable `expenses` zu deklarieren und weisen Sie ihr die Summe von `rent`, `food`, `utilities` und `entertainment` zu (beachten Sie, dass die Reihenfolge für die Tests wichtig ist). Stellen Sie sicher, dass Sie dies nach Ihrer if-Anweisung tun.

# --hints--

Die Funktion sollte eine Variable `expenses` haben.
...
Die Variable `expenses` sollte nach Ihrer `if`-Anweisung kommen.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

Sie sollten die Summe von `rent`, `food`, `utilities` und `entertainment` in dieser Reihenfolge berechnen.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

Sie sollten die Summe von `rent`, `food`, `utilities` und `entertainment` der Variable `expenses` zuweisen.
