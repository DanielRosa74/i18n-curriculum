...
O flexbox do CSS é um modelo de layout unidimensional que permite organizar elementos em linhas ou colunas dentro de um container. Você também pode controlar a ordem e a orientação deles. Desenvolvedores web usam esse modelo para criar sites e aplicações responsivos que se adaptam a diferentes tamanhos e orientações de tela. Chamamos o flexbox de modelo de layout unidimensional porque ele foca em organizar elementos ao longo de um único eixo por vez. O eixo pode ser horizontal ou vertical.

## Containers flexíveis e itens flexíveis
...
Se você definir apenas o `width`, `height` e `background-color` desses elementos `div` na folha de estilos CSS, cada elemento filho será colocado em sua própria linha porque o container não é flex por padrão (para ver as prévias, habilite o editor interativo):
...
Mas se você adicionar `display: flex` ao container `main`, os elementos `div` serão reorganizados para caber na mesma linha e eles vão encolher se necessário:
...
Ótimo. Agora vamos falar um pouco sobre o modelo flex. Esse modelo define como os itens flexíveis são organizados dentro de um container flex. Todo container flex tem dois eixos:
...
Qual é o propósito principal do flexbox do CSS?
...
Pense em como o eixo principal e o eixo cruzado estão posicionados em relação um ao outro.
