...
React Redux proporciona una pequeña API con dos funcionalidades clave: `Provider` y `connect`. Otro desafío cubre `connect`. El `Provider` es un componente envoltorio de React Redux que envuelve tu app de React. Este envoltorio te permite acceder al `store` de Redux y a las funciones `dispatch` en todo tu árbol de componentes. `Provider` recibe dos props, el store de Redux y los componentes hijos de tu app. Definir el `Provider` para un componente App podría verse así:
