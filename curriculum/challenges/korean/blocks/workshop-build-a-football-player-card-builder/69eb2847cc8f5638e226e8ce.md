...
`player` 프로프는 값이 `'PELE'`인 `name` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `98`인 `overallRating` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `'ST'`인 `position` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `'Santos FC'`인 `club` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `97`인 `pac` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `98`인 `sho` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `83`인 `pas` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `99`인 `dri` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `41`인 `def` 속성을 가져야 합니다.
...
`player` 프로프는 값이 `75`인 `phy` 속성을 가져야 합니다.
