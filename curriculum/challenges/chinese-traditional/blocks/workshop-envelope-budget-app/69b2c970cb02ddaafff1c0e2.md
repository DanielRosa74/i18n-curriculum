...
接著，創建第二個 `fieldset` 元素，並將 `id` 設為 `food`。

在該元素內，創建一個 `legend` 元素，文字為 `Food`，以及一個空的 `div`，並將 `class` 設為 `input-container`。
...
你的第二個 `fieldset` 元素應該有一個 `id` 設為 `food`。
