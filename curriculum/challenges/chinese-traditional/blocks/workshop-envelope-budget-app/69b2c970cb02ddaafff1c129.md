...
如果偵測到無效輸入，你的 `getTotalFromInputs` 函式（程式）會將全域的錯誤旗標 `isError` 設為 `true`。

為你的 `calculateBudget` 函式（程式）添加一個 `if` 述語，檢查你的全域錯誤旗標的真值，若為真則使用 `return` 結束函式（程式）執行。

# --hints--

你的 `calculateBudget` 函式（程式）應該有一個 `if` 述語，檢查 `isError` 變數的真值。
...
你的 `if` 述語應該放在最後一次呼叫 `getTotalFromInputs` 函式（程式）之後。
