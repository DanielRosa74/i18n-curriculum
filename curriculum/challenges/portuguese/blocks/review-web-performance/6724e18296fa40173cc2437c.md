...
- **Performance percebida**: É como os usuários percebem o desempenho de um site. É a forma como eles avaliam em termos de responsividade e confiabilidade. Essa é uma medida subjetiva, então é difícil quantificá-la, mas é muito importante, já que a experiência do usuário determina o sucesso ou fracasso de um site.
...
- **Chrome DevTools**: O Chrome DevTools é uma ferramenta embutida no Google Chrome que permite analisar e depurar a performance em tempo real. As ferramentas de desenvolvimento mostram tempos de carregamento, uso da CPU e atrasos na renderização. É especialmente útil para medir o First Contentful Paint, ou FCP, que indica quão rápido o usuário vê o primeiro conteúdo visível. Se seu site parecer lento, o DevTools ajudará a identificar os gargalos.
- **Lighthouse**: É uma ferramenta automatizada que verifica performance, SEO e acessibilidade.
- **WebPageTest**: Essa ferramenta permite testar como seu site carrega de diferentes locais e dispositivos. Ela oferece um detalhamento do Speed Index, Total Blocking Time e outras métricas chave de performance. Se você quer saber como usuários reais experimentam seu site globalmente, o WebPageTest é a ferramenta ideal.
...
- **Real User Monitoring**: Ferramentas RUM acompanham o comportamento real dos usuários, mostrando como visitantes reais experimentam seu site. Ferramentas populares incluem Google Analytics, que monitora tempos de carregamento e taxas de rejeição, e New Relic ou Datadog, que monitoram problemas de performance em tempo real. Se você quer dados de usuários reais, ferramentas RUM são essenciais.
...
- **Animações CSS**: Animar certas propriedades CSS, como dimensões, posição e layout, dispara um processo chamado "reflow", durante o qual o navegador recalcula a posição e geometria de certos elementos na página. Isso exige uma repintura, que é computacionalmente cara. Portanto, recomenda-se reduzir o número de animações CSS ao máximo ou pelo menos dar ao usuário a opção de ativá-las ou desativá-las.
