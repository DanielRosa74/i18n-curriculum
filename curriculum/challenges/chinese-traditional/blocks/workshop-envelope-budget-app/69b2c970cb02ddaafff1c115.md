...
你的下一步是撰寫一個函式，從使用者的輸入中取得總金額。

宣告一個名為 `getTotalFromInputs` 的函式，並給它一個名為 `list` 的參數。

# --hints--

你應該宣告一個 `getTotalFromInputs` 變數。

```js
assert.isDefined(getTotalFromInputs);
```

你的 `getTotalFromInputs` 變數應該是一個函式。

```js
assert.isFunction(getTotalFromInputs);
```

你的 `getTotalFromInputs` 函式應該接受一個名為 `list` 的參數。
