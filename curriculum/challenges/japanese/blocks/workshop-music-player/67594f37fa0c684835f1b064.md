...
プレイリストの曲をクリックすると、その曲を再生したくなります。そのためには、`song` 要素の子である各 `button` 要素に対して `click` イベントのイベントリスナーを追加する必要があります。
...
`.playlist-song` 要素の子である各 `button` 要素に対して `click` イベントのイベントリスナーを追加し、対応する曲の `playSong` を使って `id` を呼び出してください。
