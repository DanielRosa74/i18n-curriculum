...
ستُعيّن دالتك `getTotalFromInputs` علامة الخطأ العامة `isError` إلى `true` إذا تم اكتشاف مُدخَل غير صالح.

أضف جملة `if` إلى دالتك `calculateBudget` تتحقق من صحة علامة الخطأ العامة، وإذا كانت صحيحة فاستخدم `return` لإنهاء تنفيذ الدالة.

# --hints--

يجب أن تحتوي دالتك `calculateBudget` على جملة `if` تتحقق من صحة المتغير `isError`.
...
يجب وضع جملة `if` بعد آخر استدعاء لدالة `getTotalFromInputs`.
