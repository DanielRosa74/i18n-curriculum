...
غيّر لون رابط تذييل `Visit our website` إلى `gray` عندما يزور المستخدم الرابط.
...
يجب تعيين خاصية `color` إلى `gray`.
...
يجب أن يكون لـ `a:visited` خاصية `color` بقيمة `gray`.
