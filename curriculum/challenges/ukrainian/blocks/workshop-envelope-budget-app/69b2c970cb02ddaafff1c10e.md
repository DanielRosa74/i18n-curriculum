...
Створіть ще один `label` елемент (на новій лінії) в кінці вашого `HTMLString`. Цей `label` має містити текст `Expense # Amount`, використовуючи синтаксис шаблонних рядків, щоб замінити `#` на значення `entryNumber`, а атрибут `for` має бути встановлений у `X-#-amount`, де `X` — це значення `category`, а `#` — значення `entryNumber`.
...
Ваш новий `label` елемент має мати атрибут `for`, встановлений у `${category}-${entryNumber}-amount`.
...
Ваш новий `label` елемент має містити текст `Expense ${entryNumber} Amount`.
