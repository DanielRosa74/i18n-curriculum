...
A propriedade `width` especifica a largura de um elemento. Se você não especificar uma largura, o padrão será `auto`. Isso permite que o navegador determine a largura do elemento com base no seu conteúdo, pai e tipo de exibição. Para um elemento `div`, `width: auto` faz com que ele se expanda para preencher toda a largura do container pai.
...
A propriedade `min-width` especifica a largura mínima que um elemento pode ter. Mesmo que o conteúdo interno seja menor, o elemento não vai encolher abaixo desse valor.
...
Qual das seguintes opções NÃO é uma unidade válida usada com a propriedade `width`?
