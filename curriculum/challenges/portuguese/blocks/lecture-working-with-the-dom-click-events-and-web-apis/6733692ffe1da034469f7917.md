...
Para alternar uma classe em um elemento, você pode usar o método `classList.toggle()`. Neste exemplo, temos um elemento `button` com o ID `toggle-btn` e um elemento `nav` com o ID `menu`. No CSS, adicionamos alguns estilos para o menu e uma classe chamada `show` que vai alterar a propriedade `display` do `.menu` para `block`. No JavaScript, estamos acessando o elemento do menu e o elemento `button` usando o método `getElementById()`. Depois, adicionamos um listener de evento ao elemento `button` que vai alternar a classe `show` no elemento `.menu` quando o botão for clicado:
