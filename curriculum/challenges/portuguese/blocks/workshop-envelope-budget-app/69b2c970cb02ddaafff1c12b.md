...
Agora declare uma variável `netRemaining` e atribua a ela o valor da subtração de `expenses` de `income`.

# --hints--

Sua função `calculateBudget` deve ter uma variável `netRemaining`.
...
Você deve buscar o valor de `income - expenses`.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

Você deve atribuir o valor de `income - expenses` a `netRemaining`.
