...
Você deve declarar uma variável `const` chamada `dataArrIndex` usando `taskData.findIndex()`.

```js
assert.match(code, /const\s+dataArrIndex\s*=\s*taskData\.findIndex\(/)
```

Seu callback do `findIndex` deve verificar se `item.id` é igual a `currentTask.id`.
