...
無効な入力が検出された場合、`getTotalFromInputs` 関数はグローバルエラーフラグ `isError` を `true` に設定します。

`calculateBudget` 関数に、グローバルエラーフラグの真偽値をチェックする `if` 文を追加し、真であれば `return` を使って関数の実行を終了してください。

# --hints--

`calculateBudget` 関数には、`isError` 変数の真偽値をチェックする `if` 文が必要です。
...
`if` 文は、最後の `getTotalFromInputs` 関数の呼び出しの後に配置してください。
