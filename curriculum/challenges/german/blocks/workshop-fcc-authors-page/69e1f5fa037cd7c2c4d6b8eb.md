...
Ihr `catch` sollte keine verkettete Methode mehr sein.

```js
assert.notMatch(code, /\.catch/);
```

Ihr `try`-Block sollte die drei Anweisungen enthalten, die die Daten abrufen, das `json` erstellen und es dann anzeigen.
...
Ihre fCC-Autoren-Seite sollte korrekt rendern.
