...
对于 `*_data_gen` 变量，使用 `flow_from_directory` 方法。传入批次大小、目录、目标大小（`(IMG_HEIGHT, IMG_WIDTH)`）、类模式以及其他所需参数。`test_data_gen` 是最棘手的。对于 `test_data_gen`，确保向 `flow_from_directory` 方法传入 `shuffle=False`。这样可以确保最终预测结果保持测试所期望的顺序。观察 `test_data_gen` 的目录结构也会很有帮助。
