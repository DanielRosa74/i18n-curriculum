...
Declare uma variável `cellExpanded` e atribua a ela o valor da chamada de `.replace()` na sua variável `rangeExpanded`. Passe seu `cellRegex` e uma função de callback vazia. A função de callback deve receber um parâmetro `match`.
