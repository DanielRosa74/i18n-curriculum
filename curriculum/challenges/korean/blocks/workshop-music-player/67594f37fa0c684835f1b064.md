...
재생 목록의 노래를 클릭하면 그 노래를 재생하고 싶을 것입니다. 이를 위해서는 `song` 요소의 자식인 각 `button` 요소에 `click` 이벤트에 대한 이벤트 리스너를 추가해야 합니다.
...
`.playlist-song` 요소의 자식인 각 `button` 요소에 `click` 이벤트에 대한 이벤트 리스너를 추가하고, 해당 노래의 `playSong`를 사용해 `id`를 호출해야 합니다.
