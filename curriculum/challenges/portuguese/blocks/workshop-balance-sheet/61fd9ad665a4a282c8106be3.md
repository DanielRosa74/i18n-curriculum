...
Dentro do seu `thead`, crie um elemento `tr`. Nele, adicione um `td` e três elementos `th`. Dentro de cada um dos elementos `th`, adicione um elemento `span` com a `class` definida como `sr-only` e o seguinte texto, na ordem: `2019`, `2020` e `2021`.
