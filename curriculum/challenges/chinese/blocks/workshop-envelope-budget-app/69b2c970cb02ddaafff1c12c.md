...
你需要知道用户是超出预算还是还有剩余资金。当支出超过收入时，你就是超出预算（赤字）。当收入超过支出时，你有剩余资金（盈余）。剩余正好为零可以视为平衡状态。

声明两个变量：`statusText` 和 `statusClass`。将它们都初始化为空字串，使用 `""`。

# --hints--

你的 `calculateBudget` 函数应该有一个 `statusText` 变量。
...
你的 `calculateBudget` 函数应该有一个 `statusClass` 变量。
...
你应该将 `statusText` 和 `statusClass` 初始化为空字串。
