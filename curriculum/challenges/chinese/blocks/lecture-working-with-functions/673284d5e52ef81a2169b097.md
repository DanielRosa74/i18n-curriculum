...
在这个修改后的示例中，我们创建了一个名为 `greetings` 的 `const` 变量，并赋值给它一个匿名函数。大部分语法你应该都很熟悉，唯一不同的是缺少了 `function` 关键字，并且在 `name` 参数和函数体之间添加了箭头（`=>`）。如果你的参数列表只有一个参数，那么你可以像这样去掉括号：
...
那么什么时候应该使用箭头函数语法呢？这取决于情况。许多开发者在个人项目中会一直使用它。然而，在团队合作时，选择通常取决于现有代码库是使用常规函数还是箭头函数。我们将在后续课程中讲解何时使用箭头函数以及何时避免使用。
