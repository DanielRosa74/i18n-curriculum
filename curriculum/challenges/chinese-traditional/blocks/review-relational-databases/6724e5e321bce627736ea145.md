...
- <kbd>上箭頭</kbd>/<kbd>下箭頭</kbd>：在歷史命令中循環瀏覽上一個/下一個命令。
- <kbd>Tab</kbd>：自動完成命令。
- <kbd>Control</kbd> + <kbd>L</kbd>（Linux/macOS）或輸入 `cls`（Windows）：清除終端機螢幕。
- <kbd>Control</kbd> + <kbd>C</kbd>：中斷正在執行的命令（如果在 PowerShell 中選取了文字，也可用於複製）。
- <kbd>Control</kbd> + <kbd>Z</kbd>（僅限 Linux/macOS）：將任務暫停到背景；使用 `fg` 恢復執行。
- **`!!`**：立即重新執行最後執行的命令。
...
- **手冊頁**：使用 `man` 來存取命令的詳細文件。
  - 提供關於命令使用的完整資訊
  - 顯示所有可用的選項和範例
  - 使用 <kbd>方向鍵</kbd> 瀏覽，<kbd>q</kbd> 退出
  - 並非所有命令都有手冊頁（內建命令改用 `help`）
