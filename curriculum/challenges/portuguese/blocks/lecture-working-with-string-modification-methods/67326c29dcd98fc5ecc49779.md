...
`searchValue` é o valor que você quer buscar na string. Pode ser uma string ou uma expressão regular (regex), que descreve padrões no texto. Isso permite que você pesquise e manipule strings de forma flexível e poderosa. Você vai aprender mais sobre expressões regulares em lições futuras.
