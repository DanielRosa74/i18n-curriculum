...
この同じパターンに従って、`#utilities` と `#entertainment` 要素内の `number` 入力に対して `utilities` と `entertainment` 変数を宣言してください。適切な `getTotalFromInputs` の呼び出しを代入してください。

# --hints--

`calculateBudget` 関数には `utilities` 変数が必要です。
...
`utilitiesInputs` を引数にして `getTotalFromInputs` を呼び出してください。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

`getTotalFromInputs` の結果を `utilities` に代入してください。
...
`calculateBudget` 関数には `entertainment` 変数が必要です。
...
`entertainmentInputs` を引数にして `getTotalFromInputs` を呼び出してください。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

`getTotalFromInputs` の結果を `entertainment` に代入してください。
