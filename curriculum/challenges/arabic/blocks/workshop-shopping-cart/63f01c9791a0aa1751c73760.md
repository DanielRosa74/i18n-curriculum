...
لا يزال صنف `ShoppingCart` الخاص بك يحتاج إلى المزيد من الوظائف، لكن أولًا يجب أن تكون قادرًا على اختبار الكود الذي كتبته حتى الآن. ستحتاج إلى <dfn>إنشاء نسخة جديدة</dfn> من كائن `ShoppingCart` وتعيينه إلى متغير. إليك مثالًا على إنشاء نسخة من صنف `Computer` من الأمثلة السابقة:
