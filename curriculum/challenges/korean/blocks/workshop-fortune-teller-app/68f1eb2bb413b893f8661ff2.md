...
`newReading` 메서드 안에서 `showElements`를 호출하고 `this.elements.singleCardBtn`, `this.elements.multipleCardsBtn`, 그리고 `this.elements.headerTitle`을 인수로 전달하세요.

그다음 `hideElements`를 호출하고 `this.elements.singleCard`, `this.elements.multipleCard`, `this.elements.fortuneContainer`, 그리고 `this.elements.fortuneDescription`을 인수로 전달하세요.
