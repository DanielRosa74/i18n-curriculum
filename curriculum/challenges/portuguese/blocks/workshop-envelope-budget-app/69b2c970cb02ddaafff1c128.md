...
Você também precisa obter o valor da sua entrada `#income`. Você já a consultou no início do seu código e atribuiu à variável `incomeInput`. No entanto, você usou `getElementById`, que retorna um `Element`, não um `NodeList`.

Um `NodeList` é um objeto parecido com um array, o que significa que você pode iterar através dele e ele compartilha alguns métodos comuns com um array. Para sua função `getTotalFromInputs`, um array funcionará como argumento tão bem quanto um `NodeList`.

Declare uma variável `income` e atribua a ela o resultado da chamada de `getTotalFromInputs` – passe um array contendo seu `incomeInput` como argumento.

# --hints--

Sua função `calculateBudget` deve ter uma variável `income`.
...
Você deve chamar `getTotalFromInputs` com `[incomeInput]` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

Você deve atribuir o resultado de `getTotalFromInputs` a `income`.
