...
También necesitas obtener el valor de tu entrada `#income`. Ya la consultaste al inicio de tu código y la asignaste a la variable `incomeInput`. Sin embargo, usaste `getElementById`, que devuelve un `Element`, no un `NodeList`.

Un `NodeList` es un objeto similar a un arreglo, lo que significa que puedes iterar a través de él y comparte algunos métodos comunes con un arreglo. Para tu función `getTotalFromInputs`, un arreglo funcionará igual de bien como argumento que un `NodeList`.

Declara una variable `income` y asígnale el resultado de llamar a `getTotalFromInputs`: pasa un arreglo que contenga tu `incomeInput` como argumento.

# --hints--

Tu función `calculateBudget` debería tener una variable `income`.
...
Deberías llamar a `getTotalFromInputs` con `[incomeInput]` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

Deberías asignar el resultado de `getTotalFromInputs` a `income`.
