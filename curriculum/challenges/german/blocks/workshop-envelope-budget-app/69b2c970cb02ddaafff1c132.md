...
Ihr `output.innerHTML`-String benötigt ein `span`-Element. Erstellen Sie dieses und geben Sie ihm ein Klassenattribut, das auf die Variable `statusClass` gesetzt ist.

Fügen Sie innerhalb Ihres `span`-Elements die Variable `statusText` als Textinhalt hinzu.
...
Ihr `span` sollte das `class`-Attribut haben, das auf `${statusClass}` gesetzt ist.
...
Ihr `span` sollte `${statusText}` als Textinhalt haben.
