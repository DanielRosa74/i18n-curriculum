...
Aggiungi una riga alla tua funzione `calculateBudget` che chiami il metodo `preventDefault()` sul parametro `e`. Poi, azzera la flag globale di errore `isError` impostandola su `false`.

# --hints--

La tua funzione `calculateBudget` dovrebbe chiamare `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

La tua funzione `calculateBudget` dovrebbe azzerare la flag globale di errore `isError` impostandola su `false`.
