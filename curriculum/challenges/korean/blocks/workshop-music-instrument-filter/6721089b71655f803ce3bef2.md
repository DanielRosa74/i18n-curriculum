---
id: 6721089b71655f803ce3bef2
title: 2단계
challengeType: 0
dashedName: step-2
---

# --description--

이제 드롭다운에 네 개의 옵션 `All`, `Woodwinds`, `Brass`, `Percussion`를 이 순서대로 추가하세요. 각 옵션에 대해 소문자 option을 사용해 `id` 속성과 `value` 속성을 지정하세요.

# --hints--

`option` 요소 안에 `select` 요소 네 개가 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll("select>option"), 4);
```

첫 번째 `option` 요소에는 텍스트 `All`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[0].innerText,"All");
```

`All` 옵션에는 `id` 속성과 값이 `all`인 `value` 속성이 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[0].getAttribute("id"),"all");
assert.equal(document.querySelectorAll("select>option")[0].getAttribute("value"),"all");
```

두 번째 `option` 요소에는 텍스트 `Woodwinds`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[1].innerText,"Woodwinds");
```

`Woodwinds` 옵션에는 `id` 속성과 값이 `woodwinds`인 `value` 속성이 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[1].getAttribute("id"),"woodwinds");
assert.equal(document.querySelectorAll("select>option")[1].getAttribute("value"),"woodwinds");
```

세 번째 `option` 요소에는 텍스트 `Brass`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[2].innerText,"Brass");
```

`Brass` 옵션에는 `id` 속성과 값이 `brass`인 `value` 속성이 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[2].getAttribute("id"),"brass");
assert.equal(document.querySelectorAll("select>option")[2].getAttribute("value"),"brass");
```

네 번째 `option` 요소에는 텍스트 `Percussion`가 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[3].innerText,"Percussion");
```

`Percussion` 옵션에는 `id` 속성과 값이 `percussion`인 `value` 속성이 있어야 합니다.

```js
assert.equal(document.querySelectorAll("select>option")[3].getAttribute("id"),"percussion");
assert.equal(document.querySelectorAll("select>option")[3].getAttribute("value"),"percussion");
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Music Instruments product page</title>
    <link rel="stylesheet" href="./styles.css" />
  </head>
  <body>
    <h1>Student Instruments</h1>

    <main>
      <select class="select-container">
    --fcc-editable-region--
        
    --fcc-editable-region--
      </select>
      <div class="products-container">
        <div class="card">
          <h2>Flute</h2>
          <p>$500</p>
        </div>
        <div class="card">
          <h2>Clarinet</h2>
          <p>$200</p>
        </div>
        <div class="card">
          <h2>Oboe</h2>
          <p>$4000</p>
        </div>

        <div class="card">
          <h2>Trumpet</h2>
          <p>$200</p>
        </div>
        <div class="card">
          <h2>Trombone</h2>
          <p>$300</p>
        </div>
        <div class="card">
          <h2>French Horn</h2>
          <p>$4300</p>
        </div>

        <div class="card">
          <h2>Drum Set</h2>
          <p>$500</p>
        </div>
        <div class="card">
          <h2>Xylophone</h2>
          <p>$3000</p>
        </div>
        <div class="card">
          <h2>Cymbals</h2>
          <p>$200</p>
        </div>
        <div class="card">
          <h2>Marimba</h2>
          <p>$3000</p>
        </div>
      </div>
    </main>
  </body>
</html>
```

```css
*,
*::before,
*::after {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
}

:root {
  --dark-grey: #0a0a23;
  --white: #ffffff;
  --yellow: #f1be32;
}

body {
  background-color: var(--dark-grey);
  color: var(--white);
}

h1 {
  text-align: center;
  margin-top: 20px;
}

.select-container {
  display: block;
  margin: 25px auto;
  padding: 8px;
  border: 4px solid var(--white);
  border-radius: 4px;
  width: 200px;
}

.products-container {
  display: flex;
  flex-direction: column;
  flex-wrap: wrap;
  align-items: center;
  justify-content: center;
  gap: 20px;
}

@media (min-width: 760px) {
  .products-container {
    flex-direction: row;
  }
}

.card {
  background-color: var(--white);
  color: var(--dark-grey);
  border: 4px solid var(--yellow);
  border-radius: 5px;
  padding: 10px;
  width: 200px;
}
```
