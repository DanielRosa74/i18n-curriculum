...
Todavía dentro de tu tercer elemento `li`, crea un elemento `ul` para el popup. Asígnale un `className` de `sub-menu` y un `aria-label` de `Apps`.
...
Tu elemento `ul` debe tener un `className` de `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Tu elemento `ul` debe tener un `aria-label` de `Apps`.
