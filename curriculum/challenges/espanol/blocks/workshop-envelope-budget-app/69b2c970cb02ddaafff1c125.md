...
Ahora que tienes tus listas de elementos, puedes pasarlas a tu función `getTotalFromInputs` para obtener el total.

Declara una variable `rent` y asígnale el resultado de llamar a `getTotalFromInputs` con `[rentInput]` como argumento.

# --hints--

Tu función `calculateBudget` debe tener una variable `rent`.
...
Debes llamar a `getTotalFromInputs` con `[rentInput]` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

Debes asignar el resultado de `getTotalFromInputs` a `rent`.
