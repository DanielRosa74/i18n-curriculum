...
Після завершення вашого циклу потрібно очистити поля `incomeInput` та `rentInput`. Встановіть властивість `value` у `incomeInput` та `rentInput` у порожній рядок.

# --hints--

Ваша функція `clearForm` має отримати доступ до властивості `value` елемента `incomeInput`.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

Ваша функція `clearForm` має встановити властивість `value` елемента `incomeInput` у порожній рядок.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

Ви маєте змінити `incomeInput` після вашого циклу `for`.
...
Ваша функція `clearForm` має отримати доступ до властивості `value` елемента `rentInput`.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

Ваша функція `clearForm` має встановити властивість `value` елемента `rentInput` у порожній рядок.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

Ви маєте змінити `rentInput` після вашого циклу `for`.
