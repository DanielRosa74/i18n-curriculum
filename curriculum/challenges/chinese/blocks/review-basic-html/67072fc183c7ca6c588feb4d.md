...
- **HTML 实体**：HTML 实体，或字符引用，是一组用于表现 HTML 中保留字符的字符。示例包括和号符号（`&amp;`）和小于号符号（`&lt;`）。
...
- **替换元素**：替换元素是指其内容由外部资源决定，而不是由 CSS 本身决定的元素。一个例子是 `iframe` 元素。`iframe` 代表内联框架。它是一个内联元素，用于直接在 HTML 页面中嵌入其他 HTML 内容。
