...
- **Hierarquia estrutural para elementos de título**: é importante usar o elemento de título correto para manter a hierarquia estrutural do conteúdo. O elemento `h1` é o nível mais alto de título e o elemento `h6` é o nível mais baixo. Pular níveis de título interrompe a hierarquia lógica para leitores de tela e prejudica a acessibilidade.
...
- **Elemento article (`article`)**: usado para representar conteúdo independente e autocontido que pode existir sozinho, como um post de blog ou artigo de notícia.
...
- **Escolhendo CSS em vez de `i`/`em`**: como `i` e `em` carregam significado, texto que só precisa parecer itálico para fins de apresentação deve ser estilizado com CSS em vez disso.
...
- **Elemento abreviação (`abbr`)**: usado para representar uma abreviação. Para ajudar os usuários a entenderem o que é a abreviação, você pode mostrar sua forma completa, uma descrição legível, com o atributo `title`.
...
- **Elemento endereço de contato (`address`)**: usado para representar as informações de contato.
- **Elemento quebra de linha (`br`)**: usado para criar uma quebra de linha dentro do texto, como dividir as linhas de um endereço físico.
...
- **Elemento (data) hora (`time`)**: usado para representar uma data e/ou hora. O atributo `datetime` é usado para traduzir datas e horas em um formato legível por máquina.
...
- **Esquemas href `tel:` e `mailto:`**: usados para criar links clicáveis que abrem uma chamada telefônica ou compõem um e-mail quando clicados.
