...
Tu cadena `output.innerHTML` necesitará un elemento `span`. Créalo y asígnale un atributo de clase con el valor de la variable `statusClass`.

Dentro de tu elemento `span`, agrega la variable `statusText` como el contenido de texto.
...
Tu `span` debe tener el atributo `class` establecido en `${statusClass}`.
...
Tu `span` debe tener `${statusText}` como contenido de texto.
