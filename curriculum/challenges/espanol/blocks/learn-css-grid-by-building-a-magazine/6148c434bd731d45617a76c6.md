...
Esta propiedad acepta `row` o `column` como primer valor, con un segundo valor opcional `dense`. `grid-auto-flow` usa un algoritmo de auto-ubicación para ajustar el diseño de la rejilla. Configurarlo en `column` indica al algoritmo que cree nuevas columnas para el contenido según sea necesario. El valor `dense` permite que el algoritmo retroceda y rellene huecos en la rejilla con elementos más pequeños, lo que puede hacer que los elementos aparezcan fuera de orden.
