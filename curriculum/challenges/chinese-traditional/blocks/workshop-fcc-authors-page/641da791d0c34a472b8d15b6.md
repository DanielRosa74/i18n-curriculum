...
在你的 `console.log()` 述語中，將文字 `"Author Data Array:"` 作為第一個引數，`authorDataArr` 作為第二個引數。使用逗號將文字與 `authorDataArr` 分開。
...
你應該使用逗號將你的 `"Author Data Array:"` 文字和 `authorDataArr` 分開。

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

你的 `authorDataArr` 應該是 `console.log()` 述語的第二個引數。
