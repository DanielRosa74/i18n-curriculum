...
在你的 `console.log()` 语句中，将文本 `"Author Data Array:"` 作为第一个参数，`authorDataArr` 作为第二个参数。使用逗号将文本和 `authorDataArr` 分开。
...
你应该使用逗号将你的 `"Author Data Array:"` 文本和 `authorDataArr` 分开。

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

你的 `authorDataArr` 应该是 `console.log()` 语句的第二个参数。
