...
使用相同的語法，查詢 `#utilities` 元素中的 `number` 輸入，並將它們指定給 `utilitiesInputs`。

# --hints--

你應該宣告一個 `utilitiesInputs` 變數。
...
你應該使用 `#utilities input[type='number']` 呼叫 `document.querySelectorAll()`。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 呼叫的結果指定給 `utilitiesInputs`。
