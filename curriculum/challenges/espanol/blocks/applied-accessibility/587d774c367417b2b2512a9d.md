...
Camper Cat ha creado una página base para la parte del blog de su sitio web. Planea agregar una pausa visual entre sus dos artículos con una imagen decorativa de una espada samurái. Agrega un atributo `alt` a la etiqueta `img` y configúralo con una cadena vacía. (Ten en cuenta que el `src` de la imagen no enlaza a un archivo real; no te preocupes si no se muestran espadas en la pantalla.)
