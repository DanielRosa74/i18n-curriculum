...
Vamos começar com as convenções gerais de nomenclatura em JavaScript. Nas lições anteriores, você aprendeu a usar camel case para nomes de variáveis. Para variáveis booleanas, é uma prática comum usar prefixos como `is`, `has` ou `can`. Isso indica imediatamente ao leitor que a variável é um booleano:
