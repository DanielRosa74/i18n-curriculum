...
Fügen Sie ein weiteres Kontrollkästchen-`input` mit den `id`- und `value`-Attributen hinzu, die auf `"recommendation"` gesetzt sind, und ein `name`-Attribut, das auf `"choice"` gesetzt ist.
