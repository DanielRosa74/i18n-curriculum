...
8. Você deve ordenar os livros no array `filteredBooks` de acordo com o `releaseYear` em ordem crescente. Você aprendeu em uma lição anterior que o método `sort()` ordena o array no local. Isso significa que o array `filteredBooks` será modificado.
