...
Ora devi gestire il caso in cui l'utente non ha superato il budget (quando `netRemaining` è maggiore o uguale a `0`).

Aggiungi un blocco `else` alla tua istruzione `if` che imposta `statusText` su `"$X Remaining"`, dove `X` è il valore di `netRemaining`. Inoltre, imposta `statusClass` su `"surplus"`.
...
Dovresti usare un template literal per `statusText`.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

Il tuo `statusText` dovrebbe essere assegnato a `$${netRemaining} Remaining`.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

Dovresti assegnare `"surplus"` a `statusClass`.
