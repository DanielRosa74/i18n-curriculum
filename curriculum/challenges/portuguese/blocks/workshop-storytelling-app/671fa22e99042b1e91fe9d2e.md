...
Dentro do `.btn-container`, crie três botões para cada tipo de história. Dê a cada botão uma classe `btn` e um `id` de `scary-btn`, `funny-btn` e `adventure-btn`. O texto de cada botão deve ser `Scary Story`, `Funny Story` e `Adventure Story`, respectivamente.
