...
Sie benötigen noch einen dieser `fieldset` Codeblöcke für `entertainment`.
...
Ihr viertes `fieldset` Element sollte eine `id` mit dem Wert `entertainment` haben.
...
Ihr viertes `legend` Element sollte den Text `Entertainment` enthalten.
