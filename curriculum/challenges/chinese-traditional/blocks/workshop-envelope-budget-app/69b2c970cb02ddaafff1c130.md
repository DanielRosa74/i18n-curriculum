...
現在你需要處理使用者沒有超出預算的情況（當 `netRemaining` 大於或相等於 `0` 時）。

在你的 `if` 述語中添加一個 `else` 區塊，將 `statusText` 設為 `"$X Remaining"`，其中 `X` 是 `netRemaining` 的值。同時，將 `statusClass` 設為 `"surplus"`。
