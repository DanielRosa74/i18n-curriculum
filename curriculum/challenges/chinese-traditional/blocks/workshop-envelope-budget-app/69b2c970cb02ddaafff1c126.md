...
現在宣告一個 `food` 變數，並將呼叫 `getTotalFromInputs` 並傳入你的 `foodInputs` 的結果設值給它。

# --hints--

你的 `calculateBudget` 函式（程式）應該要有一個 `food` 變數。
...
你應該要用 `foodInputs` 作為引數來呼叫 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

你應該將 `getTotalFromInputs` 的結果指定給 `food`。
