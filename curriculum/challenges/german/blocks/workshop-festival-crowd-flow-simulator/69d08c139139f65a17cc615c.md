...
- Durchlaufen Sie jedes Gate im `gates` Array und rufen Sie `handleGateAtTick()` mit diesen Argumenten auf:
  ...
  - `throughputSummary`: Das lokale Objekt, das die insgesamt pro Gate verarbeiteten Teilnehmer verfolgt.
...
Sie sollten nach dem Protokollieren in der Konsole jedes Gate im `gates` Array durchlaufen und `handleGateAtTick()` mit `gates`, `gate`, `tickIndex` und `throughputSummary` aufrufen.
...
Sie sollten `tickIndex` um 1 erhöhen, nachdem Sie das `gates` Array durchlaufen haben.
