...
في دالتك الجديدة، أعلن عن متغير `total` وقم بتعيين القيمة `0` له. استخدم `let` لإعلانه، لأنك ستُعيد تعيينه لاحقًا.

# --hints--

يجب أن تعلن عن متغير `total`.
...
يجب أن يُعيَّن لمتغير `total` القيمة `0`.
...
يجب أن تعلن عن متغير `total` باستخدام `let`.
