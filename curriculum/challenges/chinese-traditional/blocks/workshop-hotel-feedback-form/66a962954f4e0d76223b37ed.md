...
在該 `input` 下面，添加一個 `label` 元素，其文字為 `Location`，並將 `for` 屬性設為 `"location"`。

在該 `label` 元素下方，添加另一個核取方塊 `input`，其 `id` 和 `value` 屬性設為 `"reputation"`，`name` 屬性設為 `"choice"`。

在該 `input` 下面，添加一個 `label` 元素，文字為 `Reputation`，並將 `for` 屬性設為 `"reputation"`。
...
你的第四個核取方塊 `input` 應該有一個 `id` 屬性，設為 `"reputation"`。
