...
<a href="https://rosettacode.org/wiki/Sorting_algorithms/Cocktail_sort" target="_blank" rel="noopener noreferrer nofollow">鸡尾酒排序</a> 是对冒泡排序的改进。给定一个数字数组，鸡尾酒排序会从头到尾遍历数组，将最大的数字移动到末尾。然后，它会反向遍历数组，将最小的数字移动到开头。它重复这两个过程，将下一个最大/最小的数字移动到数组中的正确位置，直到排序完成。
