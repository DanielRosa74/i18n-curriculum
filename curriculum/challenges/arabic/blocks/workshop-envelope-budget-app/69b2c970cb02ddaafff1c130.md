...
الآن عليك التعامل مع الحالة التي لا يكون فيها المستخدم متجاوزًا الميزانية (عندما تكون قيمة `netRemaining` أكبر من أو تساوي `0`).

أضف كتلة `else` إلى جملة `if` الخاصة بك لتعيين `statusText` إلى `"$X Remaining"`، حيث تكون `X` هي قيمة `netRemaining`. كما عيّن `statusClass` إلى `"surplus"`.
...
يجب أن تستخدم قالب نصي لـ `statusText`.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

يجب تعيين `statusText` إلى `$${netRemaining} Remaining`.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

يجب تعيين `"surplus"` إلى `statusClass`.
