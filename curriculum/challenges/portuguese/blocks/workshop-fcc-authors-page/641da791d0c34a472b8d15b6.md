...
Dentro da sua declaração `console.log()`, adicione o texto `"Author Data Array:"` como o primeiro argumento e `authorDataArr` como o segundo argumento. Use uma vírgula para separar o texto de `authorDataArr`.
...
Você deve usar uma vírgula para separar seu texto `"Author Data Array:"` e `authorDataArr`.

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

Seu `authorDataArr` deve ser o segundo argumento da declaração `console.log()`.
