...
حان الوقت لبدء تحضير حساباتك. ابدأ بتعريف متغير `expenses`، وقم بتعيين مجموع `rent` و`food` و`utilities` و`entertainment` له (لاحظ أن الترتيب مهم للاختبارات). تأكد من القيام بذلك بعد جملة if الشرطية.

# --hints--

يجب أن تحتوي الدالة على متغير `expenses`.
...
يجب أن يأتي متغير `expenses` بعد جملة `if` الشرطية.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

يجب أن تحسب مجموع `rent` و`food` و`utilities` و`entertainment` بالترتيب.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

يجب أن تعيّن مجموع `rent` و`food` و`utilities` و`entertainment` إلى `expenses`.
