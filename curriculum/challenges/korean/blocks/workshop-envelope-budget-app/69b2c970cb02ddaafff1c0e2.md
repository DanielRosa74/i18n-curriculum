...
다음으로, `id`가 `food`로 설정된 두 번째 `fieldset` 요소를 만드세요.

그 요소 안에 텍스트가 `Food`인 `legend` 요소와 `class`가 `input-container`로 설정된 빈 `div`를 만드세요.
...
두 번째 `fieldset` 요소는 `id`가 `food`로 설정되어야 합니다.
...
새로운 `legend` 요소는 텍스트가 `Food`여야 합니다.
