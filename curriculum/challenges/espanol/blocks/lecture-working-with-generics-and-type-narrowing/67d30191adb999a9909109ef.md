...
En realidad, ya has visto un tipo genérico en una lección anterior: `Array<string>`. Pero normalmente usarás tipos genéricos en funciones. De hecho, pueden considerarse como un parámetro especial que proporcionas a una función para controlar el comportamiento de la definición de tipo de la función.
...
Fíjate cómo usamos de nuevo la sintaxis de corchetes angulares (`<HTMLInputElement>`), pero esta vez en una llamada a función. Esto le indica a TypeScript que el elemento que esperamos encontrar será un elemento `input`, y ya no obtenemos un error en la propiedad value porque los elementos input sí tienen esa propiedad. Por supuesto, `querySelector` puede devolver `null` si no se encuentra un elemento, y TypeScript lanzará un error de compilación aquí porque no manejamos ese caso.
