...
`fetch` の呼び出しが非同期関数またはモジュールのメインボディ内で定義されていないため、それを修正するまでは `await` を使うことができません。
...
`initialFetch` 変数は非同期関数である必要があります。
...
`initialFetch` 関数はパラメータを取らないようにしてください。
...
`initialFetch` 関数はアロー関数の構文を使う必要があります。
