...
`label` 要素の下に、`type` 属性が `"number"` に設定され、`id` が `"age"` の `input` を追加してください。

`name` 属性は `"age"` に、`min` 属性は `"3"` に、`max` 属性は `"100"` に設定してください。
