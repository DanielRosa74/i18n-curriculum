...
`.image-wrapper` 要素内の3番目の `img` 要素に、`src` の `https://cdn.freecodecamp.org/testable-projects-fcc/images/survey-form-background.jpeg`、`alt` の `four people working on code`、`loading` の `lazy` 属性、`class` に設定された `image-3`、`600` に設定された `width` 属性、`400` に設定された `height` 属性を付けてください。
...
3番目の `img` 要素には `lazy` に設定された `loading` 属性が必要です。
...
3番目の `img` 要素には `600` に設定された `width` 属性が必要です。

```js
assert(document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('width') === '600');
```

3番目の `img` 要素には `400` に設定された `height` 属性が必要です。
