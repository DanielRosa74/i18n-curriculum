...
你在之前的课程中学到，JSX 是 JavaScript 的一种语法扩展，它允许你在 JavaScript 中编写类似 HTML 的代码，从而更容易创建和管理 React 组件。
...
这里使用 `className` 而不是 `class`，因为 `class` 是 JavaScript 中的保留关键字。所以，React 使用 `className` 来为元素应用类。
...
你的 `nav` 元素应该有一个 `className`，值为 `navbar`。
