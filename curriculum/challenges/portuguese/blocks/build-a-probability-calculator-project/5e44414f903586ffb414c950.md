...
Um chapéu sempre será criado com pelo menos uma bola. Os argumentos passados para o objeto chapéu na criação devem ser convertidos em uma variável de instância `contents`. `contents` deve ser uma lista de strings contendo um item para cada bola no chapéu. Cada item na lista deve ser o nome de uma cor representando uma única bola dessa cor. Por exemplo, se seu chapéu for `{'red': 2, 'blue': 1}`, `contents` deve ser `['red', 'red', 'blue']`.
