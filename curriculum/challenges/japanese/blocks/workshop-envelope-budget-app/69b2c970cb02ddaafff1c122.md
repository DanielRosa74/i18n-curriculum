...
`foodInputs` 変数を宣言し、セレクター `#food input[type='number']` を使って `document.querySelectorAll()` を呼び出した結果を代入してください。これにより、`#food` 要素内のすべての `number` 入力が取得されます。

# --hints--

`foodInputs` 変数を宣言してください。
...
`#food input[type='number']` を使って `document.querySelectorAll()` を呼び出してください。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` の呼び出し結果を `foodInputs` に代入してください。
