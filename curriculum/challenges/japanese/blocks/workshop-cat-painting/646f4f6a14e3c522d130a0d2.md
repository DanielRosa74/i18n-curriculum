...
`.cat-whisker-right-middle` セレクターには `absolute` に設定された `position` プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.position, 'absolute');
```

`.cat-whisker-right-middle` セレクターには `127px` に設定された `top` プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-middle')?.top, '127px');
```

`.cat-whisker-right-middle` セレクターには `109px` に設定された `left` プロパティが必要です。
