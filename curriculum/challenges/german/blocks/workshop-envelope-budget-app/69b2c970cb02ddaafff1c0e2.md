...
Erstellen Sie als Nächstes ein zweites `fieldset`-Element mit der `id`, die auf `food` gesetzt ist.

Erstellen Sie innerhalb dieses Elements ein `legend`-Element mit dem Text `Food` und ein leeres `div` mit der `class`, die auf `input-container` gesetzt ist.
...
Ihr zweites `fieldset`-Element sollte eine `id` mit dem Wert `food` haben.
...
Ihr neues `legend`-Element sollte den Text `Food` enthalten.
