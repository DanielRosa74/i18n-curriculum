...
在你的循环完成后，你需要清空 `incomeInput` 和 `rentInput` 字段。将 `incomeInput` 和 `rentInput` 的 `value` 属性设置为空字串。

# --hints--

你的 `clearForm` 函数应该访问 `incomeInput` 元素的 `value` 属性。

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

你的 `clearForm` 函数应该将 `incomeInput` 元素的 `value` 属性设置为空字串。

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

你应该在 `for` 循环之后修改 `incomeInput`。
...
你的 `clearForm` 函数应该访问 `rentInput` 元素的 `value` 属性。

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

你的 `clearForm` 函数应该将 `rentInput` 元素的 `value` 属性设置为空字串。

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

你应该在 `for` 循环之后修改 `rentInput`。
