...
你需要知道这笔开销属于哪个类别。幸运的是，你为用户添加了一个下拉菜单来选择类别。

请记住，你之前在 JavaScript 中查询了该下拉菜单并将其分配给 `entryDropdown` 变量。 你可以使用 <dfn>value</dfn> 属性来获取所选选项的值。

将 `entryDropdown` 的 `value` 属性赋值给一个名为 `category` 的变量。


# --hints--

你的 `addEntry` 函数应该有一个 `category` 变量。
...
你的 `category` 变量应该赋值为 `entryDropdown.value` 的值。
