...
首先取得 `form` 元素（使用 `id`），並將它存放在名為 `budgetForm` 的變數中。

# --hints--

你應該創建一個名為 `budgetForm` 的變數。

```js
assert.isDefined(budgetForm);
```

你的程式碼中應該包含 `document.getElementById('budget-form')`。

```js
assert.match(code, /document\.getElementById\(\s*('|")budget-form\1\s*\)/g);
```

你應該將 `document.getElementById('budget-form')` 指定給你的 `budgetForm` 變數。
