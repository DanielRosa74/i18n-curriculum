...
今、`food` 変数を宣言して、`foodInputs` を使って `getTotalFromInputs` を呼び出した値を代入してください。

# --hints--

`calculateBudget` 関数には `food` 変数が必要です。
...
`foodInputs` を引数として `getTotalFromInputs` を呼び出してください。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

`getTotalFromInputs` の結果を `food` に代入してください。
