...
체크박스에는 여전히 값이 `loving`인 `id` 속성이 있어야 합니다. `--fcc-expected--`가 예상되었으나 `--fcc-actual--`가 발견되었습니다.
...
`label` 요소에는 값이 `loving`인 `for` 속성이 있어야 합니다. `--fcc-expected--`가 예상되었으나 `--fcc-actual--`가 발견되었습니다.
