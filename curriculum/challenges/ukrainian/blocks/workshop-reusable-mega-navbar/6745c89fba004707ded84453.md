...
Створіть `a` елемент всередині вашого першого `li` елемента. Додайте йому атрибут `href` зі значенням `#` та текст `Dashboard`.

# --hints--

Ви повинні створити `a` елемент всередині вашого першого `li` елемента.

```js
assert.exists(document.querySelector('li a'));
```

Ваш `a` елемент має мати атрибут `href` зі значенням `#`.
