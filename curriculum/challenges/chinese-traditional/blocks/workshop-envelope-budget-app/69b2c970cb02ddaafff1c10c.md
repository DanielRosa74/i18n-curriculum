...
為你的 `label` 元素添加一個 `for` 屬性，值為 `X-#-name`，其中 `X` 是 `category` 變數的 `value`，`#` 是 `entryNumber` 的值。請記得 HTML 屬性應該用雙引號包起來。

# --hints--

你的 `label` 標籤應該有一個 `for` 屬性，設定為 `${category}-${entryNumber}-name`。

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

你的 `label` 標籤應該包含文字 `"Expense ${entryNumber} Name"`。
