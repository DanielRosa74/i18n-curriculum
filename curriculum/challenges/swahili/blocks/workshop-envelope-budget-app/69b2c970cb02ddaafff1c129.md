...
Kitendakazi chako cha `getTotalFromInputs` kitaweka bendera ya makosa ya ulimwengu `isError` kuwa `true` ikiwa ingizo batili itatambuliwa.

Ongeza usemi wa `if` katika kitendakazi chako cha `calculateBudget` unaokagua ukweli wa bendera yako ya makosa ya ulimwengu, na ikiwa ni kweli tumia `return` kumaliza utekelezaji wa kitendakazi.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na usemi wa `if` unaokagua ukweli wa kigezo `isError`.
...
Usemi wako wa `if` unapaswa kuwekwa baada ya wito wa mwisho wa kitendakazi cha `getTotalFromInputs`.
