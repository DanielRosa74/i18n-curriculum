...
次のステップは、ユーザーのエントリーから合計金額を取得する関数を書くことです。

`getTotalFromInputs` 関数を宣言し、`list` というパラメータを与えてください。

# --hints--

`getTotalFromInputs` という変数を宣言してください。

```js
assert.isDefined(getTotalFromInputs);
```

`getTotalFromInputs` 変数は関数である必要があります。

```js
assert.isFunction(getTotalFromInputs);
```

`getTotalFromInputs` 関数は `list` というパラメータを受け取る必要があります。
