...
حان الوقت لنقل بعض الكود من الدالة `move()` إلى الدالة `make_allowed_move()`. انقل الكود المسكن داخل أول جملة `if` (باستثناء أول استدعاء لـ `print()`) إلى دالتك الجديدة. انتبه جيدًا للمسافة البادئة.
لا تنسَ إزالة الكلمة المفتاحية `pass`.

# --hints--

يجب أن تنقل الكود المسكن داخل أول جملة `if` (باستثناء أول استدعاء لـ `print()`) إلى دالتك الجديدة.
