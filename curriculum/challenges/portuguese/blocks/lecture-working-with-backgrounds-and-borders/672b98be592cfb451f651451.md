...
Este CSS cria um gradiente linear que faz a transição do `red` na `left` para o `yellow` na `right`. O gradiente é aplicado a um elemento com altura de `40%` da altura do viewport. Você vai aprender mais sobre unidades `vh` em uma lição futura.
