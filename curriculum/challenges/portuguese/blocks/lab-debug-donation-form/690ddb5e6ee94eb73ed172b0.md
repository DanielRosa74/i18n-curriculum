...
Seu primeiro elemento `input` deve ter um atributo `name` com o valor `name`.

```js
assert.exists(document.querySelector("input:nth-of-type(1)[name='name']"));
```

Seu segundo elemento `input` deve ter um atributo `name` com o valor `email`.

```js
assert.exists(document.querySelector("input:nth-of-type(2)[name='email']"));
```

Seu terceiro elemento `input` deve ter um atributo `name` com o valor `amount`.

```js
assert.exists(document.querySelector("input:nth-of-type(3)[name='amount']"));
```

Seu quarto elemento `input` deve ter um atributo `name` com o valor `newsletter`.

```js
assert.exists(document.querySelector("input:nth-of-type(4)[name='newsletter']"));
```

Seu quinto elemento `input` deve ter um atributo `type` com o valor `submit`.
