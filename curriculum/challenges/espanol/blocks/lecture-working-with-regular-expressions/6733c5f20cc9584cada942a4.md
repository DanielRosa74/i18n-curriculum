...
Ahora podemos actualizar nuestra referencia en la expresión regular para referirnos a este grupo. Una referencia nombrada comienza con una barra invertida seguida de la letra `k` en JavaScript. Luego agregas el nombre, nuevamente entre signos de menor (`<`) y mayor (`>`). Veamos eso:
