...
Neste exemplo, `sayHello` é um método do `person` objeto. A palavra-chave `this` permite que o método `sayHello` acesse as propriedades do objeto chamado `person`. Você aprenderá mais sobre a palavra-chave `this` em lições futuras.
