...
Ahora declara una variable `netRemaining` y asígnale el valor de restar `expenses` de `income`.

# --hints--

Tu función `calculateBudget` debe tener una variable `netRemaining`.
...
Debes calcular el valor de `income - expenses`.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

Debes asignar el valor de `income - expenses` a `netRemaining`.
