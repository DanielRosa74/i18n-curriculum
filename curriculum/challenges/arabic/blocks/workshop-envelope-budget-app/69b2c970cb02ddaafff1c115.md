...
خطوتك التالية هي كتابة دالة تستخرج المبالغ الإجمالية من مدخلات المستخدم.

أعلن عن دالة `getTotalFromInputs`، وأعطها معلمة تسمى `list`.

# --hints--

يجب أن تعلن عن متغير `getTotalFromInputs`.

```js
assert.isDefined(getTotalFromInputs);
```

يجب أن يكون متغير `getTotalFromInputs` دالة.

```js
assert.isFunction(getTotalFromInputs);
```

يجب أن تأخذ دالة `getTotalFromInputs` معلمة تسمى `list`.
