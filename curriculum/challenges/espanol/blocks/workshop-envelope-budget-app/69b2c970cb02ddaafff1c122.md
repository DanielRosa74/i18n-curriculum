...
Declara una variable `foodInputs` y asígnale el valor de llamar a `document.querySelectorAll()` con el selector `#food input[type='number']`. Esto devolverá cualquier entrada `number` que esté en el elemento `#food`.

# --hints--

Debes declarar una variable `foodInputs`.
...
Debes llamar a `document.querySelectorAll()` con `#food input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada a `document.querySelectorAll()` a `foodInputs`.
