...
كما ترى، أصبح المجموع الآن فارغًا تمامًا. احذف كل من استدعائي `clear` و `has` من كودك.

# --hints--

يجب ألا يحتوي كودك على `plantsSet.clear()`.

```js
assert.notMatch(__helpers.removeJSComments(code), /plantsSet\s*\.\s*clear\s*\(\s*\)/);
```

يجب ألا يحتوي كودك على `plantsSet.has(ballerina)`.
