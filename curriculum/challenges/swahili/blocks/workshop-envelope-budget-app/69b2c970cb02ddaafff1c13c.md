...
Baada ya mzunguko wako kukamilika, unahitaji kufuta sehemu za `incomeInput` na `rentInput`. Weka sifa ya `value` ya `incomeInput` na `rentInput` kuwa mfuatano wa herufi tupu.

# --hints--

Kitendakazi chako cha `clearForm` kinapaswa kupata upatikanaji wa sifa ya `value` ya kipengele cha `incomeInput`.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

Kitendakazi chako cha `clearForm` kinapaswa kuweka sifa ya `value` ya kipengele cha `incomeInput` kuwa mfuatano wa herufi tupu.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

Unapaswa kubadilisha `incomeInput` baada ya mzunguko wako wa `for`.
...
Kitendakazi chako cha `clearForm` kinapaswa kupata upatikanaji wa sifa ya `value` ya kipengele cha `rentInput`.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

Kitendakazi chako cha `clearForm` kinapaswa kuweka sifa ya `value` ya kipengele cha `rentInput` kuwa mfuatano wa herufi tupu.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

Unapaswa kubadilisha `rentInput` baada ya mzunguko wako wa `for`.
