...
- `gates` 配列の各ゲートをループし、次の引数で `handleGateAtTick()` を呼び出してください：
  ...
  - `throughputSummary`: 各ゲートで処理された合計参加者数を追跡するローカルオブジェクト。
- 次のティックに進むために `tickIndex` を1増やしてください。

# --hints--

次のメッセージをコンソールにログ出力してください：`"\nTick " + (tickIndex + 1)`。
...
コンソールにログ出力した後、`gates` 配列の各ゲートをループし、`gates`、`gate`、`tickIndex`、`throughputSummary` を引数にして `handleGateAtTick()` を呼び出してください。
...
`gates` 配列のループの後に `tickIndex` を1増やしてください。
