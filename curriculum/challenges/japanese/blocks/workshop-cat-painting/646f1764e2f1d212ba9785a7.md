...
`.cat-whisker-left-bottom`セレクターには`absolute`に設定された`position`プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.position, 'absolute');
```

`.cat-whisker-left-bottom`セレクターには`134px`に設定された`top`プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-left-bottom')?.top, '134px');
```

`.cat-whisker-left-bottom`セレクターには`52px`に設定された`left`プロパティが必要です。
