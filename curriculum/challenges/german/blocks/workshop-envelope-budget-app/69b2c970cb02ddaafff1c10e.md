...
Erstellen Sie ein weiteres `label`-Element (in einer neuen Zeile) am Ende Ihres `HTMLString`. Dieses `label` sollte den Text `Expense # Amount` enthalten, wobei Sie Ihre Template-Literal-Syntax verwenden, um `#` durch den Wert von `entryNumber` zu ersetzen, und das `for`-Attribut auf `X-#-amount` setzen, wobei `X` der Wert von `category` und `#` der Wert von `entryNumber` ist.
...
Ihr neues `label`-Element sollte ein `for`-Attribut mit dem Wert `${category}-${entryNumber}-amount` haben.
...
Ihr neues `label`-Element sollte den Text `Expense ${entryNumber} Amount` haben.
