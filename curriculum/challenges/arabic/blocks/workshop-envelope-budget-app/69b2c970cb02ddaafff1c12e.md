...
داخل جملة `if`، عليك تعيين `statusText` لعرض مقدار تجاوز المستخدم للميزانية.

عيّن `statusText` إلى نص قالب يعرض `"Over Budget by $X"`، حيث `X` هو القيمة المطلقة لـ `netRemaining`. استخدم `Math.abs()` للحصول على القيمة المطلقة.

# --hints--

يجب تعيين قيمة لـ `statusText` داخل جملة `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*statusText\s*=/);
```

يجب أن يستخدم `statusText` نص قالب.

```js
assert.match(code, /statusText\s*=\s*`/);
```

يجب أن يحتوي نص القالب على النص "Over Budget by $."

```js
assert.match(code, /statusText\s*=\s*`Over\s+Budget\s+by\s+\$\$\{/);
```

يجب استخدام `Math.abs(netRemaining)` للحصول على القيمة المطلقة.
