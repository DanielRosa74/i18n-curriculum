...
如果我们在 JavaScript 代码中使用 `getElementById()` 获取对该 HTML 元素的引用，就可以访问该元素的 `innerText` 属性：
...
`innerText` 和 `textContent` 之间最重要的区别是，`textContent` 始终返回一个 HTML 元素及其子元素的完整文本内容，无论其是可见还是隐藏。
...
如果一个 HTML 元素不可见，就像你在这里看到的，我们隐藏了第二个段落，它的文本仍然会包含在这个属性中：
...
JavaScript 中的 `innerText`、`textContent` 和 `innerHTML` 属性提供了不同的方式来访问和操作 HTML 元素的内容。如果你想有效地在 JavaScript 中处理 HTML 内容，你应该了解这些属性之间的区别。

# --questions--

## --text--

以下哪个属性返回 HTML 元素的可见文本内容，不包括任何隐藏文本或隐藏元素内的文本？
