...
지출이 어느 카테고리에 속하는지 알아야 합니다. 다행히도 사용자가 카테고리를 선택할 수 있도록 드롭다운을 추가했어요.

이전에 JavaScript에서 해당 드롭다운을 조회하고 `entryDropdown` 변수에 할당한 것을 기억하세요. 선택된 옵션의 값을 가져오려면 <dfn>value</dfn> 속성을 사용할 수 있습니다.

`entryDropdown`의 `value` 속성을 `category`라는 변수에 할당하세요.


# --hints--

`addEntry` 함수에 `category` 변수가 있어야 합니다.
...
`category` 변수에 `entryDropdown.value` 값을 할당해야 합니다.
