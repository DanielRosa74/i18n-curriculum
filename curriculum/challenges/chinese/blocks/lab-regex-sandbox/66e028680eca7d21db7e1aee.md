...
当 `stringToTest` 的内部 HTML 为 `Gu1n34 P1g5`，且 `regexPattern` 的值为 `\d+`，并且选中了全局标记时，点击 `testButton` 按钮后，`stringToTest.innerHTML` 应变为 `Gu<span class="highlight">1</span>n<span class="highlight">34</span> P<span class="highlight">1</span>g<span class="highlight">5</span>`。
...
当 `stringToTest` 的内部 HTML 为 `Gu1n34 P1g5`，且 `regexPattern` 的值为 `G`，同时全局和不区分分支的标记都被选中时，点击 `testButton` 按钮后，`stringToTest.innerHTML` 应变为 `<span class="highlight">G</span>u1n34 P1<span class="highlight">g</span>5`。
