...
Ifuatayo, tengeneza kipengele cha pili cha `fieldset` chenye `id` iliyowekwa kuwa `food`.

Ndani ya kipengele hicho, tengeneza kipengele cha `legend` chenye maandishi `Food`, na `div` tupu yenye `class` iliyowekwa kuwa `input-container`.
...
Kipengele chako cha pili cha `fieldset` kinapaswa kuwa na `id` iliyowekwa kuwa `food`.
...
Kipengele chako kipya cha `legend` kinapaswa kuwa na maandishi `Food`.
