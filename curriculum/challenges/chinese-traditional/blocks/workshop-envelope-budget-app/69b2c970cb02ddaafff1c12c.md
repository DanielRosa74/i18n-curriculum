...
你需要知道使用者是超出預算還是有剩餘金額。當支出超過收入時，你就是超出預算（赤字）。當收入超過支出時，你有剩餘金額（盈餘）。剩餘正好為零時，可以視為收支平衡。

宣告兩個變數：`statusText` 和 `statusClass`。將兩者都初始化為空字串，使用 `""`。

# --hints--

你的 `calculateBudget` 函式（程式）應該有一個 `statusText` 變數。
...
你的 `calculateBudget` 函式（程式）應該有一個 `statusClass` 變數。
...
你應該將 `statusText` 和 `statusClass` 初始化為空字串。
