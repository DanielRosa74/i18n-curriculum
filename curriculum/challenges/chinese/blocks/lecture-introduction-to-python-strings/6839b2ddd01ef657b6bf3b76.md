...
方法是你对一个值调用的函数。要调用字串方法，写出字串或其变量名，后面跟一个点和方法调用。当你学习类和对象时，会了解更多关于方法的内容。以下是一些常见的字串方法：
...
- `split(separator)`：根据指定的分隔符将字串拆分成一个字串列表。列表是用方括弧括起来的值的集合。如果未指定分隔符，`split()` 会根据空白字符拆分。
