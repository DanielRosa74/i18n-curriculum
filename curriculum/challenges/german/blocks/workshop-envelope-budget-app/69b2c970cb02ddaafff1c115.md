...
Ihr nächster Schritt ist es, eine Funktion zu schreiben, die die Gesamtbeträge aus den Einträgen des Benutzers ermittelt.

Deklarieren Sie eine `getTotalFromInputs`-Funktion und geben Sie ihr einen Parameter namens `list`.

# --hints--

Sie sollten eine `getTotalFromInputs`-Variable deklarieren.

```js
assert.isDefined(getTotalFromInputs);
```

Ihre `getTotalFromInputs`-Variable sollte eine Funktion sein.

```js
assert.isFunction(getTotalFromInputs);
```

Ihre `getTotalFromInputs`-Funktion sollte einen Parameter namens `list` annehmen.
