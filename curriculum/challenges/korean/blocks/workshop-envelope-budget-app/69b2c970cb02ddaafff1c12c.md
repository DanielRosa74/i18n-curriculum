...
사용자가 예산을 초과했는지 아니면 남은 돈이 있는지 알아야 합니다. 지출이 수입을 초과하면 예산을 초과한 상태(적자)입니다. 수입이 지출을 초과하면 남은 돈이 있는 상태(흑자)입니다. 정확히 0이 남으면 균형 상태로 볼 수 있습니다.

`statusText`와 `statusClass` 두 변수를 선언하세요. 둘 다 `""`를 사용해 빈 문자열로 초기화하세요.

# --hints--

`calculateBudget` 함수에 `statusText` 변수가 있어야 합니다.
...
`calculateBudget` 함수에 `statusClass` 변수가 있어야 합니다.
...
`statusText`와 `statusClass`를 빈 문자열로 초기화해야 합니다.
