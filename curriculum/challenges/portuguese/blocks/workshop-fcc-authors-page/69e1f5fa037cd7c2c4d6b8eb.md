...
Seu `catch` não deve mais ser um método encadeado.

```js
assert.notMatch(code, /\.catch/);
```

Seu bloco `try` deve conter as três declarações que buscam os dados, criam o `json` e depois exibem.
...
Sua página de autores do fCC deve renderizar corretamente.
