...
Тепер оголосіть змінну `food` і призначте їй значення виклику `getTotalFromInputs` з вашими `foodInputs`.

# --hints--

У вашій функції `calculateBudget` має бути змінна `food`.
...
Ви повинні викликати `getTotalFromInputs`, передаючи `foodInputs` як аргумент.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

Ви повинні призначити результат `getTotalFromInputs` змінній `food`.
