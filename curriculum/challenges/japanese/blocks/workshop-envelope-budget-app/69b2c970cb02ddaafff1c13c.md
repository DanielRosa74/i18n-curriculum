...
ループが完了したら、`incomeInput` と `rentInput` のフィールドをクリアする必要があります。`incomeInput` と `rentInput` の `value` プロパティを空の文字列に設定してください。

# --hints--

あなたの `clearForm` 関数は `incomeInput` 要素の `value` プロパティにアクセスする必要があります。

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

あなたの `clearForm` 関数は `incomeInput` 要素の `value` プロパティを空の文字列に設定する必要があります。

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

`for` ループの後に `incomeInput` を変更してください。
...
あなたの `clearForm` 関数は `rentInput` 要素の `value` プロパティにアクセスする必要があります。

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

あなたの `clearForm` 関数は `rentInput` 要素の `value` プロパティを空の文字列に設定する必要があります。

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

`for` ループの後に `rentInput` を変更してください。
