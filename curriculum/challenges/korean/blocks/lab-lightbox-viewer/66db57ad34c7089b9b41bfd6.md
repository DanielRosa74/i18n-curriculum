...
`.gallery` 요소 안에 `gallery-item` 클래스를 가진 세 개의 `img` 요소가 있어야 합니다.

```js
assert.lengthOf(document.querySelectorAll(".gallery .gallery-item"), 3)
```

`.gallery` 요소 안에 `https://cdn.freecodecamp.org/curriculum/labs/stonehenge-thumbnail.jpg`로 설정된 `src`를 가진 `img` 요소가 있어야 합니다.
...
`.gallery` 요소 안에 `https://cdn.freecodecamp.org/curriculum/labs/storm-thumbnail.jpg`로 설정된 `src`를 가진 `img` 요소가 있어야 합니다.
...
`.gallery` 요소 안에 `https://cdn.freecodecamp.org/curriculum/labs/trees-thumbnail.jpg`로 설정된 `src`를 가진 `img` 요소가 있어야 합니다.
...
`.lightbox` 요소 안에 `close-btn`로 설정된 `id`를 가진 `button` 요소가 있어야 합니다.

```js
assert.equal(document.querySelector(".lightbox #close-btn")?.tagName, "BUTTON");
```

`.lightbox` 요소 안에 `lightbox-image`로 설정된 `id`를 가진 `img` 요소가 있어야 합니다.
