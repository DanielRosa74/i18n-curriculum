...
在 `ul` 内，创建两个列表项。在第一个列表项内，创建一个 `span` 元素并将 HTML 实体 `&#10003;` 作为其文本添加。紧接着该 `span` 但仍在列表项内，添加文本 `Ad-supported streaming`。
...
你的第一个 `li` 元素应包含一个带有 HTML 字符实体 `&#10003;` 的 `span` 元素。
...
你的第二个 `li` 元素应包含一个带有 HTML 字符实体 `&#10003;` 的 `span` 元素。
