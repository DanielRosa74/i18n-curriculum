...
이제 사용자가 예산을 초과했는지 아니면 예산 내에 있는지에 따라 적절한 상태를 결정해야 합니다.

`netRemaining`이 `0`보다 작은지 확인하는 `if` 문을 만들어 시작하세요.

# --hints--

`calculateBudget` 함수에는 `if` 문이 있어야 합니다.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

`if` 문은 `netRemaining`이 `0`보다 작은지 확인해야 합니다.
