...
Luego, crea un segundo elemento `fieldset` con el `id` establecido en `food`.

Dentro de ese elemento, crea un elemento `legend` con el texto `Food` y un `div` vacío con la `class` establecida en `input-container`.
...
Tu segundo elemento `fieldset` debe tener un `id` establecido en `food`.
...
Tu nuevo elemento `legend` debe tener el texto `Food`.
