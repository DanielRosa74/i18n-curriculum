...
Deklarieren Sie in Ihrer neuen Funktion eine `total`-Variable und weisen Sie ihr den Wert `0` zu. Verwenden Sie `let` zur Deklaration, da Sie ihr später einen neuen Wert zuweisen werden.

# --hints--

Sie sollten eine `total`-Variable deklarieren.
...
Ihrer `total`-Variable sollte der Wert `0` zugewiesen sein.
...
Sie sollten Ihre `total`-Variable mit `let` deklarieren.
