...
Sie müssen auch den Wert Ihrer `#income` Eingabe erhalten. Sie haben diese bereits am Anfang Ihres Codes abgefragt und der Variablen `incomeInput` zugewiesen. Allerdings haben Sie `getElementById` verwendet, das ein `Element` zurückgibt, nicht eine `NodeList`.

Eine `NodeList` ist ein array-ähnliches Objekt, was bedeutet, dass Sie es durchlaufen können und es einige gemeinsame Methoden mit einem Array teilt. Für Ihre `getTotalFromInputs` Funktion funktioniert ein Array als Argument genauso gut wie eine `NodeList`.

Deklarieren Sie eine `income` Variable und setzen Sie sie auf das Ergebnis des Aufrufs von `getTotalFromInputs` – übergeben Sie ein Array, das Ihre `incomeInput` enthält, als Argument.

# --hints--

Ihre `calculateBudget` Funktion sollte eine `income` Variable haben.
...
Sie sollten `getTotalFromInputs` mit `[incomeInput]` als Argument aufrufen.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

Sie sollten das Ergebnis von `getTotalFromInputs` der Variable `income` zuweisen.
