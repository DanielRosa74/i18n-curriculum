...
Neste laboratório, você vai construir uma função que converte uma representação de lista de adjacência de um grafo em uma matriz de adjacência. Uma lista de adjacência é um objeto onde cada chave representa um nó, e o valor correspondente é um array de nós aos quais o nó chave está conectado. Uma matriz de adjacência é um array 2D onde a entrada na posição `[i][j]` é `1` se existe uma aresta do nó `i` para o nó `j`, e `0` caso contrário.
