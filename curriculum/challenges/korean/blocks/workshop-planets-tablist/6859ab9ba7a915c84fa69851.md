...
두 번째 `tabpanel` 요소에 값이 `"panel-saturn"`인 `id` 속성과 값이 `"tab-saturn"`인 `aria-labelledby` 속성을 추가하세요.

세 번째 `tabpanel` 요소에 값이 `"panel-mars"`인 `id` 속성과 값이 `"tab-mars"`인 `aria-labelledby` 속성을 추가하세요.
