...
Em seguida, crie um segundo elemento `fieldset` com o `id` definido como `food`.

Dentro desse elemento, crie um elemento `legend` com o texto `Food` e um `div` vazio com a `class` definida como `input-container`.
...
Seu segundo elemento `fieldset` deve ter um `id` definido como `food`.
...
Seu novo elemento `legend` deve ter o texto `Food`.
