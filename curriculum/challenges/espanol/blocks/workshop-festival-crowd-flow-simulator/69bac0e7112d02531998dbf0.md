...
Tu función `handleGateAtTick` debe primero registrar el mensaje `"Overflow of " + result.overflow + " attendees. Rerouting..."` cuando exista desbordamiento (`result.overflow > 0`).
...
Tu función `handleGateAtTick` debe redirigir el desbordamiento llamando a `rerouteOverflow()` con `gates`, `gate`, `tickIndex` y `result.overflow` cuando exista desbordamiento (`result.overflow > 0`).
