...
Mfuatano wako wa `output.innerHTML` utahitaji kipengele cha `span`. Unda hicho, na kipa sifa ya kipengele ya darasa la vitu iliyowekwa kwa kigezo cha `statusClass`.

Ndani ya kipengele chako cha `span`, ongeza kigezo cha `statusText` kama maudhui ya maandishi.
...
`span` yako inapaswa kuwa na sifa ya kipengele ya `class` iliyowekwa kwa `${statusClass}`.
...
Kipengele chako cha `span` kinapaswa kuwa na `${statusText}` kama maudhui ya maandishi.
