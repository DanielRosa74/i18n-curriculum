...
当用户点击一个表单控件或使用键盘的 <kbd>Tab</kbd> 键选择它时，这意味着它处于 `focused` 状态。当一个输入处于 `focused` 状态时，大多数浏览器会在输入周围显示蓝色高亮边框。但你可以选择在 CSS 中添加额外的样式。

点击预览窗口中任何空白区域，然后按 <kbd>Tab</kbd> 键以查看焦点状态。要查看预览，你需要启用交互式编辑器。
...
另一种形式状态是 `readonly` 状态。这是指像输入这样的控件不能被用户编辑。这里有一个将电子邮件输入设置为只读的示例。`value` 属性用于设置输入字段中显示的值。
...
当用户点击一个输入或使用 <kbd>Tab</kbd> 键选择它以聚焦时。
