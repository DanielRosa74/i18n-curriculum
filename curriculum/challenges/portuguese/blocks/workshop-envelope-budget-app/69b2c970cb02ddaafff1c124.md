...
Seguindo o mesmo padrão, faça uma consulta para seus inputs do tipo `number` no elemento `#entertainment` e atribua-os a `entertainmentInputs`.

# --hints--

Você deve declarar uma variável `entertainmentInputs`.
...
Você deve fazer uma chamada a `document.querySelectorAll()` com `#entertainment input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Você deve atribuir o resultado da sua chamada `document.querySelectorAll()` a `entertainmentInputs`.
