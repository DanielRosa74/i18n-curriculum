...
Dentro da sua declaração `if`, você precisa atribuir um valor a `statusText` para mostrar o quanto o usuário está estourando o orçamento.

Atribua a `statusText` um template literal que exiba `"Over Budget by $X"`, onde `X` é o valor absoluto de `netRemaining`. Use `Math.abs()` para obter o valor absoluto.

# --hints--

Você deve atribuir um valor a `statusText` dentro da declaração `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*statusText\s*=/);
```

Seu `statusText` deve usar um template literal.

```js
assert.match(code, /statusText\s*=\s*`/);
```

Seu template literal deve conter o texto "Over Budget by $."

```js
assert.match(code, /statusText\s*=\s*`Over\s+Budget\s+by\s+\$\$\{/);
```

Você deve usar `Math.abs(netRemaining)` para obter o valor absoluto.
