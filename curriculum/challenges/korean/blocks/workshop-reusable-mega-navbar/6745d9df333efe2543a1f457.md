...
세 번째 `li` 요소 안에 계속해서 팝업용 `ul` 요소를 만드세요. `className`을 `sub-menu`로, `aria-label`을 `Apps`로 지정하세요.
...
만든 `ul` 요소는 `className`이 `sub-menu`여야 합니다.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

만든 `ul` 요소는 `aria-label`이 `Apps`여야 합니다.
