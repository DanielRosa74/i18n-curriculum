---
id: 5f35e5c4321f818cdc4bed30
title: Passo 25
challengeType: 0
dashedName: step-25
---

# --description--

Agora que as coisas estão boas, é hora de começar a adicionar alguns itens do menu.

Adicione um `article` vazio abaixo do título `Coffee`. Ele vai conter o sabor e o preço de um dos cafés que você oferece atualmente.

# --hints--

Você deve ter um elemento `article`.

```js
assert.exists(document.querySelector('article'));
```

Você deve ter uma tag de fechamento `</article>`.

```js
assert.match(code, /<\/article>/i);
```

O elemento preexistente `h2` deve permanecer o mesmo.

```js
assert.lengthOf(document.querySelectorAll('h2'), 1);
```

O elemento `article` deve vir depois do elemento `h2`.

```js
const article = document.querySelector('article');
assert.equal(article?.previousElementSibling?.tagName, 'H2');
```

# --seed--

## --seed-contents--

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Cafe Menu</title>
    <link href="styles.css" rel="stylesheet" />
  </head>
  <body>
    <div class="menu">
      <main>
        <h1>CAMPER CAFE</h1>
        <p>Est. 2020</p>
        <section>
          <h2>Coffee</h2>
--fcc-editable-region--
          
--fcc-editable-region--
        </section>
      </main>
    </div>
  </body>
</html>
```

```css
body {
  background-image: url(https://cdn.freecodecamp.org/curriculum/css-cafe/beans.jpg);
}

h1, h2, p {
  text-align: center;
}

.menu {
  width: 80%;
  background-color: burlywood;
  margin-left: auto;
  margin-right: auto;
}
```

