...
Vamos a crear una API simple creando una ruta que responda con JSON en la ruta `/json`. Puedes hacerlo como de costumbre, con el método `app.get()`. Dentro del manejador de la ruta, usa el método `res.json()`, pasando un objeto como argumento. Este método cierra el ciclo de solicitud-respuesta, devolviendo los datos. Detrás de escena, convierte un objeto JavaScript válido en una cadena, luego establece los encabezados apropiados para indicarle a tu navegador que estás sirviendo JSON y envía los datos de vuelta. Un objeto válido tiene la estructura habitual `{key: data}`. `data` puede ser un número, una cadena, un objeto anidado o un arreglo. `data` también puede ser una variable o el resultado de una llamada a función, en cuyo caso se evaluará antes de convertirse en cadena.
