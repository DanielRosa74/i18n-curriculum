...
在你的第三个 `li` 元素内，创建一个带有 `aria-expanded` 属性且值为 `false` 的 `button` 元素。同时，将该 `button` 的文本设置为 `Apps`。

`aria-expanded` 属性会向屏幕阅读器用户传达该按钮触发一个可展开的子菜单。

# --hints--

你应该在你的第三个 `li` 元素内创建一个 `button` 元素。

```js
assert.exists(document.querySelector('li:nth-of-type(3) button'));
```

你的 `button` 元素应具有 `aria-expanded` 属性，且值为 `false`。
