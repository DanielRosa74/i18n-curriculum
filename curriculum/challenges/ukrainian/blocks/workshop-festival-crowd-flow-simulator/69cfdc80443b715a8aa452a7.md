...
- Використовуйте цикл `for`, щоб ітерувати по кожних воротах у вхідному масиві `gates`. Для кожних воріт додайте властивість до `summary`, де:
  - Ключем є `id` воріт.
  - Значенням є `0`.
...
Ваша функція `initializeThroughput` має повертати `{}` при виклику з `[]`, не змінюючи вхідний масив.
...
Ваша функція `initializeThroughput` має повертати `{ North: 0 }` при виклику з `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]`, не змінюючи вхідний масив.
...
Ваша функція `initializeThroughput` має повертати `{ North: 0, East: 0, South: 0 }` при виклику з `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]`, не змінюючи вхідний масив.
...
Ваша функція `initializeThroughput` має повертати `summary` після циклу, як фінальну лінію у функції.
