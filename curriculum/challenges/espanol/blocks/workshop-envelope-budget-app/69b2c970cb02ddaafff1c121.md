...
Agrega una línea a tu función `calculateBudget` que llame al método `preventDefault()` sobre el parámetro `e`. Luego, reinicia tu bandera global de error `isError` a `false`.

# --hints--

Tu función `calculateBudget` debe llamar a `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

Tu función `calculateBudget` debe reiniciar la bandera global de error `isError` a `false`.
