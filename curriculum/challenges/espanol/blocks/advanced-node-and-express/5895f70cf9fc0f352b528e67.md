...
No estás cargando un objeto de usuario real porque la base de datos no está configurada. Conéctate a la base de datos una vez, cuando inicies el servidor, y mantén una conexión persistente durante todo el ciclo de vida de la app. Para hacer esto, agrega la cadena de conexión de tu base de datos (por ejemplo: `mongodb+srv://<username>:<password>@cluster0-jvwxi.mongodb.net/?retryWrites=true&w=majority`) a la variable de entorno `MONGO_URI`. Esto se usa en el archivo `connection.js`.
