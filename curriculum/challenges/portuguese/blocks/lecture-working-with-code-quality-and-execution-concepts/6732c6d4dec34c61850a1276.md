...
Neste código, mesmo depois que `createLargeArray` termina de executar, `largeArray` não pode ser coletado pelo garbage collector porque a função retornada ainda tem acesso a ele. Isso é um closure, e embora closures sejam úteis, às vezes podem levar a um uso de memória maior do que você poderia esperar. Você vai aprender mais sobre closures em lições futuras.
