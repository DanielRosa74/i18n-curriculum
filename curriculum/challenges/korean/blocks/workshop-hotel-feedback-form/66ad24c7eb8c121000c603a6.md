...
체크박스 `input`을 기본적으로 선택된 상태로 만들려면 `checked` 속성을 추가하세요.
...
기본적으로 선택된 상태로 만들려면 `id`가 `"reputation"`인 체크박스 `input`에 `checked` 속성을 추가하세요.

# --hints--

`id`가 `"reputation"`인 체크박스 `input`에 `checked` 속성이 있어야 합니다.
