...
`hasOwnProperty` 是定义在 `Object.prototype` 上的一个方法，尽管在 `Bird.prototype` 和 `duck` 上并没有定义该方法，但是我们依然可以在这两个对象上访问到。 这就是 `prototype` 链的一个例子。 在这个 `prototype` 链中，`Bird` 是 `duck` 的 `supertype`，而 `duck` 是 `subtype`。 `Object` 是 `Bird` 和 `duck` 两者的 `supertype`。 `Object` 是 JavaScript 中所有对象的 `supertype`。 因此，所有对象都可以访问 `hasOwnProperty` 方法。
