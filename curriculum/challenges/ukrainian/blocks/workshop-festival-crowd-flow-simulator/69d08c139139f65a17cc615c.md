...
- Пройдіться циклом по кожних воротах у масиві `gates` та викличте `handleGateAtTick()` з такими аргументами:
  - `gates`: повний масив об’єктів воріт.
  - `gate`: поточні ворота, які обробляються.
  - `tickIndex`: поточна позиція тіку в масиві `queue`.
  - `throughputSummary`: локальний об’єкт, що відстежує загальну кількість відвідувачів, оброблених на кожних воротах.
- Збільшіть `tickIndex` на 1, щоб перейти до наступного тіку.

# --hints--

Ви повинні вивести в консоль таке повідомлення: `"\nTick " + (tickIndex + 1)`.
...
Після виведення в консоль пройдіться циклом по кожних воротах у масиві `gates` та викличте `handleGateAtTick()` з аргументами `gates`, `gate`, `tickIndex` і `throughputSummary`.
...
Після циклу по масиву `gates` збільшіть `tickIndex` на 1.
