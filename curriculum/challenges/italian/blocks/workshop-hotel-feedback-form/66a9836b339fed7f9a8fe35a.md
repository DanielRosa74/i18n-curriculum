...
Per il tuo elemento `textarea`, aggiungi gli attributi `id` e `name` con il valore `"comments"`.

Per l'attributo `cols`, imposta il valore a `"30"`, e per l'attributo `rows`, imposta il valore a `"10"`.
