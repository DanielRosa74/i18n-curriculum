...
En este ejemplo, hemos declarado una función llamada `greet`. Dentro de esa función, tenemos un `console.log` que muestra el mensaje `Hello, Jessica!`. Si intentáramos ejecutar este código, no veríamos el mensaje aparecer en la consola. Esto se debe a que necesitamos llamar a la función.
