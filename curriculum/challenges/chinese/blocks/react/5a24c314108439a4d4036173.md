...
React 期望你永远不要直接修改 `state`，而是在状态发生变化时总是使用 `this.setState()`。你还应该注意，React 可能会批量处理多个状态更新以提升性能。这意味着通过 `setState` 方法的状态更新可能是异步的。`setState` 方法有一种备选语法，可以解决这个问题。虽然很少需要用到，但记住它是有好处的！请查阅我们的<a href="https://www.freecodecamp.org/news/what-is-state-in-react-explained-with-examples/" target="_blank" rel="noopener noreferrer nofollow">React 文章</a>了解更多细节。
