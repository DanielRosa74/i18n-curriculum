...
O array `watchList` contém objetos com informações sobre vários filmes. Use `map` no `watchList` para atribuir um novo array de objetos à variável `ratings`. Cada filme no novo array deve ter apenas uma chave `title` com o nome do filme e uma chave `rating` com a avaliação do IMDB. O código no editor atualmente usa um `for` loop para fazer isso, então você deve substituir a funcionalidade do loop pela sua expressão com `map`.
