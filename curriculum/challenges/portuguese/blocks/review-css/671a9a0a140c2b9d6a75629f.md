...
- **Alinhamento**: É como os elementos são posicionados em relação uns aos outros. Usar o alinhamento corretamente ajuda a deixar o design limpo e organizado.
- **Composição**: É o ato de organizar elementos para criar um design harmonioso. Determina como elementos como imagens, texto e formas se relacionam e contribuem para o design de forma artística.
- **Balanceamento**: É como o peso visual é distribuído dentro de uma composição. Designers buscam criar um equilíbrio por meio de arranjos simétricos ou assimétricos.
- **Escala**: Refere-se a comparar as dimensões ou tamanho de um elemento com outro.
- **Hierarquia**: Estabelece a ordem de importância dos elementos em um design. Trata-se de garantir que a informação mais importante seja notada primeiro.
- **Contraste**: É o processo de criar distinções claras entre os elementos. Você pode fazer isso por meio de variações em cor, tamanho, forma, textura ou qualquer outra característica visual. Um contraste forte também ajuda a melhorar a legibilidade.
- **Espaço em branco (espaço negativo)**: É o espaço vazio em um design. É a área que envolve os elementos.
- **UI (interface do usuário)**: UI inclui os elementos visuais e interativos que os usuários veem na tela, como ícones, imagens, texto, menus, links e botões.
- **UX (experiência do usuário)**: UX trata de como os usuários se sentem ao usar um produto ou serviço. Uma aplicação com uma boa experiência do usuário é intuitiva, fácil de usar, eficiente, acessível e agradável.
- **Briefing de design**: É um documento que descreve os objetivos, metas e requisitos de um projeto. É um roteiro que guia o processo de design e garante que o produto final atenda às necessidades do cliente.
- **Design baseado em vetor**: Envolve criar arte digital usando fórmulas matemáticas para definir linhas, formas e cores.
- **Prototipagem**: Refere-se ao processo de criar um modelo interativo de um produto ou interface do usuário.
...
- **Definição**: Uma família de fontes é um grupo de fontes que compartilham um design comum. Todas as fontes que pertencem à mesma família são baseadas no mesmo tipo de letra principal, mas também apresentam variações em estilo, espessura e largura. Você pode especificar múltiplas famílias de fontes em ordem de prioridade, da mais alta para a mais baixa, separando-as por vírgulas. Essas fontes alternativas funcionarão como opções de fallback. Você deve sempre incluir uma família de fontes genérica no final da lista de font-family.
...
- **Definição**: Regras at são declarações que fornecem instruções ao navegador. Você pode usá-las para definir vários aspectos da folha de estilos, como media queries, keyframes, fontes e mais.
- **`@font-face`**: Permite definir uma fonte personalizada especificando o arquivo da fonte, formato e outras propriedades importantes, como peso e estilo. Para que a regra at `@font-face` seja válida, você também precisa especificar a propriedade `src`. Essa propriedade contém referências aos recursos da fonte.
- **Formatos de fonte**: Para cada recurso de fonte, você também pode especificar o formato. Isso é opcional. É uma dica para o navegador sobre o formato da fonte. Se o formato for omitido, o recurso será baixado e o formato será detectado após o download. Se o formato for inválido, o recurso não será baixado. Formatos possíveis incluem `collection`, `embedded-opentype`, `opentype`, `svg`, `truetype`, `woff` e `woff2`.
- **`woff` e `woff2`**: `woff` significa "Web Open Font Format". É um formato de fonte amplamente usado desenvolvido pela Mozilla em colaboração com Type Supply, LettError e outras organizações. A diferença entre `woff` e `woff2` está no algoritmo usado para comprimir os dados.
- **OpenType**: É um formato para fontes escaláveis desenvolvido pela Microsoft e Adobe que permite acessar recursos adicionais em uma fonte. É amplamente usado nos principais sistemas operacionais.
- **`tech()`**: Usado para especificar a tecnologia do recurso da fonte. É opcional.
...
- **Google Fonts**: Serviço do Google que oferece uma coleção de fontes, muitas delas projetadas especificamente para desenvolvimento web.
- **Font Squirrel**: Outro recurso popular que você pode usar para adicionar fontes externas personalizadas aos seus projetos.

## Propriedade `text-shadow`

- **Definição**: Essa propriedade é usada para aplicar sombras ao texto. Você precisa especificar o deslocamento X e Y, que representam a distância horizontal e vertical da sombra em relação ao texto, respectivamente. Esses valores são obrigatórios. Valores positivos para o deslocamento X e Y moverão a sombra para a direita e para baixo, respectivamente, enquanto valores negativos moverão a sombra para a esquerda e para cima.
- **Cor da sombra**: Você também pode personalizar a cor da sombra especificando esse valor antes ou depois do deslocamento. Se a cor não for especificada, o navegador determinará a cor automaticamente, então geralmente é melhor definir explicitamente.
- **Raio de desfoque**: O raio de desfoque é opcional, mas deixará a sombra muito mais suave e sutil. O valor padrão do raio de desfoque é zero. Quanto maior o valor, maior o desfoque, o que significa que a sombra fica mais clara.
...
No exemplo acima, a largura da imagem será 250px se a largura da viewport for menor que 1000 pixels (já que 250px é maior que 25% dessa largura da viewport). Se a largura da viewport for maior que 1000 pixels, a largura da imagem será 25vw. Isso porque 25vw equivale a 25% da largura da viewport.
...
- **Posicionamento fixo**: Quando um elemento é posicionado com `position: fixed`, ele é removido do fluxo normal do documento e colocado em relação à viewport, ou seja, permanece na mesma posição mesmo quando o usuário rola a página. Isso é frequentemente usado para elementos como cabeçalhos ou barras de navegação que precisam permanecer visíveis o tempo todo.
- **Posicionamento sticky**: Esse tipo de posicionamento funciona como um híbrido entre o posicionamento relativo e fixo. Inicialmente, o elemento se comporta como se estivesse posicionado relativamente, permanecendo dentro do fluxo do documento. No entanto, quando o usuário rola a página e o elemento ultrapassa um certo ponto, ele "gruda" na viewport (geralmente no topo) e se comporta como se estivesse fixo. Isso é ótimo para criar elementos como barras de navegação sticky, que só ficam fixas quando o usuário rola até uma certa posição.
