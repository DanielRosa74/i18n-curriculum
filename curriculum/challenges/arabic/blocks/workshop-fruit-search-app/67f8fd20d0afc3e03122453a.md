...
أولًا، أضف Hook من نوع `useEffect` ليُنفذ في كل مرة يتغير فيها حالة `query`. تأكد من تمرير دالة سهمية كالمعلمة الأولى إلى `useEffect`، وضمّن `query` في مصفوفة التبعيات.

# --hints--

يجب عليك استخدام Hook `useEffect`.
...
يجب أن تضمّن `query` في مصفوفة التبعيات الخاصة بـ Hook `useEffect`.
