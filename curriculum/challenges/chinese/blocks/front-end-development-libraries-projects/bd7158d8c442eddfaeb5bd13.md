...
你可以使用 HTML、JavaScript、CSS、Bootstrap、SASS、React、Redux 和 jQuery 的任意组合来完成这个项目。你应该使用一个前端框架（例如 React），因为本部分是关于学习前端框架的。不建议使用上述未列出的其他技术，使用它们风险自负。我们正在考虑支持其他前端框架，如 Angular 和 Vue，但目前尚未支持。我们会接受并尝试修复所有使用本项目建议技术栈的问题报告。祝你编码愉快！
