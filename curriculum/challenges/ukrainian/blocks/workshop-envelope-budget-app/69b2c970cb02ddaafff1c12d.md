...
Тепер потрібно визначити відповідний статус залежно від того, чи користувач перевищив бюджет або залишився в межах бюджету.

Почніть зі створення `if` інструкції, яка перевіряє, чи `netRemaining` менше за `0`.

# --hints--

У вашій функції `calculateBudget` має бути `if` інструкція.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

Ваша `if` інструкція має перевіряти, чи `netRemaining` менше за `0`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

Тіло вашої `if` інструкції поки має бути порожнім.
