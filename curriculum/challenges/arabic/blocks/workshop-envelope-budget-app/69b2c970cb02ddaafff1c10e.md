...
أنشئ عنصر `label` آخر (في سطر جديد) في نهاية `HTMLString` الخاص بك. يجب أن يحتوي هذا الـ `label` على النص `Expense # Amount`، باستخدام صيغة القالب النصي لاستبدال `#` بقيمة `entryNumber`، ويُعيّن له الخاصية `for` بالقيمة `X-#-amount`، حيث تكون `X` هي قيمة `category` و`#` هي قيمة `entryNumber`.
...
يجب أن يحتوي عنصر `label` الجديد على الخاصية `for` بقيمة `${category}-${entryNumber}-amount`.
...
يجب أن يحتوي عنصر `label` الجديد على النص `Expense ${entryNumber} Amount`.
