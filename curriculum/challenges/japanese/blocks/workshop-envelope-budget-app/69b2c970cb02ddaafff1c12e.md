...
`if`文の中で、ユーザーがどれだけ予算オーバーしているかを表示するために`statusText`を設定する必要があります。

`statusText`をテンプレートリテラルに設定し、`"Over Budget by $X"`と表示してください。ここで`X`は`netRemaining`の絶対値です。絶対値を取得するには`Math.abs()`を使います。

# --hints--

`if`文の中で`statusText`に値を代入してください。

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*statusText\s*=/);
```

`statusText`はテンプレートリテラルを使う必要があります。

```js
assert.match(code, /statusText\s*=\s*`/);
```

テンプレートリテラルには「Over Budget by $」というテキストを含めてください。

```js
assert.match(code, /statusText\s*=\s*`Over\s+Budget\s+by\s+\$\$\{/);
```

絶対値を取得するには`Math.abs(netRemaining)`を使ってください。
