...
`.key.black--key::after`에 `absolute`로 설정된 `position` 속성과 `-18px`로 설정된 `left` 속성을 주세요.

# --hints--

`.key.black--key::after` 선택자에 `absolute`로 설정된 `position` 속성이 있어야 합니다.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.key.black--key::after')?.position, 'absolute');
```

`.key.black--key::after` 선택자에 `-18px`로 설정된 `left` 속성이 있어야 합니다.
