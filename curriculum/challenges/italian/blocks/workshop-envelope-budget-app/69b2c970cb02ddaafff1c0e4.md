...
Ti serve un altro di questi blocchi di codice `fieldset` per `entertainment`.
...
Il tuo quarto elemento `fieldset` dovrebbe avere un `id` impostato su `entertainment`.
...
Il tuo quarto elemento `legend` dovrebbe contenere il testo `Entertainment`.
