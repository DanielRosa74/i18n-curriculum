...
Tu `foundCard` debe ser asignado llamando a `find` sobre `this.cards`, pasando un callback que tome un parámetro `card` y devuelva el resultado de comparar `card.name_short` con `cardId` usando igualdad estricta.
