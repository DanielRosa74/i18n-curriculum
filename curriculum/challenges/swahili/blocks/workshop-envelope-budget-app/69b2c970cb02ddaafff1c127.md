...
Kufuata muundo huu huo, tangaza vigezo `utilities` na `entertainment` kwa ajili ya ingizo za `number` katika vipengele vya `#utilities` na `#entertainment`. Weka thamani zao kwa wito sahihi wa `getTotalFromInputs`.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `utilities`.
...
Unapaswa kufanya wito wa `getTotalFromInputs` ukiwa na `utilitiesInputs` kama hoja.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya `getTotalFromInputs` kwa `utilities`.
...
Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `entertainment`.
...
Unapaswa kufanya wito wa `getTotalFromInputs` ukiwa na `entertainmentInputs` kama hoja.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya `getTotalFromInputs` kwa `entertainment`.
