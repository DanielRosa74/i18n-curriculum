...
Embora uma variável declarada com `let` possa ser reatribuída, ela não pode ser redeclarada. Se você tentar declarar a mesma variável novamente usando `let`, receberá um erro:
...
O mesmo vale para `const`: uma variável declarada com `const` também não pode ser redeclarada.
...
Você também pode usar a palavra-chave `var`, mas ela não é mais tão recomendada. A palavra-chave `var` é meio que como `let`, só que tem um escopo mais amplo, o que tende a causar mais problemas no seu programa.
