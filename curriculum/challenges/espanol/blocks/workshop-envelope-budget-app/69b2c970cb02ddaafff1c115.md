...
Tu siguiente paso es escribir una función que obtenga los montos totales de las entradas del usuario.

Declara una función `getTotalFromInputs` y dale un parámetro llamado `list`.

# --hints--

Debes declarar una variable `getTotalFromInputs`.

```js
assert.isDefined(getTotalFromInputs);
```

Tu variable `getTotalFromInputs` debe ser una función.

```js
assert.isFunction(getTotalFromInputs);
```

Tu función `getTotalFromInputs` debe recibir un parámetro llamado `list`.
