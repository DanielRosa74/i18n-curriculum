...
Usando esa misma sintaxis, consulta tus entradas de `number` en el elemento `#utilities` y asígnalas a `utilitiesInputs`.

# --hints--

Debes declarar una variable `utilitiesInputs`.
...
Debes llamar a `document.querySelectorAll()` con `#utilities input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada a `document.querySelectorAll()` a `utilitiesInputs`.
