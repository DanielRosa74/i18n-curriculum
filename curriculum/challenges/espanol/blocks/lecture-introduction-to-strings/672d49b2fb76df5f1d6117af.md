...
Cada vez que uses el operador `+` para concatenar cadenas, es importante revisar cuidadosamente cualquier posible problema de espacios.
...
Es importante recordar que las cadenas son inmutables, lo que significa que una vez creada una cadena, no puedes modificarla.

En este caso, la cadena original `Hello` no se modifica. En cambio, `greeting` ahora hace referencia a la nueva cadena `Hello, John!`.
...
Para concluir, el operador `+` es mejor para concatenaciones simples, especialmente cuando necesitas combinar unas pocas cadenas o variables.
