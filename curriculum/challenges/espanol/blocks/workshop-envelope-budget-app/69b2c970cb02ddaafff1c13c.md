...
Después de que tu ciclo termine, necesitas limpiar los campos `incomeInput` y `rentInput`. Establece la propiedad `value` de `incomeInput` y `rentInput` a una cadena vacía.

# --hints--

Tu función `clearForm` debe acceder a la propiedad `value` del elemento `incomeInput`.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

Tu función `clearForm` debe establecer la propiedad `value` del elemento `incomeInput` a una cadena vacía.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

Debes modificar `incomeInput` después de tu ciclo `for`.
...
Tu función `clearForm` debe acceder a la propiedad `value` del elemento `rentInput`.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

Tu función `clearForm` debe establecer la propiedad `value` del elemento `rentInput` a una cadena vacía.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

Debes modificar `rentInput` después de tu ciclo `for`.
