...
عنصر `input` الخاص بالدخل يحتاج إلى بعض الخصائص الإضافية لضمان إدخال المستخدمين لقيم صحيحة.

حدّث العنصر `#income` الذي أنشأته بالخصائص التالية:

- تعيين `type` إلى `number` (للسماح فقط بالمُدخلات الرقمية)
- تعيين `min` إلى `0` (للسماح فقط بالأعداد الموجبة)
- تعيين `placeholder` إلى `e.g. 2000`
- أخيرًا، اجعل عنصر `input` مطلوبًا (`required`).
...
يجب أن يحتوي عنصر `input` على خاصية `placeholder` مع القيمة `e.g. 2000`.
