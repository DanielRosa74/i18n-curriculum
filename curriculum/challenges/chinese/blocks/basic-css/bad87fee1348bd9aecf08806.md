...
你可以看到我们在 `<style>` 标签内创建了一个名为 `blue-text` 的 CSS 类。你可以这样将类应用到一个 HTML 元素上：`<h2 class="blue-text">CatPhotoApp</h2>`。请注意，在你的 CSS `style` 元素中，类名以句点开头。而在 HTML 元素的 class 属性中，类名不包含句点。
