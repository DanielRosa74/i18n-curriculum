...
Sotto quell'`input`, aggiungi un elemento `label` con il testo `Location` e l'attributo `for` impostato su `"location"`.
...
Sotto quell'`input`, aggiungi un elemento `label` con il testo `Reputation` e l'attributo `for` impostato su `"reputation"`.
...
Il tuo quarto `input` casella di spunta dovrebbe avere un attributo `id` impostato su `"reputation"`.
