...
同じパターンに従って、`#entertainment` 要素内の `number` 入力をクエリし、それらを `entertainmentInputs` に代入してください。

# --hints--

`entertainmentInputs` 変数を宣言してください。
...
`document.querySelectorAll()` を `#entertainment input[type='number']` で呼び出してください。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` の呼び出し結果を `entertainmentInputs` に代入してください。
