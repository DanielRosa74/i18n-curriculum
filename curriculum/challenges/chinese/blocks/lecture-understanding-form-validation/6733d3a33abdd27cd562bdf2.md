...
DOM 中触发的每个事件都有某种默认行为。复选框上的 `click` 事件默认会切换该复选框的状态。按下聚焦按钮上的 <kbd>Spacebar</kbd> 会激活该按钮。这些事件对象上的 `preventDefault()` 方法可以阻止这些行为发生。
...
`e.key` 会给你按下的键的值，比如 <kbd>A</kbd> 键对应 `a`，<kbd>Enter</kbd> 键对应 `Enter`。
