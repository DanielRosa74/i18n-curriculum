...
title: 使用 PropTypes 定义你期望的 Props
challengeType: 6
forumTopicId: 301419
...
在上面的例子中，`PropTypes.func` 部分检查 `handleClick` 是否是一个函数。添加 `isRequired` 告诉 React `handleClick` 是该组件的必需属性。如果没有提供该属性，你会看到警告。还要注意，`func` 代表 `function`。在七种 JavaScript 原生类型中，`function` 和 `boolean`（写作 `bool`）是唯一两个拼写不寻常的类型。除了原生类型外，还有其他类型可用。例如，你可以检查一个属性是否是 React 元素。请参考<a href="https://reactjs.org/docs/typechecking-with-proptypes.html#proptypes" target="_blank" rel="noopener noreferrer nofollow">文档</a>了解所有选项。
