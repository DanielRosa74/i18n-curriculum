...
Il tuo prossimo passaggio è scrivere una funzione che ottenga gli importi totali dagli elementi dell'utente.

Dichiara una funzione `getTotalFromInputs` e assegnale un parametro chiamato `list`.

# --hints--

Dovresti dichiarare una variabile `getTotalFromInputs`.

```js
assert.isDefined(getTotalFromInputs);
```

La tua variabile `getTotalFromInputs` dovrebbe essere una funzione.

```js
assert.isFunction(getTotalFromInputs);
```

La tua funzione `getTotalFromInputs` dovrebbe prendere un parametro chiamato `list`.
