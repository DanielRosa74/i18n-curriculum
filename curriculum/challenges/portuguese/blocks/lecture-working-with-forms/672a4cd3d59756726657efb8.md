...
Dentro deste elemento `form`, há um elemento `label` e um elemento `input` para o endereço de e-mail do usuário. Quando o usuário clicar no botão de envio, os dados serão enviados para o servidor e processados. O terceiro valor possível para o atributo `type` é o valor `reset`. Aqui está um exemplo de um elemento `form` com botões de reset e envio.
