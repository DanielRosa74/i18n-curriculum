...
Seguendo lo stesso pattern, dichiara le variabili `utilities` e `entertainment` per gli input di tipo `number` negli elementi `#utilities` e `#entertainment`. Assegna loro le chiamate appropriate a `getTotalFromInputs`.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere una variabile `utilities`.
...
Dovresti chiamare `getTotalFromInputs` con `utilitiesInputs` come argomento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

Dovresti assegnare il risultato di `getTotalFromInputs` a `utilities`.
...
La tua funzione `calculateBudget` dovrebbe avere una variabile `entertainment`.
...
Dovresti chiamare `getTotalFromInputs` con `entertainmentInputs` come argomento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

Dovresti assegnare il risultato di `getTotalFromInputs` a `entertainment`.
