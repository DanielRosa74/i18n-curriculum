...
在你的 `span` 元素中，創建一個 `label` 元素，並將其 `for` 屬性設為 `entry-dropdown`，文字為 `Add expense to:`。接著創建一個 `select` 元素，`id` 設為 `entry-dropdown`，`name` 設為 `options`。在其下方，添加一個 `button` 元素，`id` 設為 `add-entry`，文字為 `Add Entry`。
...
你新的 `label` 元素應該有文字 `Add expense to:`。
...
你新的 `button` 元素應該有一個 `type` 屬性，設為 `button`。
