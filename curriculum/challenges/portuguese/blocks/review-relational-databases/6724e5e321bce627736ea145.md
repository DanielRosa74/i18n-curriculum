...
- <kbd>Seta para cima</kbd>/<kbd>Seta para baixo</kbd>: Navegar pelos comandos anteriores/próximos no histórico.
- <kbd>Tab</kbd>: Autocompletar comandos.
- <kbd>Control</kbd> + <kbd>L</kbd> (Linux/macOS) ou digitar `cls` (Windows): Limpar a tela do terminal.
- <kbd>Control</kbd> + <kbd>C</kbd>: Interromper um comando em execução (também usado para copiar no PowerShell se o texto estiver selecionado).
- <kbd>Control</kbd> + <kbd>Z</kbd> (apenas Linux/macOS): Suspender uma tarefa para o plano de fundo; use `fg` para retomá-la.
- **`!!`**: Executar novamente instantaneamente o último comando executado.
...
- **Páginas de manual**: Use `man` para acessar documentação detalhada dos comandos.
  - Fornece informações completas sobre o uso do comando
  - Mostra todas as opções disponíveis e exemplos
  - Use as <kbd>teclas de seta</kbd> para navegar, <kbd>q</kbd> para sair
