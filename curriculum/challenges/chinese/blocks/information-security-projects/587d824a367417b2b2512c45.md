...
你可以发送 DELETE 请求到 `/api/replies/{board}`，并传递 `thread_id`、`reply_id` 和 `delete_password`。返回值将是字符串 `incorrect password` 或 `success`。成功时，`reply_id` 的文本将被更改为 `[deleted]`。
...
你可以发送 PUT 请求到 `/api/replies/{board}`，并传递 `thread_id` 和 `reply_id`。返回值将是字符串 `reported`。`reply_id` 的 `reported` 值将被更改为 `true`。
