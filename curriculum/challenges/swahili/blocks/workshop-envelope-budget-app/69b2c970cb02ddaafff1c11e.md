...
Kumbuka kuwa `return` huisha utekelezaji wa kitendakazi. Baada ya kipande chako cha `if`, unahitaji kushughulikia mantiki ya wakati ingizo ni halali. Kwa sababu taarifa yako ya if inarudisha thamani, huna haja ya taarifa ya `else`.

Tumia opereta ya kuongeza kuongeza `currVal` kwenye `total` yako. Utahitaji kutumia kijenzi cha `Number` kubadilisha `currVal` kuwa nambari.
...
Baada ya taarifa yako ya `if`, unapaswa kutumia opereta ya kuongeza kwenye `total`.
...
Unapaswa kuongeza thamani ya `Number(currVal)` kwenye `total`.
