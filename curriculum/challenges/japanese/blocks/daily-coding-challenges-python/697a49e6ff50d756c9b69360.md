...
title: "チャレンジ 183: 冬季大会 第4日目: スキージャンプ"
challengeType: 29
...
`ski_jump_medal(125.0, 58.0, 0.0, 6.0)` は `"Gold"` を返すべきです。
...
`ski_jump_medal(119.0, 50.0, 1.0, 4.0)` は `"Bronze"` を返すべきです。
...
`ski_jump_medal(122.0, 52.0, -1.0, 4.0)` は `"Silver"` を返すべきです。
...
`ski_jump_medal(118.0, 50.5, -1.5, 4.0)` は `"No Medal"` を返すべきです。
...
`ski_jump_medal(124.0, 50.5, 2.0, 5.0)` は `"Gold"` を返すべきです。
...
`ski_jump_medal(119.0, 49.5, 0.0, 3.0)` は `"No Medal"` を返すべきです。
