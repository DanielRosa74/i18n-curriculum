...
`category` 변수와 `' .input-container'`(여기서 `.input-container` 앞의 공백에 주의하세요)를 연결하여 문자열을 만들고, 그 문자열을 `querySelector()`에 전달하세요.
...
`category` 뒤에 `' .input-container'`를 연결하세요. `.input-container` 앞에 공백을 포함하는 것을 잊지 마세요.
