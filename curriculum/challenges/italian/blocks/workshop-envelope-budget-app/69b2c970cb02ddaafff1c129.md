...
La tua funzione `getTotalFromInputs` imposterà la flag globale di errore `isError` su `true` se viene rilevato un input non valido.

Aggiungi un'istruzione `if` alla tua funzione `calculateBudget` che verifichi la veridicità della tua flag globale di errore e, se è vera, usa `return` per terminare l'esecuzione della funzione.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere un'istruzione `if` che verifica la veridicità della variabile `isError`.
...
La tua istruzione `if` dovrebbe essere posizionata dopo l'ultima chiamata alla funzione `getTotalFromInputs`.
