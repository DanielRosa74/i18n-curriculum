...
أنشئ عنصر `a` آخر داخل العنصر الثاني `li`. أعطه خاصية `href` بقيمة `#` والنص `Widgets`.

# --hints--

يجب أن تنشئ عنصر `a` داخل العنصر الثاني `li`.

```js
assert.lengthOf(document.querySelectorAll('li a'), 2);
```

يجب أن يحتوي عنصر `a` على خاصية `href` بقيمة `#`.
