---
id: 6821ebe4237de8297eaee794
title: "도전 과제 18: 두 번째로 비싼 것"
challengeType: 29
dashedName: challenge-18
---

# --description--

서로 다른 노트북 가격을 나타내는 정수 배열과 예산을 나타내는 정수가 주어졌을 때, 다음을 반환하세요:

1. 예산 내에 있다면 목록에서 두 번째로 비싼 노트북,
2. 그렇지 않으면 구매할 수 있는 가장 비싼 노트북,
3. 예산 내에 노트북이 없으면 `0`을 반환하세요.

- 중복된 가격은 무시하세요.

# --hints--

`get_laptop_cost([1500, 2000, 1800, 1400], 1900)`는 `1800`을 반환해야 합니다

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_laptop_cost([1500, 2000, 1800, 1400], 1900), 1800)`)
}})
```

`get_laptop_cost([1500, 2000, 2000, 1800, 1400], 1900)`는 `1800`을 반환해야 합니다

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_laptop_cost([1500, 2000, 2000, 1800, 1400], 1900), 1800)`)
}})
```

`get_laptop_cost([2099, 1599, 1899, 1499], 2200)`는 `1899`를 반환해야 합니다

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_laptop_cost([2099, 1599, 1899, 1499], 2200), 1899)`)
}})
```

`get_laptop_cost([2099, 1599, 1899, 1499], 1000)`는 `0`을 반환해야 합니다

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_laptop_cost([2099, 1599, 1899, 1499], 1000), 0)`)
}})
```

`get_laptop_cost([1200, 1500, 1600, 1800, 1400, 2000], 1450)`는 `1400`을 반환해야 합니다

```js
({test: () => { runPython(`
from unittest import TestCase
TestCase().assertEqual(get_laptop_cost([1200, 1500, 1600, 1800, 1400, 2000], 1450), 1400)`)
}})
```

# --seed--

## --seed-contents--

```py
def get_laptop_cost(laptops, budget):

    return laptops
```

# --solutions--

```py
def get_laptop_cost(laptops, budget):
    laptops = sorted(set(laptops), reverse=True)

    if len(laptops) >= 2 and budget >= laptops[1]:
        return laptops[1]
    if not laptops or budget < laptops[-1]:
        return 0

    for price in laptops[2:]:
        if budget >= price:
            return price

    return 0
```
