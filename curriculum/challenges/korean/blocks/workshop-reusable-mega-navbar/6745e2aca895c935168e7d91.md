...
마무리하려면 첫 번째 `a` 요소의 텍스트를 `Calendar`로, 두 번째는 `Chat`으로, 세 번째는 `Email`로 설정하세요.

이렇게 하면 `Navbar` 컴포넌트가 완성됩니다!

# --hints--

첫 번째 `a` 요소의 텍스트를 `Calendar`로 설정하세요.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

두 번째 `a` 요소의 텍스트를 `Chat`으로 설정하세요.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

세 번째 `a` 요소의 텍스트를 `Email`로 설정하세요.
