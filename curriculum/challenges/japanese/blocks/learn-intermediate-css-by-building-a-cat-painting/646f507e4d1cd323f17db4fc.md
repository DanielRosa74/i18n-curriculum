...
`.cat-whisker-right-bottom` セレクターには `absolute` に設定された `position` プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.position === 'absolute')
```

`.cat-whisker-right-bottom` セレクターには `134px` に設定された `top` プロパティが必要です。

```js
assert(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-bottom')?.top === '134px')
```

`.cat-whisker-right-bottom` セレクターには `109px` に設定された `left` プロパティが必要です。
