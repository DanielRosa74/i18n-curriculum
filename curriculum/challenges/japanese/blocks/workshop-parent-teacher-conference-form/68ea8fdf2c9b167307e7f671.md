...
まず、テキストが`Parent/Guardian Name: `で、`for`属性が`"parent-name"`に設定された`label`要素を追加してください。

次に、`label`要素の下に、`type`属性が`"text"`に設定された`input`要素を追加してください。`name`と`id`属性は`"parent-name"`に設定する必要があります。`placeholder`属性は`"E.g., Nancy Doe"`に設定してください。最後に、`input`は必須にしてください。
