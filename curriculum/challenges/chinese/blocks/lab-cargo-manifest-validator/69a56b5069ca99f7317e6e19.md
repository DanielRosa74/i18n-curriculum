...
**注意：** `normalizeUnits`、`validateManifest` 和 `processManifest` 都不应使用 `const` 声明，因为测试需要重新赋值它们。
...
`normalizeUnits`、`validateManifest` 和 `processManifest` 都不应使用 `const` 声明，因为测试需要重新赋值它们。
