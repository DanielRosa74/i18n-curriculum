...
## ¿Qué es el atributo `aria-expanded`?
...
## Configurar `aria-expanded` en `true` o `false`
...
## Conectar el control con su contenido

Además, las propiedades `aria-controls` y `aria-owns` pueden usarse en combinación con `aria-expanded` para establecer una conexión programática entre el elemento controlador y el elemento que controla.
