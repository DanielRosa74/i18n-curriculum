...
`.cat-whisker-right-top`セレクターには`absolute`に設定された`position`プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.position, 'absolute');
```

`.cat-whisker-right-top`セレクターには`120px`に設定された`top`プロパティが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-whisker-right-top')?.top, '120px');
```

`.cat-whisker-right-top`セレクターには`109px`に設定された`left`プロパティが必要です。
