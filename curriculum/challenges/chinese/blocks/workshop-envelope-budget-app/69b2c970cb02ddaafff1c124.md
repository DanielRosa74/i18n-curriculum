...
按照相同的模式，在 `#entertainment` 元素中查询你的 `number` 输入，并将它们赋值给 `entertainmentInputs`。

# --hints--

你应该声明一个 `entertainmentInputs` 变量。
...
你应该使用 `#entertainment input[type='number']` 调用 `document.querySelectorAll()`。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你应该将 `document.querySelectorAll()` 调用的结果赋值给 `entertainmentInputs`。
