...
现在，让我们看另一个例子。这个例子的 HTML 是一个带有文本 `freeCodeCamp is Awesome` 的 `div` 元素。CSS 使 `div` 成为一个矩形，并隐藏任何超出视口左右的内容。JavaScript 代码在每次调用带有 `animate` 函数作为回调的 `requestAnimationFrame(animate)` 时，将矩形向右移动 `2px`：
