...
在你的 `span` 元素中，创建一个 `label` 元素，设置 `for` 属性为 `entry-dropdown`，并赋予它文本 `Add expense to:`。然后创建一个 `select` 元素，设置 `id` 为 `entry-dropdown`，`name` 为 `options`。在其下方，添加一个 `button` 元素，设置 `id` 为 `add-entry`，文本为 `Add Entry`。
...
你新建的 `label` 元素应包含文本 `Add expense to:`。
...
你新建的 `button` 元素应设置 `type` 属性为 `button`。
