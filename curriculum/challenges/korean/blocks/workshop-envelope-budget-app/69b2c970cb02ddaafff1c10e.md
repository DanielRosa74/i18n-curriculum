...
새로운 `label` 요소를 `HTMLString`의 끝에 (새 줄에) 하나 더 만드세요. 이 `label`은 템플릿 리터럴 구문을 사용해 `#`를 `entryNumber` 값으로 바꾼 `Expense # Amount` 텍스트를 가져야 하며, `for` 속성은 `X-#-amount`로 설정하세요. 여기서 `X`는 `category` 값이고 `#`는 `entryNumber` 값입니다.
...
새로운 `label` 요소의 `for` 속성은 `${category}-${entryNumber}-amount`로 설정하세요.
...
새로운 `label` 요소는 `Expense ${entryNumber} Amount` 텍스트를 가져야 합니다.
