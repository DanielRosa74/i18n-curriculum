...
接下来，创建第二个 `fieldset` 元素，并将其 `id` 设置为 `food`。

在该元素内，创建一个文本为 `Food` 的 `legend` 元素，以及一个 `class` 设置为 `input-container` 的空 `div`。
...
你的第二个 `fieldset` 元素应将 `id` 设置为 `food`。
