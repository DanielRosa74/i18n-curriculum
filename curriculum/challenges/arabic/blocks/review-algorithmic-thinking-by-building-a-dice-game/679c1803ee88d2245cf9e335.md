...
أخيرًا، يجب أن ترتب كودك قليلًا عبر إعادة هيكلة! قد تلاحظ أن كودك فعّل زر الاختيار الأخير ("None of the Above") عدة مرات داخل دوال `getHighestDuplicates` و`detectFullHouse` و`checkForStraights`. أعد هيكلة كودك بحيث لا يتم تفعيل زر الاختيار الأخير في نهاية كل دالة، بل يُفعل مرة واحدة فقط داخل `rollDiceBtn`.
