...
Puedes realizar cálculos con valores que representan longitud, ángulo, tiempo, porcentajes y números. También puedes combinar diferentes unidades como píxeles, porcentajes y ems.

Para la suma y la resta, todos los valores en la expresión, también llamados operandos, deben tener sus unidades correspondientes, como `px`, `em` y porcentaje (`%`). Dependiendo del operador, los operandos pueden tener diferentes unidades.
...
También debes saber que si usas el operador de multiplicación, uno de los operandos debe ser sin unidad. Esta expresión no sería válida porque ambos operandos tienen unidades (píxeles):

```css
calc(5px * 50px)
```

Necesitarías omitir la unidad en uno de ellos. Ambas alternativas serían válidas:
...
Para el operador de división, dividir por un número sin unidad escala el valor y mantiene su unidad:

```css
calc(50% / 5)
```

Dividir dos valores con unidades iguales también es válido, pero cancela las unidades para devolver un número sin unidad (como `10`). Ten en cuenta que los valores sin unidad solo pueden usarse para ciertas propiedades CSS (como `line-height` o `flex-grow`):
