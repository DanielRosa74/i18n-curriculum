...
- Faça um loop por cada portão no array `gates` e chame `handleGateAtTick()` com estes argumentos:
  ...
  - `throughputSummary`: O objeto local que acompanha o total de participantes processados por portão.
...
Você deve fazer um loop por cada portão no array `gates` após registrar no console, e chamar `handleGateAtTick()` com `gates`, `gate`, `tickIndex` e `throughputSummary`.
...
Você deve incrementar `tickIndex` em 1 após seu loop pelo array `gates`.
