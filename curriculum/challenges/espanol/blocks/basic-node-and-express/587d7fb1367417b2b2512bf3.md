...
Supongamos que montas esta función en una ruta. Cuando una solicitud coincide con la ruta, muestra la cadena “I’m a middleware…”, luego ejecuta la siguiente función en la pila. En este ejercicio, vas a construir middleware a nivel raíz. Como viste en el desafío 4, para montar una función middleware a nivel raíz, puedes usar el método `app.use(<mware-function>)`. En este caso, la función se ejecutará para todas las solicitudes, pero también puedes establecer condiciones más específicas. Por ejemplo, si quieres que una función se ejecute solo para solicitudes POST, podrías usar `app.post(<mware-function>)`. Existen métodos análogos para todos los verbos HTTP (GET, DELETE, PUT, …).
