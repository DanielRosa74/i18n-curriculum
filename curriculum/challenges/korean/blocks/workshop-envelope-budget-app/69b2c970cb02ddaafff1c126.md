...
이제 `food` 변수를 선언하고, `foodInputs`를 인수로 하여 `getTotalFromInputs`를 호출한 값을 할당하세요.

# --hints--

`calculateBudget` 함수에 `food` 변수가 있어야 합니다.
...
`foodInputs`를 인수로 하여 `getTotalFromInputs`를 호출해야 합니다.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

`getTotalFromInputs`의 결과를 `food`에 할당해야 합니다.
