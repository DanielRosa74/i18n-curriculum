...
باتباع نفس النمط، أعلن عن متغيري `utilities` و `entertainment` لمُدخلات `number` في العنصرين `#utilities` و `#entertainment`. عيّن لهما استدعاءات `getTotalFromInputs` المناسبة.

# --hints--

يجب أن تحتوي دالة `calculateBudget` على متغير `utilities`.
...
يجب استدعاء `getTotalFromInputs` مع `utilitiesInputs` كمعلمة.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

يجب تعيين نتيجة `getTotalFromInputs` إلى `utilities`.
...
يجب أن تحتوي دالة `calculateBudget` على متغير `entertainment`.
...
يجب استدعاء `getTotalFromInputs` مع `entertainmentInputs` كمعلمة.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

يجب تعيين نتيجة `getTotalFromInputs` إلى `entertainment`.
