...
Всередині елемента `#results` використайте тернарний оператор, щоб перевірити, чи довжина масиву `results` більша за нуль. Якщо так, спроєктуйте масив і відобразіть кожен елемент у `p` елементі з класом `result-item`. Обов’язково додайте атрибут `key` до кожного абзацу.
