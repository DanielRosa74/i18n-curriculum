...
為你的 `calculateBudget` 函式添加一行，呼叫 `e` 參數的 `preventDefault()` 方法。然後，將你的全域錯誤旗標 `isError` 重設為 `false`。

# --hints--

你的 `calculateBudget` 函式應該呼叫 `e.preventDefault()`。

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

你的 `calculateBudget` 函式應該將全域錯誤旗標 `isError` 重設為 `false`。
