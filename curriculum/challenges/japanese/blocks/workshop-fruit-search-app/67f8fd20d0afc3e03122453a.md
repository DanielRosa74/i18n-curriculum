...
まず、`query`の状態が変わるたびに実行されるように、`useEffect`フックを追加してください。`useEffect`の最初の引数にはアロー関数を渡し、依存関係の配列には`query`を含めるようにしてください。

# --hints--

`useEffect`フックを使ってください。
...
`useEffect`フックの依存関係の配列に`query`を含めてください。
