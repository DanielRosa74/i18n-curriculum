...
Neste exemplo, declaramos uma função chamada `greet`. Dentro dessa função, temos um `console.log` que exibe a mensagem `Hello, Jessica!`. Se tentássemos executar esse código, não veríamos a mensagem aparecer no console. Isso acontece porque precisamos fazer a chamada da função.
