...
`sessionStorage` é outra parte da API Web Storage que armazena dados durante a sessão da página, ou seja, os dados ficam disponíveis enquanto a aba ou janela do navegador estiver aberta. Porém, diferente de `localStorage`, os dados em `sessionStorage` são apagados quando a aba ou janela é fechada. Você vai aprender mais sobre `sessionStorage` na próxima lição.
