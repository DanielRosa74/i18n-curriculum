...
Sua string `output.innerHTML` vai precisar de um elemento `span`. Crie esse elemento e dê a ele um atributo class definido pela variável `statusClass`.

Dentro do seu elemento `span`, adicione a variável `statusText` como o conteúdo de texto.
...
Seu `span` deve ter o atributo `class` definido como `${statusClass}`.
...
Seu `span` deve ter `${statusText}` como o conteúdo de texto.
