...
Você já viu um tipo genérico em uma lição anterior: `Array<string>`. Mas geralmente você vai usar tipos genéricos em funções. Na verdade, eles podem ser vistos como um parâmetro especial que você fornece a uma função para controlar o comportamento da definição de tipo da função.
