...
O próximo comando é `ls`, que lista o conteúdo do seu diretório atual. Isso é útil para ver se um arquivo ou pasta existe no seu diretório. O comando `ls` também aceita flags para mostrar arquivos ocultos (a flag `-a`), permissões de arquivos (a flag `-l`) e outros recursos. Você vai aprender mais sobre flags de comando na próxima lição. Você também pode usar `less` ou `cat` para visualizar o conteúdo de um arquivo.

E se o diretório não contiver o arquivo ou pasta que você precisa? Existem comandos chamados `mkdir` e `touch` que permitem criá-los. O `mkdir` cria um novo diretório, ou pasta, e o `touch` cria um novo arquivo. Ambos os comandos aceitam o nome do arquivo ou pasta que você quer criar. Por exemplo, `touch readme.md` cria um novo arquivo Markdown chamado `readme.md`.
