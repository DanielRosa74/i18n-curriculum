...
이제 사용자가 예산을 초과하지 않은 경우(`netRemaining`이 `0` 이상일 때)를 처리해야 합니다.

`if` 문에 `else` 블록을 추가하여 `statusText`를 `"$X Remaining"`으로 설정하세요. 여기서 `X`는 `netRemaining`의 값입니다. 또한 `statusClass`를 `"surplus"`로 설정하세요.
