...
Um alles abzuschließen, setzen Sie den Text des ersten `a`-Elements auf `Calendar`, den zweiten auf `Chat` und den dritten auf `Email`.

Damit ist Ihre `Navbar`-Komponente fertig!

# --hints--

Sie sollten den Text des ersten `a`-Elements auf `Calendar` setzen.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Sie sollten den Text des zweiten `a`-Elements auf `Chat` setzen.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Sie sollten den Text des dritten `a`-Elements auf `Email` setzen.
