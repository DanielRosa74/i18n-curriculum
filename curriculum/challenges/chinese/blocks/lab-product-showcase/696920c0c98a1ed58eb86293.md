...
在本实验中，你已经获得了所有的 HTML 和 CSS。你将使用 TypeScript 完成一个产品展示。
...
1. 你应该定义一个名为 `Item` 的接口，包含：
   ...
   - 一个类型为 `string` 的 `id` 属性。
   - 一个类型为 `number` 的 `price` 属性。
...
1. 你应该定义一个名为 `renderProduct` 的函数，带有类型为 `Product` 的参数。它应返回一个表示产品的 HTML 字串，包含：
   - 一个类名为 `item` 且 ID 为产品 `id` 值的元素。
   - 一个类名为 `price` 且文本为产品 `price` 的元素。
   - 一个包含额外产品信息的元素。

1. `renderProduct` 函数应使用类型缩小，根据产品类型渲染不同的 HTML：

   - 对于书籍，渲染的 HTML 应符合 `Book: [title] by [author]` 格式，其中 `[title]` 和 `[author]` 分别替换为标题和作者。
   - 对于电子产品，渲染的 HTML 应符合 `Electronics: [item] - [model]` 格式。如果产品有保修，应追加 ` - Warranty: [warranty] year(s)`。将 `[item]`、`[model]` 和 `[warranty]` 替换为项、型号和保修。
   - 对于服装，渲染的 HTML 应符合 `Clothing: [item] by [brand]` 格式。如果产品有尺码，应追加 ` - Size [size]`。将 `[item]`、`[brand]` 和 `[size]` 替换为项、品牌和尺码。
...
1. 你应通过调用 `renderProduct` 函数将每个产品转换为 HTML 字串，并将合并结果注入 `#output` 元素来渲染产品。
...
你的 `Item` 接口应有一个类型为 `string` 的 `id` 属性。
...
你的 `Item` 接口应有一个类型为 `number` 的 `price` 属性。
...
你的 `Book` 接口应有一个类型为 `string` 的 `title` 属性。
...
你的 `Book` 接口应有一个类型为 `string` 的 `author` 属性。
...
你的 `Electronics` 接口应有一个类型为 `string` 的 `item` 属性。
...
你的 `Electronics` 接口应有一个类型为 `string` 的 `model` 属性。
...
你的 `Electronics` 接口应有一个可选的类型为 `number` 的 `warranty` 属性。
...
你的 `Clothing` 接口应有一个类型为 `string` 的 `item` 属性。
...
你的 `Clothing` 接口应有一个类型为 `string` 的 `brand` 属性。
