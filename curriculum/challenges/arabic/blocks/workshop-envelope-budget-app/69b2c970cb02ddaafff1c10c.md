...
أضف إلى عنصر `label` خاصية `for` بقيمة `X-#-name`، حيث يكون `X` هو `value` لمتغير `category` و`#` هو قيمة `entryNumber`. تذكّر أن خصائص HTML يجب أن تُحاط بعلامات اقتباس مزدوجة.

# --hints--

يجب أن يحتوي عنصر `label` على خاصية `for` مضبوطة على `${category}-${entryNumber}-name`.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

يجب أن يحتوي عنصر `label` على النص `"Expense ${entryNumber} Name"`.
