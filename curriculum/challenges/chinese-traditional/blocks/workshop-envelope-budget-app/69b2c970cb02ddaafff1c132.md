...
你的 `output.innerHTML` 字串需要一個 `span` 元素。創建它，並將其類別屬性設為 `statusClass` 變數。

在你的 `span` 元素內，將 `statusText` 變數作為文字內容加入。
...
你的 `span` 應該有一個 `class` 屬性，並設為 `${statusClass}`。
