...
`@media` のために、`only screen` の `max-width` を持つ最後の `420px` クエリを作成してください。その中に、`4.5rem` に設定された `font-size` プロパティを持つ `.hero-title` セレクターを作成してください。
...
`.hero-title` セレクターには `4.5rem` に設定された `font-size` プロパティがあるはずです。
