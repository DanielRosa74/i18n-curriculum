...
이 `input` 안에 있는 마지막 `label`와 `fieldset`에 대해, `input`와 `id` 속성이 `value`로 설정된 체크박스 `"price"`를 추가하세요. 그리고 `"choice"`로 설정된 `name` 속성을 추가하세요.

그런 다음, 텍스트가 `Price`이고 `for` 속성이 `"price"`로 설정된 `label` 요소를 추가하세요.
