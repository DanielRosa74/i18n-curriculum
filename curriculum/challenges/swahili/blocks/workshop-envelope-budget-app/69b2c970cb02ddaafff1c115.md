...
Hatua yako inayofuata ni kuandika kitendakazi ambacho kitapata jumla ya kiasi kutoka kwa maingizo ya mtumizi.

Tangaza kitendakazi cha `getTotalFromInputs`, na kipa kigezo kinachoitwa `list`.

# --hints--

Unapaswa kutangaza kigezo cha `getTotalFromInputs`.

```js
assert.isDefined(getTotalFromInputs);
```

Kigezo chako cha `getTotalFromInputs` kinapaswa kuwa kitendakazi.

```js
assert.isFunction(getTotalFromInputs);
```

Kitendakazi chako cha `getTotalFromInputs` kinapaswa kuchukua kigezo kinachoitwa `list`.
