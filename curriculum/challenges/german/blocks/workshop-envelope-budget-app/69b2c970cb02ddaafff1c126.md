...
Deklarieren Sie jetzt eine Variable `food` und weisen Sie ihr den Wert zu, der durch den Aufruf von `getTotalFromInputs` mit Ihren `foodInputs` entsteht.

# --hints--

Ihre `calculateBudget`-Funktion sollte eine Variable `food` enthalten.
...
Sie sollten `getTotalFromInputs` mit `foodInputs` als Argument aufrufen.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

Sie sollten das Ergebnis von `getTotalFromInputs` der Variable `food` zuweisen.
