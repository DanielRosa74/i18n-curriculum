...
Fügen Sie Ihrem `textarea`-Element die `id`- und `name`-Attribute mit dem Wert `"comments"` hinzu.

Setzen Sie für das `cols`-Attribut den Wert auf `"30"` und für das `rows`-Attribut den Wert auf `"10"`.
