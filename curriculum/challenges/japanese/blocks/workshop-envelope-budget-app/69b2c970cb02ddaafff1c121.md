...
`calculateBudget` 関数に、`e` パラメータの `preventDefault()` メソッドを呼び出す行を追加してください。その後、グローバルのエラーフラグ `isError` を `false` にリセットしてください。

# --hints--

`calculateBudget` 関数は `e.preventDefault()` を呼び出す必要があります。

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

`calculateBudget` 関数はグローバルのエラーフラグ `isError` を `false` にリセットする必要があります。
