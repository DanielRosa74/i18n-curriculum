...
今、`netRemaining` 変数を宣言して、`income` から `expenses` を引いた値を代入してください。

# --hints--

`calculateBudget` 関数には `netRemaining` 変数が必要です。
...
`income - expenses` の値を求めてください。

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

`income - expenses` の値を `netRemaining` に代入してください。
