...
Il tuo `catch` non dovrebbe più essere un metodo concatenato.

```js
assert.notMatch(code, /\.catch/);
```

Il tuo blocco `try` dovrebbe contenere le tre istruzioni che recuperano i dati, creano il `json` e poi lo mostrano.
...
La tua pagina degli autori di fCC dovrebbe renderizzare correttamente.
