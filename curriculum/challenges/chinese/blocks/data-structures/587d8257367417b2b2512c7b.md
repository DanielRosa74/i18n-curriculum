...
现在这个有序关系非常容易看出。注意，根节点 8 左边的每个值都小于 8，右边的每个值都大于 8。还要注意，这种关系同样适用于每个子树。例如，第一个左子节点本身就是一个子树。3 是父节点，它恰好有两个子节点——根据二叉搜索树的规则，我们甚至不用看就知道这个节点的左子节点（以及它的所有子节点）都小于 3，右子节点（以及它的所有子节点）都大于 3（但也小于整个结构的根节点值），依此类推。
