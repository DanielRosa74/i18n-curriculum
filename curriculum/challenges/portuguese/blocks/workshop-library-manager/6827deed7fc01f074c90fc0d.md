...
Seria legal testar sua função `getBooksByAuthor` com outro autor.

Comece registrando a mensagem `"\nList of books by James Clear:\n"` no console.

Abaixo daquele `console.log()`, adicione outro `console.log()`. Dentro desse `console.log()`, faça a chamada da função `getBooksByAuthor()` com `library` e `"James Clear"` como argumentos.
