---
id: 6734e879c78ee6c61db25b90
title: O que são Components em React e como eles funcionam?
challengeType: 19
dashedName: what-are-components-in-react-and-how-do-they-work
---

# --description--

Componentes são os blocos de construção das aplicações React que permitem aos desenvolvedores dividir interfaces de usuário complexas em partes menores e gerenciáveis, facilitando o desenvolvimento e a manutenção de aplicações em grande escala.

Os dois tipos de componentes em React são componentes funcionais e baseados em classe. No React moderno, os desenvolvedores usam componentes funcionais, e todo exemplo nesta lição é um componente funcional.

De forma geral, você pode pensar nos componentes como funções JavaScript que retornam elementos que descrevem a interface do usuário.

Esta interface de usuário é descrita usando JSX, uma extensão de sintaxe para JavaScript que se parece com HTML mas permite que você escreva elementos de interface de forma mais declarativa.

Vamos ver um exemplo de um componente React:

```jsx
function Greeting() {
  const name = "John";
  return (
    <h1 className="title">
      {/* The result will be Hello John */}
      Hello {name}
    </h1>
  );
}
```

Neste exemplo, definimos um componente chamado `Greeting`. As chaves `{}` dentro das tags `h1` nos permitem incorporar expressões JavaScript, permitindo acessar a variável `name` dentro do elemento `h1`.

Também estamos aplicando uma classe chamada `title` ao elemento `h1` usando o atributo `className`.

Mas por que estamos usando `className` em vez de `class` como nos elementos HTML comuns?

Isso acontece porque em JavaScript, `class` é uma palavra-chave reservada. Então, precisamos usar `className` em vez disso.

Também estamos usando um comentário dentro do JSX. Um comentário dentro do JSX precisa estar envolvido em chaves. Fora do JSX, no corpo do componente, você escreve um comentário JavaScript normal:

```jsx
{/* Block Comments */}
```

Outra coisa que você pode ter notado é que estamos usando uma letra maiúscula no início do nome do componente. Mas por que não podemos usar todas as letras minúsculas?

Isso ocorre porque o React trata componentes com letra maiúscula como componentes personalizados, enquanto elementos com letras minúsculas são considerados elementos HTML embutidos.

Quando o React encontra uma tag em minúsculas, como `<div>` ou `<span>`, ele assume que é um elemento HTML padrão. No entanto, se o nome do componente começar com uma letra maiúscula, o React o tratará como um componente definido pelo usuário e o renderizará de acordo. Essa distinção ajuda o React a diferenciar entre tags HTML nativas e componentes que você cria.

Para usar este componente `Greeting` em nossa aplicação, escreveríamos algo assim:

```jsx
<Greeting />
```

Isto renderizaria um elemento `h1` com o texto `Hello John` na página. Mas dê uma olhada mais de perto na sintaxe aqui. Quando usamos o componente, ele termina com uma barra e depois o símbolo de maior.

Ao trabalhar com JSX, todas as tags e usos de componentes precisam ser explicitamente fechados. Então, se o componente ou tag não tiver filhos, você precisa fechá-lo explicitamente como mostrado aqui:

```jsx
<Greeting /> {/* /> is required */}
```

Até agora, só temos analisado como renderizar um único elemento `h1`. Mas você pode realmente renderizar múltiplos elementos.

Vamos dar uma olhada no seguinte código de exemplo aqui:

```jsx
function Greeting() {
  const name = "John";
  // This will throw an error
  return (
    <h1>Hello {name}</h1>
    <p>Nice to meet you.</p>
  );
}
```

Estamos tentando adicionar outra frase de `Nice to meet you` mas ela não está sendo exibida corretamente na página.

Parece haver uma mensagem de erro em vez disso. A mensagem de erro diz `Adjacent JSX elements must be wrapped in an enclosing tag.`

O motivo pelo qual você está recebendo essa mensagem de erro é que múltiplos elementos irmãos precisam estar envolvidos em um elemento pai. Embora você possa envolver os elementos `h1` e `p` em um simples `div`, existe outra forma de atender a esse requisito.

Fragmentos React são usados para agrupar elementos juntos. Aqui está como o exemplo revisado ficará:

```jsx
import { Fragment } from "react";

function Greeting() {
  const name = "John";
  return (
    <Fragment>
      <h1>Hello {name}</h1>
      <p>Nice to meet you.</p>
    </Fragment>
  );
}
```

Você também pode optar por usar tags JSX vazias que podem servir como uma forma abreviada para fragments:

```jsx
function Greeting() {
  const name = "John";
  return (
    <>
      <h1>Hello {name}</h1>
      <p>Nice to meet you.</p>
    </>
  );
}
```

Nas próximas aulas, continuaremos a aprender mais sobre como trabalhar com componentes e JSX. Mas por enquanto, você teve uma introdução sólida à construção de interfaces de usuário com componentes, estabelecendo uma base forte para o que está por vir.

# --questions--

## --text--

Qual é a principal razão pela qual o React usa `className` em vez de `class` para elementos HTML em JSX?

## --answers--

`class` é uma palavra-chave reservada em JavaScript.

---

`className` soa melhor.

### --feedback--

Pense no que `class` já significa em JavaScript.

---

Usar `className` resulta em melhor segurança em apps React.

### --feedback--

Pense no que `class` já significa em JavaScript.

---

Usar `className` resulta em desempenho mais rápido.

### --feedback--

Pense no que `class` já significa em JavaScript.

## --video-solution--

1

## --text--

No contexto dos componentes React, o que é JSX?

## --answers--

Uma nova linguagem de programação.

### --feedback--

Considere como o JSX permite que você escreva código de UI em componentes React.

---

Uma linguagem de consulta de banco de dados.

### --feedback--

Considere como o JSX permite que você escreva código de UI em componentes React.

---

Uma extensão de sintaxe para JavaScript que se parece com HTML.

---

Um framework CSS.

### --feedback--

Considere como o JSX permite que você escreva código de UI em componentes React.

## --video-solution--

3

## --text--

Como você renderizaria um componente `Message` em React?

## --answers--

`</ Message>`

### --feedback--

Revise a seção onde explica como os componentes são renderizados.

---

`<</Message>`

### --feedback--

Revise a seção onde explica como os componentes são renderizados.

---

`Message`

### --feedback--

Revise a seção onde explica como os componentes são renderizados.

---

`<Message />`

## --video-solution--

4
