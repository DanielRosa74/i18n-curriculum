...
Você pode realizar cálculos com valores que representam comprimento, ângulo, tempo, porcentagens e números. Também pode combinar diferentes unidades como pixels, porcentagens e ems.

Para adição e subtração, todos os valores na expressão, também chamados de operandos, devem ter suas unidades correspondentes, como `px`, `em` e porcentagem (`%`). Dependendo do operador, operandos diferentes podem ter unidades diferentes.
...
Você também deve saber que, se usar o operador de multiplicação, um dos operandos deve ser sem unidade. Esta não seria uma expressão válida porque ambos os operandos têm unidades (pixels):

```css
calc(5px * 50px)
```

Você precisaria omitir a unidade em um deles. Ambas essas alternativas seriam válidas:
...
Para o operador de divisão, dividir por um número sem unidade escala o valor e mantém sua unidade:

```css
calc(50% / 5)
```

Dividir dois valores com unidades iguais também é válido, mas cancela as unidades para retornar um número sem unidade (como `10`). Note que valores sem unidade só podem ser usados para certas propriedades CSS (como `line-height` ou `flex-grow`):
