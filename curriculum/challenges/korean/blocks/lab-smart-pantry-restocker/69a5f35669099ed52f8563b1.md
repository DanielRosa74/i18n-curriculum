...
1. 문자열 배열을 받아 `{ sku, name, qty, expires, zone }` 속성을 가진 객체 배열을 반환하는 `parseShipment(rawData)` 함수를 구현해야 합니다.
...
하나의 매개변수 `actions`을 받는 `groupByZone` 함수를 정의하세요.
...
하나의 매개변수 `pantry`을 받는 `clonePantry` 함수를 정의하세요.
