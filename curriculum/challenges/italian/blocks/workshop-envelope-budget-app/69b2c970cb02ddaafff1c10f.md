...
Infine, su una nuova riga dopo la tua seconda `label`, crea un altro `input` elemento. Dai a questo un attributo `type` impostato su `number`, un attributo `min` impostato su `0` (per assicurarti che non possano essere aggiunti importi negativi), un attributo `placeholder` impostato su `Amount` e un attributo `id` che corrisponda all'attributo `for` del tuo secondo `label` elemento.
...
Il tuo `input` elemento dovrebbe avere un attributo `placeholder` impostato su `Amount`.
...
Il tuo `input` elemento dovrebbe avere un attributo `id` impostato su `${category}-${entryNumber}-amount`.
...
Il tuo nuovo `input` elemento dovrebbe avere un attributo `min` impostato su `0`.
