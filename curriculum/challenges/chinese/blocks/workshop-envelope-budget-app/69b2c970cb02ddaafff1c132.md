...
你的 `output.innerHTML` 字串需要一个 `span` 元素。创建它，并将它的类属性设置为 `statusClass` 变量。

在你的 `span` 元素内，添加 `statusText` 变量作为文本内容。
...
你的 `span` 应该将 `class` 属性设置为 `${statusClass}`。
