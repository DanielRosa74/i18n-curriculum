...
Como `null` é um valor nullish, a string `default` será exibida no console. O operador de coalescência nullish é extremamente útil em situações onde `null` ou `undefined` são os únicos valores que devem acionar um fallback ou valor padrão. Aqui está um exemplo de como lidar com as configurações de preferência de um usuário:
...
No exemplo acima, temos um objeto chamado `userSettings` que contém as propriedades `theme`, `volume` e `notifications`. Estamos acessando o `theme` usando notação de ponto, como `userSettings.theme`. Você aprenderá mais sobre como trabalhar com objetos em uma lição futura. Como o `theme` do usuário está atualmente definido como `null`, a string `light` será exibida no console.
