...
在 DOM 中觸發的每個事件都有某種預設行為。預設情況下，勾選框的 `click` 事件會切換該勾選框的狀態。當按下聚焦按鈕的 <kbd>Spacebar</kbd> 時，會啟動該按鈕。這些事件物件上的 `preventDefault()` 方法會阻止該行為發生。
...
`e.key` 會給你按下鍵的值，例如 <kbd>A</kbd> 鍵的 `a` 或 <kbd>Enter</kbd> 鍵的 `Enter`。
