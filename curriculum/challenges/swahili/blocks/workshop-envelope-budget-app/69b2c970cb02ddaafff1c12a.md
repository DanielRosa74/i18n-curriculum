...
Ni wakati wa kuanza kuandaa mahesabu yako. Anza kwa kutangaza kigezo cha `expenses`, na weka thamani jumla ya `rent`, `food`, `utilities`, na `entertainment` (kumbuka kwamba mpangilio ni muhimu kwa majaribio). Hakikisha kufanya hivi baada ya taarifa yako ya if.

# --hints--

Kitendakazi kinapaswa kuwa na kigezo cha `expenses`.
...
Kigezo cha `expenses` kinapaswa kuja baada ya taarifa yako ya `if`.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

Unapaswa kuhesabu jumla ya `rent`, `food`, `utilities`, na `entertainment`, kwa mpangilio.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

Unapaswa kuweka thamani jumla ya `rent`, `food`, `utilities`, na `entertainment` katika `expenses`.
