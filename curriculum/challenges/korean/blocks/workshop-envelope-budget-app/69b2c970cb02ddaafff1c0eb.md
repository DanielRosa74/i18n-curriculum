...
`id`를 사용해 `form` 요소를 가져와 `budgetForm`이라는 변수에 저장하는 것부터 시작하세요.

# --hints--

`budgetForm`이라는 변수를 만들어야 합니다.

```js
assert.isDefined(budgetForm);
```

`document.getElementById('budget-form')`가 코드에 있어야 합니다.

```js
assert.match(code, /document\.getElementById\(\s*('|")budget-form\1\s*\)/g);
```

`document.getElementById('budget-form')`를 `budgetForm` 변수에 할당해야 합니다.
