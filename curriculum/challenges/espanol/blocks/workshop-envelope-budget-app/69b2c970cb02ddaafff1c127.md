...
Siguiendo este mismo patrón, declara las variables `utilities` y `entertainment` para las entradas de tipo `number` en los elementos `#utilities` y `#entertainment`. Asígnales las llamadas apropiadas a `getTotalFromInputs`.

# --hints--

Tu función `calculateBudget` debe tener una variable `utilities`.
...
Debes llamar a `getTotalFromInputs` con `utilitiesInputs` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

Debes asignar el resultado de `getTotalFromInputs` a `utilities`.
...
Tu función `calculateBudget` debe tener una variable `entertainment`.
...
Debes llamar a `getTotalFromInputs` con `entertainmentInputs` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

Debes asignar el resultado de `getTotalFromInputs` a `entertainment`.
