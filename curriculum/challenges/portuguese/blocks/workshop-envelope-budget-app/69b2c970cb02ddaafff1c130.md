...
Agora você precisa lidar com o caso em que o usuário não está acima do orçamento (quando `netRemaining` é maior ou igual a `0`).

Adicione um bloco `else` à sua declaração `if` que atribua `statusText` a `"$X Remaining"`, onde `X` é o valor de `netRemaining`. Também atribua `statusClass` a `"surplus"`.
...
Você deve usar um template literal para `statusText`.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

Seu `statusText` deve ser atribuído como `$${netRemaining} Remaining`.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

Você deve atribuir `"surplus"` a `statusClass`.
