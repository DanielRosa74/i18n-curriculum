...
**注意：** `normalizeUnits`、`validateManifest` 和 `processManifest` 都不應該用 `const` 宣告，因為測試需要重新指定它們。
...
`normalizeUnits`、`validateManifest` 和 `processManifest` 都不應該用 `const` 宣告，因為測試需要重新指定它們。
