...
你还需要获取你的 `#income` 输入的值。你已经在代码顶部查询过它，并将其赋值给了 `incomeInput` 变量。不过，你使用了 `getElementById`，它返回的是一个 `Element`，而不是 `NodeList`。

`NodeList` 是一个类数组对象，这意味着你可以对它进行迭代，并且它与数组共享一些常用方法。对于你的 `getTotalFromInputs` 函数，数组作为参数的效果和 `NodeList` 一样好。

声明一个 `income` 变量，并将调用 `getTotalFromInputs` 的结果赋值给它——传入一个包含你的 `incomeInput` 的数组作为参数。

# --hints--

你的 `calculateBudget` 函数应该有一个 `income` 变量。
...
你应该用 `[incomeInput]` 作为参数调用 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

你应该将 `getTotalFromInputs` 的结果赋值给 `income`。
