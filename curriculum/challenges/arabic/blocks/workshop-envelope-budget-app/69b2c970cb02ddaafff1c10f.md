...
أخيرًا، في سطر جديد بعد `label` الثاني، أنشئ عنصر `input` آخر. اجعل خاصية `type` له تساوي `number`، وخاصية `min` تساوي `0` (لضمان عدم إضافة مبالغ سالبة)، وخاصية `placeholder` تساوي `Amount`، وخاصية `id` تطابق خاصية `for` في عنصر `label` الثاني.
...
يجب أن يحتوي عنصر `input` على خاصية `placeholder` تساوي `Amount`.
...
يجب أن يحتوي عنصر `input` على خاصية `id` تساوي `${category}-${entryNumber}-amount`.
...
يجب أن يحتوي عنصر `input` الجديد على خاصية `min` تساوي `0`.
