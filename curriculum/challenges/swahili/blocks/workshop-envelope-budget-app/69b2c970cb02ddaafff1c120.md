...
Sasa ni wakati wa kuanza kuunganisha yote pamoja. Tangaza kitendakazi tupu cha `calculateBudget`, ambacho kinachukua kigezo kinachoitwa `e`. Kitendakazi hiki kitakuwa msikilizaji mwingine wa tukio, kwa hivyo hoja ya kwanza itakayotumwa itakuwa tukio la kivinjari – `e` ni jina la kawaida kwa kigezo hiki.

# --hints--

Unapaswa kutangaza kigezo cha `calculateBudget`.

```js
assert.isDefined(calculateBudget);
```

Kigezo chako cha `calculateBudget` kinapaswa kuwa kitendakazi.

```js
assert.isFunction(calculateBudget);
```

Kitendakazi chako cha `calculateBudget` kinapaswa kuchukua hoja ya `e`.
