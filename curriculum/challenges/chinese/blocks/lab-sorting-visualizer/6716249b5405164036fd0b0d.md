...
本实验已为你提供了所有的 HTML 和 CSS。你将使用 JavaScript 完成冒泡排序可视化工具，使其能够可视化冒泡排序算法对五个整数数组进行排序所需的每一步。
...
1. 你应该有一个名为 `fillArrContainer` 的函数，第一个参数是一个 HTML 元素，第二个参数是一个数组。
...
1. 你应该有一个名为 `highlightCurrentEls` 的函数，它接受一个 HTML 元素和一个数字索引。
