...
Tu ciclo necesita actualizarse para ejecutarse también cuando `i` sea `8`. Según tu lógica, esto significa que tu ciclo debe ejecutarse cuando `i` sea <dfn>menor o igual que</dfn> `count`. Puedes usar el operador menor o igual que `<=` para esto.
