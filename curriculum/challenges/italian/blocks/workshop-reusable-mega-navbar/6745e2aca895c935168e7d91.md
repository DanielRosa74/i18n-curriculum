...
Per concludere, imposta il testo del primo elemento `a` su `Calendar`, il secondo su `Chat` e il terzo su `Email`.

Con questo, il tuo componente `Navbar` è completo!

# --hints--

Dovresti impostare il testo del primo elemento `a` su `Calendar`.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Dovresti impostare il testo del secondo elemento `a` su `Chat`.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Dovresti impostare il testo del terzo elemento `a` su `Email`.
