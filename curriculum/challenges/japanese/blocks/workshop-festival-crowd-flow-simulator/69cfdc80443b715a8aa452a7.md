...
- 入力の`gates`配列の各ゲートを`for`ループで繰り返し処理してください。各ゲートに対して、`summary`に次のプロパティを追加します：
  - キーはゲートの`id`です。
  - 値は`0`です。
- ループの後に、`summary`オブジェクトを返してください。

`initializeThroughput` 関数は、元の入力配列 `gates` を変更してはいけません。

# --hints--

`initializeThroughput`関数は、`[]`を渡されたときにソースの入力を変更せずに`{}`を返す必要があります。
...
`initializeThroughput`関数は、`[{ id: "North", capacity: 5, queue: [1, 2, 3] }]`を渡されたときにソースの入力を変更せずに`{ North: 0 }`を返す必要があります。
...
`initializeThroughput`関数は、`[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]`を渡されたときにソースの入力を変更せずに`{ North: 0, East: 0, South: 0 }`を返す必要があります。
...
`initializeThroughput`関数は、ループの後に`summary`を返す必要があり、それが関数の最後の行になります。
