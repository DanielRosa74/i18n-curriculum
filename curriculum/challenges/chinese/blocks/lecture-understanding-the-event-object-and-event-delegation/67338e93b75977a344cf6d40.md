...
然后，当你点击 `span` 元素时，你会在控制台看到 `<span>Click me~!</span>` 的日志。
...
现在，每次我们直接点击一个 `span`，该元素的文本都会变成红色。

因为监听器附加在父级 `p` 元素上，所以点击 `p` 元素的任何地方，包括 `span` 元素周围或之间的空白区域，也会触发事件。当你点击那里时，`event.target` 是 `p` 元素本身，因此红色会应用到 `p` 元素上。并且由于 `color` 是一个继承的 CSS 属性，所有四个 `span` 元素会同时变成红色。
