...
オーバーフローが存在する場合（`result.overflow > 0`）、`handleGateAtTick` 関数は最初にメッセージ `"Overflow of " + result.overflow + " attendees. Rerouting..."` をログに記録する必要があります。
...
オーバーフローが存在する場合（`result.overflow > 0`）、`handleGateAtTick` 関数は `gates`、`gate`、`tickIndex`、および `result.overflow` を引数にして `rerouteOverflow()` を呼び出し、オーバーフローをルート変更する必要があります。
