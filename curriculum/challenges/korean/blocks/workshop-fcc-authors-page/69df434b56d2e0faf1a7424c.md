...
`fetch` 호출이 비동기 함수나 모듈의 메인 본문에 정의되어 있지 않기 때문에, 이를 고치기 전에는 `await`를 사용할 수 없습니다.
...
`initialFetch` 함수는 화살표 함수 구문을 사용해야 합니다.
