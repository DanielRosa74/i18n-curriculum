...
Sua função `handleGateAtTick` deve primeiro registrar a mensagem `"Overflow of " + result.overflow + " attendees. Rerouting..."` quando houver overflow (`result.overflow > 0`).
...
Sua função `handleGateAtTick` deve redirecionar o overflow chamando `rerouteOverflow()` com `gates`, `gate`, `tickIndex` e `result.overflow` quando houver overflow (`result.overflow > 0`).
