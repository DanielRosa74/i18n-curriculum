...
في هذا المختبر، ستُولد رمز مرور لمرة واحدة (OTP) مكون من 6 أرقام وتعرضه للمستخدم. سينتهي صلاحية الرمز بعد 5 ثوانٍ، وسيتم توليد رمز جديد عند نقر المستخدم على زر `"Generate OTP"`.

**الهدف:** إكمال قصص المستخدم أدناه واجتياز جميع الاختبارات لإتمام المختبر.

**قصص المستخدم:**

1. يجب استخدام الخطاف `useEffect` لإدارة مؤقت العد التنازلي.
...
3. يجب أن يحتوي عنصر `div` الذي يحمل الصنف `container` على العناصر التالية:
    ...
    - عنصر `h2` يحمل المعرف `otp-display` يعرض إما الرسالة `"Click 'Generate OTP' to get a code"` أو يعرض رمز OTP المولد إذا كان متاحًا.
    - عنصر `p` يحمل المعرف `otp-timer` وبه خاصية `aria-live` بقيمة صحيحة بحيث:
        - يبدأ فارغًا.
        - يعرض الرسالة `"Expires in: X seconds"` بعد نقر الزر، حيث يمثل `X` الوقت المتبقي قبل انتهاء صلاحية الرمز.
        - يعرض الرسالة `"OTP expired. Click the button to generate a new OTP."` بمجرد وصول العد التنازلي إلى `0`.
    - عنصر `button` يحمل المعرف `generate-otp-button` وعليه النص `"Generate OTP"`. عند النقر عليه، يجب توليد رمز OTP جديد وبدء عد تنازلي لمدة 5 ثوانٍ.
    - يجب تعطيل زر `"Generate OTP"` أثناء تشغيل العد التنازلي.

4. يجب التأكد من توقف مؤقت العد التنازلي تلقائيًا بمجرد وصوله إلى `0` ثانية لمنع التحديثات غير الضرورية.
5. يجب أن يكون رمز OTP المولد مكونًا من 6 أرقام.
...
لا يجب إزالة التعيين الحالي `const { useState, useEffect, useRef } = React;` من الكود.
...
يجب أن يحتوي عنصر `.container` على عنصر `h1` يحمل المعرف `otp-title` والنص `"OTP Generator"`.
...
يجب أن يحتوي عنصر `.container` على عنصر `h2` يحمل المعرف `otp-display` يعرض رمز OTP عند توليده.
...
يجب أن يعرض عنصر `h2` داخل `.container` في البداية الرسالة `"Click 'Generate OTP' to get a code"`.
...
يجب أن يحتوي عنصر `p` على خاصية `aria-live` بقيمة `assertive` أو `polite`.
...
يجب أن يكون عنصر `p` فارغًا في البداية.
...
يجب أن يحتوي عنصر `.container` على زر `button` يحمل المعرف `generate-otp-button` والنص `"Generate OTP"`.
...
يجب أن يعرض عنصر `h2` الذي يحمل المعرف `otp-display` رمز OTP جديدًا عند نقر الزر.
...
يجب أن يكون رمز OTP المولد مكونًا من 6 أرقام.
...
يجب أن يعرض عنصر `p` الذي يحمل المعرف `otp-timer` عدًا تنازليًا لمدة 5 ثوانٍ عند نقر الزر.
...
يجب أن يحدث عنصر `p` الذي يحمل المعرف `otp-timer` رسالته كل ثانية ليعرض الوقت المتبقي.
...
يجب عرض الرسالة `OTP expired. Click the button to generate a new OTP.` عندما يصل مؤقت العد التنازلي إلى `0`.
