...
Ihre `handleGateAtTick`-Funktion sollte zuerst die Nachricht `"Overflow of " + result.overflow + " attendees. Rerouting..."` protokollieren, wenn ein Überlauf vorliegt (`result.overflow > 0`).
...
Ihre `handleGateAtTick`-Funktion sollte den Überlauf umleiten, indem sie `rerouteOverflow()` mit `gates`, `gate`, `tickIndex` und `result.overflow` aufruft, wenn ein Überlauf vorliegt (`result.overflow > 0`).
