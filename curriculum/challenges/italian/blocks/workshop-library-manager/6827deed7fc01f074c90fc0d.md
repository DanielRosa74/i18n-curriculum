...
Sarebbe bello testare la tua funzione `getBooksByAuthor` con un altro autore.

Inizia registrando il messaggio `"\nList of books by James Clear:\n"` nella console.

Sotto quel `console.log()`, aggiungi un altro `console.log()`. All'interno di quel `console.log()`, chiama la funzione `getBooksByAuthor()` con `library` e `"James Clear"` come argomenti.
