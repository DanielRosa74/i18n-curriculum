...
到目前为止，你已经了解了如何使用 `if/else`、`&&` 和三元操作符（`condition ? expressionIfTrue : expressionIfFalse`）来有条件地决定渲染什么以及何时渲染。然而，还有一个重要的话题需要讨论，它让你可以将这些概念中的任何一个或全部与另一个强大的 React 特色结合起来：props。使用 props 来有条件地渲染代码在 React 开发者中非常常见——也就是说，他们使用某个 prop 的值来自动决定渲染什么。
