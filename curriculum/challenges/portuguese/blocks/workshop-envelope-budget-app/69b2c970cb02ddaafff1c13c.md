...
Depois que seu loop terminar, você precisa limpar os campos `incomeInput` e `rentInput`. Defina a propriedade `value` de `incomeInput` e `rentInput` como uma string vazia.

# --hints--

Sua função `clearForm` deve acessar a propriedade `value` do elemento `incomeInput`.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

Sua função `clearForm` deve definir a propriedade `value` do elemento `incomeInput` como uma string vazia.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

Você deve modificar o `incomeInput` depois do seu loop `for`.
...
Sua função `clearForm` deve acessar a propriedade `value` do elemento `rentInput`.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

Sua função `clearForm` deve definir a propriedade `value` do elemento `rentInput` como uma string vazia.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

Você deve modificar o `rentInput` depois do seu loop `for`.
