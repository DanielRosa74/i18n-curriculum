...
Puedes enviar una solicitud <b>POST</b> que contenga `comment` como datos del cuerpo del formulario a `/api/books/{_id}` para agregar un comentario a un libro. La respuesta devuelta será el objeto del libro similar a la solicitud <b>GET</b> `/api/books/{_id}` en una prueba anterior. Si no se incluye `comment` en la solicitud, devuelve la cadena `missing required field comment`. Si no se encuentra el libro, devuelve la cadena `no book exists`.
...
Puedes enviar una solicitud <b>DELETE</b> a `/api/books/{_id}` para eliminar un libro de la colección. La respuesta devuelta será la cadena `delete successful` si tiene éxito. Si no se encuentra el libro, devuelve la cadena `no book exists`.
