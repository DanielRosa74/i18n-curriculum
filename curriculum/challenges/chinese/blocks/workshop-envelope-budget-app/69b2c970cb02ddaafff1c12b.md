...
现在声明一个 `netRemaining` 变量，并赋值为 `income` 减去 `expenses` 的结果。

# --hints--

你的 `calculateBudget` 函数应该有一个 `netRemaining` 变量。
...
你应该查找 `income - expenses` 的值。

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

你应该将 `income - expenses` 的值赋值给 `netRemaining`。
