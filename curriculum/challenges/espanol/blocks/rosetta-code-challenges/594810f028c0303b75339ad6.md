...
El número decimal once puede escribirse como `0*13 + 1*8 + 0*5 + 1*3 + 0*2 + 0*1` o `010100` en notación posicional donde las columnas representan la multiplicación por un miembro particular de la secuencia. Se eliminan los ceros a la izquierda, por lo que 11 decimal se convierte en `10100`. Sin embargo, 10100 no es la única forma de obtener 11 a partir de los números de Fibonacci; `0*13 + 1*8 + 0*5 + 0*3 + 1*2 + 1*1` o 010011 también representaría el 11 decimal. Para un número Zeckendorf verdadero existe la restricción adicional de que *no se pueden usar dos números de Fibonacci consecutivos*, lo que conduce a la solución única anterior.
