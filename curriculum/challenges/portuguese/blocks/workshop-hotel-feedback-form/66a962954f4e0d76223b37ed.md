...
Abaixo dessa `input`, adicione um elemento `label` com o texto `Location` e o atributo `for` definido como `"location"`.
...
Abaixo dessa `input`, adicione um elemento `label` com o texto `Reputation` e o atributo `for` definido como `"reputation"`.
...
Sua quarta `input` do tipo caixa de seleção deve ter um atributo `id` definido como `"reputation"`.
