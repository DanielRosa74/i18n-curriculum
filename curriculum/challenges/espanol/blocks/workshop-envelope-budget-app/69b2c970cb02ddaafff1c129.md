...
Tu función `getTotalFromInputs` establecerá la bandera global de error `isError` en `true` si se detecta una entrada inválida.

Agrega una sentencia `if` a tu función `calculateBudget` que verifique la veracidad de tu bandera global de error, y si es verdadera usa `return` para terminar la ejecución de la función.

# --hints--

Tu función `calculateBudget` debe tener una sentencia `if` que verifique la veracidad de la variable `isError`.
...
Tu sentencia `if` debe colocarse después de la última llamada a la función `getTotalFromInputs`.
