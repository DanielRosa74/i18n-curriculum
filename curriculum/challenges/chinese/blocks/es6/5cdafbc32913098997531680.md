...
一个 promise 有三种状态：`pending`、`fulfilled` 和 `rejected`。你在上一个挑战中创建的 promise 永远停留在 `pending` 状态，因为你没有添加完成 promise 的方法。传递给 promise 参数的 `resolve` 和 `reject` 用来完成这个操作。当你希望 promise 成功时使用 `resolve`，当你希望它失败时使用 `reject`。它们是带有参数的方法，如下所示。
