...
`textarea` 要素に、値が `"comments"` の `id` 属性と `name` 属性を追加してください。

`cols` 属性には値を `"30"` に、`rows` 属性には値を `"10"` に設定してください。

# --hints--

`textarea` 要素には `"comments"` に設定された `id` 属性が必要です。

```js
assert.exists(document.querySelector('textarea#comments'));
```

`textarea` 要素には `"comments"` に設定された `name` 属性が必要です。

```js
assert.exists(document.querySelector('textarea[name="comments"]'));
```

`textarea` 要素には `"30"` に設定された `cols` 属性が必要です。

```js
assert.exists(document.querySelector('textarea[cols="30"]'));
```

`textarea` 要素には `"10"` に設定された `rows` 属性が必要です。
