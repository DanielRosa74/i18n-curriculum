...
За тим самим шаблоном зробіть запит для ваших `number` вхідних даних у елементі `#entertainment` і призначте їх змінній `entertainmentInputs`.

# --hints--

Ви повинні оголосити змінну `entertainmentInputs`.
...
Ви повинні зробити виклик `document.querySelectorAll()` з аргументом `#entertainment input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Ви повинні призначити результат виклику `document.querySelectorAll()` змінній `entertainmentInputs`.
