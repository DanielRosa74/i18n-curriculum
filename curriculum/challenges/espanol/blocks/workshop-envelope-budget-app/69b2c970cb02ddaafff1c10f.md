...
Finalmente, en una nueva línea después de tu segundo `label`, crea otro elemento `input`. Dale un atributo `type` con valor `number`, un atributo `min` con valor `0` (para asegurar que no se puedan agregar cantidades negativas), un atributo `placeholder` con valor `Amount` y un atributo `id` que coincida con el atributo `for` de tu segundo elemento `label`.
...
Tu elemento `input` debe tener un atributo `placeholder` con valor `Amount`.
...
Tu elemento `input` debe tener un atributo `id` con valor `${category}-${entryNumber}-amount`.
...
Tu nuevo elemento `input` debe tener un atributo `min` con valor `0`.
