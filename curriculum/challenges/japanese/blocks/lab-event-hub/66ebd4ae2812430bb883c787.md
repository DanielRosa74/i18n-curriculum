...
1. `header` 要素を用意し、`body` の最初の要素にしてください。
1. `header` 要素の中に、テキストが `Event Hub` の `h1` 要素と、`nav` 要素を入れてください。
1. `nav` 要素の中に、ページの異なるセクションへのリンクを含む2つの項目からなる番号なしリストを入れてください。最初の項目はテキストが `Upcoming Events`、2番目の項目はテキストが `Past Events` である必要があります。
1. それぞれのリンクは、対応するページのセクション `#upcoming-events` と `#past-events` にリンクする `href` 属性を持つ `a` 要素で表してください。
1. ページの異なるセクションを含む `main` 要素を用意してください。
1. `main` 要素の中に2つの `section` 要素を入れてください。
1. 最初の `section` 要素には、`id` 属性に `upcoming-events` を設定してください。
1. `#upcoming-events` セクションの中には、
