...
Seu elemento de `input` para renda precisa de alguns atributos adicionais para garantir que os usuários insiram valores válidos.

Atualize o elemento `#income` que você criou com os seguintes atributos:

- `type` definido como `number` (para permitir apenas entradas numéricas)
- `min` definido como `0` (para permitir apenas números positivos)
- `placeholder` definido como `e.g. 2000`
- Por fim, marque o elemento `input` como `required`.
...
Seu elemento `input` deve ter um atributo `placeholder` definido como `e.g. 2000`.
