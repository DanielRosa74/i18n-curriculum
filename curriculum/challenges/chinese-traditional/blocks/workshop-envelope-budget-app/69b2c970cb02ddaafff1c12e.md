...
在你的 `if` 述語中，你需要設值 `statusText` 來顯示使用者超出預算的金額。

將 `statusText` 設為一個模板字串，顯示 `"Over Budget by $X"`，其中 `X` 是 `netRemaining` 的絕對值。使用 `Math.abs()` 來取得絕對值。

# --hints--

你應該在 `if` 述語內為 `statusText` 指定一個值。

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)\s*\{[\s\S]*statusText\s*=/);
```

你的 `statusText` 應該使用模板字串。

```js
assert.match(code, /statusText\s*=\s*`/);
```

你的模板字串應包含文字「Over Budget by $」。

```js
assert.match(code, /statusText\s*=\s*`Over\s+Budget\s+by\s+\$\$\{/);
```

你應該使用 `Math.abs(netRemaining)` 來取得絕對值。
