...
تحتاج إلى معرفة ما إذا كان المستخدم تجاوز الميزانية أو لديه أموال متبقية. عندما تتجاوز النفقات الدخل، تكون قد تجاوزت الميزانية (عجز). عندما يتجاوز الدخل النفقات، يكون لديك أموال متبقية (فائض). يمكن اعتبار أن يكون المتبقي صفرًا تمامًا على أنه توازن.

أعلن عن متغيرين: `statusText` و `statusClass`. ابدأ كلاهما كسلاسل نصية فارغة باستخدام `""`.

# --hints--

يجب أن يحتوي `calculateBudget` على متغير `statusText`.
...
يجب أن يحتوي `calculateBudget` على متغير `statusClass`.
...
يجب أن تبدأ `statusText` و `statusClass` كسلاسل نصية فارغة.
...
يجب إعلان كلا المتغيرين باستخدام `let`.
