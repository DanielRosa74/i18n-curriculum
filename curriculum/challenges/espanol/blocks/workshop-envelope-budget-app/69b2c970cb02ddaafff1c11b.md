...
En JavaScript, los valores pueden ser <dfn>verdaderos</dfn> o <dfn>falsos</dfn>. Un valor es verdadero si se evalúa como `true` al convertirse en un booleano. Un valor es falso si se evalúa como `false` al convertirse en un booleano. `null` es un ejemplo de un valor falso.
...
Debes agregar una sentencia `if` a tu función `getTotalFromInputs`.
