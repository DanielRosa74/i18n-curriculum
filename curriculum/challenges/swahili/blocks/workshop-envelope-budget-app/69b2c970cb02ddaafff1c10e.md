...
Tengeneza kipengele kingine cha `label` (katika mstari mpya) mwishoni mwa `HTMLString` yako. `label` hii inapaswa kuwa na maandishi `Expense # Amount`, ukitumia sintaksia yako ya template literal kubadilisha `#` na thamani ya `entryNumber`, na sifa ya `for` iwe imewekwa kuwa `X-#-amount`, ambapo `X` ni thamani ya `category` na `#` ni thamani ya `entryNumber`.
...
Kipengele chako kipya cha `label` kinapaswa kuwa na sifa ya `for` iliyowekwa kuwa `${category}-${entryNumber}-amount`.
...
Kipengele chako kipya cha `label` kinapaswa kuwa na maandishi `Expense ${entryNumber} Amount`.
