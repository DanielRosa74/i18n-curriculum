...
Todo evento que é disparado no DOM tem algum tipo de comportamento padrão. O evento `click` em uma caixa de seleção alterna o estado dessa caixa, por padrão. Pressionar a <kbd>barra de espaço</kbd> em um botão com foco ativa o botão. O método `preventDefault()` nesses objetos de evento impede que esse comportamento aconteça.
...
`e.key` fornece o valor da tecla pressionada, como `a` para a tecla <kbd>A</kbd> ou `Enter` para a tecla <kbd>Enter</kbd>.
