...
`body-parser` 已经安装并且在你的项目的 `package.json` 文件中。在 `myApp.js` 文件顶部使用 `require` 引入它，并将其保存到名为 `bodyParser` 的变量中。处理 URL 编码数据的中间件由 `bodyParser.urlencoded({extended: false})` 返回。将前一个方法调用返回的函数传递给 `app.use()`。像往常一样，中间件必须挂载在所有依赖它的路由之前。
