...
ユーザーが予算オーバーでない場合（`netRemaining` が `0` 以上の場合）を処理する必要があります。

`if` 文に `else` ブロックを追加し、`statusText` を `"$X Remaining"` に設定してください。ここで `X` は `netRemaining` の値です。また、`statusClass` を `"surplus"` に設定してください。
