...
A função totiente de Euler, ${\phi}(n)$ (às vezes chamada de função phi), é usada para determinar o número de números positivos menores ou iguais a `n` que são primos relativos a `n`. Por exemplo, como 1, 2, 4, 5, 7 e 8 são todos menores que nove e primos relativos a nove, ${\phi}(9) = 6$. O número 1 é considerado primo relativo a todo número positivo, então ${\phi}(1) = 1$.
