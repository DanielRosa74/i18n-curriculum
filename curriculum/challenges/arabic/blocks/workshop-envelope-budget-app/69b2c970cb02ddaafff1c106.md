...
استخدم الربط لدمج متغير `category` مع `' .input-container'` (لاحظ المسافة قبل `.input-container`)، ومرر تلك السلسلة إلى `querySelector()`.
...
يجب أن تربط `' .input-container'` بعد `category`. تذكر تضمين المسافة في بداية `.input-container`.
