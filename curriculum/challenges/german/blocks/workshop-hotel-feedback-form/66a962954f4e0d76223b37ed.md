...
Fügen Sie unter diesem `input` ein `label`-Element mit dem Text `Location` und dem `for`-Attribut mit dem Wert `"location"` hinzu.
...
Fügen Sie unter diesem `input` ein `label`-Element mit dem Text `Reputation` und dem `for`-Attribut mit dem Wert `"reputation"` hinzu.
...
Ihre vierte Kontrollkästchen-`input` sollte ein `id`-Attribut mit dem Wert `"reputation"` haben.
