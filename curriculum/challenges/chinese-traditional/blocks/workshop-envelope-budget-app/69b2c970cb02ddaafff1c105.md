...
你需要知道這筆支出屬於哪個類別。幸好你已經為使用者添加了一個下拉選單來選擇類別。

請記得你先前在 JavaScript 中查詢過該下拉選單並將它指定給 `entryDropdown` 變數。你可以使用 <dfn>value</dfn> 屬性來取得所選擇選項的值。

將 `entryDropdown` 的 `value` 屬性指定給一個名為 `category` 的變數。


# --hints--

你的 `addEntry` 函式（程式）應該有一個 `category` 變數。
...
你的 `category` 變數應該被指定為 `entryDropdown.value` 的值。
