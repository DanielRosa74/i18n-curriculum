...
下面是一个在 HTML 文档中使用 `script` 元素的示例。删除 `//` 前面的 `alert("Welcome to freeCodeCamp");`，你应该会看到弹出警告。要查看预览，你需要启用交互式编辑器。
...
这里使用 `src` 属性来指定外部 JavaScript 文件的位置。`src` 代表“source”（来源）。不建议将所有 JavaScript 代码都放在超文本标记语言文档中，是因为关注点分离。关注点分离是一种设计原则，你将程序分成不同的部分，每个部分负责处理不同的关注点。在这里，我们希望将 JavaScript 代码与超文本标记语言代码分开。
