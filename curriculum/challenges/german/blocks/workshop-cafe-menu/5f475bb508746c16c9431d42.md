...
Das Bild, das Sie hinzugefügt haben, ist nicht horizontal zentriert wie die `Coffee`-Überschrift darüber. `img`-Elemente sind inline-Elemente, daher befinden sie sich im Textfluss.

Damit sich das Bild wie ein Blockelement wie eine Überschrift verhält, erstellen Sie einen `img`-Typselektor. Setzen Sie die `display`-Eigenschaft auf `block` und verwenden Sie die Eigenschaften `margin-left` und `margin-right`, um es horizontal zu zentrieren.
