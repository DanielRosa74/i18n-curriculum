...
Cada evento que se activa en el DOM tiene algún tipo de comportamiento predeterminado. El evento `click` en una casilla de verificación cambia el estado de esa casilla, por defecto. Presionar la <kbd>barra espaciadora</kbd> en un botón con enfoque activa el botón. El método `preventDefault()` en estos objetos de evento detiene que ese comportamiento ocurra.
...
`e.key` te da el valor de la tecla presionada, como `a` para la tecla <kbd>A</kbd> o `Enter` para la tecla <kbd>Enter</kbd>.
