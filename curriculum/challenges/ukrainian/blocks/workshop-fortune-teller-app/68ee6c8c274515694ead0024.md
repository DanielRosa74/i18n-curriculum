...
Прогноз має бути випадковим, тому потрібна допоміжна функція для вибору випадкового прогнозу. Оголосіть змінну з ім’ям `chosenCard` всередині методу `singleCardSelected`. Використайте функцію `getRandomItem`, щоб отримати випадкову карту, передавши `this.cards` як аргумент.
