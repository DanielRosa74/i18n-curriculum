...
`@media`의 `only screen`를 가진 `max-width`에 대해 세 번째 `550px` 쿼리를 만드세요. 그 안에 `6rem`로 설정된 `font-size`를 가진 `.hero-title` 선택자, `1.8rem`로 설정된 `font-size`를 가진 `.hero-subtitle, .author, .quote, .list-title` 선택자, `2rem`로 설정된 `font-size`를 가진 `.social-icons` 선택자, 그리고 `1.6rem`로 설정된 `font-size`를 가진 `.text` 선택자를 만드세요.
