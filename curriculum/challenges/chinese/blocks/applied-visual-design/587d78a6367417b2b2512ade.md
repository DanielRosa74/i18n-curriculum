...
世界上最受欢迎的形状之一是心形，在本挑战中你将使用纯 CSS 创建一个心形。但首先，你需要了解 `::before` 和 `::after` 伪元素。`::before` 创建一个伪元素，作为所选元素的第一个子元素；`::after` 创建一个伪元素，作为所选元素的最后一个子元素。在下面的示例中，使用了一个 `::before` 伪元素为具有 `heart` 类的元素添加一个矩形：
