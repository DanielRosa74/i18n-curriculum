...
`@media`에 대해 `only screen`의 `max-width`를 가진 마지막 `420px` 쿼리를 하나 만드세요. 그 안에 `4.5rem`로 설정된 `font-size` 속성을 가진 `.hero-title` 선택자를 만드세요.
...
`.hero-title` 선택자는 `4.5rem`로 설정된 `font-size` 속성을 가져야 합니다.
