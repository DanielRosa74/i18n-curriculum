...
- Andika ujumbe, `"\nTick " + (tickIndex + 1)`.
- Pitia kila lango katika safu ya data ya `gates` na fanya wito wa `handleGateAtTick()` kwa hoja hizi:
  - `gates`: Safu ya data kamili ya vitu vya lango.
  - `gate`: Lango linalosindikwa kwa sasa.
  - `tickIndex`: Nafasi ya tick ya sasa katika safu ya data ya `queue`.
  - `throughputSummary`: Kitu cha ndani kinachofuatilia jumla ya watu waliosindikwa kwa kila lango.
- Ongeza `tickIndex` kwa 1 ili uende kwenye tick inayofuata.
...
Unapaswa kupitia kila lango katika safu ya data ya `gates` baada ya kuandika kwenye konsoli, na fanya wito wa `handleGateAtTick()` kwa kutumia `gates`, `gate`, `tickIndex`, na `throughputSummary`.
...
Unapaswa kuongeza `tickIndex` kwa 1 baada ya kuzunguka kwenye safu ya data ya `gates`.
