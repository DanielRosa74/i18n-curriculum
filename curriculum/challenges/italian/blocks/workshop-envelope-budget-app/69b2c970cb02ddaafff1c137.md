...
Se clicchi sul pulsante `Calculate Budget`, vedrai che non succede nulla. Devi ancora aggiungere il listener per l'evento.

Aggiungi un listener per l'evento al tuo elemento `budgetForm`. Il tipo di evento dovrebbe essere `submit` e la funzione di callback dovrebbe essere `calculateBudget`.

# --hints--

Dovresti usare il metodo `.addEventListener()` del tuo elemento `budgetForm`.
...
Dovresti passare `calculateBudget` come secondo argomento a `.addEventListener()`.
