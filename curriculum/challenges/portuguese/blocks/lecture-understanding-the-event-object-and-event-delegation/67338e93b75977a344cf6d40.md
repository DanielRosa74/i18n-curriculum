...
Então, quando você clicar no elemento `span` verá `<span>Click me~!</span>` registrado no console.
...
Agora, toda vez que clicamos diretamente em um `span`, o texto desse elemento fica vermelho.

Como o listener está anexado ao elemento pai `p`, clicar em qualquer lugar dentro do `p`, incluindo o espaço vazio ao redor ou entre os elementos `span`, também dispara o evento. Quando você clica ali, `event.target` é o próprio elemento `p`, então a cor vermelha é aplicada ao elemento `p`. E como `color` é uma propriedade CSS herdada, todos os quatro elementos `span` ficam vermelhos ao mesmo tempo.
