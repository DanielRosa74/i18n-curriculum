...
Sempre all'interno del tuo terzo elemento `li`, crea un elemento `ul` per il popup. Dagli un `className` di `sub-menu` e un `aria-label` di `Apps`.
...
Il tuo elemento `ul` dovrebbe avere un `className` di `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Il tuo elemento `ul` dovrebbe avere un `aria-label` di `Apps`.
