...
Agora que você tem suas listas de elementos, pode passá-las para sua função `getTotalFromInputs` para extrair o total.

Declare uma variável `rent` e atribua a ela o resultado da chamada de `getTotalFromInputs` com `[rentInput]` como argumento.

# --hints--

Sua função `calculateBudget` deve ter uma variável `rent`.
...
Você deve chamar `getTotalFromInputs` com `[rentInput]` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

Você deve atribuir o resultado de `getTotalFromInputs` a `rent`.
