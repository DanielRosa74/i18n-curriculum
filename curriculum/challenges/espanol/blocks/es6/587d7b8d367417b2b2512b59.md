...
La sintaxis difiere en un punto clave. El valor importado, `add`, no está rodeado por llaves (`{}`). `add` aquí es simplemente un nombre de variable para lo que sea la exportación predeterminada del archivo `math_functions.js`. Puedes usar cualquier nombre aquí al importar un predeterminado.
