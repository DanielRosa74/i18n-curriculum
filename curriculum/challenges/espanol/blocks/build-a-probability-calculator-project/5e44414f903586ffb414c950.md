...
Un sombrero siempre se creará con al menos una bola. Los argumentos pasados al objeto sombrero al crearlo deben convertirse en una variable de instancia `contents`. `contents` debe ser una lista de cadenas que contenga un elemento por cada bola en el sombrero. Cada elemento en la lista debe ser el nombre de un color que represente una sola bola de ese color. Por ejemplo, si tu sombrero es `{'red': 2, 'blue': 1}`, `contents` debe ser `['red', 'red', 'blue']`.
