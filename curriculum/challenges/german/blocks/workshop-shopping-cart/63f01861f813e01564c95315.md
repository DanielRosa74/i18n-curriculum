...
Fügen Sie innerhalb Ihres `div` zwei `p`-Elemente hinzu. Setzen Sie den Text des zweiten `p`-Elements auf ein Dollarzeichen, gefolgt vom Wert der Variablen `price`.
...
Ihr zweites `p`-Element sollte ein Dollarzeichen gefolgt vom Wert der Variablen `price` enthalten.
