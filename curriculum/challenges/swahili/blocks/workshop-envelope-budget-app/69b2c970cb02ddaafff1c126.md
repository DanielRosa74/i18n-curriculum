...
Sasa tangaza `food` kama kigezo, na weka thamani yake kuwa wito wa `getTotalFromInputs` ukiwa na `foodInputs` yako.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `food`.
...
Unapaswa kufanya wito wa `getTotalFromInputs` ukiwa na `foodInputs` kama hoja.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya `getTotalFromInputs` kwa `food`.
