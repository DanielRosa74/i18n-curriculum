...
Sasa unahitaji kushughulikia hali ambapo mtumizi hayazidi bajeti (wakati `netRemaining` ni kubwa au sawa na `0`).

Ongeza kipande cha `else` kwenye usemi wako wa `if` kinachoweka `statusText` kuwa `"$X Remaining"`, ambapo `X` ni thamani ya `netRemaining`. Pia, weka `statusClass` kuwa `"surplus"`.
...
Unapaswa kutumia template literal kwa ajili ya `statusText`.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

`statusText` yako inapaswa kupewa thamani `$${netRemaining} Remaining`.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

Unapaswa kuweka `"surplus"` kwa `statusClass`.
