...
Cuando usas la propiedad `length` en una cadena primitiva, JavaScript envuelve temporalmente la cadena primitiva en un objeto cadena para realizar la operación. Por eso puedes usar la propiedad `length` y los diferentes métodos como `repeat()`, `concat()` y `slice()`. A este tipo de métodos y propiedades se les llama métodos de instancia, propiedades de instancia y métodos estáticos. Aprenderás en detalle cómo funciona esto en módulos futuros.
