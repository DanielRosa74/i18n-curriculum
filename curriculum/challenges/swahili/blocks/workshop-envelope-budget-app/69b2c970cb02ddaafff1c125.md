...
Sasa umepata orodha zako za vipengele, unaweza kuzipitisha kwa kitendakazi chako cha `getTotalFromInputs` ili kutoa jumla.

Tangaza kigezo cha `rent`, na weka thamani yake kuwa matokeo ya wito wa `getTotalFromInputs` ukiwa na `[rentInput]` kama hoja.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `rent`.
...
Unapaswa kufanya wito wa `getTotalFromInputs` ukiwa na `[rentInput]` kama hoja.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya `getTotalFromInputs` katika `rent`.
