...
`category` 変数と `' .input-container'`（`.input-container` の前にスペースがあることに注意）を連結し、その文字列を `querySelector()` に渡してください。
...
`category` の後に `' .input-container'` を連結してください。`.input-container` の先頭にスペースを入れることを忘れないでください。
