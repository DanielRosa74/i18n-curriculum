...
有些作者简介比其他的要长得多。为了让卡片看起来统一，你可以提取每个简介的前 50 个字符，并用省略号 `"..."` 替换剩余部分。否则，你也可以显示完整的简介。
...
如果 `bio` 超过 `50` 个字符，你应该使用 `slice()` 提取前 50 个字符，并将其余部分替换为 `"..."`。别忘了索引是从零开始的。
...
如果 `bio` 的字符数不超过 50 个，你应该直接使用该文本。
