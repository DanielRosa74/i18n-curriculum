...
`article`要素の中に、`Mr. Whiskers' First Bath`のテキストを持つ`h3`要素を追加してください。

`h3`要素の下に、2つの段落のロレム・イプサムテキストを追加してください。

# --hints--

`article`要素の中に、`posts`に設定された`id`を持つ2つ目の`section`要素があるはずです。
...
`h3`要素の中に、`Mr. Whiskers' First Bath`のテキストを持つ`article`要素があるはずです。
