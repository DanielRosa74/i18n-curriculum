...
現在你已經有了元素的列表，可以將它們傳給你的 `getTotalFromInputs` 函式來取得總和。

宣告一個 `rent` 變數，並將呼叫 `getTotalFromInputs` 並以 `[rentInput]` 作為引數的結果指定給它。

# --hints--

你的 `calculateBudget` 函式應該要有一個 `rent` 變數。
...
你應該要用 `[rentInput]` 作為引數來呼叫 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

你應該將 `getTotalFromInputs` 的結果指定給 `rent`。
