...
删除 `console.log(catalog.length)` 语句。

然后创建一个名为 `findByAuthor` 的函数，它接受 `catalog` 和 `author` 作为参数。在函数内部，声明一个名为 `searchTerm` 的变量，并将其设置为 `author` 的小写版本，以便后续实现不区分大小写的匹配。

还要声明一个名为 `results` 的变量，并赋值为空数组，用来存放匹配项。

# --hints--

你应该删除 `console.log(catalog.length)` 语句。
