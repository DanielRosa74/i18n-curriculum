...
Dentro de tu elemento `span`, crea un elemento `label` con un atributo `for` establecido en `entry-dropdown` y asígnale el texto `Add expense to:`. Luego crea un elemento `select` con el `id` establecido en `entry-dropdown` y un `name` establecido en `options`. Debajo de eso, agrega un elemento `button` con el `id` establecido en `add-entry` y el texto `Add Entry`.
...
Tu nuevo elemento `label` debe tener el texto `Add expense to:`.
...
Tu nuevo elemento `button` debe tener un atributo `type` establecido en `button`.
