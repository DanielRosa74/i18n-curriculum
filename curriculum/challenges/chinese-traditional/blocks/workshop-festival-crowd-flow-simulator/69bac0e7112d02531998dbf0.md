...
當存在上限溢位（`result.overflow > 0`）時，你的 `handleGateAtTick` 函式應該先記錄訊息 `"Overflow of " + result.overflow + " attendees. Rerouting..."`。
...
當存在上限溢位（`result.overflow > 0`）時，你的 `handleGateAtTick` 函式應該透過呼叫 `rerouteOverflow()` 並傳入 `gates`、`gate`、`tickIndex` 和 `result.overflow` 來重新導向上限溢位。
