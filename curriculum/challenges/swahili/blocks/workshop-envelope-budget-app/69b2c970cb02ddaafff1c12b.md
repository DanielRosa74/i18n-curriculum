...
Sasa tangaza `netRemaining` kama kigezo, na weka thamani yake kuwa matokeo ya kutoa `expenses` kutoka `income`.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `netRemaining`.
...
Unapaswa kupata thamani ya `income - expenses`.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

Unapaswa kuweka thamani ya `income - expenses` katika `netRemaining`.
