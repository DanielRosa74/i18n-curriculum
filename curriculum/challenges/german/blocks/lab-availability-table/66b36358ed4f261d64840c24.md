...
1. Die erste Zeile sollte Kopfzeilenzellen enthalten, die die Wochentage anzeigen.
1. Die erste Spalte sollte Kopfzeilenzellen mit der `class` `time` enthalten, die Zeitwerte als Text haben.
1. Alle Datenzellen sollten die `class` `available-#` haben, wobei `#` eine Zahl von `0` bis `5` ist, die die Anzahl der verfügbaren Personen zu dieser Zeit angibt.
1. Ihre Tabelle sollte ein `caption`-Element mit dem Text `Weekly Meeting Availability Schedule` haben.
1. Alle Kopfzeilenzellen in der ersten Zeile (die Wochentage) sollten ein `scope`-Attribut mit dem Wert `col` haben.
1. Alle Kopfzeilenzellen in der ersten Spalte (die Zeit-Zellen) sollten ein `scope`-Attribut mit dem Wert `row` haben.
...
1. Sie sollten dem Element `#legend-gradient` einen linearen Farbverlauf geben, der zwischen allen Farben von `--color0` bis `--color5` mit harten Übergängen wechselt.
