...
All'interno dell'elemento `#results`, usa un operatore ternario per verificare se la lunghezza dell'array `results` è maggiore di zero. Se lo è, itera sugli elementi e mostra ciascuno in un elemento `p` con la classe `result-item`. Assicurati anche di includere un attributo `key` in ogni paragrafo.
