...
现在是开始准备你的计算的时候了。先声明一个 `expenses` 变量，并将 `rent`、`food`、`utilities` 和 `entertainment` 的总和赋值给它（注意测试中顺序很重要）。确保在你的 if 语句之后执行这一步。

# --hints--

函数中应该有一个 `expenses` 变量。
