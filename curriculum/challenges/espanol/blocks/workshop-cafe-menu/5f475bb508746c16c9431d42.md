...
La imagen que agregaste no está centrada horizontalmente como el título `Coffee` que está arriba. Los elementos `img` son elementos de nivel en línea, por lo que se sitúan en el flujo del texto.

Para que la imagen se comporte como un elemento de nivel bloque, como un título, crea un selector de tipo `img`. Configura la propiedad `display` en `block` y usa las propiedades `margin-left` y `margin-right` para centrarla horizontalmente.
