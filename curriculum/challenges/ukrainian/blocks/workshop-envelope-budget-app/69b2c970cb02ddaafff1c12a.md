...
Час почати готувати свої обчислення. Спочатку оголосіть змінну `expenses` і призначте їй суму `rent`, `food`, `utilities` та `entertainment` (зверніть увагу, що порядок має значення для тестів). Обов’язково зробіть це після вашого оператора if.

# --hints--

Функція має містити змінну `expenses`.
...
Змінна `expenses` має йти після вашого оператора `if`.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

Ви маєте обчислити суму `rent`, `food`, `utilities` та `entertainment` у вказаному порядку.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

Ви маєте призначити суму `rent`, `food`, `utilities` та `entertainment` змінній `expenses`.
