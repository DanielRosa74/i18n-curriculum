...
أعلن عن متغير `netRemaining`، وعيّن له قيمة طرح `expenses` من `income`.

# --hints--

يجب أن تحتوي دالة `calculateBudget` على متغير `netRemaining`.
...
يجب أن تحسب قيمة `income - expenses`.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

يجب أن تعيّن قيمة `income - expenses` إلى `netRemaining`.
