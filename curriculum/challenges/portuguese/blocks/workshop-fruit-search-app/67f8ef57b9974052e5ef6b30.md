...
Dentro do elemento `#results`, use um operador ternário para verificar se o comprimento do array `results` é maior que zero. Se for, faça um map nos itens e exiba cada um em um elemento `p` com a classe `result-item`. Certifique-se de incluir também um atributo `key` em cada parágrafo.
