...
- 使用 `for` 迴圈來迭代輸入的 `gates` 陣列中的每個閘門。對每個閘門，為 `summary` 添加一個屬性：
  - 鍵是該閘門的 `id`。
  - 值是 `0`。
- 迴圈結束後，傳回 `summary` 物件。
...
當以 `[]` 呼叫時，你的 `initializeThroughput` 函式應該傳回 `{}`，且不會修改原始輸入。
...
當以 `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]` 呼叫時，你的 `initializeThroughput` 函式應該傳回 `{ North: 0 }`，且不會修改原始輸入。
...
當以 `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]` 呼叫時，你的 `initializeThroughput` 函式應該傳回 `{ North: 0, East: 0, South: 0 }`，且不會修改原始輸入。
...
你的 `initializeThroughput` 函式應該在迴圈結束後，將 `summary` 作為函式的最後一行傳回。
