...
L'immagine che hai aggiunto non è centrata orizzontalmente come l'intestazione `Coffee` sopra di essa. Gli elementi `img` sono elementi di livello in linea, quindi si trovano nel flusso del testo.

Per far comportare l'immagine come un elemento di livello blocco come un'intestazione, crea un selettore di tipo `img`. Imposta la proprietà `display` su `block` e usa le proprietà `margin-left` e `margin-right` per centrarla orizzontalmente.
