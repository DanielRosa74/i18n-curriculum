...
Finalmente, em uma nova linha após seu segundo `label`, crie outro `input` elemento. Dê a ele um atributo `type` definido como `number`, um atributo `min` definido como `0` (para garantir que valores negativos não possam ser adicionados), um atributo `placeholder` definido como `Amount` e um atributo `id` que corresponda ao atributo `for` do seu segundo `label` elemento.
...
Seu `input` elemento deve ter um atributo `placeholder` definido como `Amount`.
...
Seu `input` elemento deve ter um atributo `id` definido como `${category}-${entryNumber}-amount`.
...
Seu novo `input` elemento deve ter um atributo `min` definido como `0`.
