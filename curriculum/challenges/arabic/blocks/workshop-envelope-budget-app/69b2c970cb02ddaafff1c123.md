...
باستخدام نفس الصيغة النحوية، استعلم عن مُدخَلات `number` داخل عنصر `#utilities` وقم بتعيينها إلى `utilitiesInputs`.

# --hints--

يجب إعلان متغير `utilitiesInputs`.
...
يجب استدعاء `document.querySelectorAll()` مع المحدد `#utilities input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `utilitiesInputs`.
