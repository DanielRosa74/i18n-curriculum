...
다른 작가로 `getBooksByAuthor` 함수를 테스트해보면 좋겠습니다.

먼저 `"\nList of books by James Clear:\n"` 메시지를 콘솔에 기록하세요.

그 `console.log()` 아래에 또 다른 `console.log()`를 추가하세요. 그 `console.log()` 안에서 `library`와 `"James Clear"`를 인수로 하여 `getBooksByAuthor()` 함수를 호출하세요.
