...
你還需要取得 `#income` 輸入欄位的值。你已經在程式碼頂端查詢過它，並將它設值給 `incomeInput` 變數。不過，你使用的是 `getElementById`，它會傳回一個 `Element`，而不是 `NodeList`。

`NodeList` 是一種類陣列物件，這表示你可以對它進行迭代，且它與陣列共享一些常用方法。對於你的 `getTotalFromInputs` 函式，傳入陣列作為引數和傳入 `NodeList` 一樣都可以正常運作。

宣告一個 `income` 變數，並將呼叫 `getTotalFromInputs` 的結果設值給它——傳入一個包含你的 `incomeInput` 的陣列作為引數。

# --hints--

你的 `calculateBudget` 函式應該要有一個 `income` 變數。
...
你應該用 `[incomeInput]` 作為引數來呼叫 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

你應該將 `getTotalFromInputs` 的結果指定給 `income`。
