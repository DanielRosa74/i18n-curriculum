...
Quando o gatilho acontece, o React entra na fase de renderização. Aqui, o React reavalia seus componentes e decide o que exibir. Para isso, o React usa uma cópia leve do DOM "real" chamada DOM virtual. Com o DOM virtual, o React pode verificar rapidamente o que precisa mudar no componente.
...
- **Hook `useEffect()`**: No React, um efeito é qualquer coisa que acontece fora do processo de renderização do componente. Ou seja, algo que o React não gerencia diretamente como parte da renderização da interface do usuário. Exemplos comuns incluem buscar dados, atualizar o título da aba do navegador, ler ou gravar no armazenamento local do navegador, obter a localização do usuário e muito mais. Essas operações interagem com o mundo fora do React e são conhecidas como efeitos colaterais. O React oferece o hook `useEffect` para você lidar com esses efeitos colaterais. O `useEffect` permite executar uma função depois que o componente renderiza ou atualiza.
