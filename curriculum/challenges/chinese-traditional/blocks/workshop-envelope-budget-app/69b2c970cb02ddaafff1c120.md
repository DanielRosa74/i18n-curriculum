...
現在是時候開始將所有部分整合在一起。宣告一個空的 `calculateBudget` 函式，該函式接受一個名為 `e` 的參數。這個函式將會是另一個事件監聽器，所以傳入的第一個引數會是瀏覽器事件——`e` 是這個參數的常用名稱。

# --hints--

你應該宣告一個 `calculateBudget` 變數。

```js
assert.isDefined(calculateBudget);
```

你的 `calculateBudget` 變數應該是一個函式。

```js
assert.isFunction(calculateBudget);
```

你的 `calculateBudget` 函式應該接受一個 `e` 引數。
