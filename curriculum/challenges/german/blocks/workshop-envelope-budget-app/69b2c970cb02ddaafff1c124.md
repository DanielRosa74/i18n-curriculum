...
Folgen Sie dem gleichen Muster, führen Sie eine Abfrage für Ihre `number` Eingaben im `#entertainment` Element durch und weisen Sie sie `entertainmentInputs` zu.

# --hints--

Sie sollten eine Variable `entertainmentInputs` deklarieren.
...
Sie sollten `document.querySelectorAll()` mit `#entertainment input[type='number']` aufrufen.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Sie sollten das Ergebnis Ihres `document.querySelectorAll()` Aufrufs `entertainmentInputs` zuweisen.
