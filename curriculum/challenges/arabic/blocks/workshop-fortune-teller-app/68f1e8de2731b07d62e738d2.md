...
يجب تعيين `foundCard` عن طريق استدعاء `find` على `this.cards`، مع تمرير استدعاء راجع يأخذ معلمة `card` ويُرجع نتيجة مقارنة `card.name_short` مع `cardId` باستخدام المساواة الصارمة.
