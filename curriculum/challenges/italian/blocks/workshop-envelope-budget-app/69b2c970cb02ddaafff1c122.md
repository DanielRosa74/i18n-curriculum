...
Dichiara una variabile `foodInputs` e assegnale il valore della chiamata a `document.querySelectorAll()` con il selettore `#food input[type='number']`. Questo restituirà tutti gli input di tipo `number` che si trovano nell'elemento `#food`.

# --hints--

Dovresti dichiarare una variabile `foodInputs`.
...
Dovresti chiamare `document.querySelectorAll()` con `#food input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Dovresti assegnare il risultato della chiamata a `document.querySelectorAll()` a `foodInputs`.
