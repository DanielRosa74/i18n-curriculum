...
Antes de comenzar, debes saber qué es una regla-at en CSS. Las reglas-at son sentencias que dan instrucciones al navegador. Puedes usarlas para definir varios aspectos de la hoja de estilos, como consultas de medios, fotogramas clave, fuentes y más. Aprenderás más sobre conceptos como consultas de medios y fotogramas clave en futuras lecciones. Esta vez, aprenderás sobre la regla-at `@font-face`.
...
Dentro de los corchetes, necesitarás incluir descriptores para personalizar tu fuente. Veamos algunos de los más comunes. El descriptor font-family especifica el nombre que usarás en toda la hoja de estilos para referirte a esa fuente. Por ejemplo, supongamos que defines esta `@font-face rule`. Tiene el descriptor `font-family` definido y su valor es `MyCustomFont`:
...
woff significa "Web Open Font Format". La diferencia entre WOFF y WOFF2 es el algoritmo usado para comprimir los datos. OpenType es un formato para fuentes escalables de computadora desarrollado por Microsoft y Adobe que permite a los usuarios acceder a funcionalidades adicionales en una fuente. Es ampliamente usado en los principales sistemas operativos.
