...
Devi sapere se l'utente ha superato il budget o se ha soldi rimanenti. Quando le spese superano il reddito, sei oltre il budget (un deficit). Quando il reddito supera le spese, hai soldi rimanenti (un surplus). Avere esattamente zero rimanenti può essere considerato bilanciato.

Dichiara due variabili: `statusText` e `statusClass`. Inizializza entrambe come stringhe vuote usando `""`.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere una variabile `statusText`.
...
La tua funzione `calculateBudget` dovrebbe avere una variabile `statusClass`.
...
Dovresti inizializzare `statusText` e `statusClass` come stringhe vuote.
...
Entrambe le variabili dovrebbero essere dichiarate usando `let`.
