...
为你的 `calculateBudget` 函数添加一行，调用 `e` 参数的 `preventDefault()` 方法。然后，将你的全局误差标记 `isError` 重置为 `false`。

# --hints--

你的 `calculateBudget` 函数应该调用 `e.preventDefault()`。

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

你的 `calculateBudget` 函数应该将全局误差标记 `isError` 重置为 `false`。
