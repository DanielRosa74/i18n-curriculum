...
تحتاج عقد `text` إلى خاصيتي `x` و`y` لتحديد موقعها على SVG. في هذا التحدي، يمكن لقيمة `y` (التي تحدد الارتفاع) أن تستخدم نفس القيمة التي يستخدمها `circle` في خاصية `cy`. يمكن أن تكون قيمة `x` أكبر قليلاً من قيمة `cx` في `circle`، ليكون المسمى مرئيًا. هذا سيدفع المسمى إلى يمين النقطة المرسومة.
