...
`isSelect`라는 이름의 함수를 만들고, `EventTarget | null` 타입의 단일 매개변수 `element`를 받으세요. 이 함수는 선택한 요소가 `<select>` HTML 요소인지 확인할 것입니다.
...
`isSelect` 함수는 `EventTarget | null` 타입의 `element`라는 매개변수를 가져야 합니다.
