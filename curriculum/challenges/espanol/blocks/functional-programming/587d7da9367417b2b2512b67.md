...
El último desafío introdujo el método `concat` como una forma de fusionar arreglos en un nuevo arreglo sin mutar los arreglos originales. Compara `concat` con el método `push`. `push` agrega elementos al final del mismo arreglo en el que se llama, lo que muta ese arreglo. Aquí tienes un ejemplo:
