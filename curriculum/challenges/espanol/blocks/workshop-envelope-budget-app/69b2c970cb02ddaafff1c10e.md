...
Crea otro elemento `label` (en una nueva línea) al final de tu `HTMLString`. Este `label` debe tener el texto `Expense # Amount`, usando la sintaxis de plantilla literal para reemplazar `#` con el valor de `entryNumber`, y el atributo `for` configurado como `X-#-amount`, donde `X` es el valor de `category` y `#` es el valor de `entryNumber`.
...
Tu nuevo elemento `label` debe tener un atributo `for` configurado como `${category}-${entryNumber}-amount`.
...
Tu nuevo elemento `label` debe tener el texto `Expense ${entryNumber} Amount`.
