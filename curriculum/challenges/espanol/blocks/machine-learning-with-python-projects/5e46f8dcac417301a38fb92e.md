...
Para las variables `*_data_gen`, usa el método `flow_from_directory`. Pasa el tamaño del lote, el directorio, el tamaño objetivo (`(IMG_HEIGHT, IMG_WIDTH)`), el modo de clase y cualquier otro parámetro necesario. `test_data_gen` será el más complicado. Para `test_data_gen`, asegúrate de pasar `shuffle=False` al método `flow_from_directory`. Esto garantizará que las predicciones finales se mantengan en el orden que espera nuestra prueba. Para `test_data_gen` también será útil observar la estructura del directorio.
