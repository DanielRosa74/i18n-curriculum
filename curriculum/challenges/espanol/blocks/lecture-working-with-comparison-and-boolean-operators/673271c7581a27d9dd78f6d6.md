...
Los booleanos son un tipo de dato que solo tiene los valores `true` y `false`. Son útiles porque te permiten hacer algo basado en ciertas condiciones. Los booleanos son esenciales cuando quieres evaluar si algo debe ocurrir o no, como decidir si alguien puede tener acceso a una determinada funcionalidad en tu app. Aquí tienes un ejemplo de cómo asignar el valor `true` a una variable llamada `isOldEnoughToDrive`:
...
La siguiente comparación será `false`, porque un tipo de dato cadena no es igual a un tipo de dato número.

## Usar operadores de desigualdad

Si necesitas verificar si algo no es igual a otro valor, puedes usar los operadores de desigualdad o de desigualdad estricta. Aquí tienes un ejemplo de usar el operador de desigualdad (`!=`) para comparar un número con una cadena.
