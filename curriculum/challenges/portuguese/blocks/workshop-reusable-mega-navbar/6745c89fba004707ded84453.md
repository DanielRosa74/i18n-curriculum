...
Crie um elemento `a` dentro do seu primeiro elemento `li`. Dê a ele um atributo `href` com o valor `#` e o texto `Dashboard`.

# --hints--

Você deve criar um elemento `a` dentro do seu primeiro elemento `li`.

```js
assert.exists(document.querySelector('li a'));
```

Seu elemento `a` deve ter um atributo `href` com o valor `#`.
