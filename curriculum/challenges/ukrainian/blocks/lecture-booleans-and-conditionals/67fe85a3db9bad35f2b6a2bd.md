...
* Тіло інструкції `if` становить <dfn>блок коду</dfn>, який є групою інструкцій, що належать разом. Пробіли на початку лінії називаються відступом. В Python відступ визначає, які інструкції належать до блоку коду.
...
Чотири пробіли перед `print('You are an adult')` створюють відступ для цього рядка і розміщують його всередині блоку `if`.
...
`You are an adult` буде виведено в термінал.
...
`You are a teenager` буде виведено в термінал.
...
`You are a child` буде виведено в термінал.

---

В термінал буде виведена помилка.
