...
Agora, vamos falar sobre encadeamento de promises. Um dos recursos poderosos das promises é que podemos encadear várias operações assíncronas juntas. Cada `.then()` pode retornar uma nova promise, permitindo que você execute uma sequência de operações assíncronas uma após a outra. Aqui está um exemplo:
