...
إذا نفذت هذا بشكل صحيح، سيحتوي كتلة `try` على ثلاثة أسطر من الكود، وستحتوي كتلة `catch` على سطر واحد فقط. كما سيُحل خطأ `SyntaxError` ويجب أن يعمل التطبيق بشكل طبيعي.

أثنِ على نفسك! لقد تعلمت كيف تجعل كودك غير المتزامن أكثر قابلية للقراءة باستخدام العاملين `async` و `await`! الآن صفحة المؤلف الخاصة بك مكتملة.
...
يجب أن يحتوي كتلة `try` على الثلاث تعليمات التي تجلب البيانات، وتنشئ `json`، ثم تعرضها.
...
يجب أن تعرض صفحة المؤلفين في fCC بشكل صحيح.
