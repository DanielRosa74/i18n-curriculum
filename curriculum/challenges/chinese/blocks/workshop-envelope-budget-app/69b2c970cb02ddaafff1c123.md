...
使用相同的语法，在 `#utilities` 元素中查询你的 `number` 输入，并将它们赋值给 `utilitiesInputs`。

# --hints--

你应该声明一个 `utilitiesInputs` 变量。
...
你应该使用 `#utilities input[type='number']` 调用 `document.querySelectorAll()`。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你应该将 `document.querySelectorAll()` 调用的结果赋值给 `utilitiesInputs`。
