...
Dentro de este elemento `form`, hay un elemento `label` y un elemento `input` para la dirección de correo electrónico del usuario. Cuando el usuario hace clic en el botón de enviar, sus datos se enviarán al servidor y serán procesados. El tercer valor posible para el atributo `type` es el valor `reset`. Aquí tienes un ejemplo de un elemento `form` con botones de reset y enviar.
