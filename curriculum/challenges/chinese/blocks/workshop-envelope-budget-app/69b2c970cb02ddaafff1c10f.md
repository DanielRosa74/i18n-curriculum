...
最后，在你的第二个 `label` 后面另起一行，创建另一个 `input` 元素。为它添加一个 `type` 属性，值为 `number`，一个 `min` 属性，值为 `0`（确保不能添加负数），一个 `placeholder` 属性，值为 `Amount`，以及一个 `id` 属性，其值要与第二个 `label` 元素的 `for` 属性匹配。
...
你的 `input` 元素应有一个 `placeholder` 属性，值为 `Amount`。
...
你的 `input` 元素应有一个 `id` 属性，值为 `${category}-${entryNumber}-amount`。
