...
Ihre `getTotalFromInputs`-Funktion setzt die globale Fehlerflagge `isError` auf `true`, wenn eine ungültige Eingabe erkannt wird.

Fügen Sie Ihrer `calculateBudget`-Funktion eine `if`-Anweisung hinzu, die die Wahrheit Ihrer globalen Fehlerflagge überprüft, und verwenden Sie `return`, um die Ausführung der Funktion zu beenden, wenn diese wahr ist.

# --hints--

Ihre `calculateBudget`-Funktion sollte eine `if`-Anweisung enthalten, die die Wahrheit der Variable `isError` überprüft.
...
Ihre `if`-Anweisung sollte nach dem letzten Aufruf der Funktion `getTotalFromInputs` platziert werden.
