...
لإنهاء الأمور، عيّن نص العنصر الأول `a` إلى `Calendar`، والثاني إلى `Chat`، والثالث إلى `Email`.

بهذا، يصبح مكوِّن `Navbar` الخاص بك مكتملًا!

# --hints--

يجب أن تعيّن نص العنصر الأول `a` إلى `Calendar`.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

يجب أن تعيّن نص العنصر الثاني `a` إلى `Chat`.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

يجب أن تعيّن نص العنصر الثالث `a` إلى `Email`.
