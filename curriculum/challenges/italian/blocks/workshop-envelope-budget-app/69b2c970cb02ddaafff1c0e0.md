...
Il tuo elemento `input` per il reddito ha bisogno di alcuni attributi aggiuntivi per assicurarti che gli utenti inseriscano valori validi.

Aggiorna l'elemento `#income` che hai creato con i seguenti attributi:

- `type` impostato su `number` (per permettere solo input numerici)
- `min` impostato su `0` (per permettere solo numeri positivi)
- `placeholder` impostato su `e.g. 2000`
- Infine, segna l'elemento `input` come `required`.
...
Il tuo elemento `input` dovrebbe avere un attributo `placeholder` impostato su `e.g. 2000`.
