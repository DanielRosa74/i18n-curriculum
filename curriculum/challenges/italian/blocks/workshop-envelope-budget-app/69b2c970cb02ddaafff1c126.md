...
Dichiara ora una variabile `food` e assegnale il valore della chiamata a `getTotalFromInputs` con i tuoi `foodInputs`.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere una variabile `food`.
...
Dovresti chiamare `getTotalFromInputs` con `foodInputs` come argomento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

Dovresti assegnare il risultato di `getTotalFromInputs` a `food`.
