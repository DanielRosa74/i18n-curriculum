...
记住，`return` 会结束函数的执行。在你的 `if` 块之后，你需要处理输入有效时的逻辑。因为你的 if 语句会返回一个值，所以不需要 `else` 语句。

使用加法赋值操作符将 `currVal` 添加到你的 `total`。你需要使用 `Number` 构造函数将 `currVal` 转换为数字。
...
在你的 `if` 语句之后，你应该对 `total` 使用加法赋值操作符。
...
你应该将 `Number(currVal)` 的值添加到 `total`。
