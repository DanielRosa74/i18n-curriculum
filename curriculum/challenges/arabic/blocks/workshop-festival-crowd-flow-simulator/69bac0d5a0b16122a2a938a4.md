...
بعد ذلك، حدّث كائن `throughputSummary` المُدخَل ليعكس عدد الحضور الذين تم معالجتهم بنجاح عبر هذه البوابة في التوقيت الحالي. بالتحديد، خذ قيمة `processed` في `result` وأضفها إلى الخاصية `throughputSummary` باستخدام المفتاح `gate.id`.
