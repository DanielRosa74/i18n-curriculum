...
أعلن الآن عن متغير `food`، وعيّن له قيمة استدعاء `getTotalFromInputs` مع `foodInputs` الخاص بك.

# --hints--

يجب أن تحتوي دالة `calculateBudget` على متغير `food`.
...
يجب استدعاء `getTotalFromInputs` مع `foodInputs` كمعلمة.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

يجب تعيين نتيجة `getTotalFromInputs` إلى `food`.
