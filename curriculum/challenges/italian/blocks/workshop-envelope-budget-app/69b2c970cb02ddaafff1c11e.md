...
Ricorda che `return` termina l'esecuzione di una funzione. Dopo il tuo blocco `if`, devi gestire la logica per quando l'input è valido. Poiché la tua istruzione if restituisce un valore, non hai bisogno di un'istruzione `else`.

Usa l'operatore di assegnazione con addizione per aggiungere `currVal` al tuo `total`. Dovrai usare il costruttore `Number` per convertire `currVal` in un numero.
...
Dopo la tua istruzione `if`, dovresti usare l'operatore di assegnazione con addizione su `total`.
...
Dovresti aggiungere il valore di `Number(currVal)` a `total`.
