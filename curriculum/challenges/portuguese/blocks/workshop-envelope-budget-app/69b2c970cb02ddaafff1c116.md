...
Na sua nova função, declare uma variável `total` e atribua a ela o valor `0`. Use `let` para declará-la, já que você vai reatribuir esse valor depois.

# --hints--

Você deve declarar uma variável `total`.
...
Sua variável `total` deve ser atribuída com o valor `0`.
...
Você deve declarar sua variável `total` com `let`.
