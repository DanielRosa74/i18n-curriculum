...
Це тип кібернападу, коли хакер таємно опиняється посередині між двома людьми або системами, які спілкуються один з одним. Хакер може бачити, змінювати або викрадати інформацію, що передається, без відома будь-якої зі сторін. Це складне слово, тому не забудьте використати `-`. Перші літери першого та четвертого слова мають бути з великої літери.
