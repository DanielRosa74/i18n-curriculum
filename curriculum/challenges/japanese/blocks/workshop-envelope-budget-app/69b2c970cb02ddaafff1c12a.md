...
計算の準備を始めましょう。まず、`expenses` 変数を宣言し、`rent`、`food`、`utilities`、`entertainment` の合計を代入してください（テストでは順序が重要です）。この処理は if文 の後に行ってください。

# --hints--

関数には `expenses` 変数が必要です。
