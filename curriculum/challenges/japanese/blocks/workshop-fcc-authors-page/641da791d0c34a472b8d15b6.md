...
`console.log()` の中で、最初の引数にテキスト `"Author Data Array:"` を、2番目の引数に `authorDataArr` を追加してください。テキストと `authorDataArr` はカンマで区切ってください。
...
`"Author Data Array:"` のテキストと `authorDataArr` はカンマで区切ってください。

```js
assert.match(code, /console\.log\(\s*("|'|`)Author\s+Data\s+Array:\s*\1\s*,/)
```

`authorDataArr` は `console.log()` の2番目の引数にしてください。
