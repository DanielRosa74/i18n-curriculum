...
Uma vez que o gatilho acontece, o React entra na fase de renderização. Aqui, o React reavalia seus componentes e determina o que deve ser exibido. Para isso, o React usa uma cópia leve do DOM "real" chamada DOM virtual. Com o DOM virtual, o React pode verificar rapidamente o que precisa mudar no componente.
...
- **Hook `useEffect()`**: No React, um efeito é qualquer coisa que acontece fora do processo de renderização do componente. Ou seja, qualquer coisa que o React não gerencia diretamente como parte da renderização da interface. Exemplos comuns incluem buscar dados, atualizar o título da aba do navegador, ler ou gravar no armazenamento local do navegador, obter a localização do usuário e muito mais. Essas operações interagem com o mundo externo e são conhecidas como efeitos colaterais. O React fornece o hook `useEffect` para você lidar com esses efeitos. O `useEffect` permite executar uma função após o componente renderizar ou atualizar.
...
- **Redux**: O Redux gerencia o estado fornecendo uma store central e controle rigoroso sobre as atualizações de estado. Ele usa um padrão previsível com actions, reducers e middleware. Actions são cargas de informação que enviam dados da sua aplicação para a store do Redux, geralmente acionadas por interações do usuário. Reducers são funções que especificam como o estado deve mudar em resposta a essas actions, garantindo que o estado seja atualizado de forma imutável. Middleware, por sua vez, atua como uma ponte entre o despacho da action e o reducer, permitindo estender a funcionalidade do Redux (por exemplo, logging, tratamento de operações assíncronas) sem modificar o fluxo principal.
...
- **Performance percebida**: É como os usuários percebem o desempenho de um site. É a forma como eles avaliam em termos de responsividade e confiabilidade. Essa é uma medida subjetiva, então é difícil quantificá-la, mas é muito importante, pois a experiência do usuário determina o sucesso ou fracasso de um site.
- **Performance real**: É o desempenho objetivo e mensurável do site. É medido usando métricas como tempo de carregamento da página, tempo de resposta do servidor e tempo de renderização. Essas medições são influenciadas por múltiplos fatores relacionados à rede e ao próprio código.
...
- **Como funciona a renderização**: Primeiro o navegador analisa o HTML e constrói o DOM. Em seguida, o navegador processa o CSS, construindo o CSS Object Model, ou CSSOM. Essa é outra estrutura em árvore que determina como os elementos devem ser estilizados. Por fim, o navegador pinta os pixels na tela, renderizando cada elemento com base nos estilos e layout calculados. Em páginas complexas, isso pode envolver múltiplas camadas que são compostas para formar a saída visual final.

## Como o Desempenho Impacta a Sustentabilidade

- **Informações de fundo**: A internet responde por cerca de 2% das emissões globais de carbono — o mesmo que a indústria aérea! Cada byte transferido requer eletricidade, desde data centers até dispositivos dos usuários. Arquivos maiores e scripts ineficientes significam maior consumo de energia. Um site de alta performance não é apenas mais rápido, ele também reduz processamento desnecessário e uso de energia.
...
- **Chrome DevTools**: O Chrome DevTools é uma ferramenta embutida no Google Chrome que permite analisar e depurar performance em tempo real. O DevTools mostra tempos de carregamento, uso da CPU e atrasos na renderização. É especialmente útil para medir o First Contentful Paint, ou FCP, que indica quão rápido o usuário vê o primeiro conteúdo visível. Se seu site parecer lento, o DevTools ajuda a identificar os gargalos.
- **Lighthouse**: É uma ferramenta automatizada que verifica performance, SEO e acessibilidade.
- **WebPageTest**: Essa ferramenta permite testar como seu site carrega a partir de diferentes locais e dispositivos. Ela oferece um detalhamento do Speed Index, Total Blocking Time e outras métricas chave de performance. Se quiser saber como usuários reais experimentam seu site globalmente, o WebPageTest é a ferramenta ideal.
- **PageSpeed Insights**: Essa ferramenta analisa seu site e sugere melhorias rápidas para mobile e desktop. Ela indica o que está deixando seu site lento e oferece recomendações específicas, como otimizar imagens, remover scripts que bloqueiam a renderização e reduzir tempos de resposta do servidor. O PageSpeed Insights é uma forma rápida e fácil de verificar como o Google vê a performance do seu site.
- **Real User Monitoring**: Ferramentas RUM monitoram o comportamento real dos usuários, mostrando como visitantes reais experimentam seu site. Ferramentas populares incluem Google Analytics, que rastreia tempos de carregamento e taxas de rejeição, e New Relic ou Datadog, que monitoram problemas de performance em tempo real. Se quiser dados de usuários reais, ferramentas RUM são essenciais.
...
- **Animações CSS**: Animar certas propriedades CSS, como dimensões, posição e layout, dispara um processo chamado "reflow", durante o qual o navegador recalcula a posição e geometria de certos elementos na página. Isso exige um repaint, que é computacionalmente custoso. Portanto, recomenda-se reduzir o número de animações CSS ao máximo ou pelo menos oferecer ao usuário a opção de ativá-las ou desativá-las.
...
- **Mocking**: Mocking é o processo de substituir dados reais por dados falsos que simulam o comportamento de componentes reais. Por exemplo, você pode mockar a resposta da API em testes em vez de fazer chamadas contínuas para buscar os dados.
- **Stubbing**: Stubs são objetos que retornam respostas pré-definidas ou dados fictícios para um comportamento esperado em uma aplicação. Por exemplo, você pode stubar o comportamento de uma conexão com banco de dados nos seus testes sem precisar depender de uma conexão real.
