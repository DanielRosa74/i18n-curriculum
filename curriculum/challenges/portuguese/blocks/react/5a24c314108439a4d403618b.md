...
title: Dê aos elementos irmãos um atributo key único
challengeType: 6
forumTopicId: 301394
...
O editor de código tem um array com alguns frameworks de frontend e um componente funcional sem estado chamado `Frameworks()`. O `Frameworks()` precisa mapear o array para uma lista não ordenada, assim como no último desafio. Termine de escrever o callback do `map` para retornar um elemento `li` para cada framework no array `frontEndFrameworks`. Desta vez, certifique-se de dar a cada `li` um atributo `key`, definido com um valor único. Os elementos `li` também devem conter texto do `frontEndFrameworks`.
