...
Um exemplo comum de validação embutida em formulários é usar o atributo `required` nas entradas. O atributo `required` especifica que o usuário precisa preencher essa parte do formulário antes que ele seja enviado. Aqui está um exemplo de uso do atributo `required` em uma entrada de e-mail.
