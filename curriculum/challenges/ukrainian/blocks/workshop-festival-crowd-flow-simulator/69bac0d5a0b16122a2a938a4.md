...
Далі оновіть вхідний об’єкт `throughputSummary`, щоб відобразити, скільки учасників було успішно опрацьовано цими воротами на поточному такті. Зокрема, візьміть значення `processed` у `result` і додайте його до властивості `throughputSummary` з ключем `gate.id`.
