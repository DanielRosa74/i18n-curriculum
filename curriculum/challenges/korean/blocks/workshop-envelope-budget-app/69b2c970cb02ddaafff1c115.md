...
다음 단계는 사용자의 항목에서 총합을 구하는 함수를 작성하는 것입니다.

`getTotalFromInputs` 함수를 선언하고, `list`라는 매개변수를 주세요.

# --hints--

`getTotalFromInputs` 변수를 선언하세요.

```js
assert.isDefined(getTotalFromInputs);
```

`getTotalFromInputs` 변수는 함수여야 합니다.

```js
assert.isFunction(getTotalFromInputs);
```

`getTotalFromInputs` 함수는 `list`라는 매개변수를 받아야 합니다.
