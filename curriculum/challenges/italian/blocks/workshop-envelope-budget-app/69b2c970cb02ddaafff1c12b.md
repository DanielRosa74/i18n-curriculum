...
Ora dichiara una variabile `netRemaining` e assegna il valore della sottrazione di `expenses` da `income`.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere una variabile `netRemaining`.
...
Dovresti trovare il valore di `income - expenses`.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

Dovresti assegnare il valore di `income - expenses` a `netRemaining`.
