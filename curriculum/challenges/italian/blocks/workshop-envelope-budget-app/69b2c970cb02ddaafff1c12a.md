...
È il momento di iniziare a preparare i tuoi calcoli. Inizia dichiarando una variabile `expenses` e assegnale la somma di `rent`, `food`, `utilities` e `entertainment` (nota che l'ordine è importante per i test). Assicurati di farlo dopo la tua istruzione if.

# --hints--

La funzione dovrebbe avere una variabile `expenses`.
...
La variabile `expenses` dovrebbe venire dopo la tua istruzione `if`.

```js
assert.isAbove(calculateBudget.toString().indexOf('expenses'), calculateBudget.toString().indexOf('if'));
```

Dovresti calcolare la somma di `rent`, `food`, `utilities` e `entertainment`, in ordine.

```js
assert.match(calculateBudget.toString(), /rent\s*\+\s*food\s*\+\s*utilities\s*\+\s*entertainment\s*;?\s*$/m);
```

Dovresti assegnare la somma di `rent`, `food`, `utilities` e `entertainment` a `expenses`.
