...
Crea un elemento `a` all'interno del tuo primo elemento `li`. Imposta un attributo `href` su `#` e il testo `Dashboard`.

# --hints--

Dovresti creare un elemento `a` all'interno del tuo primo elemento `li`.

```js
assert.exists(document.querySelector('li a'));
```

Il tuo elemento `a` dovrebbe avere un attributo `href` impostato su `#`.
