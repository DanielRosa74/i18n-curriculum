...
`Calculate Budget` ボタンをクリックしても何も起こらないことがわかります。まだイベントリスナーをマウントする必要があります。

`budgetForm` 要素にイベントリスナーを追加してください。イベントタイプは `submit`、コールバック関数は `calculateBudget` にしてください。

# --hints--

`budgetForm` 要素の `.addEventListener()` メソッドを使ってください。
...
`.addEventListener()` の第2引数に `calculateBudget` を渡してください。
