...
Тепер потрібно обробити випадок, коли користувач не перевищує бюджет (коли `netRemaining` більший або дорівнює `0`).

Додайте блок `else` до вашої інструкції `if`, який призначить `statusText` значення `"$X Remaining"`, де `X` — це значення `netRemaining`. Також призначте `statusClass` значення `"surplus"`.
...
Ви повинні використати шаблонний рядок для `statusText`.

```js
assert.match(code, /else\s*\{[\s\S]*statusText\s*=\s*`/);
```

Вашому `statusText` слід призначити `$${netRemaining} Remaining`.

```js
assert.match(code, /statusText\s*=\s*`\s*\$\s*\$\{\s*netRemaining\s*\}\s*Remaining\s*`/);
```

Ви повинні призначити `"surplus"` для `statusClass`.
