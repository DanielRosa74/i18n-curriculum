...
title: Dar a los elementos hermanos un atributo key único
challengeType: 6
forumTopicId: 301394
...
El editor de código tiene un arreglo con algunos frameworks de front-end y un componente funcional sin estado llamado `Frameworks()`. `Frameworks()` debe mapear el arreglo a una lista desordenada, muy parecido al último desafío. Termina de escribir el callback de `map` para devolver un elemento `li` por cada framework en el arreglo `frontEndFrameworks`. Esta vez, asegúrate de darle a cada `li` un atributo `key`, asignado a un valor único. Los elementos `li` también deben contener texto de `frontEndFrameworks`.
