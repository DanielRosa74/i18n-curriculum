...
- Usa un ciclo `for` para iterar a través de cada puerta en el arreglo de entrada `gates`. Para cada puerta, agrega una propiedad a `summary` donde:
...
Tu función `initializeThroughput` debe devolver `{}` cuando se llame con `[]`, sin mutar el arreglo de entrada original.
...
Tu función `initializeThroughput` debe devolver `{ North: 0 }` cuando se llame con `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]`, sin mutar el arreglo de entrada original.
...
Tu función `initializeThroughput` debe devolver `{ North: 0, East: 0, South: 0 }` cuando se llame con `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]`, sin mutar el arreglo de entrada original.
...
Tu función `initializeThroughput` debe devolver `summary` después del ciclo, como la última línea de la función.
