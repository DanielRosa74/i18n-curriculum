...
Ainda dentro do seu terceiro `li` elemento, crie um `ul` elemento para o popup. Dê a ele um `className` de `sub-menu` e um `aria-label` de `Apps`.
...
Seu `ul` elemento deve ter um `className` de `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Seu `ul` elemento deve ter um `aria-label` de `Apps`.
