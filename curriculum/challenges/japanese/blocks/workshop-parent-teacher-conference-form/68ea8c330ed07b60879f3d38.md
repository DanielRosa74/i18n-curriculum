...
まず、テキストが `Student Grade: ` で `for` 属性が `"grade"` に設定された別の `label` 要素を追加してください。

次に、`label` 要素の下に、`type` 属性が `"number"` に設定された `input` 要素を追加してください。`name` と `id` 属性は `"grade"` に設定する必要があります。`placeholder` 属性は `"E.g., 4"` に設定してください。最後に、数値の `input` は必須にしてください。
