...
Тепер оголосіть змінну `netRemaining` і призначте їй значення, що дорівнює різниці `income` та `expenses`.

# --hints--

У вашій функції `calculateBudget` має бути змінна `netRemaining`.
...
Ви маєте знайти значення `income - expenses`.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

Ви маєте призначити значення `income - expenses` змінній `netRemaining`.
