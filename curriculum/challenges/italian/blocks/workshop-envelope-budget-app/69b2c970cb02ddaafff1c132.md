...
La tua stringa `output.innerHTML` avrà bisogno di un elemento `span`. Crealo e assegna un attributo class impostato sulla variabile `statusClass`.

All'interno del tuo elemento `span`, aggiungi la variabile `statusText` come contenuto testuale.
...
Il tuo `span` dovrebbe avere l'attributo `class` impostato su `${statusClass}`.
...
Il tuo `span` dovrebbe avere `${statusText}` come contenuto testuale.
