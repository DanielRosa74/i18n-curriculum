...
Recuerda que `return` termina la ejecución de una función. Después de tu bloque `if`, necesitas manejar la lógica para cuando la entrada es válida. Como tu sentencia if devuelve un valor, no necesitas una sentencia `else`.

Usa el operador de asignación de suma para agregar `currVal` a tu `total`. Necesitarás usar el constructor `Number` para convertir `currVal` a un número.
...
Después de tu sentencia `if`, debes usar el operador de asignación de suma en `total`.
...
Debes agregar el valor de `Number(currVal)` a `total`.
