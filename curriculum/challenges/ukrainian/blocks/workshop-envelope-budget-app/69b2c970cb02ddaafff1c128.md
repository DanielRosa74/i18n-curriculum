...
Вам також потрібно отримати значення вашого `#income` вхідного елемента. Ви вже отримали його на початку свого коду і призначили змінній `incomeInput`. Однак ви використали `getElementById`, який повертає `Element`, а не `NodeList`.

`NodeList` — це об’єкт, схожий на масив, що означає, що ви можете ітерувати по ньому, і він має деякі загальні методи з масивом. Для вашої функції `getTotalFromInputs` масив підійде як аргумент так само добре, як і `NodeList`.

Оголосіть змінну `income` і призначте їй результат виклику `getTotalFromInputs` — передайте масив, що містить ваш `incomeInput`, як аргумент.

# --hints--

У вашій функції `calculateBudget` має бути змінна `income`.
...
Ви повинні викликати `getTotalFromInputs` з `[incomeInput]` як аргументом.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

Ви повинні призначити результат `getTotalFromInputs` змінній `income`.
