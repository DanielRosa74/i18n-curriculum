...
Use `const` para criar uma função `fetchMoreAuthors` com a sintaxe de arrow function. Ainda não coloque nada dentro dela. Certifique-se de usar chaves porque você terá mais de uma expressão dentro da função.
...
Sua variável `fetchMoreAuthors` deve ser uma função.

```js
assert.isFunction(fetchMoreAuthors)
```

Sua função `fetchMoreAuthors` não deve receber nenhum parâmetro.
...
Sua função `fetchMoreAuthors` deve usar a sintaxe de arrow function e ter um corpo vazio.
