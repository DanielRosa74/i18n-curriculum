...
`li` 要素内で、`item` テキストの上に、`type` 属性が `checkbox` に設定された `input` 要素を追加してください。また、その `onChange` 属性を、`toggleItem()` を引数にして `item` を呼び出す無名関数に設定してください。

最後に、より良いアクセシビリティのために、`input` と `item` テキストを `label` 要素で囲んでください。

# --hints--

`checkbox` に設定された `type` 属性を持つ別の `input` 要素を追加してください。
