...
Un objeto `range` es inmutable, lo que significa que no se puede cambiar después de ser creado. Una lista es mutable, por lo que puedes cambiar sus valores.

Para generar una lista de enteros, pasa un rango al constructor `list()`. El constructor `list()` convierte un iterable en una lista. Aquí tienes un ejemplo de cómo generar una lista de enteros pares entre `2` y `10`:
