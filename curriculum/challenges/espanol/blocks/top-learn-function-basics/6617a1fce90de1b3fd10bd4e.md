...
La última línea, `favoriteAnimal('Goat')`, es donde estás llamando a tu función `favoriteAnimal` y pasando el valor `'Goat'` dentro de esa función. Aquí, `'Goat'` es tu argumento. Le estás diciendo a la función `favoriteAnimal`: "Por favor, envía `'Goat'` a la función `favoriteAnimal` y usa `'Goat'` dondequiera que esté el marcador de posición `'animal'`." Gracias a la flexibilidad que proporciona usar un parámetro, puedes declarar cualquier animal como tu favorito.
