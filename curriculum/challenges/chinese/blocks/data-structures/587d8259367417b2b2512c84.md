...
让我们创建一个 trie 来保存单词。它将通过一个 `add` 方法接受单词，并将这些单词保存到 trie 数据结构中。它还允许我们通过 `isWord` 方法查询给定的字串是否是一个单词，并通过 `print` 方法检索所有输入到 trie 中的单词。`isWord` 应该返回一个布尔值，`print` 应该返回一个包含所有这些单词字串的数组。为了验证这个数据结构是否正确实现，我们为树中的每个节点提供了一个 `Node` 结构。每个节点都是一个对象，带有一个 `keys` 属性，它是一个 JavaScript Map 对象，用于保存每个节点有效的单个字母键。我们还在节点上创建了一个 `end` 属性，如果该节点表示一个单词的终止，可以将其设置为 `true`。
