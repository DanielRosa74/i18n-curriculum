...
次に、`id` が `food` に設定された2つ目の `fieldset` 要素を作成してください。

その要素の中に、テキストが `Food` の `legend` 要素と、`class` が `input-container` に設定された空の `div` を作成してください。
...
2つ目の `fieldset` 要素には `id` が `food` に設定されている必要があります。
...
新しい `legend` 要素にはテキスト `Food` が含まれている必要があります。
