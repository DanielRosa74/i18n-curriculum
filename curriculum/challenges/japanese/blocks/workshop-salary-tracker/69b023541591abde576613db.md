...
`level` セッターが作成されたので、`__init__` を更新して `self._level = level` の代わりに `self.level = level` を使うようにしてください。これにより、`level` の代入がセッター経由で行われ、次のステップでバリデーションが追加されます。

また、次のステップでセッターがバリデーションを処理するため、`if not isinstance(level, str)` から `__init__` のチェックを削除してください。

# --hints--

`__init__` メソッドは `self._level = level` の代わりに `self.level = level` を使うべきです。
