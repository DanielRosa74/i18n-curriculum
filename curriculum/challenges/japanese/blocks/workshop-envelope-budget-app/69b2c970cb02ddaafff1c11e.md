...
`return` は関数の実行を終了させることを覚えておいてください。`if` ブロックの後で、入力が有効な場合のロジックを処理する必要があります。if文が値を返すので、`else` 文は必要ありません。

加算代入演算子を使って `currVal` を `total` に加算してください。`currVal` を数値に変換するために `Number` コンストラクタを使う必要があります。
...
`if` 文の後で、`total` に対して加算代入演算子を使ってください。
...
`Number(currVal)` の値を `total` に加算してください。
