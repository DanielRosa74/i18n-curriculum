...
Esta lição vai focar no módulo `fs` do Node, como ele funciona e as diferentes formas de usar os métodos que ele oferece.
...
A flexibilidade que o módulo `fs` oferece é que você pode usar seus métodos tanto de forma síncrona quanto assíncrona. Muitos métodos do módulo `fs` estão disponíveis nas formas assíncrona e síncrona, enquanto alguns estão disponíveis apenas de forma assíncrona.
...
Não para por aí. Dependendo do método, você pode usá-lo de diferentes maneiras:
...
A API baseada em promises também oferece uma forma assíncrona de trabalhar com arquivos. Você pode usar `async/await` com promises, o que pode tornar o código assíncrono mais fácil de ler e manter.

Tanto as APIs baseadas em callback quanto as baseadas em promises estão disponíveis, e a melhor escolha depende das suas necessidades.

O método `writeFile()` permite que você escreva em um arquivo. Se o arquivo não existir, ele será criado no caminho especificado. Se você fornecer um caminho relativo, o arquivo será criado relativo ao diretório de trabalho atual.
...
O método `appendFile()` permite que você adicione conteúdo a um arquivo. Se o arquivo não existir, ele será criado. Veja como usá-lo:
...
Qual dos seguintes NÃO é um método válido para escrever dados em um arquivo usando o módulo `fs` do Node?
...
De forma assíncrona, por exemplo `fs.writeFileAsync()`.
