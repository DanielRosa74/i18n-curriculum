...
title: Usar PropTypes para definir las props que esperas
challengeType: 6
forumTopicId: 301419
...
En el ejemplo anterior, la parte `PropTypes.func` verifica que `handleClick` sea una función. Agregar `isRequired` indica a React que `handleClick` es una propiedad obligatoria para ese componente. Verás una advertencia si no se proporciona esa prop. También observa que `func` representa `function`. Entre los siete tipos primitivos de JavaScript, `function` y `boolean` (escrito como `bool`) son los únicos dos que usan una ortografía inusual. Además de los tipos primitivos, hay otros tipos disponibles. Por ejemplo, puedes verificar que una prop sea un elemento React. Consulta la <a href="https://reactjs.org/docs/typechecking-with-proptypes.html#proptypes" target="_blank" rel="noopener noreferrer nofollow">documentación</a> para todas las opciones.
