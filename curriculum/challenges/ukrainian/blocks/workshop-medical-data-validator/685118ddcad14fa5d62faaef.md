...
Оператори `is` та `is not` перевіряють, чи посилаються два значення на один і той самий об’єкт. Це називається порівнянням ідентичності. Ви можете використовувати `is None`, щоб перевірити, чи є значення об’єктом `None`, і `is not None`, щоб перевірити, чи не є воно об’єктом `None`.
