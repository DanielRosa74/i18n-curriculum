...
本挑战强调了一种有前景的新防御措施，它可以显著降低现代浏览器中多种攻击的风险和影响。通过设置和配置内容安全策略，你可以防止任何非预期内容注入到你的页面中。这将保护你的应用免受 XSS 漏洞、不良跟踪、恶意帧等多种威胁。内容安全策略通过定义一个受信任的内容源允许列表来工作。你可以为网页可能需要的每种资源（脚本、样式表、字体、帧、媒体等）配置这些列表。可用的指令有很多，因此网站所有者可以实现细粒度控制。更多详情请参见 HTML 5 Rocks 和 KeyCDN。不幸的是，旧版浏览器不支持内容安全策略。
