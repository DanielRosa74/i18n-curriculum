...
- <kbd>Flecha arriba</kbd>/<kbd>Flecha abajo</kbd>: Navega por los comandos anteriores/siguientes en el historial.
- <kbd>Tab</kbd>: Autocompleta comandos.
- <kbd>Control</kbd> + <kbd>L</kbd> (Linux/macOS) o escribir `cls` (Windows): Limpia la pantalla del terminal.
- <kbd>Control</kbd> + <kbd>C</kbd>: Interrumpe un comando en ejecución (también se usa para copiar en PowerShell si hay texto seleccionado).
- <kbd>Control</kbd> + <kbd>Z</kbd> (solo Linux/macOS): Suspende una tarea en segundo plano; usa `fg` para reanudarla.
- **`!!`**: Vuelve a ejecutar instantáneamente el último comando ejecutado.
...
- **Páginas del manual**: Usa `man` para acceder a documentación detallada de comandos.
  - Proporciona información completa sobre el uso del comando
  - Muestra todas las opciones disponibles y ejemplos
  - Usa las <kbd>teclas de flecha</kbd> para navegar, <kbd>q</kbd> para salir
