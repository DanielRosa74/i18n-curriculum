...
Ваш наступний крок — написати функцію, яка отримає загальні суми з записів користувача.

Оголосіть функцію `getTotalFromInputs` і додайте їй параметр з назвою `list`.

# --hints--

Ви повинні оголосити змінну `getTotalFromInputs`.

```js
assert.isDefined(getTotalFromInputs);
```

Ваша змінна `getTotalFromInputs` має бути функцією.

```js
assert.isFunction(getTotalFromInputs);
```

Ваша функція `getTotalFromInputs` має приймати параметр з назвою `list`.
