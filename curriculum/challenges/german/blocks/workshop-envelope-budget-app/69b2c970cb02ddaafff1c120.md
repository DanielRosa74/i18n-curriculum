...
Jetzt ist es an der Zeit, alles zusammenzufügen. Deklarieren Sie eine leere `calculateBudget`-Funktion, die einen Parameter namens `e` entgegennimmt. Diese Funktion wird ein weiterer Ereignis-Listener sein, daher ist das erste übergebene Argument das Browser-Ereignis – `e` ist ein gebräuchlicher Name für diesen Parameter.

# --hints--

Sie sollten eine Variable `calculateBudget` deklarieren.

```js
assert.isDefined(calculateBudget);
```

Ihre Variable `calculateBudget` sollte eine Funktion sein.

```js
assert.isFunction(calculateBudget);
```

Ihre Funktion `calculateBudget` sollte ein Argument `e` entgegennehmen.
