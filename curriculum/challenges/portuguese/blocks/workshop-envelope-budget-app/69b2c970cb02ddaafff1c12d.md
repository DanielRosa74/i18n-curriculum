...
Agora você precisa determinar o status apropriado com base em o usuário estar acima ou abaixo do orçamento.

Comece criando uma declaração `if` que verifique se `netRemaining` é menor que `0`.

# --hints--

Sua função `calculateBudget` deve ter uma declaração `if`.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

Sua declaração `if` deve verificar se `netRemaining` é menor que `0`.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

O corpo da sua declaração `if` deve estar vazio por enquanto.
