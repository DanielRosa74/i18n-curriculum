...
Debajo de ese `input`, agrega un elemento `label` con el texto `Location` y el atributo `for` establecido en `"location"`.
...
Debajo de ese `input`, agrega un elemento `label` con el texto `Reputation` y el atributo `for` establecido en `"reputation"`.
...
Tu cuarta casilla de verificación `input` debe tener un atributo `id` establecido en `"reputation"`.
