...
A previsão deve ser aleatória, então é necessário uma função auxiliar para selecionar uma previsão aleatória. Declare uma variável chamada `chosenCard` dentro do método `singleCardSelected`. Use a função `getRandomItem` para obter um cartão aleatório e passe `this.cards` como argumento.
