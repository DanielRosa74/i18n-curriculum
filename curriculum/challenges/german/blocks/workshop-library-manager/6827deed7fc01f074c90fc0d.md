...
Es wäre schön, Ihre `getBooksByAuthor`-Funktion mit einem anderen Autor zu testen.

Beginnen Sie damit, die Nachricht `"\nList of books by James Clear:\n"` in der Konsole auszugeben.

Fügen Sie unter diesem `console.log()` einen weiteren `console.log()` hinzu. Rufen Sie innerhalb dieses `console.log()` die `getBooksByAuthor()`-Funktion mit `library` und `"James Clear"` als Argumente auf.
