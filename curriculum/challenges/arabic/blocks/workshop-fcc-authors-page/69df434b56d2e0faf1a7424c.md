...
بما أن استدعاء `fetch` الخاص بك غير معرف داخل دالة غير متزامنة أو في الجسم الرئيسي لوحدة، لا يمكنك استخدام `await` قبل إصلاح ذلك.
...
يجب أن تكون المتغير `initialFetch` دالة.
...
يجب أن تكون المتغير `initialFetch` دالة غير متزامنة.
...
يجب ألا تأخذ الدالة `initialFetch` أي معلمات.
...
يجب أن تستخدم الدالة `initialFetch` صيغة الدالة السهمية.
