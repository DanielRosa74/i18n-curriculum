...
أعلن عن متغير `foodInputs`، وعيّن له قيمة استدعاء `document.querySelectorAll()` باستخدام المحدد `#food input[type='number']`. هذا سيُرجع أي مُدخَلات من نوع `number` داخل عنصر `#food`.

# --hints--

يجب إعلان متغير `foodInputs`.
...
يجب استدعاء `document.querySelectorAll()` باستخدام `#food input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `foodInputs`.
