...
Додайте до вашого елемента `label` атрибут `for` зі значенням `X-#-name`, де `X` — це `value` змінної `category`, а `#` — значення `entryNumber`. Пам’ятайте, що атрибути HTML мають бути взяті в подвійні лапки.

# --hints--

Ваш `label` елемент повинен мати атрибут `for`, встановлений у `${category}-${entryNumber}-name`.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

Ваш `label` елемент повинен містити текст `"Expense ${entryNumber} Name"`.
