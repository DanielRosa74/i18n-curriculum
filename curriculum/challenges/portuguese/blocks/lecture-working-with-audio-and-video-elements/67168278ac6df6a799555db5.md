...
## Adicionando áudio com o atributo `src`
...
## Adicionando controles de reprodução com o atributo `controls`
...
O atributo `controls` habilita os usuários a gerenciar a reprodução de áudio, incluindo pausar ou retomar a reprodução. O atributo `controls` é um atributo booleano que pode ser adicionado a um elemento para habilitar controles de reprodução embutidos. Se omitido, nenhum controle será exibido.
...
## Repetindo áudio com o atributo `loop`
...
## Silenciando áudio com o atributo `muted`

Outro atributo que você pode usar é o atributo `muted`. Quando presente no elemento `audio`, esse atributo booleano fará o áudio começar em estado mudo. Aqui está um exemplo de uso do atributo `muted`.
...
## Exibindo uma imagem de pré-visualização com o atributo `poster`

Para o atributo `src`, ou atributo de fonte, estamos usando um vídeo chamado "Big Buck Bunny" do archive.org. Se você quiser exibir uma imagem enquanto o vídeo está sendo baixado, pode usar o atributo `poster`. Esse atributo não está disponível para elementos `audio` e é exclusivo do elemento `video`. Aqui está um exemplo de uso do atributo `poster` com conteúdo fornecido por peach.blender.org.
