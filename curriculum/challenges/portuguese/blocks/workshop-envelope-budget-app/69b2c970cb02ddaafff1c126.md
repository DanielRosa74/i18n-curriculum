...
Agora declare uma variável `food` e atribua a ela o valor da chamada de `getTotalFromInputs` com seus `foodInputs`.

# --hints--

Sua função `calculateBudget` deve ter uma variável `food`.
...
Você deve fazer a chamada de `getTotalFromInputs` com `foodInputs` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

Você deve atribuir o resultado de `getTotalFromInputs` a `food`.
