...
이를 해결하려면 두 번째 `legend` 내 `label` 요소들 앞에 텍스트 `Account type (required)`가 포함된 `fieldset` 요소를 추가하여 필요한 내용을 설명할 수 있습니다. 그런 다음 필수 데이터를 포함하여 폼이 제출되도록 두 번째 `checked` 입력에 `Personal` 속성을 추가하세요.
...
`legend` 요소에 `Account type (required)` 텍스트를 추가해야 합니다.
