...
Ahora, presentemos el método `forEach`. `forEach` es un método integrado para arreglos en JavaScript. Te permite iterar sobre cada elemento en un arreglo y realizar una operación en cada elemento. La operación que quieres realizar se define en una función callback que proporcionas a `forEach`.
