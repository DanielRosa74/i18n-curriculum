...
El constructor `Number()` se usa para crear un objeto number. El objeto number contiene algunas propiedades y métodos útiles como `isNaN` y el método `toFixed`. Aquí tienes un ejemplo usando el constructor `Number()` con la palabra clave `new`:
