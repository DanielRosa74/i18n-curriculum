...
次に、`type`属性が`range`に設定された`input`要素を追加して、音量スライダーを作成してください。デフォルトの音量レベルを定義するために、`min`属性を`0`に、`max`属性を`100`に、`value`属性を`50`に設定してください。
...
`input`要素には値が`range`の`type`属性が必要です。
...
`input`要素には値が`100`の`max`属性が必要です。
...
`input`要素には値が`50`の`value`属性が必要です。
