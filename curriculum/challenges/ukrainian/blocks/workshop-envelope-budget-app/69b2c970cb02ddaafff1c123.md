...
Використовуючи той самий синтаксис, зробіть запит до ваших `number` вхідних даних у елементі `#utilities` і призначте їх у `utilitiesInputs`.

# --hints--

Ви повинні оголосити змінну `utilitiesInputs`.
...
Ви повинні зробити виклик `document.querySelectorAll()` з `#utilities input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Ви повинні призначити результат вашого виклику `document.querySelectorAll()` змінній `utilitiesInputs`.
