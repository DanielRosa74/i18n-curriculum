...
Toa kipengele chako cha `label` sifa ya kipengele `for` yenye thamani `X-#-name`, ambapo `X` ni `value` ya mabadiliko ya `category` na `#` ni thamani ya `entryNumber`. Kumbuka kwamba sifa za HTML zinapaswa kufunikwa na alama za nukuu maradufu.

# --hints--

Kipengele chako cha `label` kinapaswa kuwa na sifa ya kipengele `for` iliyowekwa kuwa `${category}-${entryNumber}-name`.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

Kipengele chako cha `label` kinapaswa kuwa na maandishi `"Expense ${entryNumber} Name"`.
