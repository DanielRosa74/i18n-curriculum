...
Unahitaji kujua kama mtumizi amezidi bajeti au ana pesa iliyobaki. Wakati matumizi yanapozidi mapato, uko juu ya bajeti (upungufu). Wakati mapato yanapozidi matumizi, una pesa iliyobaki (zuzuzu). Kuwa na sifuri kabisa iliyobaki kunaweza kufikiriwa kama usawa.

Tangaza vigezo viwili: `statusText` na `statusClass`. Anzisha zote kama mfuatano wa herufi tupu kwa kutumia `""`.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `statusText`.
...
Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `statusClass`.
...
Unapaswa kuanzisha `statusText` na `statusClass` kama mfuatano wa herufi tupu.
...
Vigezo vyote viwili vinapaswa kutangazwa kwa kutumia `let`.
