...
Sie müssen wissen, ob der Benutzer über dem Budget liegt oder noch Geld übrig hat. Wenn die Ausgaben das Einkommen übersteigen, sind Sie über dem Budget (ein Defizit). Wenn das Einkommen die Ausgaben übersteigt, haben Sie Geld übrig (ein Überschuss). Genau null übrig zu haben, kann als ausgeglichen betrachtet werden.

Deklarieren Sie zwei Variablen: `statusText` und `statusClass`. Initialisieren Sie beide als leere Strings mit `""`.

# --hints--

Ihre `calculateBudget`-Funktion sollte eine Variable `statusText` haben.
...
Ihre `calculateBudget`-Funktion sollte eine Variable `statusClass` haben.
...
Sie sollten `statusText` und `statusClass` als leere Strings initialisieren.
...
Beide Variablen sollten mit `let` deklariert werden.
