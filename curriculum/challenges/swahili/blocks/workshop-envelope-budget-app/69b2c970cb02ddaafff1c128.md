...
Pia unahitaji kupata thamani ya ingizo lako la `#income`. Tayari umeuliza hili juu ya msimbo wako, na ukaweka katika kigezo cha `incomeInput`. Hata hivyo, ulitumia `getElementById`, ambayo inarudisha `Element`, si `NodeList`.

`NodeList` ni kitu kinachofanana na safu ya data, ambayo ina maana unaweza kurudia kupitia na kinashiriki baadhi ya njia za kawaida na safu ya data. Kwa kitendakazi chako cha `getTotalFromInputs`, safu ya data itafanya kazi kwa hoja sawa na `NodeList`.

Tangaza kigezo cha `income` na ukaweke kwa matokeo ya wito wa `getTotalFromInputs` – pita safu ya data inayojumuisha `incomeInput` kama hoja.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kuwa na kigezo cha `income`.
...
Unapaswa kuita `getTotalFromInputs` ukiwa na `[incomeInput]` kama hoja.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

Unapaswa kuweka matokeo ya `getTotalFromInputs` katika `income`.
