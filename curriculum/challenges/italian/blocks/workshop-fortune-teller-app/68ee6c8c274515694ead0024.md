...
La fortuna deve essere casuale, quindi serve una funzione di supporto per selezionare una fortuna casuale. Dichiara una variabile chiamata `chosenCard` all'interno del metodo `singleCardSelected`. Usa la funzione `getRandomItem` per ottenere una carta casuale e passa `this.cards` come argomento.
