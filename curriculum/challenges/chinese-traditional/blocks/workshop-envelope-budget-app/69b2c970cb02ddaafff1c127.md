...
依照相同的樣式，為 `#utilities` 和 `#entertainment` 元素中的 `number` 輸入欄位宣告 `utilities` 和 `entertainment` 變數。將它們指定為適當的 `getTotalFromInputs` 呼叫結果。

# --hints--

你的 `calculateBudget` 函式（程式）應該有一個 `utilities` 變數。
...
你應該使用 `utilitiesInputs` 作為引數來呼叫 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

你應該將 `getTotalFromInputs` 的結果指定給 `utilities`。
...
你的 `calculateBudget` 函式（程式）應該有一個 `entertainment` 變數。
...
你應該使用 `entertainmentInputs` 作為引數來呼叫 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

你應該將 `getTotalFromInputs` 的結果指定給 `entertainment`。
