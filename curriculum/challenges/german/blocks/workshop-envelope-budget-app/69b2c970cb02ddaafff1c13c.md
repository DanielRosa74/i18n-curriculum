...
Nachdem Ihre Schleife abgeschlossen ist, müssen Sie die Felder `incomeInput` und `rentInput` löschen. Setzen Sie die `value`-Eigenschaft von `incomeInput` und `rentInput` auf einen leeren String.

# --hints--

Ihre `clearForm`-Funktion sollte auf die `value`-Eigenschaft des Elements `incomeInput` zugreifen.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

Ihre `clearForm`-Funktion sollte die `value`-Eigenschaft des Elements `incomeInput` auf einen leeren String setzen.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

Sie sollten `incomeInput` nach Ihrer `for`-Schleife ändern.
...
Ihre `clearForm`-Funktion sollte auf die `value`-Eigenschaft des Elements `rentInput` zugreifen.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

Ihre `clearForm`-Funktion sollte die `value`-Eigenschaft des Elements `rentInput` auf einen leeren String setzen.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

Sie sollten `rentInput` nach Ihrer `for`-Schleife ändern.
