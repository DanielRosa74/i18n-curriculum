...
1. `header` 요소가 있어야 하며, `body` 안에서 첫 번째 요소여야 합니다.
1. `header` 요소 안에는 `Event Hub`라는 텍스트를 포함한 `h1` 요소와 `nav` 요소가 있어야 합니다.
1. `nav` 요소 안에는 페이지의 다른 섹션으로 연결되는 두 개의 항목이 있는 순서 없는 목록이 있어야 합니다. 첫 번째 항목은 `Upcoming Events`라는 텍스트를, 두 번째 항목은 `Past Events`라는 텍스트를 가져야 합니다.
1. 각 링크는 해당 페이지 섹션인 `#upcoming-events`와 `#past-events`로 연결되는 `href` 속성을 가진 `a` 요소로 나타내야 합니다.
1. 페이지의 여러 섹션을 포함하는 `main` 요소가 있어야 합니다.
1. `main` 요소 안에는 두 개의 `section` 요소가 있어야 합니다.
1. 첫 번째 `section` 요소는 `id` 속성 값이 `upcoming-events`여야 합니다.
1. `#upcoming-events` 섹션 안에는:
