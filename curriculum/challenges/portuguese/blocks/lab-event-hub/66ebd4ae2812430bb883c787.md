...
1. Você deve ter um elemento `header`, e ele deve ser o primeiro elemento dentro do `body`.
...
1. Cada link deve ser representado por um elemento `a` com um atributo `href` que linka para a respectiva seção da página, `#upcoming-events` e `#past-events` respectivamente.
