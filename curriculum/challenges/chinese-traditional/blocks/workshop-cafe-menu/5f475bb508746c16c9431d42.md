...
你添加的圖片沒有像上方的 `Coffee` 標題那樣水平置中。`img` 元素是行內層級元素，因此它們會置於文字流程中。

要讓圖片像標題這類區塊層級元素一樣表現，請建立一個 `img` 型別選擇器。將 `display` 屬性設為 `block`，並使用 `margin-left` 和 `margin-right` 屬性來水平置中它。
