...
تحتاج أيضًا إلى الحصول على قيمة الـ `#income` مُدخَل. لقد استعلمت عنها بالفعل في بداية الكود، وعيّنتها إلى المتغير `incomeInput`. مع ذلك، استخدمت `getElementById`، التي تُرجع `Element`، وليس `NodeList`.

`NodeList` هو كائن يشبه المصفوفة، مما يعني أنه يمكنك التكرار خلاله ويشارك بعض الطرق الشائعة مع المصفوفة. بالنسبة لدالتك `getTotalFromInputs`، ستعمل المصفوفة كمعلمة تمامًا كما يعمل `NodeList`.

أعلن عن متغير `income` وعينه إلى نتيجة استدعاء `getTotalFromInputs` – مرر مصفوفة تحتوي على `incomeInput` كمعلمة.

# --hints--

يجب أن تحتوي دالة `calculateBudget` على متغير `income`.
...
يجب أن تستدعي `getTotalFromInputs` مع `[incomeInput]` كمعلمة.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

يجب أن تعين نتيجة `getTotalFromInputs` إلى `income`.
