...
1. 你應該有一個 `header` 元素，且它必須是 `body` 內的第一個元素。
1. 在 `header` 元素內，你應該有一個包含文字 `Event Hub` 的 `h1` 元素，以及一個 `nav` 元素。
1. 在 `nav` 元素內，你應該有一個包含兩個項目的無序列表，這兩個項目包含指向頁面不同區段的連結。第一個項目的文字應為 `Upcoming Events`，第二個項目的文字應為 `Past Events`。
1. 每個連結應由一個帶有 `href` 屬性的 `a` 元素表示，分別連結到頁面對應的區段 `#upcoming-events` 和 `#past-events`。
1. 你應該有一個包含頁面不同區段的 `main` 元素。
1. 在 `main` 元素內，你應該有兩個 `section` 元素。
1. 第一個 `section` 元素應該有一個值為 `upcoming-events` 的 `id` 屬性。
