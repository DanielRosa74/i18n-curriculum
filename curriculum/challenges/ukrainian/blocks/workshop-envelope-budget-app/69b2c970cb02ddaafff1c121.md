...
Додайте лінію у вашу `calculateBudget` функцію, яка викликає метод `preventDefault()` на параметрі `e`. Потім скиньте глобальний прапорець помилки `isError` у значення `false`.

# --hints--

Ваша `calculateBudget` функція має викликати `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

Ваша `calculateBudget` функція має скинути глобальний прапорець помилки `isError` у значення `false`.
