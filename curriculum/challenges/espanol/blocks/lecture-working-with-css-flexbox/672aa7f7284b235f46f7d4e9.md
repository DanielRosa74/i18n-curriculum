...
CSS flexbox es un modelo de diseño unidimensional que te permite organizar elementos en filas o columnas dentro de un contenedor. También puedes controlar su orden y orientación. Los desarrolladores web lo usan para crear sitios web y aplicaciones web adaptables que se ajustan a diferentes tamaños y orientaciones de pantalla. Nos referimos a flexbox como un modelo de diseño unidimensional porque se enfoca en organizar elementos a lo largo de un solo eje a la vez. El eje puede ser horizontal o vertical.
...
Si solo configuras el `width`, `height` y `background-color` de estos elementos `div` en la hoja de estilos CSS, cada elemento hijo se colocará en su propia fila porque el contenedor no es flex por defecto (para ver las vistas previas, habilita el editor interactivo):
...
Pero si agregas `display: flex` al contenedor `main`, los elementos `div` se reorganizarán para caber en la misma fila y se encogerán si es necesario:
...
Genial. Ahora hablemos un poco sobre el modelo flex. Este modelo define cómo se organizan los elementos flexibles dentro de un contenedor flex. Cada contenedor flex tiene dos ejes:
...
Piensa en cómo se posicionan el eje principal y el eje transversal en relación uno con el otro.
