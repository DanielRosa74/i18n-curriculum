...
Declare uma variável `foodInputs` e atribua a ela o valor da chamada `document.querySelectorAll()` com o seletor `#food input[type='number']`. Isso vai retornar qualquer entrada `number` que esteja dentro do elemento `#food`.

# --hints--

Você deve declarar uma variável `foodInputs`.
...
Você deve fazer a chamada `document.querySelectorAll()` com `#food input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Você deve atribuir o resultado da sua chamada `document.querySelectorAll()` à variável `foodInputs`.
