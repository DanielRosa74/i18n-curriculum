...
Quando você usa a propriedade `length` em uma string primitiva, o JavaScript envolve temporariamente a string primitiva em um objeto string para realizar a operação. É por isso que você pode usar a propriedade `length` e os diferentes métodos como `repeat()`, `concat()` e `slice()`. Esses tipos de métodos e propriedades são chamados de métodos de instância, propriedades de instância e métodos estáticos. Você vai aprender em detalhes como isso funciona em módulos futuros.
