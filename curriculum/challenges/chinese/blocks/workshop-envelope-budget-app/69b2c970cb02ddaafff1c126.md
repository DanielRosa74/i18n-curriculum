...
现在声明一个 `food` 变量，并将调用 `getTotalFromInputs` 并传入你的 `foodInputs` 的结果赋值给它。

# --hints--

你的 `calculateBudget` 函数应该有一个 `food` 变量。
...
你应该用 `foodInputs` 作为参数调用 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

你应该将 `getTotalFromInputs` 的结果赋值给 `food`。
