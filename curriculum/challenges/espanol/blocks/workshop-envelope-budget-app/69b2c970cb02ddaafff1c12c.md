...
Necesitas saber si el usuario está por encima del presupuesto o si le queda dinero. Cuando los gastos superan los ingresos, estás por encima del presupuesto (un déficit). Cuando los ingresos superan los gastos, te queda dinero (un superávit). Tener exactamente cero restante se puede considerar equilibrado.

Declara dos variables: `statusText` y `statusClass`. Inicializa ambas como cadenas vacías usando `""`.

# --hints--

Tu función `calculateBudget` debe tener una variable `statusText`.
...
Tu función `calculateBudget` debe tener una variable `statusClass`.
...
Debes inicializar `statusText` y `statusClass` como cadenas vacías.
...
Ambas variables deben declararse usando `let`.
