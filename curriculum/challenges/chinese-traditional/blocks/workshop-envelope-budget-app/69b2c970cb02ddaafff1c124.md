...
依照相同的樣式，在 `#entertainment` 元素中查詢你的 `number` 輸入，並將它們指定給 `entertainmentInputs`。

# --hints--

你應該宣告一個 `entertainmentInputs` 變數。
...
你應該使用 `#entertainment input[type='number']` 來呼叫 `document.querySelectorAll()`。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 呼叫的結果指定給 `entertainmentInputs`。
