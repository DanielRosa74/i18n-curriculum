...
Seguindo esse mesmo padrão, declare as variáveis `utilities` e `entertainment` para as entradas `number` nos elementos `#utilities` e `#entertainment`. Atribua a elas as chamadas apropriadas de `getTotalFromInputs`.

# --hints--

Sua função `calculateBudget` deve ter uma variável `utilities`.
...
Você deve chamar `getTotalFromInputs` com `utilitiesInputs` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

Você deve atribuir o resultado de `getTotalFromInputs` para `utilities`.
...
Sua função `calculateBudget` deve ter uma variável `entertainment`.
...
Você deve chamar `getTotalFromInputs` com `entertainmentInputs` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

Você deve atribuir o resultado de `getTotalFromInputs` para `entertainment`.
