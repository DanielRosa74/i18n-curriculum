---
id: 690a57fef66aac39dffc61a2
title: الخطوة 5
challengeType: 1
dashedName: step-5
---

# --description--

الآن بعد أن عرفت ما هي القيم الصحيحة والقيم الخاطئة، احذف كل من إعلان المتغير وعبارة `console.log` من كودك.

# --hints--

يجب ألا يكون لديك متغير `truthyOrFalsy`.

```js
const explorer = await __helpers.Explorer(code);
assert.notExists(explorer.variables.truthyOrFalsy);
```

يجب ألا يحتوي كودك على `console.log(Boolean(truthyOrFalsy))`.

```js
assert.notMatch(__helpers.removeJSComments(code), /console\s*\.\s*log\s*\(\s*Boolean\s*\(\s*truthyOrFalsy\s*\)\s*\)\s*;?/);
```

# --seed--

## --seed-contents--

```js
--fcc-editable-region--
const truthyOrFalsy = "";

console.log(Boolean(truthyOrFalsy));
--fcc-editable-region--
```
