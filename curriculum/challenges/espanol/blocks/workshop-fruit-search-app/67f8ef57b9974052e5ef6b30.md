...
Dentro del elemento `#results`, usa un operador ternario para verificar si la longitud del arreglo `results` es mayor que cero. Si lo es, recorre los elementos y muestra cada uno en un elemento `p` con la clase `result-item`. Asegúrate de incluir también un atributo `key` en cada párrafo.
