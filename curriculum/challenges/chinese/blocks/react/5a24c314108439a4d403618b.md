...
代码编辑器中有一个包含一些前端框架的数组和一个名为 `Frameworks()` 的无状态函数组件。`Frameworks()` 需要像上一个挑战一样，将数组映射为一个无序列表。完成 `map` 回调函数的编写，为 `frontEndFrameworks` 数组中的每个框架返回一个 `li` 元素。这次，确保为每个 `li` 添加一个唯一值的 `key` 属性。`li` 元素还应包含来自 `frontEndFrameworks` 的文本。
