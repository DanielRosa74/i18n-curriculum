...
1. 你应该在页面上显示 `headCoach`、`team` 和 `year` 的值。这些值应显示在具有 `id` 值为 `head-coach`、`team` 和 `year` 的 HTML 元素中。
...
你应该在具有 `id` 值为 `head-coach`、`team` 和 `year` 的 HTML 元素中显示 `footballTeam` 对象的 `headCoach`、`team` 和 `year` 值。 
