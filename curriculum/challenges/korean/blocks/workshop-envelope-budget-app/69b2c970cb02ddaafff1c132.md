...
`output.innerHTML` 문자열에 `span` 요소가 필요합니다. 그것을 만들고, `statusClass` 변수로 설정된 클래스 속성을 부여하세요.

`span` 요소 안에 `statusText` 변수를 텍스트 내용으로 추가하세요.
...
`span`에 `class` 속성이 `${statusClass}`로 설정되어야 합니다.
...
`span`에 `${statusText}`가 텍스트 내용으로 있어야 합니다.
