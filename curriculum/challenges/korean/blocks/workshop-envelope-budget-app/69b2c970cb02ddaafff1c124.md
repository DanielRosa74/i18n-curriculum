...
같은 패턴을 따라 `#entertainment` 요소 안의 `number` 입력을 쿼리하고 `entertainmentInputs`에 할당하세요.

# --hints--

`entertainmentInputs` 변수를 선언하세요.
...
`document.querySelectorAll()`을 `#entertainment input[type='number']`와 함께 호출하세요.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

`document.querySelectorAll()` 호출 결과를 `entertainmentInputs`에 할당하세요.
