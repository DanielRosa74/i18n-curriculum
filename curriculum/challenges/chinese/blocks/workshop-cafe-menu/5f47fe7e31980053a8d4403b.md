...
如果 `h2` 元素和它们关联的图标之间的垂直间距更小会更好。`h2` 元素有默认的上下边距，所以你可以将 `h2` 元素的下边距改为 `0` 或其他值。

有一个更简单的方法，只需为 `img` 元素添加一个负的上边距，就可以把它们从当前位置往上拉。负值是通过在数值前加上 `-` 实现的。要完成本练习，请在 `img` 类型选择器中使用 `25px` 的负上边距。
