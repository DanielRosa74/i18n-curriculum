...
Deklarieren Sie nach demselben Muster die Variablen `utilities` und `entertainment` für die `number`-Eingaben in den Elementen `#utilities` und `#entertainment`. Weisen Sie ihnen die entsprechenden `getTotalFromInputs`-Aufrufe zu.

# --hints--

Ihre `calculateBudget`-Funktion sollte eine Variable `utilities` haben.
...
Sie sollten `getTotalFromInputs` mit `utilitiesInputs` als Argument aufrufen.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

Sie sollten das Ergebnis von `getTotalFromInputs` der Variable `utilities` zuweisen.
...
Ihre `calculateBudget`-Funktion sollte eine Variable `entertainment` haben.
...
Sie sollten `getTotalFromInputs` mit `entertainmentInputs` als Argument aufrufen.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

Sie sollten das Ergebnis von `getTotalFromInputs` der Variable `entertainment` zuweisen.
