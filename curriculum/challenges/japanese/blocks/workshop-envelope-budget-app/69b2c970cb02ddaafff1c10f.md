...
最後に、2つ目の`label`の後の新しい行に、もう1つ`input`要素を作成してください。これには`type`属性に`number`、`min`属性に`0`（マイナスの金額を追加できないようにするため）、`placeholder`属性に`Amount`、そして2つ目の`label`要素の`for`属性と一致する`id`属性を設定してください。
...
`input` 要素には、`placeholder` 属性を `Amount` に設定してください。
...
あなたの`input`要素には、`id`属性が`${category}-${entryNumber}-amount`に設定されている必要があります。
...
新しい`input`要素には、`min`属性が`0`に設定されている必要があります。
