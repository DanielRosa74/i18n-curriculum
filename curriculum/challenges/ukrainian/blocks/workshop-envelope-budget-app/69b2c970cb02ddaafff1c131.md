...
Вам потрібно створити HTML-рядок, який буде відображатися в `output` елементі.

Почніть з призначення порожнього шаблонного рядка властивості `innerHTML` елемента `output` на новій лінії в кінці інструкції `if`.
