...
`#income` 입력값도 가져와야 합니다. 이미 코드 상단에서 이 값을 조회해 `incomeInput` 변수에 할당했지만, `getElementById`를 사용했기 때문에 `NodeList`가 아니라 `Element`를 반환합니다.

`NodeList`는 배열과 유사한 객체로, 순차적으로 반복할 수 있고 배열과 공통된 메서드를 일부 공유합니다. `getTotalFromInputs` 함수에는 배열이 인수로 들어가도 `NodeList`만큼 잘 작동합니다.

`income` 변수를 선언하고 `getTotalFromInputs`를 호출한 결과로 할당하세요. 이때 인수로 `incomeInput`을 포함하는 배열을 전달하세요.

# --hints--

`calculateBudget` 함수에는 `income` 변수가 있어야 합니다.
...
`getTotalFromInputs`를 호출할 때 인수로 `[incomeInput]`을 전달해야 합니다.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

`getTotalFromInputs`의 결과를 `income`에 할당해야 합니다.
