...
Ваш `catch` більше не повинен бути методом у ланцюжку.

```js
assert.notMatch(code, /\.catch/);
```

Ваш блок `try` має містити три інструкції, які отримують дані, створюють `json`, а потім відображають їх.
...
Ваша сторінка авторів fCC має відтворюватися правильно.
