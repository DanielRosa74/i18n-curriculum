...
## 將 `aria-expanded` 設為 `true` 或 `false`
...
此外，屬性 `aria-controls` 和 `aria-owns` 可以與 `aria-expanded` 一起使用，以在控制元件和其所控制的元素之間建立程式化的連接。
