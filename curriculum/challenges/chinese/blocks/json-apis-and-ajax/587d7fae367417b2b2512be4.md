...
<blockquote>[ ] -> 方括弧表现数组。<br>{ } -> 括弧状的括弧表现对象。<br>" " -> 双引号表现字串。它们也用于 JSON 中的密钥名称。</blockquote>
...
第一个对象中另一个有趣的“点对”是 `"codeNames":["Juggernaut","Mrs. Wallace","ButterCup"]`。这里 `codeNames` 是密钥，其值是包含三个字串的数组。密钥的值也可以是对象数组，或者是数组。
