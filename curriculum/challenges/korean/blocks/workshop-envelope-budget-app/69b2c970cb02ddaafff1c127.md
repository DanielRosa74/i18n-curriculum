...
이와 같은 패턴을 따라 `#utilities`와 `#entertainment` 요소의 `number` 입력에 대해 `utilities`와 `entertainment` 변수를 선언하세요. 적절한 `getTotalFromInputs` 호출을 할당하세요.

# --hints--

`calculateBudget` 함수에 `utilities` 변수가 있어야 합니다.
...
`utilitiesInputs`를 인수로 하여 `getTotalFromInputs`를 호출해야 합니다.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

`getTotalFromInputs`의 결과를 `utilities`에 할당해야 합니다.
...
`calculateBudget` 함수에 `entertainment` 변수가 있어야 합니다.
...
`entertainmentInputs`를 인수로 하여 `getTotalFromInputs`를 호출해야 합니다.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

`getTotalFromInputs`의 결과를 `entertainment`에 할당해야 합니다.
