...
Ваша функція `getTotalFromInputs` встановить глобальний прапорець помилки `isError` у значення `true`, якщо буде виявлено недійсні вхідні дані.

Додайте інструкцію `if` у вашу функцію `calculateBudget`, яка перевіряє істинність глобального прапорця помилки, і якщо він істинний, використайте `return` для завершення виконання функції.

# --hints--

У вашій функції `calculateBudget` має бути інструкція `if`, яка перевіряє істинність змінної `isError`.
...
Інструкція `if` має бути розміщена після останнього виклику функції `getTotalFromInputs`.
