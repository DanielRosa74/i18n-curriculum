...
Siguiendo el mismo patrón, haz una consulta para tus entradas de `number` en el elemento `#entertainment` y asígnalas a `entertainmentInputs`.

# --hints--

Debes declarar una variable `entertainmentInputs`.
...
Debes llamar a `document.querySelectorAll()` con `#entertainment input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Debes asignar el resultado de tu llamada a `document.querySelectorAll()` a `entertainmentInputs`.
