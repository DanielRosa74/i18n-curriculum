...
Ahora es momento de empezar a juntar todo. Declara una función vacía `calculateBudget`, que tome un parámetro llamado `e`. Esta función será otro listener de evento, así que el primer argumento que se pase será el evento del navegador – `e` es un nombre común para este parámetro.

# --hints--

Debes declarar una variable `calculateBudget`.

```js
assert.isDefined(calculateBudget);
```

Tu variable `calculateBudget` debe ser una función.

```js
assert.isFunction(calculateBudget);
```

Tu función `calculateBudget` debe tomar un argumento `e`.
