...
باتباع نفس النمط، استعلم عن مُدخَلات `number` داخل عنصر `#entertainment` وقم بتعيينها إلى `entertainmentInputs`.

# --hints--

يجب إعلان متغير `entertainmentInputs`.
...
يجب استدعاء `document.querySelectorAll()` مع المحدد `#entertainment input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

يجب تعيين نتيجة استدعاء `document.querySelectorAll()` إلى `entertainmentInputs`.
