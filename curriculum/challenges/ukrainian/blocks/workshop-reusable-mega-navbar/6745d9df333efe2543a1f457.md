...
Все ще всередині вашого третього `li` елемента створіть `ul` елемент для спливаючого вікна. Дайте йому `className` зі значенням `sub-menu` та `aria-label` зі значенням `Apps`.
...
Ваш `ul` елемент повинен мати `className` зі значенням `sub-menu`.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Ваш `ul` елемент повинен мати `aria-label` зі значенням `Apps`.
