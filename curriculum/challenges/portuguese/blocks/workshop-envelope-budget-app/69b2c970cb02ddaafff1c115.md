...
Seu próximo passo é escrever uma função que vai obter os valores totais das entradas do usuário.

Declare uma função `getTotalFromInputs` e dê a ela um parâmetro chamado `list`.

# --hints--

Você deve declarar uma variável `getTotalFromInputs`.

```js
assert.isDefined(getTotalFromInputs);
```

Sua variável `getTotalFromInputs` deve ser uma função.

```js
assert.isFunction(getTotalFromInputs);
```

Sua função `getTotalFromInputs` deve receber um parâmetro chamado `list`.
