...
`form` 要素を (`id` を使って) 取得し、`budgetForm` という変数に格納することから始めてください。

# --hints--

`budgetForm` という変数を作成してください。

```js
assert.isDefined(budgetForm);
```

`document.getElementById('budget-form')` をコードに含めてください。

```js
assert.match(code, /document\.getElementById\(\s*('|")budget-form\1\s*\)/g);
```

`document.getElementById('budget-form')` を `budgetForm` 変数に代入してください。
