...
按照<a href="https://www.freecodecamp.org/news/how-to-set-up-a-github-oauth-application/" target="_blank" rel="noopener noreferrer nofollow">这些说明</a>从 GitHub 获取你的*客户端 ID 和密钥*。将主页 URL 设置为你的主页（**不是项目代码的 URL**），并将回调 URL 设置为相同的主页 URL，后面追加 `/auth/github/callback`。将客户端 ID 和客户端密钥储存在你的项目 `.env` 文件中，命名为 `GITHUB_CLIENT_ID` 和 `GITHUB_CLIENT_SECRET`。
