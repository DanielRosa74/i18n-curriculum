...
Crie outro elemento `label` (em uma nova linha) no final do seu `HTMLString`. Esse `label` deve ter o texto `Expense # Amount`, usando sua sintaxe de template literal para substituir `#` pelo valor de `entryNumber`, e o atributo `for` definido como `X-#-amount`, onde `X` é o valor de `category` e `#` é o valor de `entryNumber`.
...
Seu novo elemento `label` deve ter um atributo `for` definido como `${category}-${entryNumber}-amount`.
...
Seu novo elemento `label` deve ter o texto `Expense ${entryNumber} Amount`.
