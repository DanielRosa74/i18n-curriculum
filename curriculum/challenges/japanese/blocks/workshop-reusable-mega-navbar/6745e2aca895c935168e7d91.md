...
最後に、最初の`a`要素のテキストを`Calendar`に、2番目を`Chat`に、3番目を`Email`に設定してください。

これで、`Navbar`コンポーネントが完成です！

# --hints--

最初の`a`要素のテキストを`Calendar`に設定してください。

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

2番目の`a`要素のテキストを`Chat`に設定してください。

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

3番目の`a`要素のテキストを`Email`に設定してください。
