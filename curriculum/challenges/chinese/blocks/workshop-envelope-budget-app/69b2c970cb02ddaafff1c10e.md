...
在你的 `HTMLString` 末尾新建另一个 `label` 元素（新起一行）。这个 `label` 应该包含文本 `Expense # Amount`，使用你的模板字面语法将 `#` 替换为 `entryNumber` 的值，并且 `for` 属性设置为 `X-#-amount`，其中 `X` 是 `category` 的值，`#` 是 `entryNumber` 的值。
...
你新建的 `label` 元素的 `for` 属性应设置为 `${category}-${entryNumber}-amount`。
...
你新建的 `label` 元素应包含文本 `Expense ${entryNumber} Amount`。
