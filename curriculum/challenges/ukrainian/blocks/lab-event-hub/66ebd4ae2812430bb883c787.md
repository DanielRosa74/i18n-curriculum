...
1. Ви повинні мати `header` елемент, і він має бути першим елементом всередині `body`.
1. Всередині `header` елемента ви повинні мати `h1` елемент, який містить текст `Event Hub`, і `nav` елемент.
1. Всередині `nav` елемента ви повинні мати невпорядкований список з двох елементів, які містять посилання на різні розділи сторінки. Перший елемент повинен містити текст `Upcoming Events`, а другий — текст `Past Events`.
1. Кожне посилання має бути представлене `a` елементом з атрибутом `href`, який посилається на відповідний розділ сторінки — `#upcoming-events` та `#past-events` відповідно.
1. Ви повинні мати `main` елемент, який містить різні розділи сторінки.
1. Всередині `main` елемента ви повинні мати два `section` елементи.
1. Перший `section` елемент повинен мати атрибут `id` зі значенням `upcoming-events`
1. Всередині розділу `#upcoming-events` ви повинні мати:
