...
当存在溢出（`result.overflow > 0`）时，你的 `handleGateAtTick` 函数应首先记录消息 `"Overflow of " + result.overflow + " attendees. Rerouting..."`。
...
当存在溢出（`result.overflow > 0`）时，你的 `handleGateAtTick` 函数应通过调用 `rerouteOverflow()` 并传入 `gates`、`gate`、`tickIndex` 和 `result.overflow` 来重新路由溢出。
