...
类似于你的 `#stats` 元素，你的 `#monsterStats` 元素需要两个 `span` 元素。为它们添加 `stat` 类，第一个元素的文本为 `Monster Name: `，第二个元素的文本为 `Health: `。在每个文本后，添加一个包含空嵌套 `span` 元素的 `strong` 元素。第一个内部 `span` 元素的 `id` 设置为 `monsterName`，第二个内部 `span` 元素的 `id` 设置为 `monsterHealth`。
