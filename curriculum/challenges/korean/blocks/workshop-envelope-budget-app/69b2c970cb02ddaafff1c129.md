...
`getTotalFromInputs` 함수는 잘못된 입력이 감지되면 전역 오류 플래그 `isError`를 `true`로 설정합니다.

`calculateBudget` 함수에 전역 오류 플래그의 참값 여부를 확인하는 `if` 문을 추가하고, 참일 경우 `return`을 사용해 함수 실행을 종료하세요.

# --hints--

`calculateBudget` 함수에는 `isError` 변수의 참값 여부를 확인하는 `if` 문이 있어야 합니다.
...
`if` 문은 마지막 `getTotalFromInputs` 함수 호출 뒤에 위치해야 합니다.
