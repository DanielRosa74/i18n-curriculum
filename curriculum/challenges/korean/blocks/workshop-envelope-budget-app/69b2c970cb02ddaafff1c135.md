...
같은 보간 구문을 사용하여 기존 `p` 요소 아래에 텍스트 `$${expenses} Total Expenses`가 있는 두 번째 `p` 요소를 추가하세요.

# --hints--

기존 요소 아래에 두 번째 `p` 요소를 추가해야 합니다.
...
두 번째 `p` 요소에는 텍스트 `$${expenses} Total Expenses`가 있어야 합니다.
