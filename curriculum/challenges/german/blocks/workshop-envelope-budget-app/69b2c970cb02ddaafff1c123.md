...
Verwenden Sie dieselbe Syntax, um Ihre `number` Eingaben im `#utilities` Element abzufragen und weisen Sie sie `utilitiesInputs` zu.

# --hints--

Sie sollten eine Variable `utilitiesInputs` deklarieren.
...
Sie sollten `document.querySelectorAll()` mit `#utilities input[type='number']` aufrufen.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Sie sollten das Ergebnis Ihres `document.querySelectorAll()` Aufrufs `utilitiesInputs` zuweisen.
