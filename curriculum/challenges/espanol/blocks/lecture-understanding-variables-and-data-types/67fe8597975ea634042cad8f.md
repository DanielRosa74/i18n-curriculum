...
En Python, las variables son como cajas etiquetadas para almacenar y referenciar datos de diferentes tipos. Para declarar variables en Python, asignas un valor a un identificador con el operador de asignación (`=`). No necesitas usar palabras clave especiales como `let` o `const` en JavaScript, o `char` en C#.

En Python, solo escribes el nombre de la variable a la izquierda, seguido del operador de asignación, y el valor que quieres asignar a la variable a la derecha. Aquí tienes un ejemplo de cómo declarar las variables `name` y `age`:
...
- Los nombres de variables distinguen entre mayúsculas y minúsculas: `age`, `Age` y `AGE` se consideran todos únicos.
- Los nombres de variables no pueden ser palabras clave reservadas de Python como `if`, `class` o `def`.

Si rompes alguna de esas reglas, tu programa en Python generará un `SyntaxError`:
...
Primero, los nombres de variables deben estar en minúsculas, con palabras separadas por un guion bajo. Esto se llama snake case:
...
Otra convención es evitar usar nombres de variables de una sola letra. Los nombres de una sola letra son comunes en Python, pero deben evitarse porque no comunican ningún propósito ni significado:
