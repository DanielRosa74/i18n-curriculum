...
Оголосіть змінну `foodInputs` та призначте їй значення виклику `document.querySelectorAll()` з селектором `#food input[type='number']`. Це поверне будь-які `number` вхідні дані, які знаходяться в елементі `#food`.

# --hints--

Ви повинні оголосити змінну `foodInputs`.
...
Ви повинні викликати `document.querySelectorAll()` з `#food input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Ви повинні призначити результат виклику `document.querySelectorAll()` змінній `foodInputs`.
