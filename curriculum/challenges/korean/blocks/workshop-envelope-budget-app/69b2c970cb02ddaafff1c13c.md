...
루프가 완료된 후에 `incomeInput`과 `rentInput` 필드를 지우세요. `incomeInput`과 `rentInput`의 `value` 속성을 빈 문자열로 설정하세요.

# --hints--

`clearForm` 함수는 `incomeInput` 요소의 `value` 속성에 접근해야 합니다.

```js
assert.match(clearForm.toString(), /incomeInput\.value/);
```

`clearForm` 함수는 `incomeInput` 요소의 `value` 속성을 빈 문자열로 설정해야 합니다.

```js
assert.match(clearForm.toString(), /incomeInput\.value\s*=\s*('|"|`)\1/);
```

`for` 루프가 끝난 후에 `incomeInput`을 수정해야 합니다.
...
`clearForm` 함수는 `rentInput` 요소의 `value` 속성에 접근해야 합니다.

```js
assert.match(clearForm.toString(), /rentInput\.value/);
```

`clearForm` 함수는 `rentInput` 요소의 `value` 속성을 빈 문자열로 설정해야 합니다.

```js
assert.match(clearForm.toString(), /rentInput\.value\s*=\s*('|"|`)\1/);
```

`for` 루프가 끝난 후에 `rentInput`을 수정해야 합니다.
