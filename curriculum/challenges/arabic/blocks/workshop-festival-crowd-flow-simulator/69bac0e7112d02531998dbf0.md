...
يجب أن تسجل دالة `handleGateAtTick` أولاً الرسالة `"Overflow of " + result.overflow + " attendees. Rerouting..."` عندما يوجد تجاوز السعة (`result.overflow > 0`).
...
يجب أن تعيد دالة `handleGateAtTick` توجيه تجاوز السعة باستدعاء `rerouteOverflow()` مع `gates` و `gate` و `tickIndex` و `result.overflow` عندما يوجد تجاوز السعة (`result.overflow > 0`).
