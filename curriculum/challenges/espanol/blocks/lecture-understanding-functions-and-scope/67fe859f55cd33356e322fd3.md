...
Las funciones son fragmentos reutilizables de código que se ejecutan cuando las llamas. Python proporciona funciones integradas, incluyendo `print()`, que usaste en lecciones anteriores.
...
`None` es un valor especial que representa la ausencia de un valor. Es el único valor del tipo de dato `NoneType`. `None` es inmutable y falso, lo que significa que no puede cambiarse y se evalúa como `False` en un contexto booleano.
