...
你的收入 `input` 元素需要一些额外的属性，以确保用户输入有效的值。

为你创建的 `#income` 元素添加以下属性：

- 将 `type` 设置为 `number`（只允许数字输入）
- 将 `min` 设置为 `0`（只允许正数）
- 将 `placeholder` 设置为 `e.g. 2000`
- 最后，将该 `input` 元素标记为 `required`。
...
你的 `input` 元素应有一个 `placeholder` 属性，设置为 `e.g. 2000`。
