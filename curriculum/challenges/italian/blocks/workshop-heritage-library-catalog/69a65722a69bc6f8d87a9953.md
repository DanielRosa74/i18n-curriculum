...
Dovresti dichiarare una variabile `const` chiamata `header` con valore `"Title,Author,Year,Location"`.
...
Dovresti dichiarare una variabile `const` chiamata `rows` impostata su un array vuoto.
