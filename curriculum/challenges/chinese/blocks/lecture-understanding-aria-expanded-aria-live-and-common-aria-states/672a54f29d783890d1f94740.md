...
另一个常见的 ARIA 状态是 `aria-disabled`。此状态用于向使用辅助技术（例如屏幕阅读器）的人指示某个元素已被禁用。需要注意的是，`aria-disabled` 并不会真正禁用该元素。由你，开发者，负责使其看起来并表现得像一个禁用的元素。此属性也常用于本地的 HTML 元素，替代 `disabled` 属性。你选择哪一个取决于按钮所使用的上下文。
