...
Jetzt müssen Sie den passenden Status bestimmen, basierend darauf, ob der Benutzer über dem Budget oder unter dem Budget liegt.

Beginnen Sie damit, eine `if`-Anweisung zu erstellen, die überprüft, ob `netRemaining` kleiner als `0` ist.

# --hints--

Ihre `calculateBudget`-Funktion sollte eine `if`-Anweisung enthalten.

```js
assert.match(calculateBudget.toString(), /if\s*\(/g);
```

Ihre `if`-Anweisung sollte prüfen, ob `netRemaining` kleiner als `0` ist.

```js
assert.match(calculateBudget.toString(), /if\s*\(\s*netRemaining\s*<\s*0\s*\)/g);
```

Der Körper Ihrer `if`-Anweisung sollte vorerst leer sein.
