...
Ora è il momento di mettere tutto insieme. Dichiara una funzione vuota `calculateBudget`, che prende un parametro chiamato `e`. Questa funzione sarà un altro listener di eventi, quindi il primo argomento passato sarà l'evento del browser – `e` è un nome comune per questo parametro.

# --hints--

Dovresti dichiarare una variabile `calculateBudget`.

```js
assert.isDefined(calculateBudget);
```

La tua variabile `calculateBudget` dovrebbe essere una funzione.

```js
assert.isFunction(calculateBudget);
```

La tua funzione `calculateBudget` dovrebbe prendere un argomento `e`.
