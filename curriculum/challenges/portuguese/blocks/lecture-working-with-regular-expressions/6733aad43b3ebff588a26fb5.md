...
Essa expressão regular específica vai corresponder ao texto `freeCodeCamp`, com os `C`s maiúsculos, em qualquer lugar dentro de uma string.

Se o seu padrão vier de uma variável em vez de ser escrito diretamente no literal da regex, você pode criar o mesmo tipo de expressão regular com o construtor `RegExp`:
...
Mas como você pode realmente usar uma expressão regular?

## Testando uma string com `test()`
...
## Obtendo detalhes da correspondência com `match()`
