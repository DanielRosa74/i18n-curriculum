...
为 `.bb3` 填写一个 `repeating-linear-gradient`。使用 `90deg` 作为方向，前两个颜色使用你的 `building-color3`，第三个颜色在 `15%` 处使用 `window-color3`。当你没有为颜色指定距离时，它会使用合理的默认值。在这种情况下，前两个颜色默认分别是 `0%` 和 `7.5%`，因为它从 `0%` 开始，而 `7.5%` 是 `15%` 的一半。
