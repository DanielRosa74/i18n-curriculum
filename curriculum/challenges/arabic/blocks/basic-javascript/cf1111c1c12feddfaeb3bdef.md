...
تُستخدم جمل `if` لاتخاذ قرارات في الكود. الكلمة المفتاحية `if` تخبر JavaScript بتنفيذ الكود داخل الأقواس المتعرجة تحت شروط معينة، تُحدد في الأقواس العادية. تُعرف هذه الشروط باسم شروط `Boolean` ولا يمكن أن تكون إلا `true` أو `false`.
