...
運勢はランダムでなければならないので、ランダムな運勢を選ぶためのヘルパー関数が必要です。`singleCardSelected` メソッドの中に `chosenCard` という名前の変数を宣言してください。`getRandomItem` 関数を使ってランダムなカードを取得し、引数として `this.cards` を渡してください。
