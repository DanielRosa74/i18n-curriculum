...
Ukizingatia sintaksia hiyo hiyo, fanya hoja ya utafutaji kwa ingizo zako za `number` katika kipengele cha `#utilities` na weka thamani yao katika `utilitiesInputs`.

# --hints--

Unapaswa kutangaza kigezo cha `utilitiesInputs`.
...
Unapaswa kufanya wito wa `document.querySelectorAll()` kwa kutumia `#utilities input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Unapaswa kuweka thamani ya matokeo ya wito wako wa `document.querySelectorAll()` katika `utilitiesInputs`.
