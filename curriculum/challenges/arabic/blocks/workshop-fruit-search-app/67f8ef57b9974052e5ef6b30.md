...
داخل العنصر `#results`، استخدم معامل ثلاثي للتحقق مما إذا كان طول مصفوفة `results` أكبر من صفر. إذا كان كذلك، استعرض العناصر واعرض كل واحد داخل عنصر `p` مع صنف `result-item`. تأكد من تضمين خاصية `key` مع كل فقرة أيضًا.
