...
在本实验中，你已经获得了所有的 HTML 和 CSS。你将使用 TypeScript 来完成摩托车商店页面的功能。
...
2. 你应该创建一个名为 `interface` 的 `Motorcycle`，它具有以下属性：一个类型为 `id` 的 `string`，一个类型为 `name` 的 `string`，一个类型为 `manufacturer` 的 `string`，一个类型为 `category` 的 `Category`，一个类型为 `price` 的 `number`，一个类型为 `image_url` 的 `string`，一个类型为 `Date` 的 `created_at` 属性，一个类型为 `description` 的 `string`，以及一个类型为 `number` 的 year。
...
5. 你应该创建一个名为 `renderMotorcycleCard` 的函数，它接受一个单一的 `Motorcycle` 对象作为参数，并返回一个包含摩托车卡片格式化 HTML 字串的字符串。
...
6. 你应该有一个名为 `MotorcycleGalleryApp` 的类。
   ...
   2. 该类应有一个名为 `renderMotorcycles` 的函数，在 id 为 `motorcycle-grid` 的元素内渲染 `allMotorcycles` 变量对应的摩托车卡片组件。
