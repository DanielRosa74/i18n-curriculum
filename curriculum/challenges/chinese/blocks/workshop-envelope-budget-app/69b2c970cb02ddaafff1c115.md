...
你的下一步是编写一个函数，用来获取用户条目的总金额。

声明一个名为 `getTotalFromInputs` 的函数，并给它一个名为 `list` 的参数。

# --hints--

你应该声明一个 `getTotalFromInputs` 变量。

```js
assert.isDefined(getTotalFromInputs);
```

你的 `getTotalFromInputs` 变量应该是一个函数。

```js
assert.isFunction(getTotalFromInputs);
```

你的 `getTotalFromInputs` 函数应该接受一个名为 `list` 的参数。
