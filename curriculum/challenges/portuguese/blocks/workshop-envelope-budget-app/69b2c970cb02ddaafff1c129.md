...
Sua função `getTotalFromInputs` vai definir a flag global de erro `isError` como `true` se uma entrada inválida for detectada.

Adicione uma declaração `if` na sua função `calculateBudget` que verifique a veracidade da sua flag global de erro e, se for verdadeira, use `return` para encerrar a execução da função.

# --hints--

Sua função `calculateBudget` deve ter uma declaração `if` que verifique a veracidade da variável `isError`.
...
Sua declaração `if` deve ser colocada após a última chamada da função `getTotalFromInputs`.
