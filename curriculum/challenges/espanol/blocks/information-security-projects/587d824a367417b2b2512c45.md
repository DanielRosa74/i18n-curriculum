...
Puedes enviar una solicitud DELETE a `/api/replies/{board}` y pasar el `thread_id`, `reply_id` y `delete_password`. La respuesta será la cadena `incorrect password` o `success`. En caso de éxito, el texto del `reply_id` cambiará a `[deleted]`.
...
Puedes enviar una solicitud PUT a `/api/replies/{board}` y pasar el `thread_id` y `reply_id`. La respuesta será la cadena `reported`. El valor `reported` del `reply_id` cambiará a `true`.
