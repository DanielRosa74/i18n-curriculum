...
- استعرض كل بوابة في مصفوفة `gates` واستدعِ الدالة `handleGateAtTick()` مع هذه المعلمات:
  ...
  - `gate`: البوابة الحالية التي تتم معالجتها.
  - `tickIndex`: موضع التوقيت الحالي في مصفوفة `queue`.
...
يجب أن تستعرض كل بوابة في مصفوفة `gates` بعد التسجيل في وحدة التحكم، وتستدعي الدالة `handleGateAtTick()` مع `gates`، و`gate`، و`tickIndex`، و`throughputSummary`.
...
يجب أن تزيد قيمة `tickIndex` بواحد بعد استعراضك لمصفوفة `gates`.
