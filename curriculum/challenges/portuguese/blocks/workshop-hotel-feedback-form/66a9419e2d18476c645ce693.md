...
Abaixo do seu elemento `label`, adicione uma `input` com o atributo `type` definido como `"number"` e um `id` igual a `"age"`.
...
Sua `input` do tipo número deve ter um `id` igual a `"age"`.

```js
assert.isNotNull(document.querySelector('label + input#age'));
```

Sua `input` do tipo número deve ter um atributo `name` definido como `"age"`.
