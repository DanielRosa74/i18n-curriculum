...
`match`/`case` 구문을 `len(results)` 대신 `results`에 맞게 수정하세요. 그런 다음, 각 `case`를 적절한 요소 수를 가진 리스트를 사용하도록 수정하세요. 리스트에 요소가 하나일 경우 `x`를, 요소가 두 개일 경우 `x1`와 `x2`를 사용하세요.
...
`result_list` 안에 있는 f-문자열을 `result[0]` 대신 `x`를 사용하도록 수정하세요.
