...
Mas os seletores de atributo podem ser mais específicos. Suponha que você queira direcionar apenas os links que têm um atributo `title`. O atributo `title` frequentemente fornece informações adicionais sobre o link, e você pode querer estilizar esses links de forma diferente para indicar que eles têm informações extras disponíveis. Veja como fazer isso:
