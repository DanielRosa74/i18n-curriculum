...
`calculateBudget` 함수에 `e` 매개변수의 `preventDefault()` 메서드를 호출하는 한 줄을 추가하세요. 그런 다음 전역 오류 플래그 `isError`를 `false`로 재설정하세요.

# --hints--

`calculateBudget` 함수는 `e.preventDefault()`를 호출해야 합니다.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

`calculateBudget` 함수는 전역 오류 플래그 `isError`를 `false`로 재설정해야 합니다.
