...
按照相同的模式，为 `#utilities` 和 `#entertainment` 元素中的 `number` 输入声明 `utilities` 和 `entertainment` 变量。将它们赋值为相应的 `getTotalFromInputs` 调用结果。

# --hints--

你的 `calculateBudget` 函数应该有一个 `utilities` 变量。
...
你应该使用 `utilitiesInputs` 作为参数调用 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*utilitiesInputs\s*\)/);
```

你应该将 `getTotalFromInputs` 的结果赋值给 `utilities`。
...
你的 `calculateBudget` 函数应该有一个 `entertainment` 变量。
...
你应该使用 `entertainmentInputs` 作为参数调用 `getTotalFromInputs`。

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*entertainmentInputs\s*\)/);
```

你应该将 `getTotalFromInputs` 的结果赋值给 `entertainment`。
