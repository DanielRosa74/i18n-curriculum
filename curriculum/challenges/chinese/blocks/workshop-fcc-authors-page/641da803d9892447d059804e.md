...
现在创建一个图像标签，并为它添加一个 `class` 属性，值为 `"user-img"`。使用字串插值将 `src` 属性设置为 `image` 属性。将 `alt` 属性设置为 `author` 后跟文本 `"avatar"`。确保 `author` 变量和单词 `"avatar"` 之间有一个空格，例如，`"Quincy Larson avatar"`。
