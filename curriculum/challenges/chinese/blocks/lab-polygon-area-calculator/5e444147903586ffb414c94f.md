...
2. 当创建一个 `Rectangle` 对象时，应使用 `width` 和 `height` 属性进行初始化。该类还应包含以下方法：
    ...
    - `get_diagonal`：返回对角线长度 \\( \sqrt{\text{width}^2 + \text{height}^2} \\)。
    - `get_picture`：返回一个用 `*` 组成的字符串来表现形状。行数应等于高度，每行的 `*` 数量应等于宽度。每行末尾应有换行符（`\n`）。如果宽度或高度大于 `50`，应返回字符串：`Too big for picture.`。
    - `get_amount_inside`：接受另一个形状（正方形或矩形）作为参数。返回传入形状在当前形状内能完整容纳的次数（不旋转）。例如，宽度为 `4`，高度为 `8` 的矩形可以容纳两个边长为 `4` 的正方形。
