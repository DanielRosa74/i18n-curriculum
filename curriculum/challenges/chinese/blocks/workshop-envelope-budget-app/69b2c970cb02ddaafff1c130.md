...
现在你需要处理用户没有超出预算的情况（当 `netRemaining` 大于或等于 `0` 时）。

为你的 `if` 语句添加一个 `else` 块，将 `statusText` 赋值为 `"$X Remaining"`，其中 `X` 是 `netRemaining` 的值。同时，将 `statusClass` 赋值为 `"surplus"`。
