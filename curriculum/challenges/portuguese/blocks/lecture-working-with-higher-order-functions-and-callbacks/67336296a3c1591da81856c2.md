...
Muitos métodos embutidos para arrays em JavaScript, como `map()`, `filter()` e `reduce()`, são funções de ordem superior. Esses métodos recebem uma função como argumento e a aplicam aos elementos do array de várias maneiras. Você aprenderá mais sobre esses métodos em lições futuras.
