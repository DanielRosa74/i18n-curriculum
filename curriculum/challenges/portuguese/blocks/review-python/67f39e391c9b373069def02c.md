...
- **Definição**: Um shell interativo é um programa que permite digitar comandos um de cada vez e ver os resultados. Abra um app de terminal e digite `python` e pressione <kbd>Enter</kbd>. Isso iniciará um shell interativo do Python. O símbolo `>>>` significa que o Python está esperando você digitar um comando. Tente imprimir `"Hello, world!"` no terminal:
...
- **Tipos imutáveis**: Esses tipos não podem ser alterados depois de declarados, embora você possa apontar suas variáveis para algo novo, o que é chamado de reatribuição. Eles incluem inteiro, float, booleano, string, tupla, range e `None`.
- **Tipos mutáveis**: Esses tipos podem ser alterados depois de declarados. Você pode adicionar, remover ou atualizar seus itens. Eles incluem tipos de coleção como lista, conjunto e dicionário.
...
- **Definição**: Strings em Python são imutáveis, o que significa que você não pode alterá-las depois de criadas. Você pode usar aspas simples ou aspas duplas. É recomendado escolher uma regra e segui-la:
...
Python tem regras adicionais de escopo que não são abordadas aqui.
