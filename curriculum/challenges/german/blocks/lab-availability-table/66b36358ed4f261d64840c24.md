...
1. Zellen in der ersten Zeile sollten Tabellenkopfzeilen sein, die die Wochentage enthalten.
1. Zellen in der ersten Spalte sollten Tabellenkopfzeilen mit der `class` `time` sein und Zeitwerte als Text enthalten.
1. Alle Datenzellen sollten die `class` `available-#` haben, wobei `#` eine Zahl von `0` bis `5` ist, die die Anzahl der zu dieser Zeit verfügbaren Personen angibt.
1. Ihre Tabelle sollte ein `caption`-Element mit dem Text `Weekly Meeting Availability Schedule` haben.
1. Alle Tabellenkopfzellen in der ersten Zeile (die Wochentage) sollten ein `scope`-Attribut mit dem Wert `col` haben.
1. Alle Tabellenkopfzellen in der ersten Spalte (die Zeit-Zellen) sollten ein `scope`-Attribut mit dem Wert `row` haben.
1. Alle Datenzellen mit einer Verfügbarkeitsklasse sollten ein `aria-label`-Attribut haben, das den Verfügbarkeitsgrad beschreibt (z. B. `0 available` für die `available-0`-Zellen, `1 available` für die `available-1`-Zellen usw. und `5+ available` für die `available-5`-Zellen).
1. In Ihrem `:root`-Selektor sollten Sie sechs CSS-Variablen namens `--color#` definieren, wobei `#` eine Zahl von `0` bis `5` ist, und ihnen jeweils einen Farbwert zuweisen. Verwenden Sie diese Variablen, um die Hintergrundfarbe der entsprechenden `.available-#`-Elemente festzulegen.
1. In Ihrem `:root`-Selektor sollten Sie CSS-Variablen namens `--solid-border` und `--dashed-border` definieren. Verwenden Sie diese Variablen, um die unteren Rahmen Ihrer Datenzellen zu gestalten, wobei Sie je nach Zeile zwischen durchgezogenen und gestrichelten Rahmen wechseln. Geben Sie den Zeilen entweder die Klasse `sharp` oder `half`, um sie zu stylen.
1. Sie sollten ein `div`-Element mit der `id` `legend` haben. Es sollte ein `span`-Element mit dem Text `Availability` und ein `div`-Element mit der `id` `legend-gradient` enthalten.
1. Sie sollten dem `#legend-gradient`-Element einen linearen Farbverlauf geben, der mit harten Linien zwischen allen Farben von `--color0` bis `--color5` übergeht.
...
Ihre Tabelle sollte ein `caption`-Element haben.
...
Ihr `caption`-Element sollte den Text `Weekly Meeting Availability Schedule` enthalten.
...
Alle Tabellenkopfzellen in der ersten Zeile sollten ein `scope`-Attribut mit dem Wert `col` haben.
...
Alle Tabellenkopfzellen in der ersten Spalte sollten ein `scope`-Attribut mit dem Wert `row` haben.
...
Alle Datenzellen mit einer Verfügbarkeitsklasse sollten ein `aria-label`-Attribut haben.
...
Datenzellen mit der Klasse `available-0` sollten, falls vorhanden, ein `aria-label` mit dem Wert `0 available` haben.
...
Datenzellen mit der Klasse `available-1` sollten, falls vorhanden, ein `aria-label` mit dem Wert `1 available` haben.
...
Datenzellen mit der Klasse `available-2` sollten, falls vorhanden, ein `aria-label` mit dem Wert `2 available` haben.
...
Datenzellen mit der Klasse `available-3` sollten, falls vorhanden, ein `aria-label` mit dem Wert `3 available` haben.
...
Datenzellen mit der Klasse `available-4` sollten, falls vorhanden, ein `aria-label` mit dem Wert `4 available` haben.
...
Datenzellen mit der Klasse `available-5` sollten, falls vorhanden, ein `aria-label` mit dem Wert `5+ available` haben.
