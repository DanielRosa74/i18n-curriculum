...
Se o usuário digitar o nome e pressionar "OK", a variável `userName` armazenará o valor inserido. Se o usuário pressionar "Cancelar", a variável `userName` será definida como `null`. `null` significa que o usuário não forneceu nenhuma entrada. O parágrafo de saída então exibirá uma mensagem de saudação usando o nome fornecido ou `null` se o usuário cancelou. Você aprenderá técnicas para evitar exibir `null` quando o usuário cancelar o prompt em lições futuras.
