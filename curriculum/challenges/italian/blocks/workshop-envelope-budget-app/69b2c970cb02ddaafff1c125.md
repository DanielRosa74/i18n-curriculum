...
Ora che hai le tue liste di elementi, puoi passarle alla tua funzione `getTotalFromInputs` per estrarre il totale.

Dichiara una variabile `rent` e assegnale il risultato della chiamata a `getTotalFromInputs` con `[rentInput]` come argomento.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere una variabile `rent`.
...
Dovresti chiamare `getTotalFromInputs` con `[rentInput]` come argomento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

Dovresti assegnare il risultato di `getTotalFromInputs` a `rent`.
