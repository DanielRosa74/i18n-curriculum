...
الصورة التي أضفتها ليست في الوسط أفقيًا مثل عنوان `Coffee` أعلاها. عناصر `img` هي عناصر مضمَّنة، لذا تجلس ضمن تدفق النص.

لجعل الصورة تتصرف كعنصر على مستوى الكتلة مثل العنوان، أنشئ محدِّد نوع `img`. اضبط خاصية `display` على `block`، واستخدم خاصيتي `margin-left` و`margin-right` لتمركزها أفقيًا.
