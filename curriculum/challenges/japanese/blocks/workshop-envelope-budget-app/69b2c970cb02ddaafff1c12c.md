...
ユーザーが予算オーバーか、余剰金があるかを知る必要があります。支出が収入を超えると予算オーバー（赤字）です。収入が支出を超えると余剰金があります。ちょうどゼロの場合はバランスが取れていると考えられます。

`statusText` と `statusClass` の2つの変数を宣言してください。両方とも空の文字列 `""` で初期化してください。

# --hints--

`calculateBudget` 関数には `statusText` 変数が必要です。
...
`calculateBudget` 関数には `statusClass` 変数が必要です。
...
`statusText` と `statusClass` は空の文字列で初期化してください。
