...
Verwenden Sie den Additionszuweisungsoperator, um `currVal` zu Ihrem `total` hinzuzufügen. Sie müssen den `Number`-Konstruktor verwenden, um `currVal` in eine Zahl umzuwandeln.
...
Nach Ihrer `if`-Anweisung sollten Sie den Additionszuweisungsoperator auf `total` anwenden.
...
Sie sollten den Wert von `Number(currVal)` zu `total` hinzufügen.
