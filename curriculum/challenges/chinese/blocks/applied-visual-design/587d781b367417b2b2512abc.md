...
<blockquote>rgba 代表：<br>  r = 红色<br>  g = 绿色<br>  b = 蓝色<br>  a = alpha/不透明度等级</blockquote>

RGB 值的范围是 0 到 255。alpha 值的范围是 1（完全不透明或纯色）到 0（完全透明或清晰）。在这种情况下，使用 `rgba()` 非常好，因为它允许你调整不透明度。这意味着你不必完全遮挡背景。
