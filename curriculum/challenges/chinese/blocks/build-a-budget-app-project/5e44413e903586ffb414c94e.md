...
图表应显示传入函数的每个类别的支出百分比。支出百分比应仅根据取款计算，而不包括存款，并且应表示每个类别的支出金额占所有类别总支出的百分比。图表左侧应标有 0 到 100 的标签。条形图中的“条”应由字符 'o' 组成。每个条的高度应向下取整到最接近的 10。条形图下方的水平线应比最后一个条多出两个空格。每个类别名称应垂直写在条形图下方。顶部应有标题，写着“Percentage spent by category”。
