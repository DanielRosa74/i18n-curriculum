...
`output.innerHTML` の文字列には `span` 要素が必要です。それを作成し、`statusClass` 変数を設定したクラス属性を付けてください。

`span` 要素の中に、`statusText` 変数をテキストコンテンツとして追加してください。
...
`span` の `class` 属性は `${statusClass}` に設定してください。
...
`span` のテキストコンテンツは `${statusText}` にしてください。
