...
Recuerda que los arreglos tienen índices basados en cero, lo que significa que el último índice del arreglo es `length - 1`. Nuestra condición para este ciclo es `i < arr.length`, que detiene el ciclo cuando `i` es igual a `length`. En este caso, la última iteración es `i === 4`, es decir, cuando `i` se vuelve igual a `arr.length - 1` y muestra `6` en la consola. Luego `i` aumenta a `5` y el ciclo termina porque `i < arr.length` es `false`.
