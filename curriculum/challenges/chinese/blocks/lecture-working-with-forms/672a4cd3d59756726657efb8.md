...
在这个 `form` 元素中，有一个用于用户电子邮件地址的 `label` 和 `input` 元素。当用户点击提交按钮时，他们的数据将被发送到服务器并进行处理。`type` 属性的第三个可能值是 `reset`。这是一个包含重置和提交按钮的 `form` 元素示例。
...
`value` 属性用于显示按钮文本。那么，使用 `input` 和 `button` 元素有什么区别呢？`input` 元素是空元素，这意味着它们不能有子节点，比如文本，并且只能有开始标签。另一方面，`button` 元素提供了更多灵活性，因为你可以在其中嵌套文本、图像和图标。
