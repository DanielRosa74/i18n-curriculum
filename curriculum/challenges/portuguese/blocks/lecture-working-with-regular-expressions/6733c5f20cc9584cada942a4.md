...
Agora podemos atualizar nossa referência na expressão regular para se referir a esse grupo. Uma referência nomeada começa com uma barra invertida seguida da letra `k` em JavaScript. Depois, você adiciona o nome, novamente entre sinais de menor (`<`) e maior (`>`). Vamos dar uma olhada nisso:
