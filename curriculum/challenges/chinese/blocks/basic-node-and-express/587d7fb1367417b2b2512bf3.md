...
假设你将这个函数挂载到一个路由上。当请求匹配该路由时，它会显示字符串“I'm a middleware…”，然后执行栈中的下一个函数。在本练习中，你将构建根级中间件。正如你在挑战 4 中看到的，要在根级挂载中间件函数，可以使用 `app.use(<mware-function>)` 方法。在这种情况下，该函数会对所有请求执行，但你也可以设置更具体的条件。例如，如果你希望函数只对 POST 请求执行，可以使用 `app.post(<mware-function>)`。所有 HTTP 动词（GET、DELETE、PUT 等）都有类似的方法。
