...
Devido a essas potenciais inconsistências, o ES6 (a sexta edição do JavaScript, lançada em 2015) introduziu o `Number.isNaN()`. Esse método não tenta converter o parâmetro para um número antes de testar. Ele retorna `true` apenas se o valor for exatamente `NaN`:
