...
- Use um `for` loop para iterar através de cada gate no array de entrada `gates`. Para cada gate, adicione uma propriedade ao `summary` onde:
...
Sua função `initializeThroughput` deve retornar `{}` quando chamada com `[]`, sem modificar o array de entrada original.
...
Sua função `initializeThroughput` deve retornar `{ North: 0 }` quando chamada com `[{ id: "North", capacity: 5, queue: [1, 2, 3] }]`, sem modificar o array de entrada original.
...
Sua função `initializeThroughput` deve retornar `{ North: 0, East: 0, South: 0 }` quando chamada com `[{ id: "North", capacity: 5, queue: [] }, { id: "East", capacity: 3, queue: [] }, { id: "South", capacity: 4, queue: [] }]`, sem modificar o array de entrada original.
...
Sua função `initializeThroughput` deve retornar `summary` após o loop, como a última linha da função.
