...
在 `newReading` 方法内，调用 `showElements` 并传入 `this.elements.singleCardBtn`、`this.elements.multipleCardsBtn` 和 `this.elements.headerTitle` 作为参数。

然后调用 `hideElements` 并传入 `this.elements.singleCard`、`this.elements.multipleCard`、`this.elements.fortuneContainer` 和 `this.elements.fortuneDescription` 作为参数。
