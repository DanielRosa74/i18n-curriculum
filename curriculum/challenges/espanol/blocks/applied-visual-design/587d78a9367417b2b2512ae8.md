...
La función `cubic-bezier` consta de cuatro puntos principales que se ubican en esta rejilla de 1 por 1: `p0`, `p1`, `p2` y `p3`. `p0` y `p3` están definidos para ti: son los puntos inicial y final que siempre se encuentran respectivamente en el origen (0, 0) y (1, 1). Tú defines los valores x e y de los otros dos puntos, y la posición que les des en la rejilla determina la forma de la curva que seguirá la animación. Esto se hace en CSS declarando los valores x e y de los puntos "ancla" `p1` y `p2` en la forma: `(x1, y1, x2, y2)`. Resumiendo, aquí tienes un ejemplo de una curva de Bezier en código CSS:
