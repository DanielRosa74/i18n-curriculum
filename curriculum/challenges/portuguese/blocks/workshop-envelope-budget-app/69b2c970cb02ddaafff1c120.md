...
Agora é hora de começar a juntar tudo. Declare uma função vazia `calculateBudget`, que recebe um parâmetro chamado `e`. Essa função será outro listener de evento, então o primeiro argumento passado será o evento do navegador – `e` é um nome comum para esse parâmetro.

# --hints--

Você deve declarar uma variável `calculateBudget`.

```js
assert.isDefined(calculateBudget);
```

Sua variável `calculateBudget` deve ser uma função.

```js
assert.isFunction(calculateBudget);
```

Sua função `calculateBudget` deve receber um argumento `e`.
