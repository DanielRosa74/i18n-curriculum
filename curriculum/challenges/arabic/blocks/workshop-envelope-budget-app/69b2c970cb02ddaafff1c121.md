...
أضف سطرًا إلى دالة `calculateBudget` يستدعي طريقة `preventDefault()` على المعلمة `e`. ثم أعد تعيين علم الخطأ العام `isError` إلى `false`.

# --hints--

يجب أن تستدعي دالة `calculateBudget` الدالة `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

يجب أن تعيد دالة `calculateBudget` تعيين علم الخطأ العام `isError` إلى `false`.
