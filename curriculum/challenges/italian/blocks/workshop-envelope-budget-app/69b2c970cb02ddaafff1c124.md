...
Seguendo lo stesso pattern, esegui una query per i tuoi input di tipo `number` nell'elemento `#entertainment` e assegnali a `entertainmentInputs`.

# --hints--

Dovresti dichiarare una variabile `entertainmentInputs`.
...
Dovresti chiamare `document.querySelectorAll()` con `#entertainment input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#entertainment input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Dovresti assegnare il risultato della chiamata a `document.querySelectorAll()` a `entertainmentInputs`.
