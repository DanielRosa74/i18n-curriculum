...
別の著者で `getBooksByAuthor` 関数を試してみると良いでしょう。

まず、メッセージ`"\nList of books by James Clear:\n"`をコンソールにログ出力してください。

その `console.log()` の下にもう一つ `console.log()` を追加してください。その `console.log()` の中で、`library` と `"James Clear"` を引数にして `getBooksByAuthor()` 関数を呼び出してください。
