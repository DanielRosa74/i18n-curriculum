...
その`input`の下に、テキストが`Location`で`for`属性が`"location"`に設定された`label`要素を追加してください。

その下の `label` 要素の下に、`input` のチェックボックスをもう1つ追加し、`id` と `value` の属性を `"reputation"` に、`name` の属性を `"choice"` に設定してください。

その`input`の下に、テキストが`Reputation`で`for`属性が`"reputation"`に設定された`label`要素を追加してください。
...
4つ目のチェックボックス`input`は、`id`属性が`"reputation"`に設定されている必要があります。
...
評判のチェックボックスの下に、テキストが `Reputation` の `label` 要素があるはずです。
