...
Ваша функція `handleGateAtTick` спочатку має вивести повідомлення `"Overflow of " + result.overflow + " attendees. Rerouting..."`, коли існує переповнення (`result.overflow > 0`).
...
Ваша функція `handleGateAtTick` має перенаправити переповнення, викликавши `rerouteOverflow()` з аргументами `gates`, `gate`, `tickIndex` та `result.overflow`, коли існує переповнення (`result.overflow > 0`).
