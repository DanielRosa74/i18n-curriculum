...
宣告一個 `foodInputs` 變數，並將呼叫 `document.querySelectorAll()` 並使用選擇器 `#food input[type='number']` 的結果指定給它。這會傳回位於 `#food` 元素中的所有 `number` 輸入欄位。

# --hints--

你應該宣告一個 `foodInputs` 變數。
...
你應該使用 `#food input[type='number']` 呼叫 `document.querySelectorAll()`。

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

你應該將 `document.querySelectorAll()` 呼叫的結果指定給 `foodInputs`。
