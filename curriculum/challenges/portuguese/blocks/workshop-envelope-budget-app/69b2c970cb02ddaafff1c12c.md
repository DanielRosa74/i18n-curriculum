...
Você precisa saber se o usuário está acima do orçamento ou se tem dinheiro sobrando. Quando as despesas ultrapassam a renda, você está acima do orçamento (um déficit). Quando a renda ultrapassa as despesas, você tem dinheiro sobrando (um superávit). Ter exatamente zero sobrando pode ser considerado equilibrado.

Declare duas variáveis: `statusText` e `statusClass`. Inicialize ambas como strings vazias usando `""`.

# --hints--

Sua função `calculateBudget` deve ter uma variável `statusText`.
...
Sua função `calculateBudget` deve ter uma variável `statusClass`.
...
Você deve inicializar `statusText` e `statusClass` como strings vazias.
...
Ambas as variáveis devem ser declaradas usando `let`.
