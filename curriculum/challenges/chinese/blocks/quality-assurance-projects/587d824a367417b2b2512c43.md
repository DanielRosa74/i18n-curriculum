...
你可以发送一个包含 `comment` 作为形式体数据的 <b>POST</b> 请求到 `/api/books/{_id}`，以向书籍添加评论。返回的响应将是类似于之前测试中 <b>GET</b> `/api/books/{_id}` 请求的书籍对象。如果请求中未包含 `comment`，则返回字串 `missing required field comment`。如果未找到书籍，则返回字串 `no book exists`。
...
你可以发送一个 <b>DELETE</b> 请求到 `/api/books/{_id}`，以从集合中删除一本书。如果成功，返回的响应将是字串 `delete successful`。如果未找到书籍，则返回字串 `no book exists`。
