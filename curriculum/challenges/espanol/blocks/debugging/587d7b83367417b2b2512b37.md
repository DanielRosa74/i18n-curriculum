...
Hay muchos métodos para usar con `console` para mostrar mensajes. `log`, `warn` y `clear`, por nombrar algunos. La consola de freeCodeCamp solo mostrará mensajes `log`, mientras que la consola del navegador mostrará todos los mensajes. Cuando haces cambios en tu código, este se ejecuta automáticamente y muestra los registros. La consola de freeCodeCamp se limpia cada vez que tu código se ejecuta.
