...
El ámbito de bloque es un concepto introducido con las palabras clave `let` y `const` en ES6. Un bloque es cualquier sección de código dentro de llaves, `{}`, como en sentencias `if`, ciclos `for` o ciclos `while`. El concepto de ciclos se enseñará en una lección próxima.
