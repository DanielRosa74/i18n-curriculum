...
Una de las formas más populares en el mundo es la forma de corazón, y en este desafío crearás una usando solo CSS. Pero primero, necesitas entender los pseudoelementos `::before` y `::after`. `::before` crea un pseudoelemento que es el primer hijo del elemento seleccionado; `::after` crea un pseudoelemento que es el último hijo del elemento seleccionado. En el siguiente ejemplo, se usa un pseudoelemento `::before` para agregar un rectángulo a un elemento con la clase `heart`:
