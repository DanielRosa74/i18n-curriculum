...
Después de la sentencia `if`, usa un operador ternario para comprobar la veracidad de llamar a `isSpam()` con `messageInput.value` como argumento. Si es verdadero, asigna la propiedad `textContent` del elemento `result` a `"Oh no! This looks like a spam message."`. De lo contrario, asígnala a `"This message does not seem to contain any spam."`
