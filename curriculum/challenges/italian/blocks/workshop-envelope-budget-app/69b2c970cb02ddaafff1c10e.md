...
Crea un altro elemento `label` (su una nuova riga) alla fine della tua `HTMLString`. Questa `label` dovrebbe avere il testo `Expense # Amount`, usando la sintassi dei template literal per sostituire `#` con il valore di `entryNumber`, e l'attributo `for` impostato su `X-#-amount`, dove `X` è il valore di `category` e `#` è il valore di `entryNumber`.
...
Il tuo nuovo elemento `label` dovrebbe avere un attributo `for` impostato su `${category}-${entryNumber}-amount`.
...
Il tuo nuovo elemento `label` dovrebbe avere il testo `Expense ${entryNumber} Amount`.
