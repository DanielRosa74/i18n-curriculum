...
Erstellen Sie noch innerhalb Ihres dritten `li`-Elements ein `ul`-Element für das Popup. Geben Sie ihm einen `className` von `sub-menu` und ein `aria-label` von `Apps`.
...
Ihr `ul`-Element sollte einen `className` von `sub-menu` haben.

```js
assert.equal(document.querySelector('li ul')?.getAttribute('class'), 'sub-menu');
```

Ihr `ul`-Element sollte ein `aria-label` von `Apps` haben.
