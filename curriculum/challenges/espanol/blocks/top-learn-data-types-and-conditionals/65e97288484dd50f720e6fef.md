...
Las sentencias condicionales más complejas pueden incluir múltiples condiciones. Por eso se usan operadores lógicos para combinar varias condiciones. Los operadores lógicos son `&&`, `||` y `!`, que se usan para representar `and`, `or` y `not` respectivamente.
...
El operador lógico `&&` se usa para combinar dos condiciones booleanas. Devuelve `true` solo si ambas condiciones son `true`. De lo contrario, devuelve `false`:
