...
Вам потрібно знати, чи користувач перевищує бюджет або має залишок грошей. Коли витрати перевищують дохід, ви перевищуєте бюджет (дефіцит). Коли дохід перевищує витрати, у вас є залишок грошей (профіцит). Якщо залишок дорівнює нулю, це можна вважати збалансованим.

Оголосіть дві змінні: `statusText` і `statusClass`. Ініціалізуйте обидві як порожні рядки, використовуючи `""`.

# --hints--

У вашій функції `calculateBudget` має бути змінна `statusText`.
...
У вашій функції `calculateBudget` має бути змінна `statusClass`.
...
Ви повинні ініціалізувати `statusText` і `statusClass` як порожні рядки.
...
Обидві змінні слід оголосити за допомогою `let`.
