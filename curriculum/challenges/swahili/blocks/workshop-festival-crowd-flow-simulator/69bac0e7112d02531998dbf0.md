...
Kitendakazi chako cha `handleGateAtTick` kinapaswa kwanza kurekodi ujumbe `"Overflow of " + result.overflow + " attendees. Rerouting..."` wakati kuzidi kunapoonekana (`result.overflow > 0`).
...
Kitendakazi chako cha `handleGateAtTick` kinapaswa kupitisha kuzidi kwa wito wa `rerouteOverflow()` kwa kutumia `gates`, `gate`, `tickIndex`, na `result.overflow` wakati kuzidi kunapoonekana (`result.overflow > 0`).
