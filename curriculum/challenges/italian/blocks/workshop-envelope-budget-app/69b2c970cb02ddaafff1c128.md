...
Devi anche ottenere il valore del tuo `#income` input. Lo hai già interrogato all'inizio del tuo codice e assegnato alla variabile `incomeInput`. Tuttavia, hai usato `getElementById`, che restituisce un `Element`, non un `NodeList`.

Un `NodeList` è un oggetto simile a un array, il che significa che puoi iterare su di esso e condivide alcuni metodi comuni con un array. Per la tua funzione `getTotalFromInputs`, un array funzionerà altrettanto bene come argomento quanto un `NodeList`.

Dichiara una variabile `income` e assegnale il risultato della chiamata a `getTotalFromInputs` – passa un array contenente il tuo `incomeInput` come argomento.

# --hints--

La tua funzione `calculateBudget` dovrebbe avere una variabile `income`.
...
Dovresti chiamare `getTotalFromInputs` con `[incomeInput]` come argomento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*incomeInput\s*\]\s*\)/);
```

Dovresti assegnare il risultato di `getTotalFromInputs` a `income`.
