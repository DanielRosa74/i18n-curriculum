...
由於你的 `fetch` 呼叫並未定義在非同步函式或模組的主體中，因此在修正之前你無法使用 `await`。
...
你的 `initialFetch` 變數應該是一個非同步函式（程式）。
...
你的 `initialFetch` 函式（程式）不應該帶有任何參數。
...
你的 `initialFetch` 函式（程式）應該使用箭頭函式語法。
