...
`netRemaining` 변수를 선언하고, `income`에서 `expenses`를 뺀 값을 할당하세요.

# --hints--

`calculateBudget` 함수 안에 `netRemaining` 변수가 있어야 합니다.
...
`income - expenses` 값을 구하세요.

```js
assert.match(calculateBudget.toString(), /income\s*-\s*expenses/);
```

`income - expenses` 값을 `netRemaining`에 할당하세요.
