...
Ahora declara una variable `food` y asígnale el valor de llamar a `getTotalFromInputs` con tus `foodInputs`.

# --hints--

Tu función `calculateBudget` debe tener una variable `food`.
...
Debes llamar a `getTotalFromInputs` con `foodInputs` como argumento.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*foodInputs\s*\)/);
```

Debes asignar el resultado de `getTotalFromInputs` a `food`.
