...
为你的 `label` 元素添加一个 `for` 属性，值为 `X-#-name`，其中 `X` 是 `category` 变量的 `value`，`#` 是 `entryNumber` 的值。记住，超文本标记语言的属性应该用双引号括起来。

# --hints--

你的 `label` 元素应该有一个 `for` 属性，设置为 `${category}-${entryNumber}-name`。

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

你的 `label` 元素应该包含文本 `"Expense ${entryNumber} Name"`。
