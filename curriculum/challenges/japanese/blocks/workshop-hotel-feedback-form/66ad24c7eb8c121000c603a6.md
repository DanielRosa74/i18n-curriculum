...
チェックボックスの`input`をデフォルトでチェック済みにするには、`checked`属性を追加してください。
...
デフォルトでチェック済みにするために、`id`が`"reputation"`のチェックボックス`input`に`checked`属性を追加してください。

# --hints--

`id`が`"reputation"`のチェックボックス`input`には`checked`属性が必要です。
