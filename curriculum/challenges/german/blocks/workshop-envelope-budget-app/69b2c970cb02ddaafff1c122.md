...
Deklarieren Sie eine Variable `foodInputs` und weisen Sie ihr den Wert des Aufrufs von `document.querySelectorAll()` mit dem Selektor `#food input[type='number']` zu. Dies gibt alle `number`-Eingaben zurück, die sich im Element `#food` befinden.

# --hints--

Sie sollten eine Variable `foodInputs` deklarieren.
...
Sie sollten `document.querySelectorAll()` mit `#food input[type='number']` aufrufen.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#food input\[\s*type=(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Sie sollten das Ergebnis Ihres Aufrufs von `document.querySelectorAll()` der Variable `foodInputs` zuweisen.
