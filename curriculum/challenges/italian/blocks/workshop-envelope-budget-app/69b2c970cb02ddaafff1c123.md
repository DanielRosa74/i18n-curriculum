...
Usando quella stessa sintassi, esegui una query sugli input `number` nell'elemento `#utilities` e assegna il risultato a `utilitiesInputs`.

# --hints--

Dovresti dichiarare una variabile `utilitiesInputs`.
...
Dovresti chiamare `document.querySelectorAll()` con `#utilities input[type='number']`.

```js
assert.match(calculateBudget.toString(), /document\.querySelectorAll\(\s*('|"|`)#utilities input\[\s*type\s*=\s*(('|")?)number\2\s*\]\s*\1\s*\)/);
```

Dovresti assegnare il risultato della chiamata a `document.querySelectorAll()` a `utilitiesInputs`.
