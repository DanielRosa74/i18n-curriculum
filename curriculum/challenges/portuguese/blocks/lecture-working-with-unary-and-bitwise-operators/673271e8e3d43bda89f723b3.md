...
O operador bitwise NOT é um operador unário menos comumente usado. Representado por um til, `~`, ele inverte a representação binária de um número. Os computadores armazenam números em formato binário (1s e 0s). O operador `~` inverte cada bit, ou seja, transforma todos os 1s em 0s e todos os 0s em 1s. Você aprenderá mais sobre binário e bits em uma lição futura.
