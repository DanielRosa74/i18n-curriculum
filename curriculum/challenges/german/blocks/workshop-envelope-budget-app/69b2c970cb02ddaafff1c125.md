...
Da Sie jetzt Ihre Listen von Elementen haben, können Sie diese an Ihre `getTotalFromInputs`-Funktion übergeben, um die Summe zu ermitteln.

Deklarieren Sie eine `rent`-Variable und weisen Sie ihr das Ergebnis des Aufrufs von `getTotalFromInputs` mit `[rentInput]` als Argument zu.

# --hints--

Ihre `calculateBudget`-Funktion sollte eine `rent`-Variable enthalten.
...
Sie sollten `getTotalFromInputs` mit `[rentInput]` als Argument aufrufen.

```js
assert.match(calculateBudget.toString(), /getTotalFromInputs\s*\(\s*\[\s*rentInput\s*\]\s*\)/);
```

Sie sollten das Ergebnis von `getTotalFromInputs` der Variable `rent` zuweisen.
