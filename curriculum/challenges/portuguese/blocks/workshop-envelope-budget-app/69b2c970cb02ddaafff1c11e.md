...
Lembre-se de que `return` termina a execução de uma função. Depois do seu bloco `if`, você precisa lidar com a lógica para quando a entrada for válida. Como sua instrução if retorna um valor, você não precisa de uma declaração `else`.

Use o operador de atribuição de adição para adicionar `currVal` ao seu `total`. Você precisará usar o construtor `Number` para converter `currVal` em um número.
...
Depois da sua instrução `if`, você deve usar o operador de atribuição de adição em `total`.
...
Você deve adicionar o valor de `Number(currVal)` a `total`.
