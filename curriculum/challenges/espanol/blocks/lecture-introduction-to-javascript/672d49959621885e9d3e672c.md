...
Aunque una variable declarada con `let` puede ser reasignada, no puede ser redeclarada. Si intentas declarar la misma variable nuevamente usando `let`, obtendrás un error:
...
Lo mismo aplica para `const`: una variable declarada con `const` tampoco puede ser redeclarada.
...
También puedes usar la palabra clave `var`, pero ya no se recomienda tanto. La palabra clave `var` es algo así como `let`, excepto que tiene un ámbito más amplio, lo que es más probable que cause problemas en tu programa.
