...
`#results` 要素の中で、`results` 配列の長さがゼロより大きいかどうかを三項演算子で確認してください。もしそうなら、配列の各アイテムをマップして、それぞれを `result-item` クラスを持つ `p` 要素として表示してください。各段落には必ず `key` 属性も含めてください。

`results` が空の場合は、`No results found` というメッセージを含む `p` 要素をレンダリングしてください。

# --hints--

`#results` コンテナには、クエリに基づいた一致する果物のリストが含まれているはずです。各結果はクラスが `result-item` の `p` 要素で、果物の名前を表示してください。
