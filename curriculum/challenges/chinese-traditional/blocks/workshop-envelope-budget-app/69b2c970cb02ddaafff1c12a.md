...
現在是時候開始準備你的計算了。先宣告一個 `expenses` 變數，並將 `rent`、`food`、`utilities` 和 `entertainment` 的總和指定給它（注意測試時順序很重要）。務必在你的 if 述語之後執行這個動作。

# --hints--

這個函式應該要有一個 `expenses` 變數。
