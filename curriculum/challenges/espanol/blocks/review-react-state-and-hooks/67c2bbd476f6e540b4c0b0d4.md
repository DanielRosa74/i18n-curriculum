...
- **Sistema de eventos sintéticos**: Esta es la forma en que React maneja los eventos. Funciona como un envoltorio alrededor de los eventos nativos como los eventos `click`, `keydown` y `submit`. Los manejadores de eventos en React usan la convención de nomenclatura camel case. (Ej. `onClick`, `onSubmit`, etc)
...
- **Hook `useState()`**: El hook `useState` es una función que te permite declarar variables de estado en componentes funcionales. Aquí está la sintaxis básica:
