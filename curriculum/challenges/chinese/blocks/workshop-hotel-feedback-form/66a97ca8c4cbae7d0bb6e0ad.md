...
你的 `value` 为 `"satisfactory"` 的 `option` 应该有文本 `Satisfactory`。
...
你的 `value` 为 `"excellent"` 的 `option` 元素应该有 `selected` 属性。
