...
Adicione uma linha à sua função `calculateBudget` que faça a chamada do método `preventDefault()` no parâmetro `e`. Depois, redefina a flag global de erro `isError` para `false`.

# --hints--

Sua função `calculateBudget` deve fazer a chamada `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

Sua função `calculateBudget` deve redefinir a flag global de erro `isError` para `false`.
