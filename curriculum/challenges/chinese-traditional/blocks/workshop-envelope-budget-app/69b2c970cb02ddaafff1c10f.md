...
最後，在你的第二個 `label` 之後的新一行，創建另一個 `input` 元素。為這個元素設定 `type` 屬性為 `number`，`min` 屬性為 `0`（確保無法輸入負數金額），`placeholder` 屬性為 `Amount`，以及一個與你第二個 `label` 元素的 `for` 屬性相符的 `id` 屬性。
...
你的 `input` 元素應該有一個 `placeholder` 屬性，並設定為 `Amount`。
...
你的 `input` 元素應該有一個 `id` 屬性，設定為 `${category}-${entryNumber}-amount`。
...
你的新 `input` 元素應該有一個 `min` 屬性，設定為 `0`。
