---
id: 68a23004e96a105df3a21fc4
title: Construir um Assistente de Contagem de Cartas
challengeType: 26
dashedName: lab-counting-cards
---

# --description--

No jogo de casino Blackjack, um jogador pode determinar se tem uma vantagem sobre a próxima mão da casa, mantendo o número relativo de cartas altas e baixas restantes no baralho. Isso se chama "contar as cartas".

Ter cartas mais altas restantes no baralho favorece o jogador. Quando o contador for positivo, o jogador deve apostar alto. Quando a contagem for zero ou negativa, o jogador deverá apostar baixo.

**Objetivo:** Cumprir as user stories abaixo e fazer todos os testes passarem para completar o laboratório.

**Histórias de Usuário:**

1. Você deve usar `let` para declarar uma variável global chamada `count` e definir seu valor como `0`.
1. Você deve ter uma função chamada `cardCounter`.
1. A função `cardCounter` deve receber um parâmetro `card` que pode ser um número ou uma string.
    - Para valores entre `2` e `10`, o parâmetro `card` será um número.
    - Para todos os outros valores, o parâmetro `card` será uma string.
1. A função `cardCounter` deve modificar a variável global `count` com base em certos critérios.
1. A variável global `count` deve ser aumentada em `1` para as cartas `2`, `3`, `4`, `5` ou `6`.
1. A variável global `count` deve permanecer inalterada para as cartas `7`, `8`, `9`.
1. A variável global `count` deve ser diminuída em `1` para as cartas `10`, `"J"`, `"Q"`, `"K"`, `"A"`.
1. A função `cardCounter` deve retornar uma string com o valor atual de count e a string `Bet` se o count for positivo.
1. A função `cardCounter` deve retornar uma string com o valor atual de count e a string `Hold` se o count for menor ou igual a `0`.
1. Na saída da função, o valor atual de count e a decisão do jogador (`Bet` ou `Hold`) devem ser separados por um espaço. Por exemplo, `-3 Hold`.

# --hints--

Você deve usar `let` para declarar uma variável global chamada `count` e defini-la como `0`.

```js
const explorer = await __helpers.Explorer(code);
const { count } = explorer.variables;
assert.isTrue(count?.matches('let count = 0'));
```

Você deve ter uma função chamada `cardCounter`.

```js
assert.isFunction(cardCounter);
```

Sua função deve retornar o valor de `count` e o texto (`Bet` ou `Hold`) com um caractere de espaço entre eles.

```js
count = 0;
let out = cardCounter(10);
const pattern = /^-?\d+ (Bet|Hold)$/;
assert.match(out, pattern);
```

Após os cards `2`, `3`, `4`, `5`, chamar `cardCounter(6)` deve retornar a string `5 Bet`.

```js
count = 0;
cardCounter(2);
cardCounter(3);
cardCounter(4);
cardCounter(5);
const out = cardCounter(6);
assert.strictEqual(out, '5 Bet')
```

Após as cartas `7`, `8`, chamar `cardCounter(9)` deve retornar a string `0 Hold`.

```js
count = 0;
cardCounter(7);
cardCounter(8);
const out = cardCounter(9);
assert.strictEqual(out, '0 Hold');
```

Após as cartas `10`, `"J"`, `"Q"`, `"K"`, então chamar `cardCounter("A")` deve retornar a string `-5 Hold`.

```js
count = 0;
cardCounter(10);
cardCounter('J');
cardCounter('Q');
cardCounter('K');
const out = cardCounter('A');
assert.strictEqual(out, '-5 Hold');
```

Após as cartas `3`, `7`, `"Q"`, `8`, chamar `cardCounter("A")` deve retornar a string `-1 Hold`.

```js
count = 0;
cardCounter(3);
cardCounter(7);
cardCounter('Q');
cardCounter(8);
const out = cardCounter('A');
assert.strictEqual(out, '-1 Hold');
```

Após as cartas `2`, `"J"`, `9`, `2`, chamar `cardCounter(7)` deve retornar a string `1 Bet`.

```js
count = 0;
cardCounter(2);
cardCounter('J');
cardCounter(9);
cardCounter(2);
const out = cardCounter(7);
assert.strictEqual(out, '1 Bet');
```

Após as cartas `2`, `2`, chamar `cardCounter(10)` deve retornar a string `1 Bet`.

```js
count = 0;
cardCounter(2);
cardCounter(2);
const out = cardCounter(10);
assert.strictEqual(out, '1 Bet')
```

Após as cartas `3`, `2`, `"A"`, `10`, chamar `cardCounter("K")` deve retornar a string `-1 Hold`.

```js
count = 0;
cardCounter(3);
cardCounter(2);
cardCounter('A');
cardCounter(10);
const out = cardCounter('K');
assert.strictEqual(out, '-1 Hold');
```

# --seed--

## --seed-contents--


```js



```

# --solutions--

```js
let count = 0;
function cardCounter(card) {
  switch(card) {
    case 2:
    case 3:
    case 4:
    case 5:
    case 6:
      count++;
      break;
    case 10:
    case 'J':
    case 'Q':
    case 'K':
    case 'A':
      count--;
  }
  if(count > 0) {
    return count + " Bet";
  } else {
    return count + " Hold";
  }
}
```
