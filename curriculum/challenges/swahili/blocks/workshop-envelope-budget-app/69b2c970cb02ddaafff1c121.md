...
Ongeza mstari katika kitendakazi chako cha `calculateBudget` unaofanya wito wa njia ya `preventDefault()` kwenye kigezo `e`. Kisha, weka upya bendera ya makosa ya kimataifa `isError` kuwa `false`.

# --hints--

Kitendakazi chako cha `calculateBudget` kinapaswa kufanya wito wa `e.preventDefault()`.

```js
assert.match(calculateBudget.toString(), /e\.preventDefault\(\s*\)/);
```

Kitendakazi chako cha `calculateBudget` kinapaswa kuweka upya bendera ya makosa ya kimataifa `isError` kuwa `false`.
