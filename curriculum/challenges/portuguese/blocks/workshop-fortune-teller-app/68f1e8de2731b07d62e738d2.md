...
Seu `foundCard` deve ser atribuído chamando `find` em `this.cards`, passando um callback que recebe um parâmetro `card` e retorna o resultado da comparação de `card.name_short` com `cardId` usando igualdade estrita.
