...
En este ejemplo revisado, estamos creando una variable `const` llamada `greetings` y asignándole una función anónima. La mayoría de la sintaxis te resultará familiar, excepto por la ausencia de la palabra clave `function` y la adición de la flecha (`=>`) entre el parámetro `name` y el cuerpo de la función. Si tu lista de parámetros tiene solo un parámetro, entonces puedes eliminar los paréntesis así:
...
¿Entonces, cuándo deberías usar la sintaxis de función flecha? Bueno, depende. Muchos desarrolladores la usan de forma consistente en sus proyectos personales. Sin embargo, cuando trabajas en equipo, la elección suele depender de si la base de código existente usa funciones regulares o funciones flecha. En lecciones futuras, cubriremos cuándo usar funciones flecha y cuándo evitarlas.
