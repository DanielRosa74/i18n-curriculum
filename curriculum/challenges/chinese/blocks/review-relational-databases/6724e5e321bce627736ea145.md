...
- <kbd>上箭头</kbd>/<kbd>下箭头</kbd>：在历史命令中循环浏览前一个/下一个命令。
- <kbd>Tab</kbd>：自动补全命令。
- <kbd>Control</kbd> + <kbd>L</kbd>（Linux/macOS）或输入 `cls`（Windows）：清除终端屏幕。
- <kbd>Control</kbd> + <kbd>C</kbd>：中止正在运行的命令（如果选中文本，在 PowerShell 中也用于拷贝）。
- <kbd>Control</kbd> + <kbd>Z</kbd>（仅限 Linux/macOS）：将任务挂起到后台；使用 `fg` 恢复。
- **`!!`**：立即重新运行上一个执行的命令。
...
- **手册页**：使用 `man` 访问命令的详细文档。
  - 提供关于命令使用的全面信息
  - 显示所有可用选项和示例
  - 使用 <kbd>箭头键</kbd> 导航，<kbd>q</kbd> 退出
  - 并非所有命令都有手册页（内置命令使用 `help`）
