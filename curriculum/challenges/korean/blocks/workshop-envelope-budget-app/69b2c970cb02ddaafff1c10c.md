...
`label` 요소에 `for` 속성을 `X-#-name` 값으로 지정하세요. 여기서 `X`는 `category` 변수의 `value`이고 `#`는 `entryNumber`의 값입니다. HTML 속성은 큰따옴표로 감싸야 한다는 점을 기억하세요.

# --hints--

`label` 요소에 `for` 속성이 `${category}-${entryNumber}-name`으로 설정되어야 합니다.

```js
assert.match(code, /HTMLString\s*=\s*`\n\s*<label\s+for\s*=\s*"\$\{category\}-\$\{entryNumber\}-name"\s*>/)
```

`label` 요소에 `"Expense ${entryNumber} Name"` 텍스트가 있어야 합니다.
