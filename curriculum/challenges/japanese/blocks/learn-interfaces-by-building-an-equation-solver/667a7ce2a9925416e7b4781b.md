...
`match`/`case` の構造を `len(results)` ではなく `results` にマッチするように変更してください。次に、各 `case` を適切な要素数のリストを使うように変更してください。リストが1つの要素を含む場合は `x` を使い、リストが2つの要素を含む場合は `x1` と `x2` を使ってください。
...
`result_list` 内の f文字列を `result[0]` の代わりに `x` を使うように変更してください。
