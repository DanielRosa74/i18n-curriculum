...
Similar a tu elemento `#stats`, tu elemento `#monsterStats` necesita dos elementos `span`. Asígnales la clase `stat` y da al primer elemento el texto `Monster Name: ` y al segundo el texto `Health: `. Después del texto en cada uno, agrega un elemento `strong` con un elemento `span` anidado vacío. Dale al primer elemento `span` interno un `id` de `monsterName` y al segundo elemento `span` interno un `id` de `monsterHealth`.
