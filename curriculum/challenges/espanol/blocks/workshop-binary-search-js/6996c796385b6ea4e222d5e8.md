...
Dentro de tu ciclo `while`, declara una variable `mid` establecida en el promedio de `low` y `high`.

# --hints--

Dentro del ciclo while deberías declarar una variable `mid` establecida en `Math.floor((low + high) / 2)`.
