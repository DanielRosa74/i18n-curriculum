...
Para terminar, establece el texto del primer elemento `a` en `Calendar`, el segundo en `Chat` y el tercero en `Email`.

Con eso, tu componente `Navbar` está completo.

# --hints--

Debes establecer el texto del primer elemento `a` en `Calendar`.

```js
assert.equal(document.querySelectorAll('li a')[2]?.textContent, 'Calendar');
```

Debes establecer el texto del segundo elemento `a` en `Chat`.

```js
assert.equal(document.querySelectorAll('li a')[3]?.textContent, 'Chat');
```

Debes establecer el texto del tercer elemento `a` en `Email`.
