...
تذكّر أن `return` يُنهي تنفيذ الدالة. بعد كتلة `if` الخاصة بك، عليك معالجة المنطق عندما يكون المُدخَل صحيحًا. بما أن جملة if الشرطية تُعيد قيمة، فلا تحتاج إلى جملة `else`.

استخدم معامل التعيين بالإضافة لإضافة `currVal` إلى `total`. ستحتاج إلى استخدام دالة البناء `Number` لتحويل `currVal` إلى رقم.
...
بعد جملة `if`، يجب استخدام معامل التعيين بالإضافة على `total`.
...
يجب إضافة قيمة `Number(currVal)` إلى `total`.
